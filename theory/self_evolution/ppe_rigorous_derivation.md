...
$$\boxed{\text{PE}_{\text{scalar}}(p, 2j) = \sin\left(\frac{2\pi p}{\lambda_j}\right), \quad \text{PE}_{\text{scalar}}(p, 2j+1) = \cos\left(\frac{2\pi p}{\lambda_j}\right)}$$
...
$$\langle \text{PE}_{\text{scalar}}(p), \text{PE}_{\text{scalar}}(q) \rangle = \sum_{j=0}^{d/2-1} \cos\left(\frac{2\pi(p-q)}{\lambda_j}\right)$$

这定义了一个**平移不变核** $K_{\text{PE}}(\Delta) = \sum_j \cos(2\pi\Delta/\lambda_j)$，其中 $\Delta = p - q$。
...
设目标核 $k(\Delta) = \exp(-|\Delta|/\xi)$（Laplace 核）。则编码核 $K_{\text{PE}}(\Delta) = \sum_{j=0}^{d/2-1} \cos(2\pi\Delta/\lambda_j)$ 在 $L^2([0, L])$ 中最优逼近 $k(\Delta)$（最小化均方误差 $\int_0^L |K_{\text{PE}}(\Delta) - k(\Delta)|^2 d\Delta$）当且仅当 $\{\lambda_j\}$ 满足以下频谱分布：
...
**步骤 3（有限维逼近）**：编码核 $K_{\text{PE}}(\Delta) = \sum_{j=0}^{d/2-1} \cos(\omega_j \Delta)$，其中 $\omega_j = 2\pi/\lambda_j$，是一个**等权离散和**，逼近连续积分 $\int_0^{\infty} S(\omega)\cos(\omega\Delta)d\omega$。
...
**定理 1.2.2（逼近误差界）**：对 Laplace 核和 $d/2$ 个频率点的分位数采样：

$$\sup_{\Delta \in [0, L]} |K_{\text{PE}}(\Delta) - k(\Delta)| \leq \frac{2\pi^2\xi}{d} \cdot \frac{L}{\xi} + O(d^{-2})$$

随着 $d \to \infty$ 以 $O(1/d)$ 速率收敛。

*证明*：这是带限函数 Fourier 积分的标准数值积分误差。分位数采样使得每个频率区间的贡献平衡，最大误差受最大被忽略的频率分量的 Fourier 系数控制。细节略。$\square$
...
$$\boxed{L_{\text{PE}}^{\text{scalar}} = \sqrt{2 \sum_{j=0}^{d/2-1} \left(\frac{2\pi}{\lambda_j}\right)^2}}$$

*证明*：对每个维度对 $(2j, 2j+1)$：

$$\|\text{PE}_j(p) - \text{PE}_j(q)\|^2 = \left[\sin\left(\frac{2\pi p}{\lambda_j}\right) - \sin\left(\frac{2\pi q}{\lambda_j}\right)\right]^2 + \left[\cos\left(\frac{2\pi p}{\lambda_j}\right) - \cos\left(\frac{2\pi q}{\lambda_j}\right)\right]^2$$

由中值定理：$|\sin(a) - \sin(b)| \leq |a-b|$，$|\cos(a) - \cos(b)| \leq |a-b|$。因此：

$$\|\text{PE}_j(p) - \text{PE}_j(q)\|^2 \leq 2 \cdot \left(\frac{2\pi|p-q|}{\lambda_j}\right)^2 = \frac{8\pi^2|p-q|^2}{\lambda_j^2}$$

对所有 $j$ 求和：

$$\|\text{PE}_{\text{scalar}}(p) - \text{PE}_{\text{scalar}}(q)\|^2 \leq 8\pi^2|p-q|^2 \sum_{j=0}^{d/2-1} \frac{1}{\lambda_j^2}$$

开方即得所述 Lipschitz 常数。该上界是紧的：取 $p, q$ 无限接近时，所有 $\sin$ 和 $\cos$ 项同时达到最大变化率。$\square$

**推论 1.2.3（Lipschitz 常数的渐近行为）**：对分位数最优谱 $\lambda_j = 2\pi\xi \cdot \cot(\pi(2j+1)/2d)$：

$$L_{\text{PE}}^{\text{scalar}} = \frac{1}{\xi} \cdot \sqrt{\frac{2d}{\pi^2} \sum_{j=0}^{d/2-1} \tan^2\left(\frac{\pi(2j+1)}{2d}\right)} \sim \frac{1}{\xi} \cdot \frac{d}{\sqrt{3}} \quad (d \to \infty)$$

Lipschitz 常数随维度线性增长，因为高维编码包含更精细（更高频）的位置信息。
...
$$\boxed{L_{\text{PE}}^{\text{rot}} = 2\sqrt{\alpha^2 + \beta^2 + \gamma^2}}$$

或者，若使用 Frobenius 范数（更保守）：

$$\boxed{L_{\text{PE}}^{\text{rot}, F} = 2\sqrt{3(\alpha^2 + \beta^2 + \gamma^2)}}$$

*证明*：

**步骤 1（单个旋转的扰动）**：对 $\mathbf{R}_x$，作用仅在第 0、1 维：

$$\mathbf{R}_x(\alpha x) - \mathbf{R}_x(\alpha q_x) = \begin{pmatrix} \cos(\alpha x) - \cos(\alpha q_x) & -\sin(\alpha x) + \sin(\alpha q_x) \\ \sin(\alpha x) - \sin(\alpha q_x) & \cos(\alpha x) - \cos(\alpha q_x) \end{pmatrix}$$

（其余维度为 0）。该 $2\times 2$ 块的 Frobenius 范数平方：

\begin{align*}
\| \cdot \|_F^2 &= [\cos(\alpha x) - \cos(\alpha q_x)]^2 + [-\sin(\alpha x) + \sin(\alpha q_x)]^2 \\
&\quad + [\sin(\alpha x) - \sin(\alpha q_x)]^2 + [\cos(\alpha x) - \cos(\alpha q_x)]^2 \\
&= 2[\cos(\alpha x) - \cos(\alpha q_x)]^2 + 2[\sin(\alpha x) - \sin(\alpha q_x)]^2
\end{align*}

由中值定理：$|\cos(a) - \cos(b)| = |\sin(\xi)||a-b| \leq |a-b|$，$|\sin(a) - \sin(b)| = |\cos(\xi)||a-b| \leq |a-b|$。

因此 $\| \cdot \|_F^2 \leq 2\alpha^2|x - q_x|^2 + 2\alpha^2|x - q_x|^2 = 4\alpha^2|x - q_x|^2$。

$$\|\mathbf{R}_x(\alpha x) - \mathbf{R}_x(\alpha q_x)\|_F \leq 2\alpha|x - q_x|$$

同理对 $\mathbf{R}_y, \mathbf{R}_z$。

**步骤 2（乘积的扰动——三角不等式分解）**：

$$\|\mathbf{R}(\mathbf{p}) - \mathbf{R}(\mathbf{q})\|_F = \|\mathbf{R}_x(\alpha x)\mathbf{R}_y(\beta y)\mathbf{R}_z(\gamma z) - \mathbf{R}_x(\alpha q_x)\mathbf{R}_y(\beta q_y)\mathbf{R}_z(\gamma q_z)\|_F$$

分解为：

\begin{align*}
\|\mathbf{R}(\mathbf{p}) - \mathbf{R}(\mathbf{q})\|_F &\leq \|\mathbf{R}_x(\alpha x)\mathbf{R}_y(\beta y)\mathbf{R}_z(\gamma z) - \mathbf{R}_x(\alpha q_x)\mathbf{R}_y(\beta y)\mathbf{R}_z(\gamma z)\|_F \\
&\quad + \|\mathbf{R}_x(\alpha q_x)\mathbf{R}_y(\beta y)\mathbf{R}_z(\gamma z) - \mathbf{R}_x(\alpha q_x)\mathbf{R}_y(\beta q_y)\mathbf{R}_z(\gamma z)\|_F \\
&\quad + \|\mathbf{R}_x(\alpha q_x)\mathbf{R}_y(\beta q_y)\mathbf{R}_z(\gamma z) - \mathbf{R}_x(\alpha q_x)\mathbf{R}_y(\beta q_y)\mathbf{R}_z(\gamma q_z)\|_F
\end{align*}

利用旋转矩阵的**正交不变性**（$\|\mathbf{R}\mathbf{A}\|_F = \|\mathbf{A}\|_F$ 和 $\|\mathbf{A}\mathbf{R}\|_F = \|\mathbf{A}\|_F$）：

\begin{align*}
\text{Term 1} &= \|(\mathbf{R}_x(\alpha x) - \mathbf{R}_x(\alpha q_x))\mathbf{R}_y(\beta y)\mathbf{R}_z(\gamma z)\|_F = \|\mathbf{R}_x(\alpha x) - \mathbf{R}_x(\alpha q_x)\|_F \leq 2\alpha|x-q_x| \\
\text{Term 2} &= \|\mathbf{R}_x(\alpha q_x)(\mathbf{R}_y(\beta y) - \mathbf{R}_y(\beta q_y))\mathbf{R}_z(\gamma z)\|_F = \|\mathbf{R}_y(\beta y) - \mathbf{R}_y(\beta q_y)\|_F \leq 2\beta|y-q_y| \\
\text{Term 3} &= \|\mathbf{R}_x(\alpha q_x)\mathbf{R}_y(\beta q_y)(\mathbf{R}_z(\gamma z) - \mathbf{R}_z(\gamma q_z))\|_F = \|\mathbf{R}_z(\gamma z) - \mathbf{R}_z(\gamma q_z)\|_F \leq 2\gamma|z-q_z|
\end{align*}

因此：

$$\|\mathbf{R}(\mathbf{p}) - \mathbf{R}(\mathbf{q})\|_F \leq 2(\alpha|x-q_x| + \beta|y-q_y| + \gamma|z-q_z|)$$

**步骤 3（编码向量的扰动）**：

$$\|\text{PE}_{\text{rot}}(\mathbf{p}) - \text{PE}_{\text{rot}}(\mathbf{q})\| = \|\mathbf{R}(\mathbf{p})\mathbf{e}_0 - \mathbf{R}(\mathbf{q})\mathbf{e}_0\| \leq \|\mathbf{R}(\mathbf{p}) - \mathbf{R}(\mathbf{q})\|_F \cdot \|\mathbf{e}_0\|$$

取 $\|\mathbf{e}_0\| = 1$。

由 Cauchy-Schwarz：$\alpha|x-q_x| + \beta|y-q_y| + \gamma|z-q_z| \leq \sqrt{\alpha^2 + \beta^2 + \gamma^2} \cdot \|\mathbf{p} - \mathbf{q}\|_2$。

因此：

$$\|\text{PE}_{\text{rot}}(\mathbf{p}) - \text{PE}_{\text{rot}}(\mathbf{q})\| \leq 2\sqrt{\alpha^2 + \beta^2 + \gamma^2} \cdot \|\mathbf{p} - \mathbf{q}\|_2$$

$\square$

**注 1.3.1（紧致性）**：Lipschitz 常数 $2\sqrt{\alpha^2 + \beta^2 + \gamma^2}$ 是紧的。考虑 $\mathbf{p} = (x, 0, 0)$, $\mathbf{q} = (x + \varepsilon, 0, 0)$，仅 $x$ 分量变化：

$$\|\text{PE}_{\text{rot}}(\mathbf{p}) - \text{PE}_{\text{rot}}(\mathbf{q})\| = \|\mathbf{R}_x(\alpha\varepsilon)\mathbf{e}_0 - \mathbf{e}_0\|$$

（利用了 $\mathbf{R}_y(0) = \mathbf{R}_z(0) = \mathbf{I}$ 和 $\mathbf{e}_0$ 在 $x$-平面上有分量）。

$\mathbf{R}_x(\alpha\varepsilon)\mathbf{e}_0 = (\cos(\alpha\varepsilon), \sin(\alpha\varepsilon), 0, \ldots)^T$，所以：

$$\|\mathbf{R}_x(\alpha\varepsilon)\mathbf{e}_0 - \mathbf{e}_0\|^2 = (\cos(\alpha\varepsilon)-1)^2 + \sin^2(\alpha\varepsilon) = 2 - 2\cos(\alpha\varepsilon) = 4\sin^2\left(\frac{\alpha\varepsilon}{2}\right)$$

当 $\varepsilon \to 0$：$\|\text{PE}_{\text{rot}}(\mathbf{p}) - \text{PE}_{\text{rot}}(\mathbf{q})\| = 2|\sin(\alpha\varepsilon/2)| \to \alpha|\varepsilon|$。

而 $L_{\text{PE}}^{\text{rot}} \cdot \|\mathbf{p} - \mathbf{q}\| = 2\alpha \cdot |\varepsilon|$。极限比值：$\alpha|\varepsilon| / (2\alpha|\varepsilon|) = 1/2$。

这说明若只在一个空间方向变化，有效 Lipschitz 常数是 $\alpha$（而非 $2\alpha$）。上界 $2\sqrt{\alpha^2 + \beta^2 + \gamma^2}$ 是**最坏情况**（三个方向同时变化）。

**改进的（方向相关）Lipschitz 常数**：

$$\|\text{PE}_{\text{rot}}(\mathbf{p}) - \text{PE}_{\text{rot}}(\mathbf{q})\| \leq 2\|\mathbf{A}(\mathbf{p} - \mathbf{q})\|_1^{\text{sorted}}$$

其中 $\mathbf{A} = \text{diag}(\alpha, \beta, \gamma)$，$\|\mathbf{v}\|_1^{\text{sorted}}$ 是 $(|v_1|, |v_2|, |v_3|)$ 按降序排列后加权的某种范数。更紧的上界需要对旋转乘积的 Baker-Campbell-Hausdorff 展开。
...
|----------|---------------|-----------------|
| 正弦（标量） | $[0, L]$ | $\sqrt{2\sum_j (2\pi/\lambda_j)^2}$ |
| 旋转（3D） | $\mathbb{R}^3$ | $2\sqrt{\alpha^2 + \beta^2 + \gamma^2}$ |
...
**定理 2.2.1（$\delta_s^{\text{PE}} > 0$ 的充分条件——信息论形式）**

设 $S, P, Y$ 分别表示状态原子、物理位置和标签的随机变量（在数据分布下）。若：

$$\boxed{I(Y; P \mid S) > 0}$$

即：物理位置在**给定状态原子的条件下**携带关于标签的额外信息，则**存在**一个 PPE 函数 $\text{PE}: \mathcal{P} \to \mathbb{R}^{d_{\text{pe}}}$ 和编码维度 $d_{\text{pe}}$，使得对某个状态 $s$，$\delta_s^{\text{PE}} > 0$。
...
**步骤 3（从信息增益到专家行为改善）**：条件互信息 $I(Y; \text{PE}(P) \mid S) > 0$ 意味着给定 $S$，$\text{PE}(P)$ 提供关于 $Y$ 的额外"信号"。考虑一个最优分类器（贝叶斯最优）在增广空间中的表现。

对于噪声样本（标签 $y$ 是错误的）：给定 $(s, p)$，贝叶斯最优预测器会更倾向于输出**与 $y$ 不同的**真实标签（因为 $p$ 提供了额外的证据）。因此分歧概率上升：

$$p_{\text{noisy}, s}^{\text{PPE}} > p_{\text{noisy}, s}$$

对于干净样本：额外的位置信息**进一步确认**正确的标签，使得专家分歧概率**不增加**（甚至减少）：

$$p_{\text{clean}, s}^{\text{PPE}} \leq p_{\text{clean}, s}$$

因此 $\delta_s^{\text{PE}} = (p_{\text{noisy}, s}^{\text{PPE}} - p_{\text{noisy}, s}) - (p_{\text{clean}, s}^{\text{PPE}} - p_{\text{clean}, s}) > 0$。

**步骤 4（从贝叶斯最优到实际专家）**：尽管实际专家 $E_m$ 可能不是贝叶斯最优的，但假设它们**一致地逼近贝叶斯最优**（即它们从增广特征中提取信息的方向与贝叶斯最优相同，只是噪声更大）。在这种情况下，$p_{\text{noisy}}^{\text{PPE}} - p_{\text{noisy}} > 0$ 和 $p_{\text{clean}}^{\text{PPE}} - p_{\text{clean}} \leq 0$ 的符号在期望意义下保持。$\square$
...
## 2.4 $\delta_s^{\text{PE}}$ 的信息论下界（Fano 逆不等式）

**定理 2.4.1（$\delta_s^{\text{PE}}$ 的 Fano 下界）**：

设位置 $P$ 在给定状态 $S$ 下提供关于标签 $Y$ 的额外信息，且编码 PE 保留其中的 $\rho = I(Y; \text{PE}(P) \mid S) / I(Y; P \mid S) \in (0, 1]$ 部分。则存在状态 $s$ 使得：

$$\boxed{\delta_s^{\text{PE}} \geq \frac{2\rho \cdot I(Y; P \mid S) - \log 2}{\log |\mathcal{Y}|} - O\left(\frac{1}{\sqrt{M}}\right)}$$
...
其中 $P_e$ 是给定 $(S, \text{PE}(P))$ 下的贝叶斯错误率，$H_2$ 是二元熵。

等效地，下界：

$$P_e \geq \frac{H(Y \mid S, \text{PE}(P)) - 1}{\log |\mathcal{Y}|}$$

**步骤 2（与检测边际的联系）**：贝叶斯错误率 $P_e$ 与专家分歧概率的关系为：

$$P_e = \min_{f} P(f(S, \text{PE}(P)) \neq Y)$$

在 SCX 框架中，噪声标签下的分歧概率 $p_{\text{noisy}}$ 对应于模型以概率 $1 - p_{\text{noisy}}$ 输出正确标签（即"同意"噪声标签 → 犯错）。因此平均错误率：

$$\bar{P}_e = (1 - \pi_{\text{noise}}) \cdot p_{\text{clean}} + \pi_{\text{noise}} \cdot (1 - p_{\text{noisy}})$$

其中 $\pi_{\text{noise}}$ 是噪声比例。

**步骤 3（变化量）**：PPE 引入前后错误率的变化：

\begin{align*}
\Delta P_e &= \bar{P}_e^{\text{PPE}} - \bar{P}_e \\
&= (1 - \pi)(p_{\text{clean}}^{\text{PPE}} - p_{\text{clean}}) + \pi((1 - p_{\text{noisy}}^{\text{PPE}}) - (1 - p_{\text{noisy}})) \\
&= (1 - \pi)(p_{\text{clean}}^{\text{PPE}} - p_{\text{clean}}) - \pi(p_{\text{noisy}}^{\text{PPE}} - p_{\text{noisy}})
\end{align*}

对单个状态 $s$，$\delta_s^{\text{PE}} = (p_{\text{noisy}}^{\text{PPE}} - p_{\text{noisy}}) - (p_{\text{clean}}^{\text{PPE}} - p_{\text{clean}})$。

当 PPE 主要帮助检测噪声（$p_{\text{noisy}}^{\text{PPE}} \gg p_{\text{noisy}}$）而不过多干扰干净样本（$p_{\text{clean}}^{\text{PPE}} \approx p_{\text{clean}}$），有 $\Delta P_e \approx -\pi \cdot \delta_s^{\text{PE}}$（错误率下降 = 增益）。

**步骤 4（Fano 给出下界）**：

\begin{align*}
P_e - P_e^{\text{PPE}} &= \frac{H(Y|S) - H(Y|S, \text{PE}(P))}{\log |\mathcal{Y}|} \quad (\text{近似，取 Fano 的等号}) \\
&= \frac{I(Y; \text{PE}(P) \mid S)}{\log |\mathcal{Y}|} \\
&= \frac{\rho \cdot I(Y; P \mid S)}{\log |\mathcal{Y}|}
\end{align*}

由于 $P_e - P_e^{\text{PPE}} \leq \pi \cdot \delta_s^{\text{PE}}$ 对最大获益的状态 $s$ 成立，存在 $s$ 使得：

$$\delta_s^{\text{PE}} \geq \frac{\rho \cdot I(Y; P \mid S)}{\pi \cdot \log |\mathcal{Y}|}$$

简化（取 $\pi = 1/2$ 最坏情况，$\rho = 1$ 完美编码），并加上有限 $M$ 的 Hoeffding 修正 $\approx \sqrt{\log(2/\alpha)/(2M)}$：

$$\delta_s^{\text{PE}} \geq \frac{2 \cdot I(Y; P \mid S) - \log 2}{\log |\mathcal{Y}|} - O\left(\frac{1}{\sqrt{M}}\right)$$

$\square$

**物理含义**：若 $I(Y; P \mid S)$ 很小（位置几乎不提供额外信息），则 $\delta_s^{\text{PE}}$ 的下界接近零或为负——PPE 无法实质性地改善检测。这解释了为什么**不是所有物理系统都能从 PPE 中受益**。

---

## 2.5 空间局部性正则化——CC 审计报告结果的修正版

**定理 2.5.1（$\delta_s^{\text{PE}}$ 的空间光滑性）**：设 PE 满足 Lipschitz 条件（常数为 $L_{\text{PE}}$），且专家 $E_m$ 对其输入满足 $L_E$-Lipschitz。则对同属一个状态 $s$、具有物理位置 $p_i, p_j$ 的两个原子：

$$\boxed{|\delta_i^{\text{PE}} - \delta_j^{\text{PE}}| \leq 2 \cdot L_E \cdot L_{\text{PE}} \cdot d_{\mathcal{P}}(p_i, p_j) + O\left(\frac{1}{\sqrt{M}}\right)}$$

其中 $O(1/\sqrt{M})$ 来自有限专家的估计误差。

*证明*：与 CC 审计报告中的命题 2.2 基本相同，但我们的符号是修正后的 $\delta_s^{\text{PE}}$ 定义。
...
每个 probability difference 受输入变化约束：
...
（利用 $s_i = s_j = s$ 时 $\phi(s_i) = \phi(s_j)$。）两倍即得。$\square$
...
### 4.3.3 学习型 PE 真正能区分的条件

要使学习型 PE 能够区分，需要满足：**学习信号不仅仅依赖于观测分布**。这发生在：
...
**定理 4.2（学习型 PE 的可区分性——严格条件）**：

设 $\mathcal{L}_{\text{total}}(\theta) = \mathcal{L}_{\text{sup}}(\theta) + \lambda \cdot \mathcal{L}_{\text{aux}}(\theta)$ 是完整的学习目标，其中：
- $\mathcal{L}_{\text{sup}}$ 是监督损失（仅依赖 $(X, P, Y)$ 的观测分布）
- $\mathcal{L}_{\text{aux}}$ 是辅助损失（可能依赖未观测到的物理量 $Z$）

若 $\mathcal{L}_{\text{aux}}$ 在 $W_A$ 和 $W_B$ 中具有不同的总体最小值（即使观测分布 $(X, P, Y)$ 相同），即：

$$\arg\min_\theta \mathcal{L}_{\text{aux}}^{W_A}(\theta) \neq \arg\min_\theta \mathcal{L}_{\text{aux}}^{W_B}(\theta)$$

则学习到的 PE 参数 $\hat{\theta}_{W_A} \neq \hat{\theta}_{W_B}$ 以趋于 1 的概率成立（当样本量 $n \to \infty$）。两个世界变为可区分。

*构造性示例*：设辅助损失为能量一致性 $\mathcal{L}_{\text{aux}} = \mathbb{E}[|E_{\text{pred}}(x, p) - E_{\text{DFT}}(x, p)|^2]$。在 $W_A$ 中，噪声标签意味着专家判断与物理能量不匹配；而在 $W_B$ 中，困难样本的标签虽不确定，但物理能量是一致的。两个世界给出不同的最优 PE 参数。
...
\textbf{若前提 (A) 不成立: } & \text{PE 的参数通过涉及 } Y \text{（或任何与 } Y_{\text{true}} \text{ 相关的信号）的目标函数学习，} \\
& \text{且该目标函数在两个世界中具有不同的最优值，则不可区分性被破坏：} \\
& \exists d^*, \quad P_{W_A}(d^* = W_A) > 1/2 \text{ 且 } P_{W_B}(d^* = W_B) > 1/2。
\end{aligned}}$$
...
3. **通过多目标学习（包括标签相关目标）的 PE**：Theorem 3 **可能被破坏**。具体取决于辅助目标在两个世界中是否给出不同的最优参数。这是**好消息**：位置信息提供了一个额外的"窗口"，通过它可能区分噪声和困难样本。

4. **风险**：如果辅助目标的设计不好（例如引入了虚假相关性），可能导致错误的"区分"，将困难样本错误地标记为噪声。这需要仔细的实验验证。
