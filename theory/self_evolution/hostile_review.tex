...
\textbf{审稿人身份}: Annals of Statistics 最苛刻的审稿人\\
\textbf{审稿日期}: 2026-06-29\\
\textbf{审稿对象}: \texttt{paper/situs\_theory/main.tex} +
\texttt{theory/self\_evolution/ppe\_rigorous\_derivation.md}\\
\textbf{审稿标准}: 不礼貌。不妥协。只要有一个漏洞就不放过。

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{〇、总体印象}\label{ux603bux4f53ux5370ux8c61}

这篇论文声称在一个叫SCX的框架上建立了Situs（物理位置编码）的''严格数学基础''。实际读完三份文档后的结论是：\textbf{这篇文章在多个核心定理的证明上存在根本性错误或无法修复的逻辑跳跃。Theorem
...
论文正文使用 \texttt{\textbackslash{}Situs},
\texttt{\textbackslash{}Sstates}, \texttt{\textbackslash{}Ppos},
\texttt{\textbackslash{}PE},
\texttt{d\_\{\textbackslash{}pe\}}，ppe\_rigorous\_derivation.md 使用
\texttt{PPE}, \texttt{\textbackslash{}mathcal\{S\}},
\texttt{\textbackslash{}mathcal\{P\}},
\texttt{\textbackslash{}text\{PE\}},
\texttt{d\_\{\textbackslash{}text\{pe\}\}}。论文声称这些编号与
ppe\_rigorous\_derivation.md ``保持一致''（line
1343），但在所有符号上都不同。这不是LaTeX vs
Markdown的格式差异------论文用 \texttt{\textbackslash{}Ppos}
表示位置空间，md用
\texttt{\textbackslash{}mathcal\{P\}}。作者在同一个句子里宣称一致性的同时展示了不一致性。

\subsubsection{1.3 Theorem 2.3.1
的两个版本}\label{theorem-2.3.1-ux7684ux4e24ux4e2aux7248ux672c}

\textbf{论文版} (line 697-701):
\[\delta_s^{\PE} \leq \min\!\left(1 - p_{\text{clean}, s},\, \sqrt{\frac{1}{2} D_{\KL}(P_{\PE|S,Y} \| P_{\PE|S})}\right) + \delta_s^{\text{variance}}\]

\textbf{md版} (line 512-513):
\[\delta_s^{\text{PE}} \leq \min\left(1, \sqrt{\frac{1}{2} \cdot I(P; \text{PE}(P) \mid S, Y)}\right) + \text{Bernstein 修正项}\]

这\textbf{不是}同一个上界。第一个用 \(1-p_{\text{clean},s}\) 和 KL
散度（逐点条件），第二个用 \(1\)
和互信息（期望形式）。md版用''Bernstein修正''（暗示\(O(1/M)\)收敛），论文用
\(\delta_s^{\text{variance}} = O(1/\sqrt{M})\)（暗示CLT收敛）。两者在有限\(M\)下的缩放律完全不同------一个是\(1/M\)，一个是\(1/\sqrt{M}\)。
...
(\(v_m \indep v_{m'} \mid s\))}{A1: 专家独立性 (v\_m \textbackslash indep v\_\{m\textquotesingle\} \textbackslash mid s)}}\label{a1-ux4e13ux5bb6ux72ecux7acbux6027-v_m-indep-v_m-mid-s}

\textbf{原始状态}: \(M\)个专家对同一状态原子\(s\)的投票是i.i.d.
Bernoulli。

\textbf{加入Situs后}: 所有专家接收相同的增强输入
\(h_i = \phi(s_i) + \PE(p_i)\)。PE项是共享的确定性信号------如果PE携带关于标签的信息，那么所有专家的输入都包含同一条''提示''。这使得专家的条件独立性假设\textbf{被削弱}：\(\Cov(v_m, v_{m'} \mid s, p) \neq 0\)。严格地说，Chernoff-Hoeffding
bound要求的是给定状态后的条件独立，现在PE注入了一个所有专家共享的公共信号。\textbf{这破坏了i.i.d.假设的严格性。}

攻击严重度:
\textbf{中}。在\(M\)很大时，这种共享信号引入的相关性可能使Chernoff界变松一个常数因子（类似于\(\beta\)-mixing修正），但论文没有提及这个问题------它装作Chernoff-Hoeffding的结构''不变''。

\subsubsection{\texorpdfstring{A2: 检测边际为正
(\(\Delta_s > 0\))}{A2: 检测边际为正 (\textbackslash Delta\_s \textgreater{} 0)}}\label{a2-ux68c0ux6d4bux8fb9ux9645ux4e3aux6b63-delta_s-0}

\textbf{原始状态}:
\(\Delta_s = p_{\text{noisy},s} - p_{\text{clean},s} > 0\)。

\textbf{加入Situs后}: 需要
\(\Delta_s^{\Situs} = \Delta_s + \delta_s^{\PE} > 0\)。但
\(\delta_s^{\PE} \in [-\Delta_s, 1-p_{\text{clean},s}]\)。\textbf{Situs可以使检测边际减小}！如果\(\delta_s^{\PE} < 0\)且\(|\delta_s^{\PE}| > \Delta_s\)，则\(\Delta_s^{\Situs} < 0\)------Situs反转了检测方向。Theorem
2.2.1只给出了存在某个\(s\)和某个PE使\(\delta_s^{\PE} > 0\)的充分条件，但\textbf{没有排除对其他\(s\)有\(\delta_s^{\PE} < 0\)的可能性}。

攻击严重度: \textbf{高}。论文中的修正Theorem
1将所有状态的\(\Delta_s\)替换为\(\Delta_s + \delta_s^{\PE}\)，但\(\delta_s^{\PE}\)可以是负的。对某些\(s\)，\(\exp(-2M(\Delta_s + \delta_s^{\PE})^2)\)可能比\(\exp(-2M\Delta_s^2)\)更大，即\(F_1\)下界\textbf{变差}。这不是论文声称的''系统性增强''。

\subsubsection{A3:
状态空间离散性}\label{a3-ux72b6ux6001ux7a7aux95f4ux79bbux6563ux6027}

\textbf{原始状态}: \(\mathcal{S} = \{s_1, \ldots, s_N\}\)是有限离散集。
...
攻击严重度:
\textbf{低-中}。技术上可修复（在联合空间上定义分布），但论文没有讨论这个微妙之处。
...
\textbf{原始状态}: \(v_m \in \{0,1\}\)有界。

\textbf{加入Situs后}:
投票仍然在\(\{0,1\}\)中，Chernoff-Hoeffding的适用性不变。这个假设\textbf{形式上保持}。但如A1所述，独立性条件被削弱。

攻击严重度: \textbf{低}（形式保持，实质被A1牵连）。
...
攻击严重度:
\textbf{低}。这些是标准信息论的技术细节，可以处理，但论文没有处理。

\subsubsection{假设检验结论}\label{ux5047ux8bbeux68c0ux9a8cux7ed3ux8bba}

\textbf{论文声称的6个假设中，至少A1（独立性）和A2（正边际）在Situs下被显著影响。论文没有逐条验证这些假设仍然成立就声称Chernoff结构''不变''，这是不诚实的。}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{三、证明完整性：逻辑跳跃、未声明假设、错误推理}\label{ux4e09ux8bc1ux660eux5b8cux6574ux6027ux903bux8f91ux8df3ux8dc3ux672aux58f0ux660eux5047ux8bbeux9519ux8befux63a8ux7406}
...
论文自己标注这是
\texttt{{[}启发式{]}}。但这不是启发式------\textbf{这是一个无桥的鸿沟}。问题如下：

\textbf{(a)}
贝叶斯最优分类器在给定\((s, \PE(p))\)下的行为与任意一个实际训练的神经网络专家\(E_m\)的行为之间，没有任何有限样本的收敛保证。

\textbf{(b)}
``一致地逼近贝叶斯最优''意味着什么？如果专家是随机梯度下降训练的神经网络，它们的收敛点是损失景观的某个驻点，不一定是贝叶斯最优。即使是，也是在\textbf{总体分布}意义下，不保证对每个特定状态\(s\)的\(\delta_s^{\PE}\)符号正确。

\textbf{(c)}
论文声称\(p_{\text{clean},s}^{\Situs} \leq p_{\text{clean},s}\)（干净样本上分歧不增加）。贝叶斯最优分类器在获得更多信息后确实不会变得更差（数据处理不等式保证）。但实际专家可能因为PE引入了\textbf{虚假相关性}而在干净样本上表现更差------特别是当训练数据有限时，PE可能学到一个碰巧在训练集上有效但在测试集上无效的模式。这个情况下\(p_{\text{clean},s}^{\Situs} > p_{\text{clean},s}\)，且\(\delta_s^{\PE}\)可能为负。

\textbf{(d)}
整个定理2.2.1的结论------``存在PE和维度使得\(\delta_s^{\PE} > 0\)''------如果没有步骤4，只对贝叶斯最优分类器成立。对于实际SCX框架中的神经网络专家，这是一个\textbf{未经证实的猜想}。

\textbf{判决}:
定理2.2.1是本文最重要的信息论结果（连接了\(I(Y;P|S) > 0\)和检测改进）。去掉步骤4后，它只证明了贝叶斯最优分类器可以从位置信息中受益------这是平凡的。将这个结果用于SCX框架中的实际专家是\textbf{逻辑跳跃}。

\subsubsection{3.2
定理2.4.1步骤2：Fano不等式的方向性（致命）}\label{ux5b9aux74062.4.1ux6b65ux9aa42fanoux4e0dux7b49ux5f0fux7684ux65b9ux5411ux6027ux81f4ux547d}

这是我发现的最严重错误。论文声称Fano给出了\(\delta_s^{\PE}\)的\textbf{下界}。但仔细检查推导：

\textbf{步骤1}: Fano不等式标准形式（正确）------
...
\textbf{步骤3} (论文line 763-766, md line 601-607):
\[P_e - P_e^{\Situs} = \frac{H(Y|S) - H(Y|S, \PE(P))}{\log |\Y|} \quad (\text{取Fano的等号})\]

\textbf{这是根本性错误。} 逻辑链条是： 1.
\(P_e \geq \frac{H(Y|S)-1}{\log|\Y|}\)（Fano） 2.
\(P_e^{\Situs} \geq \frac{H(Y|S,\PE(P))-1}{\log|\Y|}\)（Fano） 3.
如果两个不等式都取等号 →
\(P_e - P_e^{\Situs} = \frac{H(Y|S) - H(Y|S,\PE(P))}{\log|\Y|}\)

但从(1)和(2)的\textbf{不等式}到(3)的\textbf{等式}，需要 (i)
...
-
更重要的是，即使两个Fano界都达到等号，\(a \geq A\)和\(b \geq B\)\textbf{不能推出}
\(a-b = A-B\)。例如\(P_e = 0.5\)（Fano界0.3），\(P_e^{\Situs} = 0.45\)（Fano界0.25），则\(P_e - P_e^{\Situs} = 0.05\)但\(\frac{I}{\log|\Y|} = 0.05\)碰巧成立。但也可以\(P_e = 0.5\)（界0.3），\(P_e^{\Situs} = 0.3\)（界0.15），则\(P_e - P_e^{\Situs} = 0.2\)但\(\frac{I}{\log|\Y|} = 0.15\)------Fano等式形式给出错误的下界。

md明确标注''近似，取Fano的等号''------\textbf{这承认了不是严格推导}。但论文正文中Theorem
2.4.1的陈述没有这个''近似''的限定，它被呈现为一个\textbf{定理}，带有\texttt{{[}严格{]}}/\texttt{{[}启发式{]}}混合标注（步骤1严格，步骤2-3启发式）。

\textbf{方向性错误的本质}:
论文试图从Fano的\textbf{上界}（条件熵）推出错误率改善的\textbf{下界}。Fano不等式是\(H(Y|X) \leq f(P_e)\)的形式，给出的是\(P_e\)的下界。但\(P_e - P_e^{\Situs}\)作为一个差值，不能直接从两个\(P_e\)的下界推导出来------差值的方向不受单个下界的控制。

\textbf{正确做法}:
需要的是\(P_e - P_e^{\Situs}\)的\textbf{上界}才能用Fano给出任何严格的界。或者使用更强的工具如Fano的逆形式（但这通常需要额外的分布假设）。

\textbf{判决}: \textbf{Theorem 2.4.1的证明是错的。}
...
PE是从物理位置派生的，物理位置可能与标签有\textbf{虚假相关}（在训练分布中相关，但在目标分布中不相关）。
-
...
\textbf{攻击}:
步骤3的论证假设PE提供的信息总是''有益的''------它增加噪声样本的分歧但不增加干净样本的分歧。这等于\textbf{假设了结论}：PE以某种方式区分噪声和干净样本。如果PE的信息是纯粹的额外特征（相关性但不因果），它可能以相同的幅度增加两类样本的分歧，导致\(\delta_s^{\PE} = 0\)。
...
但\(p_{\text{clean}, i}^{\text{PPE}} = \mathbb{E}[\mathbf{1}[E_m(h_i) \neq y]]\)是\textbf{指示函数的期望}。即使\(E_m\)的输出是\(L_E\)-Lipschitz的（在某种意义下），指示函数\(\mathbf{1}[\cdot \neq y]\)是\textbf{不连续的}------它在决策边界处跳跃。\(L_E\)-Lipschitz假设通常适用于网络的输出logit或softmax概率，而不是0/1决策的期望。

要这个界成立，需要额外的假设： -
专家的决策边界距离\(h_i\)和\(h_j\)都足够远（margin条件），或者 -
专家输出的是软概率且指示函数的期望恰好是Lipschitz的（这要求专家的预测概率关于输入是Lipschitz的，且阈值交叉的概率为零测集）

论文没有提供这些条件。\textbf{这不是技术细节------这是界是否非空（non-vacuous）的关键。}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{四、符号错误：不止一处}\label{ux56dbux7b26ux53f7ux9519ux8befux4e0dux6b62ux4e00ux5904}
...
\textbf{但是}：CC审计报告内部的符号本身就是不一致的。§0.2定义\(p_{\text{clean}}\)为分歧概率（\(<0.5\)），但§2.3的证明中出现了\(|p_{\text{clean}} + p_{\text{noisy}} - 1|\)这种把\(p_{\text{clean}}\)当一致性概率用的公式。所以CC报告的''sign
error''可能不是单纯的符号反了------而是更深的\textbf{符号体系内部分裂}：§0.2和§2.3
用的是完全不同的\(p\)的语义。

论文修正了这个sign
error但\textbf{没有指出CC报告内部的符号分裂}------它只是反了一下符号就宣称修正了。实际上CC报告的问题更严重。
...
\textbf{推导}:
对于\(\mathbf{e}_0 = (1,0,0,1,0,0,\ldots)^T\)（每个旋转平面第一维为1），
...
\textbf{真正的Lipschitz常数是\(\max(\alpha, \beta, \gamma)\)}，并且这个界是紧的（取\(\Delta\mathbf{p}\)沿最大频率参数的方向）。

论文证明中的错误出在步骤2的三角不等式分解（md line
300-317）。作者正确地得到了：
\[\|\mathbf{R}(\mathbf{p}) - \mathbf{R}(\mathbf{q})\|_F \leq 2\alpha|\Delta x| + 2\beta|\Delta y| + 2\gamma|\Delta z|\]

但三个差分矩阵作用于\textbf{互不相交的子空间}上（维度对(0,1), (2,3),
...
\textbf{两个松弛相乘导致最终常数高出约3.46倍}。更糟的是，作者注意到了这个问题（注记1.3.1/md
note
...
当\(d \to \infty\)，编码核\textbf{发散}至无穷而非收敛到目标核。要使编码核逼近目标核，必须归一化：
\[\frac{2}{d} K_{\PE}(\Delta) \to \int_0^\infty S(\omega)\cos(\omega\Delta)d\omega = k(\Delta)\]

但定理1.2.1的陈述和证明\textbf{完全没有}提到这个\(2/d\)因子。定理中''编码核在\(L^2([0,L])\)中最优逼近\(k(\Delta)\)``的陈述在数学上是错误的------\(K_{\PE}\)和\(k\)的量级不同（\(O(d)\)
vs \(O(1)\)）。
...
Pinsker
bound是：\(|P(A) - Q(A)| \leq \sqrt{\frac{1}{2}D_{\KL}(P\|Q)}\)，其中\(P\)和\(Q\)是\textbf{同一可测空间上的概率测度}且\(A\)是该空间中的事件。

论文应用时，\(P\)和\(Q\)是\(\PE(P)\)在给定\(S\)下的条件分布（纯净vs混杂），但被bound的量\(p_{\text{clean},s}^{\Situs} - p_{\text{clean},s}\)涉及的是\textbf{专家的输出事件}，该事件不在\(\PE(P)\)的空间中。
...
\textbf{不过这可能是可修复的}:
如果我们比较\(p_{\text{clean},s}^{\Situs}\)和\(p_{\text{clean},s}^{\text{avg}} = \int P(E_m(\phi(s)+z) \neq y) \cdot dP_{\PE(P)|S=s}(z)\)（即对PE取平均的干净分歧率），Pinsker就可以合法应用。但论文没有做这个分解。
...
\subsubsection{5.4 Theorem 2'的界}\label{theorem-2ux7684ux754c}
...
上界比原始Theorem
2松了\(\sqrt{\delta + 2I/C_F^2} / \sqrt{\delta}\)倍。当\(I(Y;P|X)\)很大（位置有很多信息但编码完全丢失）时，上界可以变得任意松。这当然不是一个''紧''的界------它是一个\textbf{非空的但可能非常宽松的}上界。论文没有声称它是紧的（这点诚实），但在摘要中说''修正了弱特征失效上界''暗示了某种精确性，实际上只是在一个已经很松的界上再加了一个正项。
...
\tightlist
\item
  构造了两个世界\(W_A, W_B\)，验证\(P_{W_A}(X,Y) = P_{W_B}(X,Y)\) ✓
\item
  引入学习型PE \(\PE_\theta(p) = \theta \cdot p\)
\item
  计算\(\mathbb{E}_{W_A}[Y|X] = \mathbb{E}_{W_B}[Y|X] = 0.6 \cdot \text{sign}(x)\)------\textbf{数值相同}
\item
  结论：\textbf{即使是学习型PE也不能区分两个世界}（论文line
  1131-1132原文：``因此即使是学习型PE也不能区分两个世界。这恰恰印证了Theorem
  3的精髓''）
\end{enumerate}

\textbf{这是一个''构造性反例''------它构造了一个学习型PE无法区分的例子，正好证明了Theorem
3的鲁棒性而非其脆弱性！}
...
这\textbf{不是构造性证明}------这是口头描述了一个设想中的场景。没有给出\(Z\)的具体定义，没有给出\(\mathcal{L}_{\text{aux}}\)的具体形式，没有验证两个世界确实给出不同的最优点。整个§4.3承诺了一个''构造性最小示例''但只交付了一个\textbf{空的声称}。
...
- 前提(A)不成立时的''可区分性''结论\textbf{没有构造性证明支撑} -
``构造性最小示例''实际上是定理的\textbf{反例的反例}------即它展示了定理的鲁棒性
-
区分噪声和困难样本的实际机制（多任务学习+物理一致性）没有被形式化、没有被证明、没有被构造

\textbf{Theorem 3'的''破坏''部分是一个未被证明的猜想。}
...
\textbf{性质}: 逻辑错误（不等号方向误用）\\
\textbf{位置}: main.tex §3.4 (line 746-776), md §2.4 (line 557-617)\\
\textbf{具体错误}:
...
\textbf{性质}: 常数因子高估约3.46倍\\
\textbf{位置}: main.tex §2.3 (line 483-492), md §1.3.4 (line 269-276)\\
\textbf{具体错误}:
...
\textbf{性质}: 不可桥接的逻辑跳跃\\
\textbf{位置}: main.tex line 659-661, md line 484\\
\textbf{具体错误}:
...
\textbf{性质}: 推导遗漏\\
\textbf{位置}: main.tex §2.2.2, md §1.2.2\\
\textbf{具体错误}:
\(K_{\PE}(0) = d/2\)而\(k(0) = 1\)。不归一化不可能有收敛。\\
\textbf{可修复性}:
...
\textbf{性质}: 未被支持的声称\\
\textbf{位置}: main.tex §4.3, md §4.3\\
\textbf{具体错误}: ``构造性最小示例''反而证明了Theorem 3的鲁棒性。\\
\textbf{可修复性}:
需要一个真正展示学习型PE能在观测等价数据上产生不同参数的例子。这要求辅助损失涉及未观测变量------这样的例子是可能存在的，但\textbf{没有被构造出来}。\\
\textbf{后果}: Theorem
...
\(\min(1-p_{\text{clean},s}, \ldots)\)
的来源不明}{8.1 定理2.3.1中 \textbackslash min(1-p\_\{\textbackslash text\{clean\},s\}, \textbackslash ldots) 的来源不明}}\label{ux5b9aux74062.3.1ux4e2d-min1-p_textcleans-ldots-ux7684ux6765ux6e90ux4e0dux660e}

为什么上界中有一个\(1-p_{\text{clean},s}\)的项？推导（line
...
的单位歧义}{8.2 定理2.4.1中 \textbackslash log 2 的单位歧义}}\label{ux5b9aux74062.4.1ux4e2d-log-2-ux7684ux5355ux4f4dux6b67ux4e49}
...
标注为\texttt{{[}启发式{]}}。指数衰减对于解析核（Cauchy）的Fourier级数逼近在某些光滑性条件下成立，但这里逼近的是核\(k(\Delta)\)，而信息丢失\(\varepsilon_{\PE}\)涉及条件分布\(P_{Y|X,P}\)和编码造成的KL散度。从核逼近误差到信息丢失，中间需要至少两次Pinsker应用和关于\(P_{Y|X,P}\)光滑性的假设。这些假设没有被陈述。指数衰减函数形式\(\exp(-d/d_0)\)是猜测级别的精度。

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{九、加分题：CC审计报告内部的额外问题}\label{ux4e5dux52a0ux5206ux9898ccux5ba1ux8ba1ux62a5ux544aux5185ux90e8ux7684ux989dux5916ux95eeux9898}
...
\subsection{十、最终评分}\label{ux5341ux6700ux7ec8ux8bc4ux5206}
...
定理2.4.1 & \textbf{1/10} & \textbf{证明是逻辑错误的，不能被称为定理} \\
...
\textbf{总体} & \textbf{4/10} &
\textbf{存在一个致命逻辑错误（2.4.1）、一个量级错误（1.3.1）、一个不可桥接的推广（2.2.1步4）。在修正这些之前不适合发表。} \\
\end{longtable}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{十一、如果要修改，最少需要做哪些事？}\label{ux5341ux4e00ux5982ux679cux8981ux4feeux6539ux6700ux5c11ux9700ux8981ux505aux54eaux4e9bux4e8b}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Theorem 2.4.1}:
  移除''下界''声称，重新标注为''启发式估计''或直接删除。如果保留，需要用不同的方法重新证明。Fano的等号假设需要替换为分布假设下的显式构造。
\item
  \textbf{Theorem 1.3.1}:
  将Lipschitz常数修正为\(\max(\alpha, \beta, \gamma)\)，并修正所有依赖此常数的后续结果（定理1.4.1、定理2.5.1）。
\item
  \textbf{Theorem 1.2.1}:
  在编码定义中加入归一化因子\(1/\sqrt{d/2}\)（或等价地，在内积中加入\(2/d\)），并相应调整定理1.2.2的误差界常数。
\item
  \textbf{Theorem 2.2.1}:
  将结论从''存在\(\PE\)和\(d\)使得\(\delta_s^{\PE} > 0\)``明确限定为''对贝叶斯最优分类器成立''。对实际专家的推广需要单独的学习理论分析（可能涉及VC维、Rademacher复杂度等），或明确标注为猜想。
\item
  \textbf{Theorem 3'}:
  要么提供真正的构造性示例（展示学习型PE在观测等价数据上导致可区分的参数），要么将''破坏方向''的结论改为开放问题。
\item
  \textbf{Theorem 2.3.1}: 澄清\(\min\)项的来源，修正Pinsker应用的条件。
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\emph{审稿人签字: 不用签。这些问题是客观的。}
