...
> **Definition:** Observer as Expert | A measurement apparatus (or conscious observer) $\Obs_m$ is an **expert**. Given a quantum state $\ket{\psi}$, $\Obs_m$ reports an outcome $o_m \in \Omega$ (the spectrum of the measured observable) with probability $\Pbb(o_m \mid \psi) = |\braket{o_m|\psi}|^2$ (Born rule). The report is a **claim** about the pre-measurement state.

> **Definition:** Measurement as Consensus | $M$ independent observers $\Obs_1,\ldots,\Obs_M$ measure identically prepared states $\ket{\psi}^{\otimes M}$. The consensus outcome is the modal report $\hat{o} = \arg\max_o \sum_m \mathbf{1}\{o_m = o\}$. The consensus strength $C = \frac{1}{M}\sum_m \mathbf{1}\{o_m = \hat{o}\}$ measures inter-observer agreement.

## Theorem 1: Multi-Observer Agreement with Finite-$M$ Tight Bounds

> **Theorem:** Multi-Observer Agreement Bound | Let $M$ independent observers measure observable $\hat{O}$ on identically prepared state $\ket{\psi}$ with spectral decomposition $\hat{O} = \sum_{o} \lambda_o \ket{o}\bra{o}$. Define $p_o = |\braket{o|\psi}|^2$ and $p_{\max} = \max_o p_o$.
...
For a state with $p_{\max} = \max_o |\braket{o|\psi}|^2 < 1$, the probability that *at least two observers disagree* exceeds $0.99$ when:
...
**(iv) Asymptotic limit.**
As $M \to \infty$, $\Pbb(all agree) = \sum_o p_o^{M} \to 0$ for any non-deterministic state ($p_{\max} < 1$). The disagreement is exponentially fast in $M$ with rate $\ln(1/p_{\max})$.

> **Proof:** **(i)** By the Born rule, each observer independently reports $o$ with probability $p_o = |\braket{o|\psi}|^2$. The events are independent across observers (identically prepared states on distinct subsystems of $\ket{\psi}^{\otimes M}$). Thus $\Pbb(all report  o) = p_o^M$. Agreement means all $M$ report the *same* outcome (any $o$), giving $\sum_o p_o^M$. Disagreement is the complement $1 - \sum_o p_o^M$.
...
**(iii)** For disagreement to be probable $\geq 0.99$, we need $\Pbb(all agree) \leq 0.01$. Since $\Pbb(all agree) = \sum_o p_o^M \leq |\Omega| \cdot p_{\max}^M$, the condition $|\Omega| \cdot p_{\max}^M \leq 0.01$ suffices. Solving $p_{\max}^M \leq 0.01/|\Omega|$ gives $M \geq \ln(0.01/|\Omega|) / \ln p_{\max}$, yielding the formula for $M_{0.99}$.

**(iv)** For each $o$ with $p_o < 1$, $p_o^M \to 0$ as $M \to \infty$. If $p_{\max} < 1$, all $p_o < 1$ and the sum vanishes. The rate is $\sim p_{\max}^M = e^{-M \ln(1/p_{\max})}$, exponential decay. $\square$

> **Remark:** SCX Interpretation | The Chernoff--Hoeffding bound quantifies how fast the empirical distribution of expert reports concentrates around the Born rule probabilities. For small $M$, expert reports are volatile --- consensus is fragile. For large $M$, the empirical distribution is tightly concentrated, yet individual *unanimity* becomes exponentially unlikely. This tension --- concentration of the histogram vs.\ divergence of individual reports --- is the mathematical core of why wavefunction collapse requires an axiom (the Heisenberg cut) rather than emerging as a theorem.

## Historical Validation: Three Experiments as SCX Special Cases

### Double-Slit: $M=1$ Observer Destroys Interference

> **Example:** Double-Slit as $M=1$ Audit Failure | In the double-slit experiment with single-photon detection:
[nosep]
...
> **Example:** Wigner's Friend as $M=2$ Consensus Failure | Wigner's friend $\Obs_F$ measures $\ket{\psi}$ inside a sealed laboratory, obtaining outcome $o_F$. Wigner $\Obs_W$ stands outside, treating the laboratory as a quantum system.
...
[nosep]
    - **$\Obs_F$ is correct**: collapse occurs upon first conscious observation. $\Obs_W$ is merely unaware.
    - **$\Obs_W$ is correct**: no collapse occurs. $\Obs_F$ is entangled with the system, forming a larger superposition.
    - **Both are correct in their own reference frames**: collapse is observer-relative (Relational QM).
...
> **Theorem:** Weak Measurement as Multi-Expert Audit | Consider a weak measurement of observable $\hat{A}$ on $M$ identically prepared copies of $\ket{\psi}$, with von Neumann interaction Hamiltonian $\hat{H}_{int} = g \cdot \hat{A} \otimes \hat{P}_{meter}$ where $g \ll 1$ is the coupling strength.
...
\brak{\hat{A}}_{weak}^{(M)} = \braket{\psi|\hat{A}|\psi} \pm \sqrt{\frac{\Var_{\psi}[\hat{A}]}{M}} + O(g^2)
...
g_{opt} = \sqrt{\frac{4\sigma^2 \delta}{M \Var_{\psi}[\hat{A}]}}, \qquad M_{opt} = \frac{\Delta_{tot}}{g_{opt}^2}

$$

The statistical error then scales as $O(M_{opt}^{-1/2}) = O(g_{opt})$.

> **Proof:** **(i)** The weak measurement unitary is $\hat{U} = \exp(-i g \hat{A} \otimes \hat{P}_{meter})$. For Gaussian meter $\ket{\Phi}_{meter}$ with $\braket{\hat{P}} = 0$, $\braket{\hat{P}^2} = 1/(4\sigma^2)$, the reduced system state after tracing out the meter is (to $O(g^2)$):
...
> **Corollary:** Why Weak Measurement Was Discovered Late | Weak measurement (Aharonov, Albert, Vaidman 1988) was historically surprising because physicists thought measurement necessarily destroys the state. SCX explains: strong measurement = $M=1$ single-expert audit (destructive). Weak measurement = $M \gg 1$ multi-expert audit (information-preserving). The surprise was not quantum --- it was audit-theoretic. The fidelity $F^{(M)} = 1 - O(M g^2)$ quantifies *how much of the original state (the ``training data'' in SCX terms) survives the audit*.

## Theorem 3: Quantum Darwinism --- Redundancy and Consensus Threshold

> **Theorem:** Quantum Darwinism = Spring Permanent Memory | Let the total environment $\mathcal{E}$ be partitioned into $N$ fragments $\{\mathcal{E}_1, \ldots, \mathcal{E}_N\}$, each interacting with the system $\mathcal{S}$ via the same pointer observable $\hat{X} = \sum_x x \ket{x}\bra{x}$. Quantum Darwinism asserts that information about $\hat{X}$ is redundantly encoded across many fragments.
...
R_\delta = \max\!\left\{k \in \{1,\ldots,N\} : I(\mathcal{S}:\mathcal{E}_j) \geq (1-\delta) \cdot \chi(\mathcal{S}:\hat{X})  for at least  k  distinct  j\right\}
...
> **Corollary:** Bell Test as SCX Audit Protocol | The Bell--CHSH experiment is the simplest non-trivial SCX multi-expert audit:
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
**SCX Component** & **Bell Test Realization** 
...
> **Corollary:** Collapse Is Not a Physical Process --- It Is a Consensus Threshold | Wavefunction collapse occurs when the consensus among observers exceeds a threshold $\theta$. For $M=1$ (single observer), collapse is instantaneous upon measurement --- the observer IS the consensus. For $M \gg 1$ (environment, $R_\delta \gg R_{\min}$), collapse is gradual --- decoherence continuously increases consensus until classicality emerges. The ``Heisenberg cut'' is the choice of $M$: how many observers (or environment fragments) constitute sufficient consensus?

> **Corollary:** The Four Theorems Form a Complete Audit Hierarchy | The four theorems span the full space of SCX multi-expert quantum audit:
[nosep]
    - **Theorem~1 (Agreement)** --- How fast does consensus decay with $M$? Answer: exponentially, with Chernoff--Hoeffding tight bounds.
    - **Theorem~2 (Fidelity)** --- How much state survives the audit? Answer: $F^{(M)} = 1 - O(M g^2)$, trading off precision vs.\ preservation.
    - **Theorem~3 (Redundancy)** --- How many copies are needed for consensus? Answer: $R_{\min} = O(\log(1/(1-\theta)))$, exponentially small in required confidence.
    - **Theorem~4 (Non-classicality)** --- When does the audit reveal structure beyond classical explanation? Answer: when $S > 2$, the experts' correlation pattern requires quantum description.

> **Remark:** HONEST LIMITATION | This paper does **not** solve the measurement problem. It reframes it: the question ``when does collapse occur?'' becomes ``how many independent observers must agree before we treat the outcome as objective?'' The answer depends on the declared $M$ (or $R_\delta$, or $N$), which is an axiom --- not a theorem. SCX cannot eliminate the need for axioms in quantum foundations. It can only make them explicit. The value added is quantitative: the tight bounds (Chernoff--Hoeffding, fidelity decay, $R_{\min}$, CHSH confidence) replace qualitative hand-waving with numerically precise statements about when and how consensus---and therefore classicality---emerges.