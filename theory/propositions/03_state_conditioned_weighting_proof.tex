...
\[\varphi(\lambda P_1 + (1-\lambda) P_2) \geq \lambda \varphi(P_1) + (1-\lambda) \varphi(P_2)\]

where the inequality holds because the minimum of a sum is at least the
sum of the minima. This establishes that \(\varphi\) is \textbf{concave}.
...
\(w_{\text{global}} \approx \mathbb{E}_s[w^*(s)]\)), we obtain:
...
\(\mathbb{E}[(\ell - \hat{R})(w - w')]\). The sign of this expectation determines whether the
...
\item
  ...
  Section 2.3
\item
  SCX mathematical foundations. \texttt{05\_Mathematical\_Roots\_and\_Proofs.md}
  Proposition 3
\item
...
\begin{longtable}[]{@{}p{0.18\linewidth}p{0.23\linewidth}p{0.23\linewidth}p{0.28\linewidth}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Date
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\bottomrule\noalign{}
\endlastfoot
2026-06-28 & B4 & \textbf{Added entropy inequality} (\S8.1):
...
MAJOR \\
2026-06-28 & B5 & \textbf{Corrected correlation claim} (\S8.2): Replaced
