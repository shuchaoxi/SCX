...
> **FIXED (2026-06-28, DEFECT-12):** 对 $K>2$ 的构造使用**完全随机专家**（$f_m \perp y^* \mid s_1$）——这是"难度"的一种极端/退化形式。对于 $K=2$，构造使用现实形式的难度（专家偏向类别 0），但 $K>2$ 的构造需要完全随机专家来确保联合分布在所有类别上匹配。这意味着：虽然**原则上**存在某种"难度"分布与噪声不可区分，但能实现这种不可区分性的"难度"类别**非常狭窄**。现实世界中的困难样本通常具有**结构化**的专家错误模式（如专家一致错误地预测某个特定错误类别），这在附加温和假设（如 A5 状态同质性或 A6 平衡误差分布）下可能与噪声区分开来。**$K=2$ 的构造具有最强的实践相关性**，因为它使用现实形式的专家偏向性；$K>2$ 的构造主要作为存在性证明，其极端假设（完全随机专家）在现实 MLIP/医学图像/LLM 场景中几乎不可能出现。
