...
生成日期：2026-06-25 \textbar{} 版本：v0.1.0-draft
\end{quote}
...
\tightlist
\item
  \hyperref[ux6a21ux5757-1ux72b6ux6001ux53d1ux73b0ux4e0eux8868ux793a-state-discovery--representation]{模块
  1：状态发现与表示}
\item
  \hyperref[ux6a21ux5757-2ux4e13ux5bb6ux53efux9760ux6027ux4f30ux8ba1-expert-reliability-estimation]{模块
  2：专家可靠性估计}
\item
  \hyperref[ux6a21ux5757-3ux6570ux636eux56dbux5206ux7c7b-data-four-classification]{模块
  3：数据四分类}
\item
  \hyperref[ux6a21ux5757-4ux72b6ux6001ux6570ux636eux4f30ux503c-state-data-valuation]{模块
  4：状态数据估值}
\item
  \hyperref[ux6a21ux5757-5ux4e13ux5bb6ux8defux7531ux4e0eux4ef2ux88c1-expert-routing--arbitration]{模块
  5：专家路由与仲裁}
\item
  \hyperref[ux6a21ux5757-6scx-compressux5197ux4f59ux538bux7f29]{模块
  6：SCX-Compress（冗余压缩）}
\item
  \hyperref[ux6a21ux5757-7ux4e3bux52a8ux5b66ux4e60ux7b56ux7565-active-learning-policy]{模块
  7：主动学习策略}
\item
  \hyperref[ux6a21ux5757-8ux4e0e-paper-1-3-ux7684ux8fdeux63a5]{模块
  8：与 Paper 1-3 的连接}
\item
  \hyperref[ux5b8cux6574ux7cfbux7edfux67b6ux6784ux56fe]{完整系统架构图}
\end{enumerate}
...
\subsubsection{1.1 问题设定}\label{ux95eeux9898ux8bbeux5b9a}

给定原始输入空间 \(\mathcal{X}\)（原子结构、图像、文本等）和一个表示映射
\(\phi: \mathcal{X} \to \mathbb{R}^d\)，状态发现的目标是将高维、异构的输入空间划分为
\(K\) 个\textbf{语义上有意义的状态}
\(S = \{S_1, S_2, \dots, S_K\}\)，使得每个状态内部的样本具有相似的结构/语义属性，而不同状态之间的样本在统计和行为上存在显著差异。

\subsubsection{1.2 数学模型}\label{ux6570ux5b66ux6a21ux578b}

\paragraph{1.2.1 表示映射}\label{ux8868ux793aux6620ux5c04}
...
\tightlist
\item
  \textbf{ACE (Atomic Cluster Expansion)}：将原子邻域环境展开为 \(d\) 维
  bispectrum 系数，\(d \sim O(N_{\text{max}}^3)\) 其中
  \(N_{\text{max}}\) 是截断阶数
\item
  \textbf{SOAP (Smooth Overlap of Atomic
  Positions)}：利用高斯核构造原子环境相似性，\(d \sim n_{\text{max}} \times l_{\text{max}}\)
\item
  \textbf{神经网络
  embedding}：\(h(x) = \text{Encoder}(x; \theta) \in \mathbb{R}^d\)
\item
  \textbf{CLIP embedding}：多模态对齐表示，\(d = 512\) 或 \(768\)
\end{itemize}

\paragraph{1.2.2 降维}\label{ux964dux7ef4}
...
\paragraph{1.2.3 状态分配}\label{ux72b6ux6001ux5206ux914d}
...
\item
  \textbf{贝叶斯信息准则
  (BIC)}：\(\text{BIC}(K) = -2 \log \mathcal{L}(K) + p_K \log N\)，选择使
  BIC 最小的 \(K\)
\item
  \textbf{轮廓系数 (Silhouette
  Score)}：\(S(i) = \frac{b(i) - a(i)}{\max(a(i), b(i))}\)，其中
  \(a(i)\) 是类内距离，\(b(i)\) 是最近类间距离
\item
  \textbf{稳定性分析}：在不同随机初始化和 bootstrap
  子集上运行聚类，选择最稳定的 \(K\)
\end{itemize}
...
\subsubsection{1.4 实现方法}\label{ux5b9eux73b0ux65b9ux6cd5}
...
\subsubsection{1.6 计算复杂度}\label{ux8ba1ux7b97ux590dux6742ux5ea6}
...
\subsubsection{1.7 关键超参数}\label{ux5173ux952eux8d85ux53c2ux6570}
...
聚类型别 & - & soft & \texttt{soft} (GMM) / \texttt{hard} (K-means) \\
...
\subsubsection{2.1 问题设定}\label{ux95eeux9898ux8bbeux5b9a-1}

给定 \(M\) 个专家 \(\{f_1, f_2, \dots, f_M\}\)，每个专家是一个函数
\(f_m: \mathcal{X} \to \mathcal{Y}\)。核心问题是：对于每个专家 \(m\)
和每个状态 \(s\)，\textbf{专家在该状态下的可靠性是多少？}
这里的可靠性在不同标注预算场景下有不同定义。

\subsubsection{2.2 数学模型}\label{ux6570ux5b66ux6a21ux578b-1}
...
\textbf{定义 1：状态条件专家风险 (State-Conditioned Expert Risk)}
...
\textbf{定义 2：SCX 可靠性 (SCX Reliability)}
...
当状态 \(s\)
的标注样本很少（\(N_s < N_{\min}\)），直接经验估计不可靠。采用以下方法：

\textbf{方法 A：贝叶斯平滑 (Bayesian Smoothing)}
...
\textbf{方法 B：层次贝叶斯模型 (Hierarchical Bayesian)}
...
\textbf{方法 C：迁移估计 (Transfer Estimation)}
...
\subsubsection{2.4 实现方法}\label{ux5b9eux73b0ux65b9ux6cd5-1}
...
\bottomrule\noalign{}
\endlastfoot
模块 3 (四分类) & \(R_m(s)\), \(\text{SCX}_m(s)\) & 计算 C(s),
ExpertConflict \\
模块 4 (估值) & \(R_m(s)\), \(\text{SCX}_m(s)\) & 计算 max\_m
SCX\_m(s) \\
模块 5 (路由) & \(R_m(s)\), \(\text{SCX}_m(s)\) & 硬路由和软加权 \\
模块 7 (主动学习) & \(\max_m \text{SCX}_m(s)\) & route 动作的效用计算 \\
\end{longtable}

\subsubsection{2.6 计算复杂度}\label{ux8ba1ux7b97ux590dux6742ux5ea6-1}
...
\subsubsection{2.7 关键超参数}\label{ux5173ux952eux8d85ux53c2ux6570-1}
...
\subsubsection{3.1 问题设定}\label{ux95eeux9898ux8bbeux5b9a-2}

数据分类不是对原始数据标签的分类，而是对每个状态 \(s\)
的\textbf{信息类型分类}。目标是判断每个状态属于以下四类中的哪一类：\textbf{Valuable}（值得标注）、\textbf{Redundant}（冗余）、\textbf{Noisy}（噪声）、\textbf{Expert-dependent}（依赖特定专家）。这个分类直接驱动后续的决策（标注、压缩、路由等）。

\subsubsection{3.2 数学模型}\label{ux6570ux5b66ux6a21ux578b-2}
...
\textbf{度量 A：标签一致性 (Label Consistency)}
...
当只有单一 oracle 标注时，用\textbf{状态内方差}：
...
\textbf{度量 B：专家预测一致性 (Expert Agreement Consistency)}
...
\textbf{度量 C：密度邻域一致性 (Density Neighborhood Consistency)}
...
分解分析： - \textbf{高误差 + 高密度 + 高一致性} →
\(\text{NoiseScore} \to 0\) → 可学习困难样本 - \textbf{高误差 + 低密度 +
低一致性} → \(\text{NoiseScore} \to \text{large}\) → 疑似噪声 -
\textbf{低误差} → \(\text{NoiseScore} \to 0\) → 已正确预测
...
\textbf{定义}：冗余度 \(D(s) \in [0,1]\) 衡量状态 \(s\)
在当前训练集中被充分覆盖的程度。
...
\paragraph{3.2.6 四分类规则}\label{ux56dbux5206ux7c7bux89c4ux5219}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Valuable} & 高误差 + 高密度 + 高可学习性 + 低冗余 &
\(\bar{r}(s) > \tau_r\), \(\rho(s) > \tau_\rho\), \(L(s) > \tau_L\),
\(D(s) < \tau_D\) \\
\textbf{Redundant} & 低误差 + 高冗余 & \(\bar{r}(s) < \tau_r\),
\(D(s) > \tau_D\) \\
\textbf{Noisy} & 高误差 + 低密度 + 低一致性 & \(\bar{r}(s) > \tau_r\),
\(\rho(s) < \tau_\rho\), \(C(s) < \tau_C\) \\
\textbf{Expert-dependent} & 高专家分歧 + 某专家明显优于其他 &
...
\subsubsection{3.4 实现方法}\label{ux5b9eux73b0ux65b9ux6cd5-2}
...
\tightlist
\item
  \textbf{空状态 (Empty State)}：若 \(\rho(s) \approx 0\)，标记为
  REDUNDANT（无数据可贡献）
\item
  \textbf{新状态 (Novel State)}：若 \(N_s < N_{\min}\)，标记为
  VALUABLE（默认值得探索）
\item
  \textbf{高冲突状态 (High Conflict)}：若
  \(\text{Conflict}(s) > \tau_{\text{conflict}}\) 且
  \(C(s) > \tau_C\)，标记为 EXPERT\_DEPENDENT
\item
  \textbf{全噪声状态 (Fully Noisy)}：若
  \(L(s) < \varepsilon_L\)，直接标记为 NOISY
\end{enumerate}
...
\(V_{\text{add}}(s)\) 和 \(V_{\text{remove}}(s)\) \\
模块 6 (压缩) & \(D(s)\), \(\text{boundary\_idx}\) &
决定保留/丢弃数据 \\
模块 7 (主动学习) & \(\text{classification}\), \(L(s)\) &
选择动作和分配预算 \\
模块 5 (路由) & \(\text{ExpertConflict}(s)\) & 触发仲裁 \\
\end{longtable}

\subsubsection{3.7 计算复杂度}\label{ux8ba1ux7b97ux590dux6742ux5ea6-2}
...
\subsubsection{3.8 关键超参数}\label{ux5173ux952eux8d85ux53c2ux6570-2}
...
\subsubsection{4.1 问题设定}\label{ux95eeux9898ux8bbeux5b9a-3}

数据估值的核心是回答两个问题： 1. \textbf{采集价值}
\(V_{\text{add}}(s)\)：如果现在从状态 \(s\)
获取更多标注样本，对模型性能的提升有多大？ 2. \textbf{压缩价值}
\(V_{\text{remove}}(s)\)：如果移除状态 \(s\)
中的现有样本，模型性能损失有多大？

这两个价值是互补的，分别驱动主动学习和数据压缩。

\subsubsection{4.2 数学模型}\label{ux6570ux5b66ux6a21ux578b-3}
...
\(\max_m \text{SCX}_m(s)\) & 最好专家的可靠性 &
有可靠专家才能获取高质量标签 \\
\end{longtable}

\textbf{动态性}：\(V_{\text{add}}(s)\)
...
其中 \(\text{Cost}_{\text{acquire}}(s)\) 是从状态 \(s\)
获取标注的成本（如 DFT 计算时间）。
...
\tightlist
\item
  高 \(\bar{r}(s)\) + 高 \(D(s)\)：模型在状态 \(s\) 上表现差但数据冗余 →
  可以移除部分冗余
\item
  低 \(\rho(s)\)：低频状态的移除影响有限
\item
...
\textbf{SCX 状态估值与 Shapley 的关系}：

SCX 的状态估值是对 Shapley 值的\textbf{状态级近似}：
...
\textbf{影响函数}（样本 \(i\) 对模型参数的影响）：

\[
\mathcal{I}(x_i) = -H_\theta^{-1} \nabla_\theta \ell(x_i; \theta)
\]

\textbf{SCX 状态估值与影响函数的关系}：
...
对于 MLIP 场景，\(\text{Cost}_{\text{label}}(x)\) 可能是 DFT
计算成本，与原子数、计算精度相关。
...
\subsubsection{4.4 实现方法}\label{ux5b9eux73b0ux65b9ux6cd5-3}
...
\bottomrule\noalign{}
\endlastfoot
模块 7 (主动学习) & \(V_{\text{add}}(s)\), \(\text{ranking\_add}\) &
采集函数的输入 \\
模块 6 (压缩) & \(V_{\text{remove}}(s)\), \(\text{ranking\_remove}\) &
压缩优先级 \\
人类可读报告 & ranking 信息 & 可视化哪些状态最有价值 \\
\end{longtable}

\subsubsection{4.6 计算复杂度}\label{ux8ba1ux7b97ux590dux6742ux5ea6-3}
...
\subsubsection{4.7 关键超参数}\label{ux5173ux952eux8d85ux53c2ux6570-3}
...
\subsubsection{5.1 问题设定}\label{ux95eeux9898ux8bbeux5b9a-4}

在多专家系统中，对于给定的输入
\(x\)，我们需要决定：\textbf{哪个（或哪些）专家最适合处理这个样本？}
路由策略分为硬路由（选一个专家）和软路由（加权组合多个专家）。当专家之间出现显著分歧时，还需要仲裁机制。

\subsubsection{5.2 数学模型}\label{ux6570ux5b66ux6a21ux578b-4}
...
\textbf{策略 A：加权集成 (Weighted Ensemble)}

使用 5.2.2 节的软权重。

\textbf{策略 B：最佳专家 (Best Expert)}

\[
f(x) = f_{m^*}(x), \quad m^* = \arg\min_m \sum_s \gamma_s(x) \cdot \hat{R}_m(s)
\]

\textbf{策略 C：置信度过滤 (Confidence Filtering)}
...
\textbf{策略 D：元专家 (Meta-Expert)}

训练一个轻量级元模型 \(g: \mathbb{R}^M \to \mathcal{Y}\)，以专家输出
\([f_1(x), \dots, f_M(x)]\) 为输入：
...
\textbf{传统 MoE (Mixture of Experts)}：
...
\textbf{互补性}：SCX 路由可以作为 MoE
门控的\textbf{初始化}或\textbf{正则化约束}。
...
\subsubsection{5.4 实现方法}\label{ux5b9eux73b0ux65b9ux6cd5-4}
...
\bottomrule\noalign{}
\endlastfoot
模块 7 (主动学习) & \(\text{conflict\_score}\) & relabel
动作的触发条件 \\
\end{longtable}

\subsubsection{5.6 计算复杂度}\label{ux8ba1ux7b97ux590dux6742ux5ea6-4}
...
\bottomrule\noalign{}
\endlastfoot
获取状态分配 & \(O(\text{batch} \cdot K \cdot d_\phi)\) &
\(O(\text{batch} \cdot K)\) \\
计算加权风险 & \(O(\text{batch} \cdot M \cdot K)\) &
\(O(\text{batch} \cdot M)\) \\
专家推理 & \(O(\text{batch} \cdot M \cdot C_f)\) &
\(O(\text{batch} \cdot M)\) \\
冲突检测 & \(O(\text{batch} \cdot M^2)\) &
\(O(\text{batch} \cdot M)\) \\
\end{longtable}

\subsubsection{5.7 关键超参数}\label{ux5173ux952eux8d85ux53c2ux6570-4}
...
SCX 置信度阈值 & \(\tau_{\text{SCX}}\) & \(0.7\) & 置信度过滤的最小
SCX \\
...
\subsubsection{6.1 问题设定}\label{ux95eeux9898ux8bbeux5b9a-5}

给定训练数据集 \(\mathcal{D} = \{(x_i, y_i)\}_{i=1}^N\)，SCX-Compress
的目标是找到一个\textbf{加权子集}
\(\mathcal{D}_c = \{(x_i, y_i, w_i)\}\)，使得： 1.
\textbf{模型性能保真}：在 \(\mathcal{D}_c\) 上训练的性能接近在
\(\mathcal{D}\) 上训练的性能 2.
\textbf{状态结构保留}：状态划分和专家可靠性估计在压缩前后的变化不超过容差
3. \textbf{压缩率最大化}：\(|\mathcal{D}_c| \ll |\mathcal{D}|\)

\subsubsection{6.2 数学模型}\label{ux6570ux5b66ux6a21ux578b-5}
...
分解分析： - \textbf{高 \(D(s_i)\)}：状态已被充分覆盖 → 数据可能冗余 -
\textbf{高 \(C(s_i)\)}：状态内一致性高 → 样本信息不独特 - \textbf{低
\(\max_m \text{SCX}_m(s_i)\)}：没有专家特别可靠 →
不需要保留专家特定的样本
...
\textbf{SCX-Compress 区别于传统 Coreset}：
...
\textbf{Dataset Distillation}：合成少量伪样本 \(\tilde{\mathcal{D}}\)
使得：
...
\textbf{关系}： - SCX-Compress 是\textbf{选择型}压缩（select existing
data points） - Dataset distillation 是\textbf{合成型}压缩（generate new
synthetic data points） - 两者可以\textbf{互补}：先用 SCX-Compress
...
\subsubsection{6.4 实现方法}\label{ux5b9eux73b0ux65b9ux6cd5-5}

\textbf{算法 6：SCX-Compress}
...
\(\text{boundary\_idx}\) & 冗余分数计算 \\
模块 4 (估值) & \(V_{\text{remove}}(s)\) & 压缩优先级排序 \\
\end{longtable}

\subsubsection{6.6 计算复杂度}\label{ux8ba1ux7b97ux590dux6742ux5ea6-5}
...
\subsubsection{6.7 关键超参数}\label{ux5173ux952eux8d85ux53c2ux6570-5}
...
\subsubsection{7.1 问题设定}\label{ux95eeux9898ux8bbeux5b9a-6}

主动学习的核心问题：在有限的标注预算 \(B\)
下，选择哪些样本进行标注，以及选择何种标注方式，能最大化模型性能提升。SCX
将这个决策从\textbf{逐样本 (point-wise)} 升级为\textbf{逐状态
(state-wise)}，并且拓展了动作空间------不只是''标注哪个''，还包括''如何标注''。

\subsubsection{7.2 数学模型}\label{ux6570ux5b66ux6a21ux578b-6}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{acquire} & 从状态 \(s\) 获取新标注样本 & 高（如 DFT 计算时间） &
增加训练数据量 \\
\textbf{relabel} & 对状态 \(s\) 中已有样本重新标注（找更强 oracle） &
中（专家时间） & 修正错误标签 \\
\textbf{downweight} & 降低状态 \(s\) 中疑似噪声样本的权重 &
低（计算操作） & 减少噪声影响 \\
\textbf{discard} & 丢弃状态 \(s\) 中的极端噪声样本 & 低（删除操作） &
消除有害数据 \\
\textbf{route} & 将状态 \(s\) 的标注任务路由到最可靠的专家 &
中（推理成本） & 提高标签质量 \\
\end{longtable}
...
其中 \(V_{\text{add}}(s)\) 来自模块 4，\(\text{Cost}_{\text{label}}(s)\)
是在状态 \(s\) 中标注一个样本的成本。
...
路由到最可靠专家，适用于 \(\text{Conflict}(s)\) 高但 \(\text{SCX}\)
差异大的状态。
...
\textbf{为什么要 State-wise}：

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{统计效率}：状态级聚合减少了估计方差（借用''同状态样本共享信息''）
\item
  \textbf{计算效率}：状态数 \(K \ll N\)，优化在 \(O(K)\) 空间而非
  \(O(N)\)
\item
  \textbf{语义意义}：状态提供可解释的决策单元
\item
  \textbf{多样性}：避免只关注不确定性最高的离群点
\end{enumerate}
...
\textbf{D-optimality}：选择设计矩阵 \(X\) 使得 \(\det(X^\top X)\)
最大，最小化参数估计的置信椭圆体积。

\textbf{对比}： - D-optimality 只关注线性模型参数估计的方差 - SCX-AL
...
- SCX-AL：\(s^* = \arg\max_s V_{\text{add}}(s)\), 然后在 \(s^*\)
...
\subsubsection{7.4 实现方法}\label{ux5b9eux73b0ux65b9ux6cd5-6}

\textbf{算法 7：SCX 主动学习策略}
...
\tightlist
\item
  \textbf{预算耗尽}: \(\text{budget\_remaining} \leq 0\)
\item
  \textbf{边际效用为零}:
  \(\max_{a,s} U(a,s) \leq 0\)（所有动作都不值得）
\item
  \textbf{性能达标}: 验证集性能达到目标阈值
\item
  \textbf{状态饱和}: 所有状态的 \(D(s) > \tau_{D,\text{stop}}\) 或
  \(L(s) < \tau_{L,\text{stop}}\)
\end{enumerate}
...
模块 2 (可靠性估计) & \(R_m(s)\), \(\text{SCX}_m(s)\) & 路由动作和
relabel 条件 \\
模块 3 (四分类) & \(\text{classification}\), \(L(s)\), \(N(s)\) &
动作选择 \\
模块 4 (估值) & \(V_{\text{add}}(s)\), \(V_{\text{remove}}(s)\) &
采集和压缩的效用核心 \\
\end{longtable}
...
\subsubsection{7.7 计算复杂度}\label{ux8ba1ux7b97ux590dux6742ux5ea6-6}
...
\subsubsection{7.8 关键超参数}\label{ux5173ux952eux8d85ux53c2ux6570-6}
...
停止冗余阈值 & \(\tau_{D,\text{stop}}\) & \(0.9\) &
所有状态超此值则停止 \\
停止可学习阈值 & \(\tau_{L,\text{stop}}\) & \(0.05\) &
所有状态低于此值则停止 \\
...
\subsubsection{8.1 问题设定}\label{ux95eeux9898ux8bbeux5b9a-7}
...
势函数的合并问题。核心挑战： - 不同 ACE 势函数使用不同的\textbf{规范
(gauge)}，不能直接线性组合 -
...
\tightlist
\item
  \textbf{专家不是等价的}：不同势函数在不同结构区域表现不同 → \(R_m(s)\)
  的动机
\item
  \textbf{全局指标不够}：势函数整体的 RMSE 掩盖了局部行为差异 → 命题 1
  的动机
\item
  \textbf{规范差异}：专家之间存在系统偏差 → SCX 中的冲突检测
\item
  \textbf{合并不是唯一方法}：与其合并专家，不如基于状态选择专家 →
  路由的动机
\end{enumerate}

\paragraph{8.3.3 形式化连接}\label{ux5f62ux5f0fux5316ux8fdeux63a5}
...
Paper 2 提出了\textbf{残差状态图 (Residual-State
Maps)}：将势函数的预测误差映射到结构状态空间，以便直观识别哪些结构区域是某个势函数的薄弱环节。

\textbf{核心方法}： 1. 对每个样本 \(x_i\)，计算残差
...
\tightlist
\item
  \textbf{状态空间的可视化}：通过残差热力图发现模式 → 状态发现模块
\item
  \textbf{高误差 ≠ 高价值}：Paper 2 显示有些高误差状态是''不可救药的'' →
  命题 2
\item
  \textbf{状态边界的过渡区域}：观察到边界附近误差最高 → 边界检测
\item
  \textbf{误差归因}：将误差归因到具体结构状态 → \(V_{\text{add}}(s)\)
  中的 \(r̄(s)\)
\end{enumerate}

\paragraph{8.4.3 形式化连接}\label{ux5f62ux5f0fux5316ux8fdeux63a5-1}
...
Paper 3 构建了一个\textbf{多专家势函数平台 (Expert Compiler)}，能够： 1.
...
\tightlist
\item
  \textbf{路由基础设施}：Paper 3 实现了初步的路由机制 → 模块 5
\item
  \textbf{蒸馏作为通道}：将专家知识传递给轻量模型 → 软路由的逻辑
\item
  \textbf{动态专家组合}：根据输入特征调整专家权重 → \(w_m(x)\) 的动机
\item
  \textbf{处理异质专家}：不同架构、不同训练数据的专家 → 通用框架的必要性
\end{enumerate}

\paragraph{8.5.3 形式化连接}\label{ux5f62ux5f0fux5316ux8fdeux63a5-2}
...
\paragraph{8.6.1 概念映射}\label{ux6982ux5ff5ux6620ux5c04}
...
\bottomrule\noalign{}
\endlastfoot
专家 \(f_m\) & ACE 势函数 \(f_1, f_2, \dots\) & 被评估的势函数 &
编译器管理的势函数池 \\
状态 \(s\) & 化学环境类型 & 残差图中的结构聚类 & 结构特征区间 \\
\(R_m(s)\) & 势函数在特定环境下 RMSE & 残差状态图的值 &
验证集上每专家每结构误差 \\
\(\text{SCX}_m(s)\) & 势函数在环境下 \textless{} 阈值概率 & (隐式)
可接受残差区域 & 专家在结构上的置信度 \\
\(V_{\text{add}}(s)\) & 增加某环境训练数据的价值 & (隐式)
高残差+高密度状态 & 需要补充训练的区域 \\
...
\textbf{统一目标}：从多专家系统中学习，提高数据效率和模型可靠性。

\textbf{方法论演化}： 1. \textbf{Paper
1}：用规范变换合并专家（消除分歧） 2. \textbf{Paper
2}：可视化专家分歧的分布（理解分歧） 3. \textbf{Paper
3}：构建系统管理分歧（利用分歧） 4. \textbf{Paper 4
...
\tightlist
\item
  \textbf{理论指导实践}：SCX 为 Paper 3 的编译器提供优化目标（最大化
  \(V_{\text{add}}\)）
\item
  \textbf{实践验证理论}：Paper 1-3 的实验数据为 SCX 的命题提供实证
\item
  \textbf{跨领域泛化}：SCX 的抽象使得方法可以迁移到 MLIP
  之外的领域（医学图像、NLP 等）
\item
  \textbf{持续迭代}：Paper 3 的编译器可作为 SCX 的实验平台，SCX
  的理论可指导编译器的改进
\end{enumerate}

\subsubsection{8.8 计算复杂度}\label{ux8ba1ux7b97ux590dux6742ux5ea6-7}
...
\bottomrule\noalign{}
\endlastfoot
Paper 1 & \(O(N \cdot C_{\text{ACE}} \cdot M)\) & 提供初始 \(R_m(s)\)
估计 \\
...
\subsection{完整系统架构图}\label{ux5b8cux6574ux7cfbux7edfux67b6ux6784ux56fe}
...
\subsubsection{数据流摘要}\label{ux6570ux636eux6d41ux6458ux8981}
...
\begin{quote}
\textbf{专家的可靠性不是全局常数，而是状态条件的；数据的价值不是固有属性，而是相对于当前模型、状态空间和可用专家的条件量。}
\end{quote}

八个子模块构成了一个完整的闭环系统： 1.
\textbf{状态发现}将异质输入空间分解为语义一致的状态 2.
\textbf{可靠性估计}量化每个专家在每个状态的表现 3.
\textbf{数据分类}将状态分为四类，驱动不同的决策 4.
\textbf{数据估值}给出每个状态的采集和压缩价值 5.
\textbf{路由仲裁}基于状态条件可靠性做专家选择 6.
\textbf{压缩}在保真约束下移除冗余数据 7.
\textbf{主动学习}在状态级做预算分配和动作选择 8.
\textbf{论文连接}将这四篇工作的层次关系和传承路径说清楚

SCX
的价值在于它不是一个拼凑的工具箱，而是一个\textbf{数学上自洽的框架}------每个模块的输出是下一个模块的输入，所有公式共享一致的状态空间基础，最终形成一个可解释、可扩展的主动学习+专家管理闭环。
