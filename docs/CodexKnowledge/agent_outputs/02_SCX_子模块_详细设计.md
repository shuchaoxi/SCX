...
1. [模块 1：状态发现与表示](#模块-1状态发现与表示-state-discovery--representation)
2. [模块 2：专家可靠性估计](#模块-2专家可靠性估计-expert-reliability-estimation)
3. [模块 3：数据四分类](#模块-3数据四分类-data-four-classification)
4. [模块 4：状态数据估值](#模块-4状态数据估值-state-data-valuation)
5. [模块 5：专家路由与仲裁](#模块-5专家路由与仲裁-expert-routing--arbitration)
6. [模块 6：SCX-Compress（冗余压缩）](#模块-6scx-compress冗余压缩)
7. [模块 7：主动学习策略](#模块-7主动学习策略-active-learning-policy)
8. [模块 8：与 Paper 1-3 的连接](#模块-8与-paper-1-3-的连接)
9. [完整系统架构图](#完整系统架构图)
...
给定原始输入空间 $\mathcal{X}$（原子结构、图像、文本等）和一个表示映射 $\phi: \mathcal{X} \to \mathbb{R}^d$，状态发现的目标是将高维、异构的输入空间划分为 $K$ 个**语义上有意义的状态** $S = \{S_1, S_2, \dots, S_K\}$，使得每个状态内部的样本具有相似的结构/语义属性，而不同状态之间的样本在统计和行为上存在显著差异。

### 1.2 数学模型

#### 1.2.1 表示映射

定义特征提取函数 $\phi: \mathcal{X} \to \mathbb{R}^d$，将原始输入映射到 $d$ 维特征空间。在科学计算场景中，常见选择：

- **ACE (Atomic Cluster Expansion)**：将原子邻域环境展开为 $d$ 维 bispectrum 系数，$d \sim O(N_{\text{max}}^3)$ 其中 $N_{\text{max}}$ 是截断阶数
- **SOAP (Smooth Overlap of Atomic Positions)**：利用高斯核构造原子环境相似性，$d \sim n_{\text{max}} \times l_{\text{max}}$
- **神经网络 embedding**：$h(x) = \text{Encoder}(x; \theta) \in \mathbb{R}^d$
- **CLIP embedding**：多模态对齐表示，$d = 512$ 或 $768$

#### 1.2.2 降维

原始表示 $\phi(x) \in \mathbb{R}^d$ 通常维度较高（ACE 可达数千维），需要降维到 $p \ll d$ 以进行有效的状态发现。
...
| 模块 3 (数据四分类) | $\gamma_s(x)$, $\rho(s)$, 状态边界 | 计算 C(s), N(s), D(s) |
...
其中 $C_\phi$ 是 $\phi$ 的计算成本，$T$ 是迭代次数，$k$ 是 UMAP 的近邻数。
...
| 降维维度 | $p$ | $2$-$50$ | 降维后维度（可视化常用2-3，聚类常用10-50） |
...
给定 $M$ 个专家 $\{f_1, f_2, \dots, f_M\}$，每个专家是一个函数 $f_m: \mathcal{X} \to \mathcal{Y}$。核心问题是：对于每个专家 $m$ 和每个状态 $s$，**专家在该状态下的可靠性是多少？** 这里的可靠性在不同标注预算场景下有不同定义。

### 2.2 数学模型

#### 2.2.1 有标签场景 (Supervised)

设有标签数据集 $\mathcal{D}_L = \{(x_i, y_i)\}_{i=1}^{N_L}$，其中 $y_i = f^*(x_i)$ 来自 oracle（如 DFT 计算、专家标注）。
...
即专家 $m$ 在状态 $s$ 中做出可接受预测（误差低于阈值 $\tau$）的概率。
...
其中 $\sigma(\cdot)$ 是 sigmoid 函数，$\mu_D, \sigma_D$ 是所有 $(m,s)$ 对的分歧均值和标准差。

#### 2.2.3 小样本场景 (Few-shot)

当状态 $s$ 的标注样本很少（$N_s < N_{\min}$），直接经验估计不可靠。采用以下方法：
...
其中 $\hat{R}_m(\cdot) = \frac{1}{K} \sum_{s'} \hat{R}_m(s')$ 是专家 $m$ 的全局平均风险，$\lambda$ 是平滑强度。
...
$$
R_m(s) \sim \mathcal{N}(\mu_m, \tau_m^2) \quad \text{(先验)}
$$
$$
\ell_{mi} \mid R_m(s) \sim \mathcal{N}(R_m(s), \sigma_m^2 / w_{si}) \quad \text{(似然)}
$$
...
| 数据消费者 | 接收的数据 | 用途 |
|-----------|-----------|------|
| 模块 3 (四分类) | $R_m(s)$, $\text{SCX}_m(s)$ | 计算 C(s), ExpertConflict |
| 模块 4 (估值) | $R_m(s)$, $\text{SCX}_m(s)$ | 计算 max_m SCX_m(s) |
| 模块 5 (路由) | $R_m(s)$, $\text{SCX}_m(s)$ | 硬路由和软加权 |
| 模块 7 (主动学习) | $\max_m \text{SCX}_m(s)$ | route 动作的效用计算 |
...
数据分类不是对原始数据标签的分类，而是对每个状态 $s$ 的**信息类型分类**。目标是判断每个状态属于以下四类中的哪一类：**Valuable**（值得标注）、**Redundant**（冗余）、**Noisy**（噪声）、**Expert-dependent**（依赖特定专家）。这个分类直接驱动后续的决策（标注、压缩、路由等）。
...
- $L(s) \to 1$：状态高度可学习（一致且无噪声）
- $L(s) \to 0$：状态不可学习（不一致或有噪声）

#### 3.2.2 一致性度量 (Consistency Measures)
...
分解分析：
- **高误差 + 高密度 + 高一致性** → $\text{NoiseScore} \to 0$ → 可学习困难样本
- **高误差 + 低密度 + 低一致性** → $\text{NoiseScore} \to \text{large}$ → 疑似噪声
- **低误差** → $\text{NoiseScore} \to 0$ → 已正确预测
...
#### 3.2.4 冗余度估计 (Redundancy Estimation)

**定义**：冗余度 $D(s) \in [0,1]$ 衡量状态 $s$ 在当前训练集中被充分覆盖的程度。
...
其中 $\mathbb{H}[\cdot]$ 是预测熵。当模型对状态内所有样本都确定时，$D(s) \to 1$。
...
- $\text{BoundaryScore} \to 0$：明确属于某个状态
- $\text{BoundaryScore} \to 1$：在状态边界上，分配模糊
...
| 类别 | 判定条件 | 符号条件 |
|------|---------|---------|
| **Valuable** | 高误差 + 高密度 + 高可学习性 + 低冗余 | $\bar{r}(s) > \tau_r$, $\rho(s) > \tau_\rho$, $L(s) > \tau_L$, $D(s) < \tau_D$ |
| **Redundant** | 低误差 + 高冗余 | $\bar{r}(s) < \tau_r$, $D(s) > \tau_D$ |
| **Noisy** | 高误差 + 低密度 + 低一致性 | $\bar{r}(s) > \tau_r$, $\rho(s) < \tau_\rho$, $C(s) < \tau_C$ |
| **Expert-dependent** | 高专家分歧 + 某专家明显优于其他 | $\max_m \text{SCX}_m(s) \gg \text{SCX}_{\text{others}}(s)$ |
...
| 模块 6 (压缩) | $D(s)$, $\text{boundary\_idx}$ | 决定保留/丢弃数据 |
| 模块 7 (主动学习) | $\text{classification}$, $L(s)$ | 选择动作和分配预算 |
| 模块 5 (路由) | $\text{ExpertConflict}(s)$ | 触发仲裁 |
...
| SCX 差距阈值 | $\tau_{\text{SCX\_gap}}$ | 0.3 | 专家可靠性的最小显著差异 |
| 边界阈值 | $\tau_{\text{boundary}}$ | 0.3 | 边界样本检测灵敏度 |
| 一致性组合权重 | $\alpha, \beta, \gamma$ | $(0.5, 0.3, 0.2)$ | 标签/专家/密度一致性的权重 |
...
数据估值的核心是回答两个问题：
1. **采集价值** $V_{\text{add}}(s)$：如果现在从状态 $s$ 获取更多标注样本，对模型性能的提升有多大？
2. **压缩价值** $V_{\text{remove}}(s)$：如果移除状态 $s$ 中的现有样本，模型性能损失有多大？

这两个价值是互补的，分别驱动主动学习和数据压缩。

### 4.2 数学模型

#### 4.2.1 采集价值 (Acquisition Value)
...
关键区别：
- Shapley 值计算需要 $O(2^{|\mathcal{D}|})$ 次模型训练 → 计算不可行
- SCX 状态估值在 $O(K)$ 级别计算 → 可扩展到大型数据集
- SCX 通过状态结构引入先验知识（状态内样本共享价值）

#### 4.2.4 与 Influence Function 的关系
...
| 数据消费者 | 接收的数据 | 用途 |
|-----------|-----------|------|
| 模块 7 (主动学习) | $V_{\text{add}}(s)$, $\text{ranking\_add}$ | 采集函数的输入 |
| 模块 6 (压缩) | $V_{\text{remove}}(s)$, $\text{ranking\_remove}$ | 压缩优先级 |
...
其中 $\alpha \geq 0$ 是温度参数：
- $\alpha \to 0$：退化为均匀加权 $w_m(x) \to 1/M$
- $\alpha \to \infty$：退化为硬路由（one-hot 权重）
...
#### 5.2.4 仲裁策略 (Arbitration Strategies)
...
训练一个轻量级元模型 $g: \mathbb{R}^M \to \mathcal{Y}$，以专家输出 $[f_1(x), \dots, f_M(x)]$ 为输入：
...
**互补性**：SCX 路由可以作为 MoE 门控的**初始化**或**正则化约束**。
...
| 下游模块 | 提供的数据 | 用途 |
|---------|-----------|------|
| 模块 7 (主动学习) | $\text{conflict\_score}$ | relabel 动作的触发条件 |
...
| SCX 置信度阈值 | $\tau_{\text{SCX}}$ | $0.7$ | 置信度过滤的最小 SCX |
| 冲突阈值 | $\tau_{\text{conflict}}$ | $0.3$ | 触发仲裁的冲突级别 |
...
给定训练数据集 $\mathcal{D} = \{(x_i, y_i)\}_{i=1}^N$，SCX-Compress 的目标是找到一个**加权子集** $\mathcal{D}_c = \{(x_i, y_i, w_i)\}$，使得：
1. **模型性能保真**：在 $\mathcal{D}_c$ 上训练的性能接近在 $\mathcal{D}$ 上训练的性能
2. **状态结构保留**：状态划分和专家可靠性估计在压缩前后的变化不超过容差
3. **压缩率最大化**：$|\mathcal{D}_c| \ll |\mathcal{D}|$

### 6.2 数学模型

#### 6.2.1 冗余分数 (Redundancy Score)
...
分解分析：
- **高 $D(s_i)$**：状态已被充分覆盖 → 数据可能冗余
- **高 $C(s_i)$**：状态内一致性高 → 样本信息不独特
- **低 $\max_m \text{SCX}_m(s_i)$**：没有专家特别可靠 → 不需要保留专家特定的样本
...
#### 6.2.3 保真约束 (Fidelity Constraints)
...
其中 $\theta_c$ 是在 $\mathcal{D}_c$ 上训练的参数，$\theta$ 是在 $\mathcal{D}$ 上训练的参数。

#### 6.2.4 边界 Anchor 保留策略
...
#### 6.2.5 与 Coreset Selection 的关系

**传统 Coreset**：选择最小加权重子集，使得在全数据上的损失近似等于在 coreset 上的加权损失：
...
#### 6.2.6 与 Dataset Distillation 的关系

**Dataset Distillation**：合成少量伪样本 $\tilde{\mathcal{D}}$ 使得：
...
**关系**：
- SCX-Compress 是**选择型**压缩（select existing data points）
- Dataset distillation 是**合成型**压缩（generate new synthetic data points）
- 两者可以**互补**：先用 SCX-Compress 选择最核心的样本，再对这些样本进行蒸馏
- SCX-Compress 的计算成本低于蒸馏，更适合科学计算场景
...
| 模块 2 (可靠性估计) | $R_m(s)$, $\text{SCX}_m(s)$ | 专家可靠性的保真约束 |
| 模块 3 (四分类) | $C(s)$, $D(s)$, $\bar{r}(s)$, $\text{boundary\_idx}$ | 冗余分数计算 |
...
| 风险保真容差 | $\delta_R$ | $0.05$ | 压缩前后专家风险的最大允许偏差 |
| 概率保真容差 | $\delta_\rho$ | $0.02$ | 压缩前后状态概率的最大允许偏差 |
| 边界阈值 | $\tau_{\text{boundary}}$ | $0.3$ | 边界判定的灵敏度 |
...
主动学习的核心问题：在有限的标注预算 $B$ 下，选择哪些样本进行标注，以及选择何种标注方式，能最大化模型性能提升。SCX 将这个决策从**逐样本 (point-wise)** 升级为**逐状态 (state-wise)**，并且拓展了动作空间——不只是"标注哪个"，还包括"如何标注"。

### 7.2 数学模型

#### 7.2.1 动作空间 (Action Space)
...
路由到最可靠专家，适用于 $\text{Conflict}(s)$ 高但 $\text{SCX}$ 差异大的状态。
...
以概率 $\varepsilon$ 选择次优状态（探索新状态），以概率 $1-\varepsilon$ 贪心选择。
...
#### 7.2.4 从 Point-wise 到 State-wise 的升级
...
其中 $a_{\text{score}}$ 是采集函数（如不确定性、多样性）。
...
$$
\text{exploration}: x_{\text{new}} = \arg\max_x \sigma^2_{\text{model}}(x) \quad \text{(模型方差)}
$$
$$
\text{exploitation}: \text{筛选低能量结构加入训练集}
$$
...
#### 7.2.6 与 D-optimality 的对比

**D-optimality**：选择设计矩阵 $X$ 使得 $\det(X^\top X)$ 最大，最小化参数估计的置信椭圆体积。

**对比**：
- D-optimality 只关注线性模型参数估计的方差
- SCX-AL 关注非线性模型的泛化性能
- D-optimality 不考虑噪声和专家可靠性
- D-optimality 是 SCX 中覆盖度 $D(s)$ 的相关概念

#### 7.2.7 与 DIRECT 的对比

**DIRECT (Diverse and Relevant Example seleCTion)**：选择既多样又相关的样本。

**对比**：
- DIRECT：$x^* = \arg\max [\lambda \cdot \text{diversity}(x) + (1-\lambda) \cdot \text{relevance}(x)]$
- SCX-AL：$s^* = \arg\max_s V_{\text{add}}(s)$, 然后在 $s^*$ 内选多样性样本
- DIRECT 是 point-wise，SCX-AL 是 two-level（state + point）
- DIRECT 的 relevance 取决于特定任务，SCX-AL 的 $V_{\text{add}}$ 是通用的
...
1. **预算耗尽**: $\text{budget\_remaining} \leq 0$
...
| 模块 3 (四分类) | $\text{classification}$, $L(s)$, $N(s)$ | 动作选择 |
| 模块 4 (估值) | $V_{\text{add}}(s)$, $V_{\text{remove}}(s)$ | 采集和压缩的效用核心 |
...
Paper 1 处理多个 ACE (Atomic Cluster Expansion) 势函数的合并问题。核心挑战：
- 不同 ACE 势函数使用不同的**规范 (gauge)**，不能直接线性组合
- 需要找到规范变换，使得势函数在"物理等价"的意义上对齐

**数学形式**：
$$
f_m(x) = \sum_{i} c_{m,i} \cdot B_i(x) \quad \text{(ACE 展开)}
$$
$$
...
1. **专家不是等价的**：不同势函数在不同结构区域表现不同 → $R_m(s)$ 的动机
2. **全局指标不够**：势函数整体的 RMSE 掩盖了局部行为差异 → 命题 1 的动机
3. **规范差异**：专家之间存在系统偏差 → SCX 中的冲突检测
4. **合并不是唯一方法**：与其合并专家，不如基于状态选择专家 → 路由的动机
...
Paper 1 隐式地使用结构类型（如 sp², sp³, 断键）作为状态，这直接启发了 SCX 的状态定义：

$$
\text{Paper 1 的状态: } s(x) \in \{\text{sp}^2, \text{sp}^3, \text{broken bond}, \text{surface}, \dots\}
$$
$$
\text{SCX 的状态: } s: \mathcal{X} \to \{1, \dots, K\} \text{ (一般化)}
$$
...
### 8.4 Paper 2: Residual-State Error Maps → SCX 的 State Discovery

#### 8.4.1 Paper 2 的核心内容

Paper 2 提出了**残差状态图 (Residual-State Maps)**：将势函数的预测误差映射到结构状态空间，以便直观识别哪些结构区域是某个势函数的薄弱环节。

**核心方法**：
1. 对每个样本 $x_i$，计算残差 $r_i = \ell(f(x_i), y_i)$
2. 对每个结构状态 $s$，计算 $\bar{r}(s) = \mathbb{E}[r_i | x_i \in s]$
3. 绘制 $\bar{r}(s)$ 的热力图

#### 8.4.2 SCX 从 Paper 2 继承的洞察

1. **状态空间的可视化**：通过残差热力图发现模式 → 状态发现模块
2. **高误差 ≠ 高价值**：Paper 2 显示有些高误差状态是"不可救药的" → 命题 2
3. **状态边界的过渡区域**：观察到边界附近误差最高 → 边界检测
4. **误差归因**：将误差归因到具体结构状态 → $V_{\text{add}}(s)$ 中的 $r̄(s)$
...
### 8.5 Paper 3: Expert Compiler Distillation → SCX 的 Expert Routing + Teacher Generation

#### 8.5.1 Paper 3 的核心内容

Paper 3 构建了一个**多专家势函数平台 (Expert Compiler)**，能够：
1. 管理多个势函数专家
2. 根据输入结构自动选择/组合专家
3. 将多专家知识蒸馏到单一学生模型
...
1. **路由基础设施**：Paper 3 实现了初步的路由机制 → 模块 5
2. **蒸馏作为通道**：将专家知识传递给轻量模型 → 软路由的逻辑
3. **动态专家组合**：根据输入特征调整专家权重 → $w_m(x)$ 的动机
4. **处理异质专家**：不同架构、不同训练数据的专家 → 通用框架的必要性
...
$$
\text{Paper 3 路由: } m^*(x) = \arg\min_m \sum_s \gamma_s(x) \cdot \underbrace{\text{validation\_err}(m,s)}_{\text{对应 } R_m(s)}
$$
...
**方法论演化**：
1. **Paper 1**：用规范变换合并专家（消除分歧）
2. **Paper 2**：可视化专家分歧的分布（理解分歧）
3. **Paper 3**：构建系统管理分歧（利用分歧）
4. **Paper 4 (SCX)**：将分歧视为信息，用数学框架统一数据价值和专家路由（理论化分歧）
...
1. **理论指导实践**：SCX 为 Paper 3 的编译器提供优化目标（最大化 $V_{\text{add}}$）
...
| 论文 | 主要计算成本 | 与 SCX 的关系 |
|------|------------|-------------|
| Paper 1 | $O(N \cdot C_{\text{ACE}} \cdot M)$ | 提供初始 $R_m(s)$ 估计 |
...
八个子模块构成了一个完整的闭环系统：
1. **状态发现**将异质输入空间分解为语义一致的状态
2. **可靠性估计**量化每个专家在每个状态的表现
3. **数据分类**将状态分为四类，驱动不同的决策
4. **数据估值**给出每个状态的采集和压缩价值
5. **路由仲裁**基于状态条件可靠性做专家选择
6. **压缩**在保真约束下移除冗余数据
7. **主动学习**在状态级做预算分配和动作选择
8. **论文连接**将这四篇工作的层次关系和传承路径说清楚
