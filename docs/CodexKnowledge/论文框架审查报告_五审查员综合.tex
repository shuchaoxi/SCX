...
\subsection{一、五审查员共识矩阵}\label{ux4e00ux4e94ux5ba1ux67e5ux5458ux5171ux8bc6ux77e9ux9635}
...
\subsection{二、核心恐惧诊断：``别人觉得我是骗子''}\label{ux4e8cux6838ux5fc3ux6050ux60e7ux8bcaux65adux522bux4ebaux89c9ux5f97ux6211ux662fux9a97ux5b50}
...
\item
  ...
  new here?''
\item
  \textbf{防御}：证明\textbf{在真实材料体系中，忽略状态条件性的代价有多大}------EGP
...
\item
  \textbf{防御}：两层描述符（ErrorDrivenEncoder）是关键------F1 从 0.253
  → 0.585 的 2.3 倍提升证明这不是 trivial clustering
\end{itemize}
...
\subsection{三、最优论文框架：``三段跳''}\label{ux4e09ux6700ux4f18ux8bbaux6587ux6846ux67b6ux4e09ux6bb5ux8df3}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{核心贡献} & gauge-fixed shared+correction ACE 参数化 \\
\textbf{目标期刊} & npj Computational Materials / Physical Review
Materials / JCTC \\
\textbf{受众} & 材料/计算化学社区 \\
\textbf{篇幅} & 4-6 页正文 + SI \\
\textbf{数据} & AlN v3: 425-534 frames DFT 已训练; GaN/AlN v4
超算运行中 \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{核心贡献} & 状态条件数据质量框架 + MLIP 验证 \\
\textbf{目标期刊} & npj Computational Materials / Nature
Communications \\
\textbf{受众} & MLIP 社区 + 数据估值社区 \\
\textbf{篇幅} & 正文 1/3 理论（精简版）+ 2/3 实验 \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{SCX-Sim} & Nature Computational Science & 需产业合作方数据 &
如有合作方则优先 \\
\textbf{SCX-Health} & npj Digital Medicine & MedMNIST 实验需 GPU
升级且结果有效 & 如无合作方则选此项 \\
\end{longtable}

\textbf{建议二选一，不做两个。} 深度做透一个领域比广撒网有效 10 倍。
...
/ NatComm & ⏳ 需重训验证 + 公开数据集 \\
\textbf{3} & \textbf{SCX-Sim 或 SCX-Health} & 跨领域扩展 & NatCompSci /
...
\subsection{五、提交前必须完成的实验清单}\label{ux4e94ux63d0ux4ea4ux524dux5fc5ux987bux5b8cux6210ux7684ux5b9eux9a8cux6e05ux5355}
...
审稿人直接拒稿------``预估不是实验'' \\
\textbf{P1} & \textbf{公开 MLIP 数据集交叉验证}：下载 Si/Cu/MgO
公开训练数据 → 运行 SCX 两层分析 → 报告噪声/冗余发现 & 1-2d 分析（零
DFT） & ``只在 AlN 上有效''的批评无法回应 \\
\textbf{P1} & \textbf{合成实验}：生成已知噪声结构的合成数据 → 验证 SCX
...
\begin{quote}
\textbf{P0 不可跳过。} 这个实验决定了论文能从 ``interesting
...
\bottomrule\noalign{}
\endlastfoot
\textbf{P0} & 删除 Arrow 不可能定理类比 & 技术错误，过度包装 \\
\textbf{P0} & 重构 Prop 1 &
...
\textbf{P2} & 状态发现抽象化 & 理论论文中将状态映射视为
oracle，ErrorDrivenEncoder 作为附录实例化 \\
\end{longtable}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{七、竞争定位改进}\label{ux4e03ux7adeux4e89ux5b9aux4f4dux6539ux8fdb}
...
\textbf{互补关系}：SCX 粗估状态 → Data Shapley 在选定状态内细估 \\
\textbf{MoE (Shazeer/DeepMD)} & 两阶段 vs 端到端 &
\textbf{场景差异}：异构专家 + 增量扩展 + 可审计 → SCX；同构端到端 → 传统
MoE \\
\textbf{LLM 数据治理} & 不做主战场 & \textbf{引言一句话 +
立刻转移}：先提 LLM 场景作为动机，然后转移到科学仿真（有 gold label） \\
\textbf{条件风险} & 新概念 & \textbf{视角差异}：离散化状态 ≠
连续条件风险；离散化是审计的前提 \\
\end{longtable}
