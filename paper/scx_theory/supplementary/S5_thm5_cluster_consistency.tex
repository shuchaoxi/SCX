% =============================================================================
% Supplementary Section S5: Theorem 5 — Cluster Consistency of State Discovery
% Nature Submission — "A Fundamental Impossibility in Data Quality"
% =============================================================================
...
\usepackage{bm, bbding, enumerate, mathrsfs}
...
% -----------------------------------------------------------------------------
% Theorem environments (numbered per section for standalone compilation)
% -----------------------------------------------------------------------------
\theoremstyle{plain}
...
\newcommand{\muk}{\mu_k}
\newcommand{\mutrue}{\{\mu_1,\dots,\mu_K\}}
\newcommand{\norm}[1]{\lVert#1\rVert}
...
\le C_3\,\sigma\sqrt{d_\phi}\cdot\exp\!\left(-\frac{\Delta_{\min}^2}{8\sigma^2}\right)
\]
...
\noindent\textbf{Step 5 (From approximate fixed point to minimizer proximity).}
...
Under \eqref{eq:S5:strongsep}, the Voronoi partition is stable: if two
center sets $\theta,\theta'$ satisfy $\norm{\theta-\theta'}<\Delta_{\min}/4$,
...
\qquad \text{whenever } \norm{\theta-\mu}_\infty\le\frac{\Delta_{\min}}{4}.
\]
...
Under \eqref{eq:S5:strongsep} with $C_0$ sufficiently large,
$2\epsilon_{\mathrm{pop}}<\Delta_{\min}/8$, because the exponential
...
grows only logarithmically in the problem parameters.  \hfill$\square$
\end{proof}
...
\;\ge\; \pi_{\min}^K > 0,
\]
a constant depending only on $K$ and $\pi_{\min}$, not on $n$.
Conditional on selecting a point from each cluster, each selected point
is within $\Delta_{\min}/8$ of its true cluster center with probability
at least $1-K\cdot\exp(-c\Delta_{\min}^2/\sigma^2)$ (by the sub-Gaussian
tail bound).  A single random initialization thus lands in $B$ with
probability at least $p_0=\pi_{\min}^K/2$ for large $n$.
