% ============================================================
% Theory and Methods — SCX: State-Conditioned Multi-Expert
...
We prove the following guarantee (full proof in Supplementary Information, Section~S1). Under assumptions A1--A5 (expert training on disjoint data, conditional independence given the state, random noise independent of the input, state-homogeneous error rates, and uniform noise distribution), the noise detection F1 score satisfies:

\begin{equation}
\mathrm{F1} \;\geq\; 1 \;-\; \frac{1}{\eta} \exp\Bigl(-2M\,\Delta^2\Bigr),
\label{eq:theorem1}
\end{equation}

where $\eta$ is the noise rate in the data, $M$ is the number of experts, and $\Delta$ is the \emph{separation gap}---the minimum difference between the per-state expert error rate on clean samples and the maximum allowed threshold $\tau(s)$. The bound is tight up to constant factors.

The exponential dependence on $M$ is the key practical takeaway. With $M \geq 20$ experts and a clean-sample error rate below $20\%$ (so that $\Delta \geq 0.3$), the bound guarantees $\mathrm{F1} \geq 0.95$ for noise rates as low as $5\%$. Even with as few as $M = 8$ experts, moderate separation ($\Delta \geq 0.2$) ensures $\mathrm{F1} \geq 0.85$ for typical noise rates. Table~\ref{tab:f1_guarantee} shows the minimum $M$ required to guarantee various F1 thresholds across different separation gaps and noise rates.

\begin{table}[t]
\caption{Minimum number of experts $M$ required to guarantee $\mathrm{F1} \geq 1 - \varepsilon$ under the bound in Equation~\eqref{eq:theorem1}, for different separation gaps $\Delta$ and noise rates $\eta$. At $\Delta = 0.3$ and $\eta = 0.1$, only $M = 5$ experts are needed to guarantee $\mathrm{F1} \geq 0.90$, and $M = 18$ to guarantee $\mathrm{F1} \geq 0.99$.}
...
\cmidrule{3-6}
& & $\Delta = 0.2$ & $\Delta = 0.3$ & $\Delta = 0.2$ & $\Delta = 0.3$ \\
\midrule
$0.10$ & $\geq 0.90$ & 10 & 5 & 9 & 4 \\
$0.05$ & $\geq 0.95$ & 16 & 8 & 15 & 7 \\
$0.01$ & $\geq 0.99$ & 27 & 14 & 25 & 12 \\
$0.001$ & $\geq 0.999$ & 39 & 20 & 37 & 18 \\
\bottomrule
\end{tabular}
\end{ruledtabular}
\end{table}

The separation gap $\Delta$ plays a critical role. It is maximized when experts are diverse---trained on different data subsets with different random initializations---so that their errors decorrelate, and when the state discovery is accurate enough that clean-sample error rates are well-characterized. This connects directly to the two-layer state discovery above: better state discovery increases $\Delta$ by making within-state error rates more homogeneous, which tightens the theoretical guarantee.

In the AlN v3 application, training $M = 12$ experts with disjoint data subsets yields a separation gap $\Delta \approx 0.35$ in the thermal state and $\Delta \approx 0.41$ in the MLMD state, where most noise is concentrated. The resulting noise detection achieves $\mathrm{F1} = 0.87$ across the full dataset (Table~\ref{tab:aln_f1}), consistent with the theoretical prediction of $\mathrm{F1} \geq 0.93$ from Equation~\eqref{eq:theorem1} given the observed $\Delta$ and $\eta$.

\begin{table}[t]
\caption{SCX noise detection performance on the AlN v3 dataset. The theoretical F1 lower bound (Equation~\eqref{eq:theorem1}) is computed from the observed $\Delta$ and $\eta$ in each state. Empirical F1 is measured against the ground truth noise labels.}
\label{tab:aln_f1}
\begin{ruledtabular}
\begin{tabular}{lcccc}
\toprule
State & Expert error rate $\rho$ & $\Delta$ & Theoretical F1 bound & Empirical F1 \\
\midrule
Equilibrium & $0.03$ & $0.47$ & $\geq 0.99$ & $1.00$ \\
EOS/strain & $0.04$ & $0.46$ & $\geq 0.99$ & $0.97$ \\
Phonon & $0.06$ & $0.44$ & $\geq 0.99$ & $1.00$ \\
Thermal & $0.18$ & $0.35$ & $\geq 0.93$ & $0.89$ \\
MLMD & $0.15$ & $0.41$ & $\geq 0.96$ & $0.91$ \\
\bottomrule
\end{tabular}
\end{ruledtabular}
\end{table}
