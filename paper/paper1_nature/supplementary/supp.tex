...
\begin{table}[H]
\centering
\caption{DrugBank targetome screening performance at 10\% controlled noise. Data cleaning improves held-out AUC by 29.2\% relative; architecture change (random forest to XGBoost) improves it by 2.1\%.}
\label{tab:S1_drugbank}
\begin{tabular}{lccc}
\toprule
Method & Noise detection F1 & Held-out AUC & Improvement \\
\midrule
Loss baseline & 0.29 & 0.71 & -- \\
SCX cleaning & 0.56 & 0.82 & 29.2\% \\
Architecture change & -- & 0.73 & 2.1\% \\
\midrule
Ratio & -- & -- & 13.9$\times$ \\
\bottomrule
\end{tabular}
...
Full theorem proofs and derivations of the bounds referenced in this work (Theorem~1: exponential detection guarantee; Theorem~2: weak feature bound; Theorem~3: Noise-Difficulty Unidentifiability; Theorem~4': exact constant minimax optimality; Proposition~6: bootstrap stability diagnostic) are provided in the companion Paper~I~[arXiv:XXXX], Supplementary Information Sections~S1--S8.

The Curation-Exploration Tradeoff framework, which explains why expert diversity must be allowed to develop before noise detection, is developed in the companion Paper~II~[arXiv:XXXX].
