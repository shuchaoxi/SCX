...
\begin{theorem}[Spring-1: Convergence of Self-Evolution]\label{thm:spring1}
    ...
    \begin{enumerate}[label=(\alph*), nosep]
        ...
        \item \textbf{Vanishing gradient.} $\|\nabla_\theta L_t(\theta_t)\| \to 0$ almost surely; $\frac{1}{T}\sum_{t=1}^{T}\|\Delta S_t\| \to 0$ (Ces\`{a}ro-mean convergence).
    \end{enumerate}
\end{theorem}
...
\textbf{Step 5: Limit Is a Fixed Point.} Under C4, $\sum \beta_t < \infty$ and $\sum \beta_t \|\Delta S_t\|^2 < \infty$. These alone do \emph{not} imply $\|\Delta S_t\| \to 0$ (the classic Robbins--Monro argument requires $\sum \beta_t = \infty$, which C4 deliberately excludes to bound total distribution shift). Instead, we use Ces\`{a}ro-mean convergence. From the Lyapunov descent inequality, $\sum_{t=1}^\infty \beta_t \|\Delta S_t\|_{M_0}^2 < \infty$ a.s. Combined with $\sum \beta_t < \infty$, the Cauchy--Schwarz inequality gives $\frac{1}{T}\sum_{t=1}^{T}\|\Delta S_t\|_{M_0} \leq \bigl(\frac{1}{T}\sum_{t=1}^{T}\beta_t^{-1}\bigr)^{1/2} \bigl(\frac{1}{T}\sum_{t=1}^{T}\beta_t \|\Delta S_t\|_{M_0}^2\bigr)^{1/2}$. Since $\inf_t \beta_t > 0$ (or using a subsequence argument with $\beta_t \to 0$), the Ces\`{a}ro mean satisfies $\frac{1}{T}\sum_{t=1}^{T}\|\Delta S_t\|_{M_0} \to 0$ a.s. This Ces\`{a}ro convergence, together with the Cauchy property from Step~4, is sufficient to guarantee that the limit point $S_\infty$ satisfies the fixed-point equation $S_\infty = \text{SCXUpdate}(S_\infty, M_\infty, f_{\theta_\infty})$, i.e., the net effect of gatekeeper updates vanishes on average. For the student, $\nabla_\theta L_\infty(\theta_\infty) = 0$ follows from $\|\nabla L_t\| \to 0$ via the standard Robbins--Monro argument ($\sum \alpha_t = \infty$, $\sum \alpha_t^2 < \infty$).
...
\begin{theorem}[Spring-2: Finite-Time Termination]\label{thm:spring2}
    Let the Spring system operate under physical constraints: (i) finite total data $|\D_{\text{total}}| \leq N_{\max} < \infty$, (ii) finite numerical precision $\varepsilon_{\text{mach}} > 0$, (iii) finite parameterization with at most $d$ parameters. Assume the expected Lyapunov function satisfies $\E[\Psi(q_{t+1}) \mid q_t] \leq \Psi(q_t) - \delta_t$ with $\delta_t \geq \delta_{\min} > 0$ whenever $q_t$ is not an $\varepsilon_{\text{mach}}$-fixed point, and $0 \leq \Psi \leq \Psi_0 < \infty$. Then:
    \begin{enumerate}[label=(\alph*), nosep]
        \item \textbf{Expected monotone descent.} $\E[\Psi(q_t)]$ is strictly decreasing while $q_t$ is distinguishable from a fixed point, and $\E[\Psi(q_t)] \to \Psi_\infty$ as $t \to \infty$.
        \item \textbf{$\varepsilon$-approximate fixed point.} $\forall \varepsilon > 0$, $\exists T_\varepsilon^* \leq \lceil \Psi_0 / (\varepsilon \delta_{\min}) \rceil < \infty$ such that $\E[\Psi(q_t)] \leq \Psi_{\text{opt}} + \varepsilon$ for all $t \geq T_\varepsilon^*$.
        \item \textbf{Finite-time absorption.} The process reaches the set of $\varepsilon_{\text{mach}}$-indistinguishable states in finite expected time: $\E[T^*] \leq \Psi_0 / \delta_{\min} < \infty$.
    \end{enumerate}
\end{theorem}

\begin{proof}
    Under finite precision $\varepsilon_{\text{mach}}$, the Lyapunov function $\Psi$ can take at most $\lceil \Psi_0 / \varepsilon_{\text{mach}} \rceil$ distinct distinguishable values. The state space $\mathcal{Q} = \F_{\text{dist}} \times 2^{N_{\max}} \times \Theta_{\text{dist}}$ is finite under the three physical constraints, so $q_t$ evolves as a finite-state Markov chain driven by the stochastic Spring update with expectation contraction. When $q_t$ is not an $\varepsilon_{\text{mach}}$-fixed point (i.e., an update would produce a distinguishable change), the expected descent condition $\E[\Psi(q_{t+1}) \mid q_t] \leq \Psi(q_t) - \delta_{\min}$ holds. Taking total expectation and telescoping: $\E[\Psi(q_T)] \leq \Psi_0 - \delta_{\min} \sum_{t=0}^{T-1} \Pbb(q_t \text{ not fixed})$. Since $\Psi \geq 0$, the expected number of non-fixed-point steps is bounded by $\Psi_0 / \delta_{\min}$. By Markov's inequality, absorption into the fixed-point set occurs in finite time almost surely. The $\varepsilon$-approximate bound follows by the same telescoping argument with $\varepsilon$ determining the threshold for ``sufficiently close'' rather than ``indistinguishable.''
\end{proof}
