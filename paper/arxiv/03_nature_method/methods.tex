% ============================================================
% Methods — SCX: State-Conditioned Multi-Expert Consistency
...
The SCX (State-Conditioned Multi-Expert Consistency) pipeline proceeds through five stages. Given a training dataset $\mathcal{D} = \{(x_i, y_i)\}_{i=1}^N$ with suspected label noise and a choice of domain-appropriate features $\phi \colon \mathcal{X} \to \mathbb{R}^{d_\phi}$:
...
\begin{algorithm}[t]
\caption{SCX: State-Conditioned Multi-Expert Consistency for Noise Detection}
\label{alg:scx}
\begin{algorithmic}[1]
