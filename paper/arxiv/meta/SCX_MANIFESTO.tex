...
SCX enters this lineage through a single theorem. \textbf{Theorem 3}---the Noise-Difficulty Unidentifiability Theorem---proves that, from observational data alone, one cannot distinguish between a mislabeled sample and a genuinely difficult-but-correct sample. The two worlds produce identical joint distributions over inputs, labels, and expert predictions. No algorithm, no amount of data, no hypothesis test has power exceeding its significance level. The boundary is hard.
