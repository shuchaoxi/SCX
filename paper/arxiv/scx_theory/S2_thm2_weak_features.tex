...
\section{Theorem 2: Weak Feature Failure Boundary (强人所难定理)}
\label{sec:thm2}
...
There exists a constant $C_F$, depending on the minimum precision and recall,
such that in the typical operating range (precision, recall $\geq 0.1$,
$C_F \leq 2$),
\begin{equation}
\boxed{\;
...
\emph{PR-AUC.} The PR-AUC involves an integral over thresholds of precision,
$\mathbb{P}(Z=1 \mid \hat{z}=1)$. The same amplification argument applies at
each threshold, yielding the identical bound
\eqref{eq:prauc-bound}. (A fully rigorous treatment requires additional steps
