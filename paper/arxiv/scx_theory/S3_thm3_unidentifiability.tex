...
\begin{enumerate}[label=\textbf{E\arabic*}, leftmargin=*, itemsep=4pt]
  \item \textbf{Assumption transparency:} the claim $\mathcal{S}$ is
    ...
    (enforced by Theorem~3, 老实人定理). If $\mathcal{A}_{\mathcal{S}}$
...
and the \textbf{SCX-knowledge operator}:
\[
\mathbf{K}_{t}(\mathcal{S}, \mathcal{C}) =
\begin{cases}
1 & \text{if } V_M(\mathcal{S}, s) \geq \theta_s \text{ for all } s
...
Under assumptions E1--E5 and the SCX framework conditions A1--A6
...
For the tightness claim, Theorem~4 (极定理) establishes that the
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Gettier immunity:} Gettier (1963) cases arise when a claim
    ...
    fails). Either way, the Gettier gap is closed;
  \item \textbf{人人平等 (Epistemic equality):} $\mathbf{K}$ depends only
