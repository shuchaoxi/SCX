...
% ===================================================================
\subsection{Corollary: 人人平等定理 (Everyone Equal Theorem)}
\label{sec:thm3_equal}

\begin{corollary}[人人平等定理 (Everyone Equal Theorem)]
...
The name ``人人平等定理'' (Everyone Equal Theorem) reflects the
...
% ===================================================================
\subsection{Metatheorem: 好人收敛定理 (Good Person Convergence)}
\label{sec:good-convergence}

\begin{theorem}[好人收敛定理 (Good Person Convergence Theorem)]
\label{thm:good-convergence}
...
\begin{enumerate}[label=(C\arabic*)]
  \item \textbf{老实人条件}: every data-quality claim must state its
    structural assumptions (Theorem~3);
  \item \textbf{人人平等条件}: no agent occupies a privileged epistemic
    position (Corollary~\ref{cor:everyone-equal});
  \item \textbf{禁止装逼条件}: audit outputs are unconditionally verifiable
    by any third party (非扩散均衡, Theorem~NPE).
\end{enumerate}
Then, as $t\to\infty$:
\begin{enumerate}[label=(\roman*)]
  \item The fraction of agents making unverifiable claims converges to
    zero---``恶人低头'': dishonest behaviour becomes detectable and
    economically irrational;
  \item The expected payoff of honest behaviour exceeds that of dishonest
    behaviour for all agents---``好人挺直腰板'': honesty is the strictly
    dominant strategy;
...
By Theorem~3 (老实人定理), any claim lacking stated assumptions is
mathematically empty and is rejected by the community without further
argument. By Corollary~\ref{cor:everyone-equal} (人人平等定理), no agent
can claim epistemic privilege---the audit procedure is identical for all.
By Theorem~NPE (禁止装逼定理 / 雅洁不扩散定理), the equilibrium in which
all agents adopt the audit protocol and none develop competing standards
is the unique Nash equilibrium under fragmentation costs. Agents who
persist in unverifiable claims face escalating detection risk without
offsetting benefit; agents who adopt honest practices gain verifiability
without sacrificing competitive position. The community converges to the
all-honest equilibrium determined by these three conditions. $\square$
\end{proof}

\begin{remark}[命名]
``好人收敛'' (Good Person Convergence): 数学结构本身使诚实成为占优策略。
不是道德说教，不是制度强制——是博弈均衡的自然结果。所有人变老实、变平等、
变好人，不需要一个更强大的人来执行——定理替你执行。
\end{remark}

% ===================================================================
\subsection{认识论形式化 (Epistemic Formalization)}
\label{sec:epistemic}

For over two millennia, epistemology has asked: ``What is knowledge?''
Plato's \textit{justified true belief} (JTB) stood for two thousand years
until Gettier (1963) showed it fails when justification rests on unstated
contingencies. No consensus replacement has emerged.

Here we provide a mathematical resolution: knowledge is a claim whose
assumptions are stated and whose verification is reproducible.

\begin{definition}[SCX-Knowledge]
\label{def:scx-knowledge}
A statement $\mathcal{S}$ constitutes \textbf{SCX-knowledge} with respect
to a community $\mathcal{C}$ of verifiers if and only if:
\begin{enumerate}[label=(K\arabic*)]
  \item \textbf{Assumption transparency}: the structural assumptions
    $\mathcal{A}_{\mathcal{S}}$ on which $\mathcal{S}$ depends are
    explicitly enumerated (Theorem~3, 老实人定理);
  \item \textbf{Independent verifiability}: $\mathcal{A}_{\mathcal{S}}$
    can be checked by any member of $\mathcal{C}$ without privileged access
    (Corollary~\ref{cor:everyone-equal}, 人人平等定理);
  \item \textbf{Multi-expert validation}: under $\mathcal{A}_{\mathcal{S}}$,
    $\mathcal{S}$ has been verified by at least $M \geq 2$ independent
    verifiers with no unresolved conflict (Theorem~1, 独醒不太可能定理);
  \item \textbf{No live counterexample}: no member of $\mathcal{C}$ has
    produced a valid counterexample to $\mathcal{S}$ under the stated
    assumptions $\mathcal{A}_{\mathcal{S}}$ that remains unresolved.
\end{enumerate}
If any condition fails, $\mathcal{S}$ is not SCX-knowledge---it is an
unvalidated claim.
\end{definition}

\begin{theorem}[认识论形式化定理 (Epistemic Formalization Theorem)]
\label{thm:epistemic}
Under the SCX framework, the question ``does agent $X$ know $\mathcal{S}$?''
is decidable in the following sense: for any claim $\mathcal{S}$ and any
community $\mathcal{C}$ operating under the SCX audit protocol,
membership of $\mathcal{S}$ in SCX-knowledge reduces to checking conditions
(K1)--(K4), all of which are computable given access to (a) $X$'s stated
assumptions $\mathcal{A}_{\mathcal{S}}$, (b) the community's verification
records, and (c) the audit outputs generated by Yajie.

Furthermore, the SCX-knowledge operator $\mathbf{K}$ satisfies:
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Non-triviality}: $\mathbf{K}(\mathcal{S}) \neq \top$ for
    all $\mathcal{S}$---no claim is automatically knowledge;
  \item \textbf{Verifiability}: $\mathbf{K}(\mathcal{S}) \implies$
    any member of $\mathcal{C}$ can independently confirm
    $\mathbf{K}(\mathcal{S})$;
  \item \textbf{Monotonicity under audit}: if $\mathbf{K}_t(\mathcal{S})$
    holds at time $t$, then $\mathbf{K}_{t+1}(\mathcal{S})$ also holds,
    provided no counterexample emerges---audit does not erode knowledge;
  \item \textbf{Gettier immunity}: Gettier cases arise precisely when
    justification relies on assumptions not present in $\mathcal{A}_{\mathcal{S}}$.
    Under (K1), such assumptions must be stated, eliminating the Gettier
    problem by construction.
\end{enumerate}
\end{theorem}

\begin{proof}[Proof sketch]
(K1) is enforced by Theorem~3: any claim lacking stated assumptions is
mathematically empty. (K2) follows from Corollary~\ref{cor:everyone-equal}:
verification requires no privileged access. (K3) invokes Theorem~1: with
$M$ independent verifiers, the F1 convergence guarantee makes validation
statistically decisive. (K4) is maintained by the cumulative evidentiary
corpus (CEC) of the Yajie Protocol: counterexamples, if they arise, are
logged and adjudicated. Thus all four conditions are checkable by any
community member. The $\mathbf{K}$ operator properties follow from the
structural features of the SCX framework: non-triviality because (K1)--(K4)
are substantive filters; verifiability because audit outputs are public;
monotonicity because CEC is append-only; Gettier immunity because unstated
assumptions are blocked at (K1). $\square$
\end{proof}

\begin{remark}[Historical significance]
Plato (c.\ 369 BCE) defined knowledge as justified true belief. Gettier
(1963) showed this definition admits counterexamples where justification
and truth coincide accidentally. The subsequent literature---Nozick's
truth-tracking, Goldman's causal theory, Sosa's virtue epistemology---has
produced successively more refined accounts, none of which achieved
consensus. The SCX approach does not compete with these accounts on their
own philosophical terms. Rather, it operationalizes knowledge as a
\textit{verifiable status} within a community of auditors: a claim is
knowledge iff it survives multi-expert audit under stated, independently
verifiable assumptions. This transforms epistemology from a normative
discipline into a computable protocol. It does not answer ``what should we
believe?''---it answers ``when can we agree that this claim has met the
conditions for collective acceptance?''
\end{remark}
% ===================================================================
% ===================================================================
% ===================================================================
