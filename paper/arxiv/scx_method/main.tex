...
\title{Data Quality and Model Architecture: A State-Conditioned View}
...
The companion Paper~II~\cite{scx2025curation} situates SCX within the broader Curation-Exploration Tradeoff framework, establishing that premature cleaning destroys the signal needed to detect errors---the very diversity among experts that SCX exploits must be allowed to develop through unconstrained exploration before any samples are removed.
...
\caption{Cross-domain comparison of data cleaning vs.\ architecture improvement. In three strong-feature domains (AlN, CIFAR-10, DrugBank), the relative performance improvement from SCX-based data cleaning (blue) exceeds that from switching model architectures (orange) by a factor of 11--19. On DermaMNIST, where low-resolution thumbnails provide weak features, both improvements are small---consistent with the theoretical weak-feature bound. Error bars show 95\% confidence intervals over 5 random training seeds.}
...
\textbf{Adaptive threshold.} For noise rates that deviate substantially from $\eta = 0.5$, the naive threshold $\theta^*$ can be suboptimal by a factor of up to 2.4. The companion Paper~I~\cite{xi2025fundamental} (Theorem~4') derives an adaptive threshold $\theta^\dagger = \theta^* + \frac{1}{M}\frac{\log((1-\eta)/\eta)}{D^*} + O(1/M^2)$ that achieves exact constant minimax optimality. This $\eta$-aware threshold is essential for rare noise scenarios ($\eta < 0.05$) where the cost of false positives is asymmetric.

\subsection{Theoretical Guarantees}

The companion Paper~I~\cite{xi2025fundamental} provides the complete theoretical foundation. We summarize the key guarantees here.

\textbf{Exponential convergence (Theorem~1).} Under assumptions A1--A6 (disjoint expert training, bounded correlation, bounded loss, uniform independent noise, state-homogeneous error rates, and balanced error distribution), the noise detection F1 satisfies:
...
SCX Research Group. ``A Fundamental Impossibility in Data Quality: Distinguishing Label Noise from Sample Difficulty is Provably Unsolvable Without Explicit Assumptions.'' arXiv preprint (2025).

\bibitem{scx2025curation}
SCX Research Group. ``State-Conditioned Expertise and the Curation-Exploration Tradeoff.'' Working paper (2026).
