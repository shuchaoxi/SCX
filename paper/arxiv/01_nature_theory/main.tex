...
\title{Detecting Label Noise by State-Conditioned Multi-Expert Consistency}
...
The quality of training data determines the quality of machine learning models~\cite{placeholder}. When training data contains label errors---samples whose given labels differ from the true labels---model performance degrades predictably and substantially~\cite{placeholder}. A large industry of data-cleaning methods has emerged to address this: loss-based filtering, confidence learning, multi-annotator consensus, and active label correction, among others~\cite{placeholder}. These methods share an implicit premise: that data errors can be distinguished from genuinely difficult samples by some observable signal.
