...
\twocolumn[
  \begin{@twocolumnfalse}
    \title{\textbf{State-Conditioned Expertise: A Unified Framework for Data Quality Across Scientific and Engineering Domains}}
    ...
    \begin{abstract}
      \small
      Data quality is widely recognized as the dominant factor in machine learning performance, yet the field lacks a rigorous, mathematically grounded framework for distinguishing meaningful label errors from intrinsic sample difficulty. The SCX (State-Conditioned eXpertise) framework fills this gap through a two-layer architecture: unsupervised state discovery partitions the data space into regions with homogeneous error characteristics, followed by multi-expert consistency scoring with provable finite-sample guarantees. This review provides a comprehensive synthesis of the SCX mathematical foundations---including the noise-difficulty unidentifiability theorem (Theorem~3), the exponential-convergence detection guarantee (Theorem~1), the minimax optimality with exact constant (Theorem~4'), the weak-feature failure bound (Theorem~2), and cluster consistency guarantees (Theorem~5)---and their instantiation across six domains: life sciences (drug discovery, medical imaging), advanced manufacturing (machine-learned interatomic potentials, quality control), embodied intelligence (robot learning, sim-to-real transfer), artificial intelligence (computer vision, natural language processing, reinforcement learning), autonomous driving (perception, behavior prediction), and the natural sciences (physics, chemistry, biology, earth science). For each domain, we present the problem formulation, the SCX adaptation with its theoretical guarantees, empirical evidence from validated results where available, and honest limitations. We identify a universal cross-domain pattern---the curation-exploration tradeoff---whereby premature data cleaning destroys the signal needed to distinguish noise from hardness, and we characterize the feature-strength regimes that determine when SCX succeeds or degrades gracefully to simpler baselines. We conclude with open challenges including automated state discovery, real-time noise detection, and the regulatory implications of certified data quality.
    \end{abstract}
...
The SCX framework resolves this unidentifiability through a minimal set of structural assumptions (A1--A6): disjoint expert training, conditional independence of expert errors, bounded loss, uniform label noise, state-homogeneous error rates, and balanced error distributions. Under these assumptions, SCX achieves provable noise detection with exponential convergence to the minimax optimal rate, including the exact asymptotic constant.
...
\begin{description}[nosep,leftmargin=*,style=nextline]
  ...
  \item[A2 (Conditional independence)] Expert errors are conditionally independent given the input for clean samples.
...
These assumptions are not merely technical conveniences---each corresponds to a specific path out of the unidentifiability trap. Removing any single assumption restores the ambiguity for some data distribution.
...
\begin{enumerate}[nosep,leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{Multi-expert training}: Train $M \geq 8$ experts on disjoint data subsets (or bootstrap samples for small datasets); use diverse random seeds and architectures to ensure conditional independence (A2).
...
The unification of data quality assessment under a single mathematical framework has practical implications: practitioners across domains can adopt a common protocol, rely on shared theoretical guarantees, and contribute to a growing body of cross-domain evidence. The framework's limitations are honestly bounded: weak features, extreme noise sparsity, and missing domain knowledge for state definition all reduce effectiveness, and the framework signals these cases unambiguously.

As machine learning systems are deployed in increasingly high-stakes domains---medical diagnosis, autonomous driving, drug discovery, climate modeling---the need for certified, mathematically grounded data quality assessment grows urgent. The SCX framework provides both the theoretical foundations and the practical protocols to meet this need.
...
The AlN v3 dataset is available from the authors upon reasonable request. CIFAR-10, DermaMNIST, BloodMNIST, and PathMNIST are publicly available benchmark datasets. The DrugBank database is available from \url{https://go.drugbank.com}. The SCX-Drug targetome analysis data is available from the authors upon request.

\paragraph*{Code availability.}
The SCX-Life module (including SCX-Health and SCX-Drug) is available at \url{https://github.com/SCX-Platform/scx-life} under the Apache 2.0 license. The SCX core framework and experiment reproduction scripts are available at \url{https://github.com/SCX-Platform/scx}.


\paragraph*{Author contributions.}
S.X. conceived the SCX framework, developed the mathematical theory, implemented the software modules, conducted experiments across all domains, and wrote the manuscript. C. contributed to theorem statements and proof verification.
...
\bibitem{paperI}
Xi, S. \& Claude. A Fundamental Impossibility in Data Quality: Distinguishing Label Noise from Sample Difficulty is Provably Unsolvable Without Explicit Assumptions. \emph{arXiv preprint} (2026).

\bibitem{paperII}
Xi, S. \& Claude. The Curation-Exploration Tradeoff: Why Premature Data Cleaning Harms Machine Learning. \emph{In preparation} (2026).

\bibitem{paperIII}
Xi, S. \& Claude. Training Data Quality Dominates Model Architecture in Scientific Machine Learning. \emph{In preparation} (2026).

\bibitem{paperIV}
Xi, S. \& Claude. Consistency-Constrained Expert Merging for Atomic Cluster Expansion Potentials. \emph{arXiv preprint} (2026).

\bibitem{paperV}
Xi, S. \& Claude. SCX for LLM Data Curation. \emph{In preparation} (2026).

\bibitem{theorems_unified}
Xi, S. SCX Mathematical Theory: Unified Theorem Document. \emph{Technical report} (2026).

\bibitem{engineering_guide}
Xi, S. SCX Engineering Practice Guide. \emph{Technical report} (2026).

\bibitem{scxlife}
SCX-Life Module. \url{https://github.com/SCX-Platform/scx-life}. Apache 2.0 license.

\bibitem{targetome_top10}
Xi, S. SCX-Drug Targetome TOP-10 Screening Report. \emph{SCX Platform Technical Report} (2026).

\bibitem{noise_module}
SCX-Noise: Label-noise detection on MedMNIST. \texttt{scx-life/health/experiments/noise/run\_noise.py}.

\bibitem{routing_module}
SCX-Routing: State-conditioned expert routing on BloodMNIST. \texttt{scx-life/health/experiments/routing/run\_routing.py}.

\bibitem{compress_module}
SCX-Compress: Redundancy scoring and compression on PathMNIST. \texttt{scx-life/health/experiments/compress/run\_compress.py}.

\bibitem{drug_state}
Xi, S. SCX-Drug State Module. \texttt{scx-life/drug/state.py}.

\bibitem{dti_module}
Xi, S. SCX-Drug DTI Operators. \texttt{scx-life/drug/operators/dti.py}.

\bibitem{medmnist}
Yang, J. \emph{et al.} MedMNIST v2: A large-scale lightweight benchmark for 2D and 3D biomedical image classification. \emph{Scientific Data} \textbf{10}, 41 (2023).
