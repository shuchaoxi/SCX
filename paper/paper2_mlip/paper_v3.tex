...
\begin{enumerate}[label=\textbf{(I\arabic*)}]
    ...
    \item \textbf{Coefficient-level gauge freedom.} When expert potentials are parameterized in a shared-correction form---decomposing the total coefficient vector into a shared component $\c0$ and element-specific corrections $\cZ$---an exact gauge transformation $\c0 \to \c0 + \g$, $\cZ \to \cZ - \g$ leaves all physical predictions invariant. Different experts exploit this freedom differently, making their coefficient vectors incompatible for direct averaging or addition.
...
\begin{enumerate}[label=\textbf{(C\arabic*)}]
    ...
    \item \textbf{Post-hoc gauge fixing via orthogonal projection.} We prove that the gauge freedom $\c0 \to \c0 + \g$, $\cZ \to \cZ - \g$ is a reparameterization of the standard multi-element ACE and propose post-hoc projection onto $\sum_Z \pi_Z \cZ = 0$, which enforces the gauge condition exactly (residual $< 5\times10^{-16}$) with zero prediction change. We systematically demonstrate that soft-penalty training fundamentally fails because the gauge constraint is nearly orthogonal to the physical loss landscape.
...
\begin{equation}
E^{(m)}(\mathbf{R}) = \sum_i \Big[ \c0^{(m)} \cdot \mathbf{B}(\mathbf{q}_i) + \cZ_i^{(m)} \cdot \mathbf{B}(\mathbf{q}_i) + b_{Z_i}^{(m)} \Big],
\label{eq:expert_ace}
\end{equation}

where $\c0^{(m)}$ is the shared coefficient vector, $\cZ^{(m)}$ is the element-specific correction for species $Z$, $b_Z^{(m)}$ is a per-species constant energy shift, $\mathbf{B}(\mathbf{q}_i)$ is the ACE basis vector for the local environment $\mathbf{q}_i$ of atom $i$, and $Z_i$ is the chemical species of atom $i$.

For a binary system of type $AB$ (e.g., AlN), the explicit form is:

\begin{equation}
E^{(m)} = \sum_i \c0^{(m)} \cdot \mathbf{B}(\mathbf{q}_i) + \sum_{i \in A} \big[ \cA^{(m)} \cdot \mathbf{B}(\mathbf{q}_i) + b_A^{(m)} \big] + \sum_{i \in B} \big[ \cB^{(m)} \cdot \mathbf{B}(\mathbf{q}_i) + b_B^{(m)} \big].
\label{eq:expert_binary}
\end{equation}

The effective per-species coefficients for deployment are:
\begin{equation}
\cZ^{\mathrm{eff},(m)} = \c0^{(m)} + \cZ^{(m)},
\quad
E^{(m)} = \sum_i \big[ \cZ_i^{\mathrm{eff},(m)} \big] \cdot \mathbf{B}(\mathbf{q}_i) + b_{Z_i}^{(m)},
...
Formally, let $\mathcal{D}_m$ be the test set of domain $m$ and $\mathcal{L}_m(\theta)$ be the prediction error of a model with parameters $\theta$ on $\mathcal{D}_m$. Given expert parameters $\theta^{(m)} = (\c0^{(m)}, \{\cZ^{(m)}\}, \{b_Z^{(m)}\})$ with home-domain losses $\mathcal{L}_m(\theta^{(m)})$, the merged parameters $\theta^{\mathrm{merged}}$ should satisfy:
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Regularization conflict}. $\ell_2$ regularization applied separately to $\c0$ and $\cZ$ is gauge-dependent, potentially biasing experts toward different non-physical coefficient configurations.
\end{enumerate}
...
This is an orthogonal linear map onto the gauge-fixing subspace $\{(\c0, \{\cZ\}) \mid \sum_Z \pi_Z \cZ = 0\}$. The proof of prediction invariance is trivial:

\begin{equation}
E' = \sum_i \big[ (\c0 + \g) + (\cZ_i - \g) \big] \cdot \mathbf{B}_i + b_{Z_i}
    = \sum_i \big[ \c0 + \cZ_i \big] \cdot \mathbf{B}_i + b_{Z_i}
    = E.
...
\begin{itemize}
    \item $\c0$ represents the stoichiometry-weighted average atomic ACE response.
...
For expert merging, this gauge convention ensures that all experts share the same reference frame: $\c0^{(m)}$ represents ``average atomic response in domain $m$'' while $\cZ^{(m)}$ represents ``element-specific deviation in domain $m$,'' both measured from a common origin. After gauge fixing, the coefficient vectors of different experts are directly comparable and combinable.
...
After gauge fixing and alignment, each expert $m$ is represented by coefficients $(\c0^{(m)}, \{\cZ^{(m)}\}, \{b_Z^{(m)}\})$ in a common reference frame. The merged base coefficient is:

\begin{equation}
\c0^{\mathrm{merged}} = \frac{1}{M} \sum_{m=1}^{M} \c0^{(m)},
...
\begin{equation}
\cZ^{\mathrm{eff,merged}} = \c0^{\mathrm{merged}} + \cZ^{\mathrm{merged}}.
...
\begin{equation}
E^{\mathrm{merged}}(\mathbf{R}) = \sum_i \big[ \cZ_i^{\mathrm{eff,merged}} \big] \cdot \mathbf{B}(\mathbf{q}_i) + b_{Z_i}^{\mathrm{merged}},
...
\begin{itemize}
    \item The gauge fixing ensures that $\c0$ and $\cZ$ coefficients have consistent meanings across experts.
...
\begin{enumerate}[label=\textbf{(F\arabic*)}]
    ...
    \item \textbf{Exact gauge fixing via orthogonal projection.} The gauge transformation $\c0 \to \c0 + \g$, $\cZ \to \cZ - \g$ leaves all physical predictions invariant. Our post-hoc projection onto $\sum_Z \pi_Z \cZ = 0$ enforces the gauge condition to machine precision ($< 10^{-15}$) with zero prediction change. Soft gauge-penalty training fundamentally fails because the gauge constraint is nearly orthogonal to the physical loss landscape, as confirmed by a systematic $\lambda$ sweep ($\lambda = 10^{-3}$ to $10^1$).
...
The SCX software framework was independently developed by the author and is released under the Apache 2.0 license for research purposes. Commercial use is subject to separate licensing terms. The mathematical methods described herein are in the public domain.
