...
%   (Xi & Claude, arXiv:XXXX, referred to as Paper I)
% ============================================================

\title{The Curation-Exploration Tradeoff: Why Premature Data Cleaning Harms Machine Learning}

\author{Shuchao Xi$^{1*}$ \& Claude$^{1}$}

\affil{$^{1}$ SCX Laboratory}
\date{}
...
where $\ell$ is a loss function and $\tau$ is an error threshold. $C(x)$ is the fraction of experts that fail to predict the given label. Under the assumptions of Paper~I (disjoint training, conditional independence, uniform noise, state homogeneity), the expected consistency differs systematically between clean and noisy samples:
...
\begin{flushleft}
\textbf{Data Availability.} The AlN v3 dataset is available at [repository]. CIFAR-10 and MedMNIST are publicly available benchmark datasets. DrugBank is available from go.drugbank.com.

\textbf{Code Availability.} The SCX implementation and experiment scripts are available at [repository].

\textbf{Companion Paper.} This paper is accompanied by a theoretical companion, ``A Fundamental Impossibility in Data Quality'' (Xi \& Claude, arXiv:XXXX), which provides the complete mathematical framework: Theorem~1 (noise detection guarantee), Theorem~2 (weak-feature bound), Theorem~3 (unidentifiability), Theorem~4' (exact constant minimax optimality), Theorem~5 (cluster consistency), and Proposition~6 (bootstrap stability diagnostic). All theorems are referenced herein as Paper~I.

\textbf{Acknowledgements.} The authors thank [to be completed].

\textbf{Author Contributions.} S.X. conceived the problem, developed the SCX framework, and conducted experiments. C. contributed to theorem statements and manuscript preparation. Both authors wrote the manuscript.
