...
% 14-scenario expanded coverage across 6 domain groups
...
\title{
  ...
  \normalsize Expanded 14-Scenario Survey Across Six Domain Groups
}
...
Table~\ref{tab:fourteen_scenarios} provides the deployment decision for fourteen canonical scientific scenarios organized into six domain groups, annotated with confidence levels and caveats.

\begin{table}[htbp]
\centering
\caption{Situs deployment decisions across fourteen scientific scenarios spanning six domain groups. $\delta_s^{\mathrm{PE}}$ is the change in detection margin; $\uparrow$ = improvement, $\approx 0$ = neutral, $\downarrow$ = harmful. Confidence: $\star\star\star$ = physically verified, $\star\star$ = theoretically grounded (unverified), $\star$ = speculative. All projected scenarios carry the $\star\star$ confidence rating: their $\delta_s^{\mathrm{PE}}$ estimates are derived from physical reasoning, not experimental measurement. Scenario N (OPC lithography) is added; Scenario C (wafer process simulation) is repositioned from Materials Science to Engineering Simulation (Semiconductor Manufacturing).}
\label{tab:fourteen_scenarios}
\footnotesize
...
\midrule
\textbf{C} & Wafer process simulation \newline{\scriptsize Eng. Simulation (Semi. Mfg)} & $\uparrow$ (cond.) & Medium & $\star\star$ & TCAD Sentaurus vs Victory Process: same process recipe, different doping profiles due to grid-dependent diffusion. $(x,y)$ wafer surface + $z$ depth Situs. Mesh convergence audit target. \\
\textbf{N} & OPC lithography \newline{\scriptsize Eng. Simulation (Semi. Mfg)} & $\uparrow$ & Large & $\star\star$ & OPC engines (Mentor Calibre, Synopsys Proteus, ASML Tachyon) compute mask pre-distortion compensating optical diffraction at $<1$\,nm precision on wafer $(x,y)$---Situs's most precise coordinate encoding. Simulation error vs real process variation = exact instance of Theorem~3: genuine lithographic defect and OPC model inaccuracy are indistinguishable from predicted contour alone. Yajie multi-engine consistency audit $\to$ hotspot localization $\to$ prioritize CD-SEM verification on maximally divergent regions. Value: shrinks verification cost from full-wafer impossible to hotspot-targeted affordable. Caveat: engines share optical models (Hopkins imaging, thin-mask approximation), weakening expert independence. \\
...
Materials science is Situs's home turf because 3D atomic coordinates \emph{causally determine} formation energies, elastic properties, and electronic structure. The two scenarios in this domain represent textbook-clear cases. (Wafer process simulation, previously Scenario C, is repositioned to Engineering Simulation \S\ref{sec:engineering} under the Semiconductor Manufacturing sub-domain alongside the new OPC lithography scenario.)
...
% Scenario C (Wafer Process Simulation) has been repositioned to
% Section 7 (Engineering Simulation) under the Semiconductor Manufacturing sub-domain,
% alongside the new OPC lithography scenario.
...
Engineering simulation encompasses two distinct but structurally analogous sub-domains: semiconductor manufacturing (where multiple TCAD and OPC engines simulate the same wafer-level physical processes) and computational mechanics (where multiple FEA/CFD solvers solve the same governing equations on the same geometry). Both share the same core Situs architecture—$(x,y,z)$ coordinates on a engineered structure—and the same fundamental challenge: distinguishing simulation artifact from physical reality.
...
Optical proximity correction (OPC) is the computational lithography step that pre-distorts photomask patterns to compensate for optical diffraction and resist effects during wafer exposure. Three commercial OPC engines—Mentor Calibre OPC, Synopsys Proteus, and ASML Tachyon—compute mask corrections independently from the same target design layout, creating a natural multi-expert audit scenario. Situs encodes wafer surface coordinates $(x,y)$ at nanometer-scale precision ($<1$\,nm), making OPC the most spatially precise Situs deployment considered in this paper: a single atomically-thin layer of photoresist defines the $z=0$ plane, and lateral positions are resolved to sub-nanometer edge-placement accuracy.

The core auditing problem is a direct instance of Theorem~3 (simulation error vs.\ real process variation indistinguishability): when two OPC engines predict different printed contours at the same $(x,y)$ location, the divergence could reflect either (i) a genuine lithographic defect mode that one engine captures and the other misses (label-relevant), or (ii) differences in optical model calibration, resist model parameterization, or numerical solver implementation (label noise). Yajie multi-engine consistency audit localizes hotspots—$(x,y)$ regions where predicted edge-placement error (EPE) diverges across engines beyond a calibrated threshold—and ranks them by divergence magnitude. CD-SEM metrology, the gold-standard but extremely low-throughput verification method ($\sim$hours per hotspot), is then prioritized on the maximally divergent regions. The value proposition is stark: full-wafer CD-SEM verification is economically impossible (millions of measurement sites), but Yajie-guided hotspot targeting reduces verification to tens to hundreds of sites—shrinking cost from impossible to affordable.

\textcolor{warning}{$\blacksquare$ Critical caveat}: OPC engines are not fully independent—they share fundamental optical models (Hopkins imaging theory, thin-mask approximation) and resist chemistry parameterizations. This shared heritage weakens the independence assumption underpinning Theorem~1's Chernoff bound and creates the risk of correlated blindness: all three engines may agree on an incorrect contour prediction because they all inherit the same thin-mask approximation error at a given feature. Furthermore, the $<1$\,nm encoding precision, while enabling the finest-grained Situs deployment, also makes the system maximally sensitive to wafer-grid alignment error—a 0.5\,nm stage misalignment turns Situs from signal into noise.
...
\item \textbf{Further domain candidates}: Magnetron sputtering, plasma bombardment, and irradiation damage—multi-scale simulation divergence in radiation-matter interaction (SRIM vs MARLOWE vs MD vs BCA) exhibits the same structural features that make Situs effective: independently developed simulation codes serving as natural experts, 3D spatial coordinates encoding collision cascade geometry, and the fundamental indistinguishability of Monte Carlo noise from genuine cascade variability (Theorem~3).
\end{enumerate}
