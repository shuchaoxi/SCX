% ============================================================
% Consistency-Constrained, Element-Guided, Gauge-Normalized ACE Potentials:
...
\title{Consistency-Constrained, Element-Guided, Gauge-Normalized ACE Potentials: \\
Identifiability, Gauge Fixing, and Shared-Correction \\
Parameterization Validated on Wurtzite AlN}
...
\subsection{Broader Limitations}

Beyond the specific issues enumerated above, several broader limitations of this work should be acknowledged. First, validation is restricted to a single binary compound (wurtzite AlN); generalization to other crystal structures, bonding types, and chemistries remains untested. Second, the gauge-fixing procedure is applied post-hoc rather than integrated into the training process, meaning the training dynamics themselves are not constrained---while prediction invariance is guaranteed, the optimization path may still explore gauge-degenerate regions in ways that affect convergence behavior. Third, the fair parameter-matched comparison (Section~\ref{sec:fair_baseline}) shows that the shared-correction architecture does not outperform a standard multi-element ACE with the same parameter count; the practical advantage is limited to physical interpretability of the coefficient decomposition, not raw accuracy. Fourth, extension to ternary (e.g., AlGaN) or quaternary (e.g., AlInGaN) systems, while theoretically straightforward, awaits empirical validation of composition-dependent accuracy. These limitations motivate the development of training-integrated gauge constraints, application to diverse material families, and rigorous testing on multi-component alloys.
...
% === CHANGELOG ===
% 2026-06-28: Title updated (Element-Guided, Gauge-Normalized); verified figure refs; added Limitations paragraph (Agent B)
\end{document}
