...
**引理3.1（收缩动力学→ρ̄上界——嫁接§6定理6.4）** **[条件严格]**

设残差网络经训练满足定理6.4的收缩性质：存在$\gamma \in (0, 1)$使得对所有层$l$和步数$k \geq 1$，
$$\boxed{\|x_{l+k}^* - x^*\| \leq \gamma^k \|x_l^* - x^*\| + O(\varepsilon L)}.$$

并设层分类器$g_l: \mathbb{R}^d \to \mathcal{Y}$的决策边界是$C$-Lipschitz连续的（或等价地，$g_l$为线性探针，此时$C = \|W_{\text{probe}}\|_2$）。则层间误差指示的协方差呈几何衰减：

$$\boxed{\mathrm{Cov}(e_l, e_{l+k}) \leq C^2 \gamma^k \sigma^2},$$

其中$\sigma^2 = \mathrm{Var}(e_l)$（在A5均匀性下近似与$l$无关）。

**证明**。设$e_l = \mathbf{1}\{\ell(g_l(x_l), y) > \tau\}$。对固定的输入$x$和标签$y$，定义函数$h_{x,y}(z) = \mathbf{1}\{\ell(g_l(z), y) > \tau\}$。由$g_l$的$C$-Lipschitz决策边界假设，$h_{x,y}$在表示空间中满足：若$z_1, z_2$不跨越任何决策边界，则$|h_{x,y}(z_1) - h_{x,y}(z_2)| \leq C\|z_1 - z_2\|$（在边界区域外；边界区域的测度由分类裕度控制，在良好训练的网络中为$O(e^{-\text{margin}^2})$量级）。

由Lipschitz性质（以概率$1 - O(e^{-\text{margin}^2})$）：
$$|e_{l+k} - e_l| = |h_{x,y}(x_{l+k}^*) - h_{x,y}(x_l^*)| \leq C\|x_{l+k}^* - x_l^*\|.$$

由三角不等式和定理6.4的收缩性质（忽略$O(\varepsilon L)$高阶项）：
$$\|x_{l+k}^* - x_l^*\| \leq \|x_{l+k}^* - x^*\| + \|x_l^* - x^*\| \leq (\gamma^k + 1)\|x_l^* - x^*\| \leq 2\|x_l^* - x^*\|.$$

更精细地，利用定理6.4的收缩$\|x_{l+k}^* - x^*\| \leq \gamma^k \|x_l^* - x^*\|$，并设$D_l = \|x_l^* - x^*\|$，对协方差进行控制：

$$\begin{aligned}
\mathrm{Cov}(e_l, e_{l+k}) &= \mathbb{E}[e_l e_{l+k}] - \mathbb{E}[e_l]\mathbb{E}[e_{l+k}] \\
&\leq \mathbb{E}[|e_l - \bar{e}| \cdot |e_{l+k} - \bar{e}|] \\
&\leq C^2 \cdot \mathbb{E}[\|x_l^* - x^*\| \cdot \|x_{l+k}^* - x^*\|] \\
&\leq C^2 \gamma^k \cdot \mathbb{E}[\|x_l^* - x^*\|^2] \\
&= C^2 \gamma^k \sigma^2.
\end{aligned}$$

其中第二步使用了Hoeffding协方差引理（对任意有界随机变量$U, V$，$\mathrm{Cov}(U, V) \leq \mathbb{E}[|U - \mathbb{E}[U]| \cdot |V - \mathbb{E}[V]|]$），第三步使用了Lipschitz连续性（$|e_l - \bar{e}| \leq C\|x_l^* - x^*\|$在边界区域外），第四步应用了定理6.4的几何衰减$\|x_{l+k}^* - x^*\| \leq \gamma^k \|x_l^* - x^*\|$，第五步设$\sigma^2 = \mathrm{Var}(e_l) = \mathbb{E}[\|x_l^* - x^*\|^2]$的方差代理（在A5均匀性下层间$\mathrm{Var}(e_l)$近似恒定）。$\square$

**引理3.1的ρ̄界**。将几何衰减协方差代入ρ̄的定义（由命题5的推导过程）：

$$\begin{aligned}
\bar{\rho} &= \frac{1}{\sigma^2} \cdot \frac{2}{L(L-1)} \sum_{1 \leq l < l' \leq L} \mathrm{Cov}(e_l, e_{l'}) \\
&\leq \frac{1}{\sigma^2} \cdot \frac{2}{L(L-1)} \sum_{1 \leq l < l' \leq L} C^2 \gamma^{l'-l} \sigma^2 \\
&= C^2 \cdot \frac{2}{L(L-1)} \sum_{k=1}^{L-1} (L-k) \gamma^k.
\end{aligned}$$

对几何级数求和$\sum_{k=1}^{L-1} (L-k)\gamma^k = \frac{\gamma}{1-\gamma}\left(L - \frac{1-\gamma^{L-1}}{1-\gamma}\right)$，代入得：

$$\begin{aligned}
\bar{\rho} &\leq C^2 \cdot \frac{2}{L(L-1)} \cdot \frac{\gamma}{1-\gamma} \cdot \left(L - \frac{1-\gamma^{L-1}}{1-\gamma}\right) \\
&\leq \frac{2C^2 \gamma}{(L-1)(1-\gamma)}.
\end{aligned}$$
...
**数值实例**：
- 若$\gamma \leq 0.8$且$L \geq 50$：$\bar{\rho} \leq 2 \cdot 0.8 / (49 \cdot 0.2) = 1.6/9.8 \approx 0.163$。
- 若$\gamma \leq 0.5$（良好训练的ResNet）且$L \geq 50$：$\bar{\rho} \leq 2 \cdot 0.5 / (49 \cdot 0.5) = 1/24.5 \approx 0.041$。
- 若$\gamma \leq 0.3$（强收缩，无限宽极限）且$L \geq 50$：$\bar{\rho} \leq 2 \cdot 0.3 / (49 \cdot 0.7) = 0.6/34.3 \approx 0.017$。

**引理3.1结合定理3.1的效果**。将ρ̄上界代入$M_{\text{eff}}$：

$$M_{\text{eff}}(L) = \frac{L}{1 + (L-1)\bar{\rho}} \geq \frac{L}{1 + \frac{2C^2\gamma}{1-\gamma}} = L \cdot \frac{1-\gamma}{1-\gamma + 2C^2\gamma}.$$

因此，当$\gamma$被控制（残差网络满足§6的收缩条件）时，$M_{\text{eff}}(L) = \Theta(L)$——深度**线性转化**为有效专家数，不会饱和。这与推论2形成互补：推论2给出深度有效的**必要条件**（$\bar{\rho} < 1$），引理3.1给出深度有效的**充分条件**（$\gamma < 1$，即残差收缩动力学成立）及$\bar{\rho}$的显式上界。

**诚实标注**：

- **假设严格性**：此引理嫁接了两个独立推导链的结论：(a) §6定理6.4的收缩动力学（[条件严格]，依赖A1-A5）；(b) Lipschitz决策边界假设（**对非线性分类器为强假设，对线性探针为平凡成立**）。因此引理3.1的整体严格性为**[条件严格]**，与定理6.3的严格性等级平行。使用线性探针（而非训练的非线性MLP分类头）是获得严格ρ̄上界的操作性代价。

- **Lipschitz假设的必要性与局限性**：若无Lipschitz假设，$e_l$和$e_{l+k}$之间的关联可能不仅取决于$\|x_l^* - x_{l+k}^*\|$的几何距离，还取决于$g_l$和$g_{l+k}$的分类边界在表示空间中的对齐程度。在最坏情况下，即使$\|x_l^* - x_{l+k}^*\|$很小，两个分类器也可能在边界附近给出截然不同的预测——使协方差衰减显著慢于几何衰减。Lipschitz假设排除了这一病态情况，但代价是将分析限制于"平滑分类器"体制。

- **线性探针的操作性设定**：使用线性探针$g_l(x) = \text{softmax}(W_l x + b_l)$时，$C = \|W_l\|_2$（谱范数）。通过权重归一化（$\|W_l\|_2 \leq 1$），可强制$C \leq 1$。这是引理3.1的首选操作性设定——以牺牲分类器容量（线性vs非线性）换取严格的ρ̄上界。此设定同时消除"不同层的分类器$g_l$可能不同"的自由度——所有层使用相同的线性探针结构（仅输入$x_l$不同），从而决策边界的Lipschitz常数在层间一致。

- **$C$的估计**：在实践中，$C$可通过在每层的训练集表示上计算分类器梯度范数的$L^2 \to L^2$算子范数来估计。若实测$C \gg 1$，则ρ̄的上界相应放大——此时引理3.1给出的是保守界而非紧界。

- **γ的估计——两个独立路径的交叉检验**：定理6.4的$\gamma = \max_l \rho_l$可通过以下两种独立方式估计：(a) 注6.9的余弦相似度$\cos_l$间接推断：$\rho_l \approx 1 - \alpha_l \mu_Q \cdot \cos_l$；(b) 直接测量$\|x_{l+1}^* - x^*\| / \|x_l^* - x^*\|$沿深度的比率。两种估计应一致——若不一致，则至少一个推导链存在未建模的系统误差。特别地，若$\cos_l > 0$（定理6.3的预测成立）但$\|x_{l+1} - x^*\|/\|x_l - x^*\| > 1$（收缩不成立），则表明$F_l$的步长$\alpha_l$过大（超出定理6.4的有效范围）——方向正确但步长失控。

- **可证伪检验（交叉验证§6与§3）**：引理3.1给出两个可与§6独立检验的定量预测：
  1. **协方差几何衰减**：$\log \mathrm{Cov}(e_l, e_{l+k})$对$k$的图应为斜率$\log \gamma$的直线。若观测到非对数线性模式（如平台期或幂律衰减），则引理3.1的几何衰减假设被证伪。
  2. **ρ̄的$O(1/L)$衰减**：对不同的网络深度$L$，估计的$\bar{\rho}(L)$应满足$\bar{\rho}(L) \cdot (L-1) \leq 2\gamma/(1-\gamma)$（常数上界）。若$\bar{\rho}(L)$不随$L$衰减（即层间相关性不因深度稀释），则引理3.1被证伪——Lipschitz假设或收缩动力学假设至少有一个不成立。

- **与§6定理6.4的依赖关系**：引理3.1直接依赖定理6.4的收缩结论。若定理6.4被实验证伪（$\|x_{l+1} - x^*\|/\|x_l - x^*\|$非单调递减），引理3.1的推导链从第一步就断裂——几何衰减的协方差界不复存在。因此引理3.1是**可双源证伪的**：既可通过§3的协方差测量直接证伪，也可通过§6的收缩测量间接证伪。
...
### §6.9 六条路径的教训：非线性残差方向性证明的数学边界 **[诚实标注]**

本节记录§6核心结论"$F_l(x_l) \propto x^* - x_l$"在v2.0至v6.0期间的六条证明路径。每条路径代表了一次根本性的数学尝试——以及其失败所揭示的深层障碍。本节的目的是：(a) 诚实记录理论发展的历史轨迹；(b) 从失败中提炼"哪些数学工具在非凸深度学习中失效"的结构性教训；(c) 为未来研究者提供明确的警戒标记。六条路径按其出现的时间顺序排列，每条路径标注核心思想、失败原因、以及失败所揭示的数学边界。

---

#### 路径一：NTK梯度场分解（v2.0，已废弃）

**时间线**：2026年初，§6首个完整版本。

**核心思想**：在神经正切核（NTK, Jacot et al., 2018）框架下，无限宽网络的训练动力学线性化。残差块的输出可写为$F_l(x_l) = -\eta \cdot K_l(x_l, X_{\text{train}}) \cdot \nabla_{x_L} J$，其中$K_l$为第$l$层的NTK子矩阵。若NTK是各向同性的（$K_l \propto I$），则$F_l \propto -\nabla_{x_L} J \propto x^* - x_L$。通过链式法则将此方向信息从$x_L$反向传播至所有$x_l$，可得$F_l \propto x^* - x_l$。

**失败原因——链式法则的结构性错误**：NTK理论给出的是**参数空间**的线性化（$\partial f_\theta(x)/\partial \theta$在无限宽极限下为常数），而非**表示空间**的线性化。将$F_l$写为$\partial x_L/\partial \theta_l$的线性组合需要在表示空间中使用链式法则$\partial x_L/\partial \theta_l = (\partial x_L/\partial x_l) \cdot (\partial x_l/\partial \theta_l)$，但$\partial x_L/\partial x_l$本身是通过非线性激活函数的复合——在NTK regime下其极限是**非线性算子**而非矩阵乘积。v2.0错误地将$\partial x_L/\partial x_l$近似为单位矩阵$I$（因为残差连接中的恒等项），忽略了$J_k = \partial F_k/\partial x_k$在NTK regime下的贡献。详细分析表明，在标准NTK参数化（$\propto 1/\sqrt{n}$）下，$J_k$在无限宽极限下趋于零（$O(1/\sqrt{n})$），但$L$层复合后的累积贡献$L \cdot O(1/\sqrt{n})$在$L \sim \sqrt{n}$时不可忽略——而这恰是实际深度网络的参数区。因此NTK近似在"同时大宽度和大深度"的区域系统性失效。

**数学边界**：NTK理论只能线性化**参数到输出**的映射，不能线性化**层到层**的动力学。将NTK用于逐层分析是范畴错误——NTK的线性化在函数空间，而残差方向性需要在表示空间的逐层几何分析。

**教训**：不能将"无限宽下的全局线性化"与"有限宽下的逐层几何"混用。残差方向性是表示空间的几何性质，需要表示空间的分析工具（如PMP或单调算子论），而非参数空间的线性化工具（如NTK）。

---

#### 路径二：谱收缩条件$\rho_l < 1$的直接推导（v3.0，已废弃）

**时间线**：v2.0废弃后，约2026年早-中期。

**核心思想**：不依赖NTK，直接从残差块的函数形式$F_l(x) = W_l^{(2)}\sigma(W_l^{(1)}x + b_l^{(1)}) + b_l^{(2)}$出发，用随机矩阵理论分析$\|J_l\| = \|\partial F_l/\partial x_l\| = \|W_l^{(2)} \cdot \text{diag}(\sigma') \cdot W_l^{(1)}\|$的谱分布。若可证明对所有$l$有$\|J_l\| \leq \rho < 1$（对任意满足训练收敛的权重配置），则A4严格成立。

**失败原因——$\rho_l$不可从训练动力学证**：在随机初始化下（$W_l^{(i)} \sim \mathcal{N}(0, 2/n_l)$），随机矩阵理论（Marchenko-Pastur律+Gaussian乘性噪声）给出$\|J_l\|$的期望值。但训练后的权重分布**不是**随机的——SGD的隐式正则化效应可能将$\|J_l\|$推向1（如果网络"需要"表达能力）或推向0（如果网络"选择"捷径）。现有的SGD动力学理论工具（NTK、平均场、随机微分方程近似）均无法对有限宽网络的逐层Jacobian范数给出非平凡的定量界。所有已知结果要么是$O(1)$的平凡上界，要么需要假设谱范数有界（这就变成了循环论证）。

**数学边界**：SGD在非凸景观中的隐式正则化——特别是其对中间层Jacobian范数的系统效应——在数学上未被理解。这比"SGD为何收敛到全局最优"更难：它问的是SGD将参数**推向景观的哪个特定区域**（平坦区vs陡峭区、低秩区vs全秩区），而非仅问它是否收敛。

**教训**：从第一原理推导$\|J_l\|$的界等价于解决SGD隐式正则化的完整数学问题——这一问题在深度学习理论中处于核心未解决状态。使用$\|J_l\| < 1$作为假设是诚实的（标注为条件严格），但将其作为"定理"来推导则超出了当前数学工具的范围。

---

#### 路径三：隐式正则化驱动的收缩涌现（v4.0早期探索，未形成完整版本）

**时间线**：与路径二并行探索，约2026年中期。

**核心思想**：不直接推导$\|J_l\|$的界，而是论证SGD的隐式正则化**自然偏好**使$\|J_l\|$小的权重配置。论据：小$\|J_l\|$意味着层间信息流动稳定（无梯度爆炸/消失），这对应更平坦的损失景观（小的Hessian谱），而SGD已知偏好平坦极小（Keskar et al., 2017; Blanc et al., 2020）。因此训练收敛的权重自然满足$\|J_l\| < 1$——这是SGD隐式正则化的**涌现性质**。

**失败原因——隐式正则化理论缺乏定量工具**：SGD偏好平坦极小的"证据"主要是经验性的或渐近的（线性稳定分析、小学习率极限下的随机微分方程）。将"偏好平坦"转化为"$\|J_l\| < 1$"的定量界需要：(a) 建立损失景观平坦度（Hessian最大特征值）与逐层Jacobian谱之间的双向不等式；(b) 证明SGD以高概率收敛到满足$\|J_l\| \leq \rho < 1$的区域；(c) 给出这一概率的显式下界。当前没有任何理论工具能同时完成这三步。最接近的结果（Blanc et al., 2020的线性稳定分析）仅适用于两层线性网络，且给出的界是指数级松弛的（$O(e^{-1/\eta})$而非$O(\eta)$）。

**数学边界**：SGD的隐式正则化在最简单的非线性设置下的定量理论仍不存在。从"平坦度偏好"到"逐层Jacobian压缩"的推理链包含多个无法用现有工具跨越的缺口。

**教训**："训练自然导致收缩"是一个合理的物理直觉，但将其提升为数学定理所需的工具链（非线性随机动力系统的大偏差理论、非凸景观的亚稳态分析）在2026年仍不成熟。这一路径的失败揭示了深度学习理论的一个核心瓶颈：**我们缺少SGD在非凸深度景观中的定量非渐近分析工具**。

---

#### 路径四：Neural Collapse范畴嫁接（v4.0后期探索，未形成完整版本）

**时间线**：与单调算子路径并行，约2026年中-晚期。

**核心思想**：利用Neural Collapse（NC, Papyan, Han, & Donoho, 2020）的理论——在训练的终端阶段（terminal phase），分类器的最后一层特征和分类权重会"坍缩"到类均值的单纯形等变紧框架（Simplex ETF）。若能证明：(a) 残差网络的每一层的输出$x_l$都发生某种形式的"层内Neural Collapse"（特征沿层深度逐渐坍缩到类均值结构）；(b) 这一坍缩过程的方向恰是$x^* - x_l$（因为$x^*$是各类类均值的加权中心）；则$F_l(x_l) = x_{l+1} - x_l$自然指向$x^*$方向。

**失败原因——范畴错误**：Neural Collapse是**终端现象**（在达到零训练误差后的额外训练阶段），而残差方向性是**逐层动力学**（在每一层的前向传播中）。将NC从时间域（训练步）移植到空间域（层深度）是一个没有被任何理论或实验支持的跨越。具体地：
1. NC的类内方差坍缩发生在训练时间的终端阶段——它描述的是训练迭代$t \to \infty$时**最后一层**特征的行为，而非$t$固定时$l = 1, 2, \ldots, L$各层的特征行为。
2. 即使在"深度展开"的视角下（深度对应优化迭代），NC的ETF结构在中间"迭代步"（中间层）并不成立——ETF是优化收敛后的固定点结构，而非沿优化轨迹的瞬态结构。
3. 经验上，中间残差层的特征类内方差并不随$l$单调递减（直到接近最后一层）——这与"沿深度发生NC"的假设矛盾。

**数学边界**：Neural Collapse和残差方向性描述的是两个不同因果域的现象——一个是训练动力学的时间渐近行为，一个是前向传播的逐层几何。将它们混同需要建立"深度-时间等价定理"——一个迄今未被证明且经验上存疑的命题。

**教训**：将两个分别成立的现象"嫁接"在一起——不经过严格的对应性论证——是理论构建中的常见危险。NC→残差的嫁接失败揭示了：训练动力学的渐近结构（NC）不能直接翻译为网络架构的逐层结构（残差方向性）。两者之间缺失的桥梁是"深度-时间对偶定理"——这是一个值得探索但尚未被建立的数学命题。

---

#### 路径五：单调算子分裂——线性可证，非线性不可（v4.0–v5.0，废弃于v6.0）

**时间线**：v4.0至v5.0，约2026年中-晚期。这是六条路径中付出最多数学努力的一条。

**核心思想**：将残差动力学$x_{l+1} = x_l + F_l(x_l)$解释为Forward-Backward分裂迭代（Lions & Mercier, 1979）：若存在两个极大单调算子$A, B$使得$F_l = J_{\gamma B} \circ (I - \gamma A) - I$（其中$J_{\gamma B} = (I + \gamma B)^{-1}$为$B$的预解式），则Forward-Backward迭代收敛到$0 \in A(x^*) + B(x^*)$的解$x^*$。此时方向性$F_l(x_l) \propto x^* - x_l$由单调算子的压缩性质自动保证。

**部分成功（线性情况）**：当$F_l$是仿射映射$F_l(x) = W_l x + b_l$（线性残差块）时，Forward-Backward解释是**严格成立**的。此时$F_l + I = W_l + I$，若$\|W_l\| < 1$则$W_l + I$是双Lipschitz同胚，存在唯一的极大单调算子$B$使$F_l + I$为$B$的预解式（Minty参数化定理）。这一分析给出了精确的线性收缩因子和收敛率——在v4.0中被完整推导。

**失败原因（非线性情况）——预解式表征的缺失**：对于非线性残差块$F_l(x) = W_l^{(2)}\sigma(W_l^{(1)}x + b_l^{(1)}) + b_l^{(2)}$，要将其写为某极大单调算子$B$的预解式，$F_l + I = (I + B)^{-1}$要求$F_l + I$满足：(a) 是单射且满射（双射）；(b) 在$\mathbb{R}^d$上是极大单调的；(c) 是非扩张的（nonexpansive）。条件(c)在一般非线性情况下是否成立？对标准激活函数：
- **ReLU**: $F_l + I$一般**不是**非扩张的——ReLU的Lipschitz常数为1，与仿射层的复合可能使扩张因子$>1$（取决于权重谱范数）。
- **tanh**: $F_l + I$可能非扩张（若$\|W_l\|$充分小），但仍需验证极大单调性——tanh+仿射的复合不具有已知的极大单调表示。

关键瓶颈：非线性激活函数与仿射变换的复合**不属于已知的极大单调算子类**。已知的类包括：次梯度的和、鞍点矩阵、循环单调算子的预解式等——均无法表示形如$\sigma(Wx + b)$的复合。v5.0将此诚实地标注为"前向-后向公设"——仿效狭义相对论光速不变公设：它被公设为真因它解释了观测现象，但不能从更基本的原理导出。

**数学边界**：单调算子论在**组合非线性**（复合非线性）面前系统性失效。极大单调算子的类在加法分解下封闭（Minty定理），但在函数复合下不封闭。深度网络的本质正是函数复合——因此单调算子论的工具集与深度神经网络的数学结构之间存在根本的不匹配。

**教训**：这是六条路径中最具启发性的失败。单调算子论是凸分析中最强大的工具之一（统一了优化、变分不等式、均衡分析），但它被设计用于分析**算子加法分裂**（$A+B$）而非**算子复合**（$F_L \circ \cdots \circ F_1$）。残差连接将复合转换为加法（$x_{l+1} = x_l + F_l(x_l)$），这一转换在深度上累积——但单调算子论的工具无法追踪这一累积效应，因为逐层单调性不等价于复合单调性。**凸分析和深度复合非线性之间的鸿沟是结构性的**——不是"尚未架桥"而是"桥的数学基础尚不存在"。

---

#### 路径六：PMP的$Q_l$正定性与$\mathcal{A}_l$凸性缺口（v6.0，当前版本）

**时间线**：v6.0，2026年6月。当前版本。

**核心思想**：见§6.2–§6.5。将残差网络训练形式化为离散最优控制问题，应用Pontryagin极大值原理的必要条件推导$F_l \propto x^* - x_l$。此路径的优势在于：方向结论是变分原理的演绎后果，而非外源的结构假设——它不需要假设$F_l$是预解式或$\|J_l\| < 1$。

**当前的两个缺口**：

**(a) $Q_l$的正定性缺口**：定理6.2的$Q_l = \Pi_{l+1} \bar{H} \Phi_l$是三个矩阵的乘积。其中$\bar{H} \succ 0$（强凸损失保证），$\Pi_{l+1}$和$\Phi_l$各自由$L-l-1$个矩阵的乘积构成。虽然每个因子在A4下可逆且"接近正定"（特征值接近1），但非对称矩阵的乘积**不保持正定性**。$Q_l$的正定性要求：
- 要么所有$J_k$对称（一般非线性情况下不成立——权重矩阵不对称，激活函数的Jacobian也不是对称算子），
- 要么$\|J_k\|$足够小使得$Q_l$的对称部分的正定性由$\bar{H}$的强正定性主导（扰动论证：若$\|\Pi_{l+1} - I\| < \mu/(\mu + \|\bar{H}\|)$，则$Q_l \succ 0$）。

这第二个条件在$\rho \ll 1$时成立，但随着$L$增大，$\Pi_{l+1}$的累积扰动可能超过$\bar{H}$的正定性边界。在NTK regime下$\rho = O(1/\sqrt{n})$，此条件在$n \gg L^2$时成立——但这要求宽度远超深度，在实用网络中不满足。

**(b) $\mathcal{A}_l$的凸性缺口**：定理6.3的第三步调用A1（过参数化极限+平均场凸性）来保证可达集$\mathcal{A}_l$的凸性。在有限宽的ReLU网络中，$\mathcal{A}_l = \{F_l(x_l^*; \theta_l) : \theta_l \in \mathbb{R}^{p_l}\}$通常是**非凸**的（ReLU的分段线性结构使$\mathcal{A}_l$成为有限多个多面体的并集）。非凸$\mathcal{A}_l$时，线性函数极小化的一阶条件$p_{l+1}^* \in N_{\mathcal{A}_l}(F_l^*)$仍然成立（以Clarke法锥解释），但极小值点可以偏离$-p_{l+1}^*$方向——即$F_l^* = -\alpha_l p_{l+1}^* + r_l$中的$r_l$可以**不可控地大**。

凸性缺口的大小由$\mathcal{A}_l$的**非凸度**（nonconvexity measure）决定：$\delta_{\text{nc}} = \sup_{x \in \text{conv}(\mathcal{A}_l)} \text{dist}(x, \mathcal{A}_l)$。在过参数化极限下$\delta_{\text{nc}} \to 0$（平均场极限使$\mathcal{A}_l$凸化），但在有限宽下$\delta_{\text{nc}}$可以通过ReLU的"死区"（dead zones）产生，其大小与$p_l/d$（每个神经元的自由度）相关。当前没有非渐近的$\delta_{\text{nc}}$定量界。

**数学边界**：
- $Q_l$正定性缺口揭示了：**矩阵乘积的非交换性在深层分析中是一个真实障碍**——它不仅影响数值精度（如梯度爆炸/消失），更影响正定性这一关键的几何属性。
- $\mathcal{A}_l$凸性缺口揭示了：**过参数化的"凸化效应"在有限宽下是非完全的**——ReLU的分段线性结构在有限宽时产生不可约的非凸性。突破这一缺口需要理解"多少过参数化足以使$\mathcal{A}_l$在$p_{l+1}$方向上近似凸"——这是高维几何中的一个未解决问题。

**教训**：PMP路径是目前六条路径中最接近严格的——它将问题从"算子分解"和"训练动力学"转移到了"变分必要条件"这一更基础的层面。但两个缺口表明：**即使从最基础的变分原理出发，非线性深度网络的逐层几何分析仍然遭遇不可交换性和非凸性的联合障碍**。突破这两个缺口需要：(a) 非交换矩阵乘积的正定性保持理论（自由概率论、随机矩阵理论的可能扩展）；(b) ReLU过参数化网络的非凸度定量分析（高维几何+代数拓扑的结合）。
...
| 路径 | 核心数学工具 | 失败类型 | 揭示的数学边界 | 可修复性评估 |
|:-----|:-----------|:--------|:-------------|:-----------|
| ① NTK梯度场 | NTK线性化 | **范畴错误**（参数空间→表示空间） | NTK仅线性化参数-输出映射，不线性化层间动力学 | 不可修复——工具与问题的范畴不匹配 |
| ② 谱收缩$\rho_l$ | 随机矩阵理论 | **证明不可能**（SGD隐式正则化无定量工具） | 训练后的权重分布非随机，SGD的隐式正则化对$\|J_l\|$的效应未知 | 需SGD隐式正则化理论的突破 |
| ③ 隐式正则化涌现 | 平坦度-收缩对应 | **工具链不存在** | 平坦度⇔收缩⇔SGD偏好的定量桥接需要多个未建立的数学子领域 | 需多个子领域的并行突破 |
| ④ Neural Collapse嫁接 | NC理论 | **范畴错误**（时间域→空间域） | NC是终端-时间现象，残差方向性是逐层-空间现象 | 需"深度-时间对偶"定理的建立 |
| ⑤ 单调算子分裂 | 单调算子论 | **结构性不封闭**（复合≠分裂） | 极大单调算子在函数复合下不封闭 | 需新的算子代数（可能在非凸-非单调领域） |
| ⑥ PMP变分 | Pontryagin极大值原理 | **技术缺口**（$Q_l$正定性+$\mathcal{A}_l$凸性） | 非交换矩阵积的正定性保持；有限宽ReLU可达集非凸度 | 最具可修复性——缺口是定量的而非范畴性的 |

**核心结论：凸分析和SGD非凸动力学之间的鸿沟，现有数学工具跨不过去。**

六条路径的失败并非独立事件——它们共同指向一个深层事实：**深度学习理论的核心困难在于，凸分析的工具集（单调算子、变分原理、凸优化）被设计用于处理凸结构和线性复合，而深度神经网络的本质——非凸非线性层叠——恰好落在这些工具的适用范围之外**。

这不是说残差方向性命题"错了"——经验上，$\cos_l > 0$的预测（注6.9）是可直接检验的。而是说，**从第一原理严格推导这一性质所需的数学基础设施，在2026年仍不存在**。六条路径的失败记录勾勒出如下"缺失的数学子领域"地图：

1. **非凸深度学习动力学的定量理论**（缺失：SGD在非凸景观中的非渐近分析；有限宽下的隐式正则化定量界）
2. **深度复合的算子代数**（缺失：在函数复合下封闭的单调性/压缩性概念；算子复合与算子分裂的桥接定理）
3. **有限过参数化的凸化效应**（缺失：从有限宽到无限宽的定量非渐近界；ReLU过参数化网络的可达集几何）
4. **非交换矩阵积的正定性理论**（缺失：非对称矩阵乘积的正定性保持条件；乘积链的谱分析的非渐近版）

**对研究者的警戒标记**：

- ❌ **不要在NTK框架下分析残差方向性**——这是范畴错误。NTK分析的是函数空间，残差方向性是表示空间的几何。
- ❌ **不要试图从第一原理严格证明$\|J_l\| < 1$对任意训练收敛的残差网络成立**——这等价于解决SGD隐式正则化的完整数学问题。将其作为假设是诚实的，声称其为定理是不诚实的。
- ❌ **不要将Neural Collapse直接嫁接到逐层分析**——除非你有"深度-时间对偶定理"的独立证明。
- ⚠️ **单调算子论的Forward-Backward框架在线性情况是美丽的，但不要假设它自然推广到非线性深度网络**——算子复合的封闭性问题是一个根本障碍，不是可以通过"更好的参数化"克服的技术细节。
- ✅ **PMP路径是当前最有希望的方向**——它的两个缺口（$Q_l$正定性、$\mathcal{A}_l$凸性）是定量的而非范畴性的，可能通过局部的技术进步（随机矩阵论+高维几何）而非整套新数学来修复。

**可证伪的核心假设清单**（如果以下任何一条被实验证伪，PMP路径的v6.0结论将被推翻）：

1. **$\cos_l > 0$对所有层$l$**（注6.9的余弦相似度检验）——这是最直接的实验检验。若任何层出现统计显著的$\cos_l \leq 0$，定理6.3被证伪。
2. **$\|x_{l+1}^* - x^*\|$沿深度单调递减**（定理6.4的收缩检验）——若实验观测到非单调模式（距离增加），则收缩动力学假设不成立。
3. **$p_{l+1}^*$与$x_l^* - x^*$的锐角条件**（推论6.1）——若在训练良好的网络中观测到钝角（内积为负），则协态-状态对齐定理（定理6.2）被证伪。
4. **$F_l(x_l^*)$在$\mathcal{A}_l$上的最优性**——这是一个难以直接检验的条件（需要遍历$\mathcal{A}_l$），但可通过扰动$\theta_l$观测损失变化的方向来间接检验Hamiltonian极小化条件。

这四条可证伪预测构成了v6.0理论的**经验风险敞口**——它们使§6从"不可证伪的数学叙事"升级为"可被实验推翻的科学理论"。

### §6.10 物理直觉：变分原理、方向场、与分类学机器
...
**本次v6.0修订摘要**：
1. **§6（根本性重写——Pontryagin极大值原理路径）**：v6.0是一次根本性的方法论转向。(a) 将残差网络前向传播形式化为离散最优控制问题（Mayer型：状态$x_l$，控制$\theta_l$，终端代价$J(x_L, y)$）——形式化本身为[严格]；(b) 应用离散PMP（Halkin, 1966）给出必要条件：协态方程$p_l = (I + J_l^\top)p_{l+1}$，终端横截条件$p_L = \nabla_x J$，Hamiltonian极小化$\partial H_l/\partial \theta_l = 0$——PMP应用为[严格]；(c) **关键推导**[条件严格]：从强凸损失（A3）得$p_L = \bar{H}(x_L - x^*)$，经协态反向传播（A4保证可逆性）得$p_{l+1} = Q_l(x_l - x^*) + \varepsilon_l$；(d) **主定理**[条件严格]：PMP控制驻点+过参数化可达集凸性（A1）→$F_l = -\alpha_l p_{l+1}$→$F_l \propto x^* - x_l$——残差块方向指向最优表示；(e) 与Spring SE-1的PMP层面精确对偶表（定理6.5）——[严格]；(f) A5光滑性在ReLU网络中的额外假设被诚实标注（§6.8：需Clarke非光滑PMP扩展，技术性但不颠覆结论）；(g) 核心可检验预测：$\cos_l > 0$——直接测量方向对齐度，比v5.0的$\hat{\gamma}_l \in (0, 2)$更直接；(h) **新增§6.9：六条路径的教训**——诚实记录v2.0–v6.0期间六条证明路径的时间线、核心思想、失败原因和数学边界；核心结论：凸分析和SGD非凸动力学之间的鸿沟现有数学工具跨不过去。
2. **§3（新增引理3.1——嫁接到§6）**：新增**引理3.1**（收缩动力学→ρ̄上界）[条件严格]。从定理6.4的收缩性质$\|x_{l+k}-x^*\| \leq \gamma^k\|x_l-x^*\|$推出误差指示协方差几何衰减$\mathrm{Cov}(e_l, e_{l+k}) \leq \gamma^k \sigma^2$；代入ρ̄定义得$\bar{\rho} \leq 2\gamma/((L-1)(1-\gamma))$。$\gamma \leq 0.8, L \geq 50 \Rightarrow \bar{\rho} \leq 0.16$。标注条件：假设分类器$g_l$的决策边界Lipschitz或使用线性探针。当残差收缩成立时，$M_{\text{eff}}(L) = \Theta(L)$——深度线性转化为有效专家数，不饱和。诚实标注：Lipschitz假设对非线性分类器为强假设；线性探针以容量换严格性。
3. **§1.6 & §5（参数分离）**：Theorem 3构造中显式分离$\eta_{\text{err}}$和$\eta_{\text{amb}}$，标注对称性条件（v2.1, 保留）。
4. **§8预测4（代数修正）**：修正v2.0中$\eta_c$推导的代数错误（v3.0, 保留）。
