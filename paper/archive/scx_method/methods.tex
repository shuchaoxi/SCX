...
\textbf{Stage~1: State Crystallization (two-layer state discovery).} State Crystallization is the process of discovering natural state boundaries in continuous physical observables and producing discrete state atoms---the ontological foundation on which subsequent Spring self-evolution and Yajie auditing operate. Unlike statistical discretization schemes such as Byte Pair Encoding (BPE) in language models, where tokens are formed by frequency-driven merging with no physical anchor, State Crystallization is \textit{physically driven}: the state boundaries are determined by the intrinsic structure of the observables (bond angles, bond lengths, local coordination environments) rather than by human-imposed labels or statistical convenience.
...
\State \textbf{Stage 1: State Crystallization}
...
Before committing to the full SCX pipeline, we recommend a lightweight diagnostic based on bootstrap stability (Proposition~6 in Paper~I~[arXiv:XXXX]). The procedure is:
