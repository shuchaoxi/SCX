...
(2)~\textbf{多样性-规模权衡定理}——$M$ 个相关专家与 $M'$ 个独立专家的效率等价条件；
(3)~\textbf{相关性结构下的共识下界}——已知相关矩阵时多数投票错误的非平凡下界；
(4)~\textbf{策略性投票的审计检测界}——审计者以有限样本检测策略性谎报的统计极限。
每个定理均附形式证明与数值示例，
并以\textcolor{red}{\textbf{诚实暴击}}标注其边界条件、
隐藏假设与不能成立的情形。

\medskip
\noindent\textbf{关键词：} 集体智能；Condorcet 陪审团定理；异质加权投票；多样性-规模权衡；
...
$\hat{\theta} = \sgn\bigl(\sum_{i=1}^n X_i\bigr)$ 的错误概率随~$n$ 指数衰减。
...
\begin{attackbox}
  ...
  \textbf{在指数衰减的决策边界附近，小权重误差可能导致灾难性误判}。
...
\end{tikzpicture}
\caption{不同共同因子载荷 $\rho$ 下多数投票错误概率的指数衰减速率。
$\rho$ 越大，有效规模增长越慢。}
\label{fig:divscale}
\end{figure}
...
\begin{attackbox}
  ...
  \medskip
  \textbf{更根本的暴击}：定理假设协方差矩阵 $\Sigmav$ \emph{已知}。
...
\subsection{基于一致性的检测统计量}

\begin{theorem}[一致性异常检测界]
\label{thm:audit}
...
\begin{attackbox}
  \textbf{一致性统计量对协调攻击的盲区}。
  定理~\ref{thm:audit} 假设策略型专家\emph{降低}跨组一致性。
  但若攻击者了解审计方法，可采取\textbf{一致性保持策略}：
...
\subsection{基于分歧图的高阶检测}

\begin{theorem}[分歧图谱检测]
\label{thm:spectral}
定义分歧图 $G = (V,E)$，其中 $V = \{1,\ldots,n\}$，边 $(i,j) \in E$ 当且仅当 $X_i \neq X_j$。
...
\begin{attackbox}
  \textbf{终极暴击：形式化框架本身的局限}。

  本文所有定理均建立在以下元假设之上：
  \begin{enumerate}[label=(\alph*)]
    \item \textbf{二元状态}：$\theta \in \{-1,+1\}$。现实决策通常是连续的、
    ...
    $\theta$ 可供事后验证。在预测性任务中这成立；
    但在规范性判断（「应该做什么」）中，$\theta$ 可能不存在。
    \item \textbf{诚实暴击标注本身}：本文的「诚实暴击」标注试图
...
\section{加权规则与贝叶斯模型平均的等价性}

本节阐明定理~\ref{thm:optimalweight} 的加权规则与贝叶斯模型平均（BMA）的等价性。
