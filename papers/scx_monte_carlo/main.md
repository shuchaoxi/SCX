...
> 
> 
> $\Situs$ 配备 Situs 度规 $h_$，由 Cercis 函数的 Hessian 定义：
...
$$<!-- label: eq:adaptive -->
    \epsilon_k = \epsilon_0 \cdot \min\!\left(1, \frac{\bar}{\lambda_{max}(h(q_k))}\right)
$$

其中 $\bar = \frac{1}{d \cdot |\Situs|} \int_ \Tr(h_(q)) \, dq$ 为
...
> $C_t = \Ocal(B^2 \cdot t)$。对于 SCX 审计，$t$ 通常为 $\Ocal(1)$ 到
...
> $$
> 且 $ar{A}_{r,r+1} pprox 0.2$–/usr/bin/bash.5$ 时，加速因子可达 $\Ocal(10^2)$–$\Ocal(10^4)$。
...
> （$m \sim 10^4$, $n \sim 10^3$），Situs 度规的求值可通过稀疏近似加速。对于大规模问题（$m \sim 10^4, n \sim 10^3$），Situs 度规矩阵尺寸达 $\mathcal{O}(10^7 \times 10^7)$，必须利用其分块对角结构 （$\partial^2 Cercis/\partial g_{ij}\partial g_{kl} = \delta_{ik}(\delta_{jl} + 2\lambda)$）实现高效求值。
...
1. **Cercis 函数符号错误（关键）**：原公式 Cercis(E) = (1/2)Σg² - λΣ(Σg)² 
   中第二项为负号，导致偏离共识时 Cercis 反而降低。修正为 +λ 项，
   确保 Boltzmann 分布适定性。同时添加说明文字澄清符号选择。

2. **Situs 度量定义不完整**：原定义使用未定义的 \mathcal{S}_{total}，
   改为显式的 Cercis(E) 的 Hessian 矩阵：h_{ij,kl} = ∂²Cercis/∂g_{ij}∂g_{kl}。

3. **哈密顿运动方程符号错误（关键）**：eq:eom_p 中度量修正项的符号为+，
   正确的哈密顿方程 dp/dt = -∂H/∂q 应给出负号。修正为 -½∇_q[p^T M^{-1}p]。

4. **广义蛙跳积分器缺失 epsilon**：步长更新中 rac{2} 和 rac{4} 
   缺少 \epsilon 分子，修正为 rac{\epsilon}{2} 和 rac{\epsilon}{4}。

5. **自适应步长公式错误**：平均特征值 ar{\lambda} 的计算公式
   误将迹(trace)当作平均特征值，缺少 1/d 因子。添加维度因子修正。

6. **ESS 概率界不严谨**：原 \ESS ≥ K/(B²log(1/δ)) 的界缺少严格推导，
   改为 \ESS ≥ K/B² - O(√(K log(1/δ))/B)。

7. **MSE 指数界过于悲观**：C_t = O(B^{2t} log t) 在 t 较大时导致
   方法不可行。在 SCX 审计典型条件下(t~1-100, B~2-5)，
   修正为 C_t = O(B² · t)。

8. **REX 混合时间公式错误**：原公式 	au_{mix}^{REX} ≤ 	au_{mix}·min{1/ar{A}·	au_{mix}(T_r)/	au_{mix}(T_1)}
   逻辑不自洽（RHS > LHS 意味着REX更慢）。修正为 	au_{mix}^{REX} ~ 	au_{mix}(T_R)·R/minar{A}。

### 裁决
R5 发现 8 个问题，其中 2 个关键符号错误（Cercis 和运动方程），
均已即时修复。文件状态可继续下一轮审查。
