...
73|\end{quote}
74|
75|\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}
76|
...
105|
106|\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}
107|
108|\subsection{Chapter 1: Origins --- The EGP Gauge-Fixing
109|Problem}\label{chapter-1-origins-the-egp-gauge-fixing-problem}
110|
111|\subsubsection{1.1 The Practical Problem}\label{the-practical-problem}
112|
...
137|
138|\textbf{(I1) Energy reference ambiguity.} Forces determine the potential
...
144|
145|\textbf{(I2) Species-shift misalignment.} The per-species constant
...
150|
151|\textbf{(I3) Coefficient-level gauge freedom.} The shared-correction ACE
...
162|active.
163|
164|\textbf{(I4) Residual meaning incompatibility.} Domain-specific
...
179|insight was \emph{negative}: a soft penalty approach---adding
...
190|\textbf{consistency constraints must be applied post-hoc, not enforced
...
203|
204|This observation---that \textbf{expert reliability is not a global
...
229|\textbf{State Crystallization}.
230|
231|\subsubsection{2.2 Why ``Crystallization''}\label{why-crystallization}
232|
...
245|\textbf{states are discovered, not defined.}
246|
...
253|
254|\begin{longtable}[]{@{}
255|  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2895}}
256|  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.5789}}
257|  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1316}}@{}}
258|\toprule\noalign{}
259|\begin{minipage}[b]{\linewidth}\raggedright
260|Dimension
261|\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
262|State Crystallization
263|\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
264|BPE
265|\end{minipage} \\
266|\midrule\noalign{}
267|\endhead
...
272|frequency \\
273|Ontology & \textbf{Discovers} natural state boundaries &
274|\textbf{Conventions} for cutting \\
275|Mathematical form & \(\mathcal{D}_{\text{phys}}\) &
276|\(\mathcal{D}_{\text{freq}}\) \\
...
284|\(\mathcal{D}_{\text{freq}} \approx \mathcal{D}_{\text{phys}}\), but the
...
293|
294|\subsubsection{2.4 Architectural Position}\label{architectural-position}
295|
296|State Crystallization was established as the \textbf{third core
297|algorithm} of SCX, completing a layered architecture:
298|
299|\begin{longtable}[]{@{}
300|  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1707}}
301|  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2683}}
302|  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2439}}
303|  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3171}}@{}}
304|\toprule\noalign{}
305|\begin{minipage}[b]{\linewidth}\raggedright
306|Layer
307|\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
308|Algorithm
309|\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
310|Function
311|\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
312|LLM Analogue
313|\end{minipage} \\
314|\midrule\noalign{}
315|\endhead
316|\bottomrule\noalign{}
317|\endlastfoot
318|State Ontology & \textbf{State Crystallization} & Continuous → discrete
319|state atoms & BPE (but physics-driven) \\
320|State Topology & \textbf{Situs} & Positional encoding of states in
321|physical space & Positional Encoding \\
322|State Evolution & \textbf{Spring} & Self-evolving gating, state-atom
323|interactions & Transformer (Self-Attention) \\
324|Audit Output & \textbf{Yajie} & Verification + audit + evidence chain &
325|--- (no LLM audit layer) \\
326|Evaluation & \textbf{Cercis Score} & \(S = Q + \eta N\) & --- \\
327|\end{longtable}
328|
...
334|
335|\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}
336|
337|\subsection{Chapter 3: Yajie --- Four Theorems on Label
338|Noise}\label{chapter-3-yajie-four-theorems-on-label-noise}
339|
340|\subsubsection{3.1 The Core Insight}\label{the-core-insight}
341|
...
345|
346|\[{\text{SCX}}_m(s) = P(\ell(f_m(x), y) < \tau \mid x \in s)\]
347|
...
350|\({\text{SCX}}_m(s)\), and the current model's deficiencies---jointly.
351|
...
360|
361|\textbf{Statement (informal).} Let \(M\) independently trained experts
...
366|
367|\[P(\text{noise survives} \mid {\text{consensus}}) \leq \exp(-2M\Delta_s^2)\]
368|
369|where \(\Delta_s = {\text{SCX}}(s) - 0.5\) is the expert reliability
370|advantage over random guessing in state \(s\).
371|
372|\textbf{Tools.} Chernoff bound + Hoeffding inequality, under assumptions
373|A1--A6 (conditional independence, bounded loss, state-conditional
374|calibration).
375|
376|\textbf{What it means.} Even a modest number of independent experts
...
379|asymptotic.
380|
381|\textbf{Bugs found and fixed.} Lemma 3 was restructured after an
...
391|
392|\textbf{Statement (informal).} If every feature \(X_j\) in state \(s\)
...
396|
397|\[{\text{AUC}} \leq 0.5 + \epsilon(\delta, |s|)\]
398|
399|where \(\epsilon \to 0\) as \(\delta \to 0\).
400|
401|\textbf{Tools.} Fano inequality, information-theoretic lower bounds on
402|detection.
403|
404|\textbf{What it means.} SCX does not work everywhere. When features are
...
410|
411|\textbf{Bugs found and fixed.} The original proof claimed optimality of
...
421|
422|\textbf{Statement (informal).} There exist two worlds---World A (noisy
...
426|
427|\textbf{Proof structure.} Constructive two-world proof. In both worlds,
...
433|
434|\textbf{What it means.} There is a \textbf{hard boundary} on what data
...
445|
446|\textbf{We discuss Theorem 3's independence and standalone significance
447|in Chapter 8.}
448|
449|\subsubsection{3.5 Theorem 4: Minimax
450|Optimality}\label{theorem-4-minimax-optimality}
451|
452|\textbf{Statement (informal).} SCX's adaptive threshold achieves the
...
455|
456|\textbf{Tools.} Bahadur-Rao refinement of the Chernoff bound (exact
...
459|
460|\textbf{What it means.} Theorem 1 says SCX works well. Theorem 4 says no
...
466|\(S = Q + \eta N\)}{3.6 The Cercis Score: S = Q + \textbackslash eta N}}\label{the-cercis-score-s-q-eta-n}
467|
...
472|
473|where: - \(Q(s)\) is the \textbf{quality} component:
474|\(1 - \overline{\text{residual}}\) across experts - \(N(s)\) is the
475|\textbf{noise} component: density-weighted + consistency-weighted
476|residual - \(\eta(t)\) is the \textbf{exploration schedule}:
477|\(\eta(0) \gg 1\), \(\eta(t) \to 0\) as \(t \to \infty\)
478|
The Cercis Score is named after \emph{Cercis chinensis} (the Chinese redbud), a
cauliflorous tree whose flowers bloom directly from old branches.
...
485|
486|\subsubsection{3.7 What Yajie Does Not Do}\label{what-yajie-does-not-do}
487|
488|The name ``Yajie'' means ``elegant purification'' in Chinese. But
489|the algorithm does not purify data. It \textbf{audits} data---it
490|identifies which samples are clean, which are noisy, and (critically)
491|which are \textbf{ambiguous}: samples where Theorem 3's uncertainty
...
497|
498|\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}
499|
