...
声明空间 $\ClaimSpace$ 是\textbf{社会现象的统一概念框架}。正如时空是物理现象的底流形，
声明空间为社会现象提供了一个统一的描述语言。但需注意：\textbf{各领域有各自的
实际底流形 $M_\Delta$}（物理学使用 $\R^{1,3}$，MoE 使用 token 空间 $\mathcal{X}$ 等），
$\ClaimSpace$ 降级为启发性统一概念（详见 §14 严格性分级）。
...
\honestcrit{这不是隐喻。这是精确的数学对应——对于物理学、MoE 和经济学三个
核心领域，SHARED 规范丛骨架（主丛 + 联络 + 曲率 + 平坦条件 $\sum g = 0$）
已被严格构造。其他领域处于不同的严格性等级。详见 §14（数学严格性分级）。}
