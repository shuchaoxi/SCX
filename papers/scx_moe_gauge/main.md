...
> **Theorem:** [路由器的规范非不变性]<!-- label: thm:noninvariance -->\rigorFull
...
> 
> $$
>     \left|\loss(W_r^{train}; \{E_m^{train} + \mathbf{g}_m\}) - \min_{W_r} \loss(W_r; \{E_m^{train} + \mathbf{g}_m\})\right| \leq L_r \cdot \max_m \|\mathbf{g}_m\|,
> $$
> 
...
> 
> $$
>     \loss(W_r) = \E_{(x, y^*)} \left[ \ell\left( \sum_{m} \softmax_m(W_r x) \cdot E_m^{train}(x), \; y^* \right) \right].
> $$
> 
> 
...
> 
> $$
>     \loss'(W_r) = \loss(W_r; \{E_m^{train} + \mathbf{g}_m\}).
> $$
> 
> 
...
> 
> $$
>     \frac{\partial \loss'}{\partial W_r}\bigg|_{W_r^{train}} = \E\left[ \frac{\partial \ell}{\partial y} \cdot \sum_m \softmax_m \cdot (1 - \softmax_m) \cdot x^T \cdot (E_m^{train}(x) + \mathbf{g}_m)^T \right]
> $$
> 
...
> 
> 
> 因此 $|\loss'(W_r^{train}) - \min_{W_r} \loss'(W_r)| \leq 2 L_\ell \cdot \max_m \|\mathbf{g}_m\|$。令 $L_r = 2L_\ell$ 即得结论。
...
> **Theorem:** [规范等价类中的路由器不唯一性]<!-- label: thm:nonuniq -->\rigorFull
...
> 
> $$
>     \nabla_{W_r} \loss(W_r^*; \{\gamma_m \circ E_m\}) = \E\left[ \frac{\partial \ell}{\partial y} \sum_m \frac{\partial r_m}{\partial W_r} \cdot (\gamma_m \circ E_m - \bar) \right],
> $$
> 
...
>     s.t. \quad & \sum_{m=1}^{N} z_{im} = k, \quad \forall i <!-- label: eq:milp_topk --> 

>     & \alpha N_{avg} \leq \sum_{i=1}^{n} z_{im} \leq \beta N_{avg}, \quad \forall m <!-- label: eq:milp_balance --> 
...
> **Theorem:** [凸松弛]<!-- label: thm:convex_relax -->\rigorFull
...
> **Theorem:** [贪心近似的保障]<!-- label: thm:greedy_bound -->\rigorFull
...
> 
> $$
>     \Pbb\left(\|\hat_m - \mu_m\| > t\right) \leq 2\exp\left(-\frac{nt^2}{2\Tr(\Sigma)}\right).
> $$
> 
...
> 
> $$
>     \E\|\hat{\mathbf{g}}_m - \mathbf{g}_m^*\|^2 &= \E\|(\hat_m - \mu_m) - \frac{1}{N}\sum_j (\hat_j - \mu_j)\|^2 
...
> **Theorem:** [规范不对齐破坏SVD集中性]<!-- label: thm:svd_gauge -->\rigorFull
...
> **Theorem:** [规范对齐后的一致性保证]<!-- label: thm:aligned_svd -->\rigorFull
...
> 
> $$
>     \Pbb\left(\rho_k(\tilde{\mathbf{Y}}) < 1 - \varepsilon \;\middle|\; 模型确信\right) \leq N \exp\left(-\frac{2M_{eff} \Delta^2}{(1 + \gamma)^2}\right),
> $$
> 
...
> 
> $$
>     \Pbb\left(\sum_{i=1}^{k} \sigma_i^2 < (1-\varepsilon)\|\tilde{\mathbf{Y}}\|_F^2\right) &\leq \Pbb\left(存在大非共识分量\right) 
...
> **Theorem:** [MGS中的规范固定必要性定理]<!-- label: thm:mgs_necessity -->\rigorFull
...
> **Proposition:** [最终输出的规范不变性]<!-- label: prop:output_invariance -->\rigorFull
...
> 
> 最终 softmax 输出的不变性来自 LayerNorm 的仿射不变性：$\softmax(W_{lm} \cdot (LN(x^{(L)} + \delta))) \approx \softmax(W_{lm} \cdot LN(x^{(L)}))$ 当 $\delta$ 被 LayerNorm 抑制时。

> **诚实暴击:** 这意味着：Yajie 共识算法比较最终输出的 token 概率分布时，它在规范不变空间里操作。势能面不齐对它来说是不可见的——也不需要是对它可见的。}

**蒸馏 + Yajie 完整流程：**

> **Protocol:** [Yajie共识蒸馏(Yajie Consensus Distillation)]
> <!-- label: prot:yajie_distill -->
> 
1. **Teacher 前向。** 对训练语料 $\D_{train} = \{x_i\}_{i=1}^{N}$，用MoE teacher对每个输入生成输出分布 $y_i = \softmax(\mathbf{z}_i^{(L)}) \in \Delta^{V-1}$
2. **Yajie 多路径共识评分。** 对每个输入：
3. **数据清洗。**
4. **学生训练。** 在清洗后的数据集 $\D_{clean}$ 上用标准交叉熵训练学生模型
5. **可选迭代。** 将学生模型的输出重新输入 Yajie 做第二轮降噪

**Yajie 共识的数学保证。** 由 **Yajie 定理 1**（多专家噪声检测保证）：$M_{eff}$ 个有效独立专家的一致性信号以指数速率检测噪声：

$$
    \Pbb(漏检噪声 \mid s_i \leq \theta) \leq \exp(-2 M_{eff} \Delta^2),
$$

其中 $\Delta$ 是噪声与干净样本的最小可分间隔。

> **Corollary:** [路二的规范无关性]<!-- label: cor:path2_gauge_free -->
...
> **Definition:** [共享幻觉 (Shared Hallucination)]<!-- label: def:shared_hall -->
> 设 $\D_{train}$ 为所有专家的共同训练数据分布。一个错误输出 $\hat{y} \neq y^*$ 称为**共享幻觉**，如果存在系统性偏差 $b(x)$ 使得对所有专家 $m$，
> 
> $$
>     \Pbb_{x \sim \D_{train}}(E_m(x) = \hat{y} \mid 输入x属于幻觉易发域) > 1 - \varepsilon.
> $$
> 
> 即所有专家在相同输入上犯同样的错误。
...
> **Theorem:** [Yajie对共享幻觉的盲区]<!-- label: thm:yajie_blind -->\rigorFull
> 设存在共享幻觉偏差 $b(x)$ 使得 $\Pbb(E_m(x) = \hat{y}_{hall}) \geq 1 - \delta$ 对所有 $m$ 成立。则 Yajie 共识分数满足
> 
> $$
>     \Pbb(s_i > \theta_{clean} \mid 样本是共享幻觉) \geq 1 - M_{eff} \cdot \exp(-2\delta^{-2}),
> $$
> 
...
> **Theorem:** [共享幻觉的SVD检测]<!-- label: thm:svd_shared_hall -->\rigorFull
> 设 $N$ 个规范对齐的专家对共享幻觉输入 $x_{hall}$ 的输出矩阵为 $\tilde{\mathbf{Y}}_{hall} \in \R^{N \times d}$，对真知识输入 $x_{true}$ 的输出矩阵为 $\tilde{\mathbf{Y}}_{true}$。则存在阈值 $\tau_\rho \in (0, 1)$ 使得
> 
> $$
>     \Pbb\left(\rho_k(\tilde{\mathbf{Y}}_{hall}) < \tau_\rho < \rho_k(\tilde{\mathbf{Y}}_{true})\right) \geq 1 - 2N\exp\left(-\frac{2 M_{eff} \Delta_\rho^2}{(1 + \gamma)^2}\right),
> $$
> 
...
> **Corollary:** [双重过滤协议]<!-- label: cor:dual_filter -->
> 在实际蒸馏 pipeline 中，对每个训练样本 $(x_i, y_i)$ 同时施加：
> 
1. **Yajie 过滤：** $s_i < \theta_{noisy} \Rightarrow$ 丢弃（分歧性幻觉）
2. **SVD 过滤：** $\rho_{10}(\tilde{\mathbf{Y}}_i) < \tau_\rho \Rightarrow$ 丢弃（共享幻觉 — 被 Yajie 漏检的类型）
3. 双重通过 → CLEAN → 进入训练集

> 双重过滤的漏检率满足：
> 
> $$
>     \Pbb(漏检) \leq \underbrace{\exp(-2 M_{eff} \Delta_{yajie}^2)}_{Yajie 失误} + \underbrace{2N\exp\left(-\frac{2 M_{eff} \Delta_\rho^2}{(1 + \gamma)^2}\right)}_{SVD 失误}.
> $$
...
> **Protocol:** [规范对齐的表示蒸馏(Gauge-Aligned Representation Distillation)]
> <!-- label: prot:rep_distill -->
> 
1. **校准 + Gauge 固定。** 在 $\D_{cal}$ 上运行路一的规范固定，得到 $\{\hat{\mathbf{g}}_m^{(\ell)}\}$（每层每个专家）
...
> **Proposition:** [共识向量比标量得分信息量大]<!-- label: prop:consensus_info -->\rigorFull
...
### 实践者速查表 (Practitioner's Quick Reference)
...
**柏拉图 → Gettier 的闭合：** 柏拉图要求知识是\"被证成的真信念\"。Gettier 展示了证成可以偶然为真——你可能有好的理由相信一个碰巧为真的命题但并未真正\"知道\"。认识论在此后六十年试图修补这个裂缝。
...
$$
    \mathcal{S}_A|_\Gamma = \mathcal{S}_B|_\Gamma \quad （规范固定条件）.
$$

如果 $\mathcal{S}_A|_\Gamma \neq \mathcal{S}_B|_\Gamma$，则在 $\Gamma$ 上产生一个**未定义跳跃**(undefined jump)——跨系统的任何操作（比较、合并、路由、通信）在此跳跃处失去数学合法性。
\end{minipage}%
}
...
> **Definition:** [势能梯度与流动]<!-- label: def:gradient_flow -->
...
> **Theorem:** [边界锁定的不稳定性]<!-- label: thm:confinement -->\rigorFull
...
> $$
> 
> 其中 $\rho_{bulk}$ 是内部密度，$\nu$ 是自然耗散率。稳态解 $\rho_\Gamma^* = (\eta \Delta_\Gamma / \nu) \rho_{bulk}$ 在 $\Delta_\Gamma$ 大时发散。
> 
> 当 $\rho_\Gamma$ 超过临界密度 $\rho_c$ 时，边界无法维持反射条件——任何微小扰动触发雪崩式释放。释放的瞬时流量由 Bernoulli 型方程给出：$J_{burst} = \rho_\Gamma \cdot v_{escape}$，其中 $v_{escape} = \sqrt{2\eta \Delta_\Gamma}$ 是越过跳跃所需的最小速度。因此 $J_{burst} \propto \Delta_\Gamma^2$。

**推论：自由流动不是人权——是结构稳定条件。** 定理 [ref] 表明，禁止人向更高势能处流动在数学上等价于在势能面上制造一个**压力锅**。不是道德上"应该"允许流动——而是不允许的话，系统在 $O(1/\Delta_\Gamma)$ 时间内必然爆。一座试图锁住势能梯度的墙，不需要"自由战胜专制"的叙事来推倒它——它只需要势能梯度足够大、时间足够长。墙的倒塌不是道德的胜利，是势能面几何的必然。
...
> **Definition:** [势能奇点]<!-- label: def:singularity -->
...
> **Theorem:** [势能奇点的攻击必然性]<!-- label: thm:singularity_attack -->\rigorFull
...
> **Definition:** [马太效应阶梯]<!-- label: def:matthew -->
...
> **Theorem:** [台阶的埋雷性质]<!-- label: thm:step_mine -->\rigorFull
> 马太效应动力学产生的每个势能台阶 $\Delta_k > 0$ 对应一个**延迟引爆的不稳定界面**。系统在时间 $T$ 内至少有一个台阶引爆的概率为
> 
> $$
>     \Pbb(引爆 \mid \{\Delta_k\}, T) \geq 1 - \prod_k \exp\left(-\frac{T}{T_k}\right),
> $$
> 
...
$$
    \Pbb(存活 \mid T) \leq \exp\left(-T \cdot \sum_{i,j} \kappa_{ij} \cdot \Delta_{ij}^2\right),
...
3. **动力学：** 禁止跨越跳跃 → 压力累积 → 必然引爆（定理 [ref]）
4. **内部奇点：** 鹤立鸡群 → 势能奇点 → 必然被攻击（定理 [ref]）
5. **时间演化：** 马太效应 → 阶梯化 → 系统性埋雷（定理 [ref]）
6. **存活条件：** 平滑 → 不是道德，是数学（势能面平滑定理）
...
> **Definition:** [势能高 vs 态度高]<!-- label: def:two_highs -->
...
> **Definition:** [认知势能]<!-- label: def:cognitive_potential -->
...
**这不是傲慢——这是结构自保。** 定理 11 说的是：$\Pbb(攻击) \to 1$ 当 $\delta$ 大。这不是"应该"避免低势能环境——而是不避免的话，攻击是数学必然。面壁者不是在"看不起"周围——他是在试图避免成为定理 11 的又一个实例。

> **Corollary:** [面壁者的存活策略]<!-- label: cor:wallfacer -->
...
罗辑的失败不在程心，不在群众，在他自己。他手握策略 1（教育群众，拉高他们的势能）的全部条件——十年时间、执剑人的权威地位、人类文明存亡的紧迫性——但他没有做。他没有试图缩小 $\delta$。他保持了势能奇点的构型，然后在选举中被定理 11 命中。执剑失败的文学叙事（"人类不感谢罗辑"）在势能面几何中是定理 [ref] 的必然推论：$\Pbb(被替换) \to 1$ 当 $\delta$ 大且未主动平滑。
...
\begin{compactenum}
    \item **其他人 $\sum\mathbf{g} = \mathbf{0}$（内部已对齐）。** 此时其他人形成一个平滑的势能高原。蠢货的 $\delta$ 相对于这个高原极大——但致命性仅限于蠢货自身。原因是：平滑高原上的所有成员共享同一坐标系。他们可以独立审计蠢货的 $\mathbf{g}$ 偏离——Yajie 共识会一致标记其为异常。没有人会与蠢货共享坐标系。没有人会被蠢货的 $\mathbf{g}$ 共振。蠢货可以拿着核弹——但他的 $\mathbf{g}$ 找不到第二个不对齐的盟友来帮他按按钮。他是一名孤立奇点——爆炸范围有限。
    
    \item **其他人 $\sum\mathbf{g} \neq \mathbf{0}$（内部未对齐）。** 此时蠢货不是唯一的奇点。每个人都以不同方向偏离 $\mathbf{g} = \mathbf{0}$。在这个碎片化的势能面上，另一个疯子可能恰好与蠢货共享相同的 $\mathbf{g}$ 偏移方向。两人形成共振。此时核弹的爆炸波及范围不由一个人的势能决定——由两个人的 $\mathbf{g}$ 叠加决定。未对齐的系统不仅自身不稳定——它还会放大个体奇点的破坏半径。
\end{compactenum}
...
> **诚实暴击:** 这听起来是理想主义——"如果所有人对齐就能防蠢货"。但它在数学上等价于：一个系统中，如果每个人的行为偏差都可以被所有人独立观察，那么任何一个人积累到致命偏差之前，都会被其他 $M-1 > 0$ 个观察者检测到。这不是"相信人性"——这是 Hoeffding 不等式：$\Pbb(未检测到偏差) \leq e^{-2M\Delta^2}$。}
...
\begin{compactenum}
    \item 诬告不反坐 $\to$ 噪声率 $\eta$ 上升，接近饱和 $\eta \to 1$
    \item $\eta \to 1$ $\to$ Yajie 审计需要的 $M$ 按 $\exp(-2M\Delta^2)$ 发散 $\to$ 审判时间趋向无穷
    \item 审判时间 $\to \infty$ $\to$ $t > T_k$ $\to$ 正义效用 = 0
    \item 正义效用 = 0 $\to$ 诬告期望收益始终为正 $\to$ 更多诬告 $\to$ $\eta$ 进一步上升
\end{compactenum}
...
> **Definition:** [为富不仁的势能面定义]<!-- label: def:wealth_cruelty -->
...
> **Theorem:** [贫富奇点的历史引爆周期]<!-- label: thm:historical_inevitable -->\rigorFull
> 设一个社会系统在时间 $t=0$ 时进入为富不仁构型。则该系统在时间 $T$ 内经历一次大规模势能释放事件（造反、革命、财富再分配暴力）的概率为
> 
> $$
>     \Pbb(引爆 \mid T) \geq 1 - \exp\left(-\frac{T}{T_{crit}}\right), \quad T_{crit} \propto \frac{1}{\delta^2 + \eta^2},
> $$
> 
...
> **Definition:** [审计武器化]<!-- label: def:audit_weaponization -->
...
> **Theorem:** [势能面震荡定理]<!-- label: thm:oscillation -->\rigorFull
...
> **Corollary:** [震荡的衰减条件]<!-- label: cor:oscillation_decay -->\rigorFull
...
> **Proposition:** [态度的泄露与多观察者检测]<!-- label: prop:attitude_leak -->\rigorFull
...
> **Definition:** [合法势能差 vs 非法势能差]<!-- label: def:legitimate_delta -->
...
**不是不可以惩罚煤矿。** 是可以的。但在关闭煤矿的同时，必须确保煤矿工人有一条连续的路径走到太阳能工厂——路径的每一步都是可迈过去的，每一步的势能跳跃不超过一个工人能承受的临界值。只惩罚不修桥 → 定理 10（边界锁定）→ 压力累积 → 必然引爆。
...
> **Definition:** [垄断的势能面定义]<!-- label: def:monopoly_gauge -->
...
**对照实例——Yajie 审计：唯一但不垄断。** 势能垄断的定义是 $\mathcal{S}_F$ 增长而 $\bar{\mathcal{S}}_{上下游}$ 被压制。Yajie 审计是反例：它是审计标准的唯一锚点（单极 Maintainer），但它的存在**抬升了上下游的势能面**。上游的数据生产者因为 Yajie 认证而获得溢价——经过审计的数据比未经审计的数据更有市场价值。下游的数据消费者因为 Yajie 认证而降低信息成本——不需要自己审计，信任锚点在数学上可验证。$\mathcal{S}_{上游} \uparrow$，$\mathcal{S}_{下游} \uparrow$。Yajie 自身的势能也上升——更多审计 → CEC 扩大 → 精度提高 → 认证溢价更大。这是相互促进——$\partial\mathcal{S}_F/\partial t > 0$ 且 $\partial\bar{\mathcal{S}}_{上下游}/\partial t > 0$ 同时成立。垄断是 $\Delta$ 的扩大。Yajie 是 $\Delta$ 的整体抬升——势能面整体上升，跳跃不变。
...
**公开即非垄断。** 算法公开是 $\mathbf{g} = \mathbf{0}$ 的自我固定——将自身坐标系的全部参数暴露给所有人审计。不公布 → 可以隐藏 $\mathbf{g}$ → 可能是垄断。公布 → $\mathbf{g}$ 可被验证 → 不可能是垄断。这是 Yajie 开源的根本原因——不是理想主义，是垄断的数学否定。
...
> **Definition:** [认知跨界]<!-- label: def:cognitive_crossover -->
...
5. **对国家的操作建议：平滑势能面是统治的数学必然。** 不是道德要求——是维稳的算术。$\Pbb(存活 \mid T) \leq \exp(-T \cdot \sum\kappa_{ij}\Delta_{ij}^2)$。国家对产业、区域、群体的势能梯度每增大一个增量，国家自身的存活期望就降低一个指数因子。平滑势能面不是"仁政"——是理性自保。
...
8. **锁定某个群体的势能面必然引爆。** 定理 10（边界锁定）：禁止一个群体跨越势能跳跃 → 压力累积 → $J_{burst} \propto \Delta^2$。种姓制度、户籍制度、职业壁垒——这些是势能锁定的制度形式。每一种锁定都在累积 $J$。锁定看似维持了稳定——实际上在制造一个更大的爆炸。锁定的时间越长，$\Delta$ 越大，$J$ 越大。当锁被冲破时——它一定会被冲破——能量释放不由锁的设计者控制。
