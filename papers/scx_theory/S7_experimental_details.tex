...
This section documents the experimental configuration for all datasets used in the main text. All experiments are designed to validate the theoretical predictions of Theorems~1--4, Heuristic Diagnostic~6, and the SCX noise detection framework under controlled conditions.
...
\paragraph{Results.}
\begin{itemize}
    \item \textbf{SimpleCNN (weak features)}: $\delta / \log K \approx 0.52$. SCX F1 $= 0.101$, loss baseline F1 $= 0.105$. SCX performance is slightly below baseline (0.101 vs 0.105), consistent with Theorem~2's prediction that SCX may actively harm when $\delta / \log K > 0.5$. Heuristic Diagnostic~6's bootstrap stability diagnostic gives $S(\Phi, K) < 0.5$, confirming weak features.
...
\paragraph{Stability Score (Heuristic Diagnostic~6).}
...
All experiments use $M \in \{8, 10, 12\}$ experts depending on the dataset. The bootstrap stability diagnostic (Heuristic Diagnostic~6) adds $B = 50$ additional $k$-means runs per dataset, which is negligible compared to expert training time. The SCX pipeline is implemented in Python 3.10 using PyTorch 2.0 (GPU experiments) and scikit-learn 1.2 (CPU experiments).
