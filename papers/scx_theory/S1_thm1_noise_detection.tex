...
\section{Theorem 1: Multi-Expert Consistency Noise Detection Guarantee (独醒不太可能定理)}
\label{sec:thm1}
...
\item[\textbf{(A3)}] \textbf{(Bounded loss).}
  $\ell(a,b) \in [0,B]$ for all $a,b \in \mathcal{Y}$ with $B < \infty$.
...
pairwise correlation $\bar{
ho} > 0$. In this case the effective number
of independent experts is reduced to
$M_{\text{eff}} = M / (1 + (M-1)\bar{
ho})$
(see Liang \& Zeger, 1986, for the generalized estimating equations
variance-inflation factor). The bound then reads
$\operatorname{F1} \geq 1 - \frac{1}{\eta}\sum_s 
ho_s
\exp(-2M_{\text{eff}}\Delta_s^2)$, which remains exponentially
convergent in $M$ but at a rate slowed by the factor $(1+(M-1)\bar{
ho})^{-1}$.
Consistent estimation of $\bar{
ho}$ via jackknife or bootstrap is
standard; see \S
ef{sec:correlation-estimation} for details.
\end{remark}
...
- \frac{1}{\eta}\sum_{s \in \mathcal{S}} \rho_s
...
$\tilde{\mu}_s = \hat{\mu}_s + B\sqrt{\log(2M/\delta_0)/(2n_s)}$.
