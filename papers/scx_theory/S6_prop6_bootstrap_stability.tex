pdfoutput=1
\pdfoutput=1
\section{Heuristic Diagnostic~6: Bootstrap ARI Stability as Feature Strength Diagnostic}
\label{sec:prop6}
...
We now establish the formal connection between clustering stability $S(\Phi, K)$ and the feature strength $\delta = I(\phi(X); S)$ that appears in Eq.~(3). The conjecture has three parts: a forward direction (strong features imply high stability), a reverse direction (weak features imply low stability), and the diagnostic rule derived from their contrapositive.

\begin{conjecture}[Bootstrap ARI stability bounds feature informativeness]
\label{prop:bootstrap_ari}

\textbf{Caveat.} This result is classified as a heuristic diagnostic rather than a formal theorem. Parts (a) and (b) are proof sketches under Gaussian isotropic assumptions (Assumptions~A1--A6); part (c) is explicitly heuristic and should not be interpreted as a rigorous bound. The diagnostic provides practical guidance for feature-strength assessment but should be validated empirically for each application domain.
...
CIFAR-10 (ResNet-18 embeddings) & $0.96_{(0.01)}$ & $0.15$ & $0.62$ & Strong \\
...
Conjecture~\ref{prop:bootstrap_ari} and the associated bootstrap stability procedure serve as a practical feature-strength diagnostic, but several limitations must be acknowledged.

\subsubsection{Stability is sufficient, not necessary}

The most important limitation is that the implication in Conjecture~\ref{prop:bootstrap_ari}(a) is one-directional: strong features imply high stability, but high stability does not guarantee that the features are informative for the specific task of state discovery in SCX. It is possible for k-means clustering to be stable (the same clusters appear across bootstrap resamples) while those clusters are not aligned with the true latent states $S$ that determine expert error rates. This can occur when:
