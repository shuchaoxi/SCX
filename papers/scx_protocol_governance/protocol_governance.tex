...
\hypersetup{
    ...
    citecolor=blue,
    urlcolor=blue,
    pdftitle={SCX协议治理：维护者轮换博弈论与免信任架构},
    pdfauthor={SCX},
...
\renewcommand{\thegovnote}{\textcolor{blue}{\textbf{[治理透视 \arabic{govnote}]}}}
...
\textit{定理审计的是维护者，不是维护者的履历}。维护者的声誉、资历、机构隶属关系或过往
贡献与SCX治理无关——只有实时的、可审计的偏差参数$g(t)$才是关键。这不是哲学立场，
而是Hoeffding界和轮换博弈的直接推论。

\medskip
\noindent\textbf{关键词：} 协议治理，维护者轮换，博弈论，Hoeffding不等式，纳什均衡，
...
\begin{center}
\small\textbf{暴击：}
迄今为止所有协议治理的根本错误，是将\textit{身份}与\textit{行为}混为一谈。
...
\noindent\textbf{中文：} 本文被分类为内部文件有一个特定的原因：它包含了SCX维护者轮换的
...
$p_{\text{det}} \cdot L > |\frac{\partial \Pi}{\partial g}|_{g=0}$的外部审计约束。
...
阶段二：审计俘获（公司任命共享其利益的审计师，使$p_{\text{det}} \to 0$）。阶段三：
...
$\{\mathcal{M}_1, \ldots, \mathcal{M}_K\}$处于活跃状态。在时刻$t_0 + T$，恰好$r$个
...
\noindent\textbf{中文：}
设共识基线$\bar{s}_{-i}$为其他$K-1$个维护者评分的\textbf{中位数}（排除维护者$i$）。
...
% ===================================================================
\section{Hoeffding保证：$P(\text{未检测偏离}) \leq e^{-2M\Delta^2}$}
\label{sec:hoeffding}
...
\noindent\textbf{中文：}
对于任何期望置信水平$1 - \varepsilon$，最小可检测偏差$\Delta_{\min}$满足
$\Delta_{\min} \leq \sqrt{\ln(1/\varepsilon) / (2M)}$。$M = 5$审计者且
$\varepsilon = 10^{-6}$时，$\Delta_{\min} \leq 1.18$。$M = 10$时，
$\Delta_{\min} \leq 0.83$。
...
未检测失真受到约束：$\E[\text{总失真}] \leq T \cdot \Delta + T \cdot \Delta_{\max} \cdot e^{-2M\Delta^2}$。
...
% ===================================================================
\section{多维护者互相审计：$\sum g = 0$是唯一均衡}
...
\noindent\textbf{中文：}
\textbf{静态警示：} 在一次性（静态）互相审计博弈中，任何对称策略
$g_i = g \neq 0$（所有$i$）也是纳什均衡：因为所有分数同步偏移，每个维护者的
分数等于（已被污染的）共识，检测失效。静态博弈\emph{不}唯一选择$g=0$。

\textbf{动态解决：} 在有限轮换重复博弈中，若检测惩罚满足
$P > \sup_g D(g) / p_{\text{det}}(g, \mathbf{0})$，则唯一的子博弈完美均衡
...
% ===================================================================
\section{为什么“伟大”无关紧要：定理审计维护者，不审计履历}
...
\textbf{履历谬误}的观念所主导：相信一个人过去的资历能预测其在治理角色中的未来行为。

这些问题中的每一个都与SCX治理无关。不是“不那么重要”或“次要”——\textit{无关}。
...
期望未检测损害受到约束：$\E[\text{最大损害}] \leq T \cdot \Delta_{\max} / K$。$K$反映
了共识机制对任何单个维护者影响的稀释。
...
超过$T \cdot \Delta_{\max} / K$的损害。这个界不取决于对手的智商、资源、耐心或创造力。
它只取决于$T$、$\Delta_{\max}$和$K$——所有这些都是SCX设置的协议参数。
...
% ===================================================================
\section{维护者数据披露：必须披露什么，绝不能披露什么}
...
SCX是第一个对治理\textit{内容偏差}而不仅仅是治理\textit{过程完整性}提供数学保证的
...
$P(\text{未检测}) \leq e^{-2M\Delta^2}$。$\sum g = 0$不被假设——被证明。
如果你的资历重要，协议就坏了。
...
% ===================================================================
\appendix
\section{附录A：Hoeffding不等式的完整陈述与证明}
...
\noindent\textbf{中文：}
设$X_1, \ldots, X_n$为独立随机变量，$X_i \in [a_i, b_i]$几乎必然。则Hoeffding不等式
给出$\Pbb(S_n - \E[S_n] \geq t) \leq \exp(-2t^2 / \sum_{i=1}^n (b_i - a_i)^2)$。

\section{附录B：重复博弈中的策略空间形式化}
...
\begin{center}
\vspace*{4cm}
{\Huge\color{red}\textbf{内部文件}}\\[16pt]
...
\textbf{未经SCX治理委员会书面授权，严禁复制、分发或引用。}\\[8pt]
\textbf{Do not copy, distribute, or cite without written authorization}\\
\textbf{from the SCX Governance Committee.}
}}\\[24pt]

{\large 文档哈希 / Document Hash:}\\[4pt]
{\small\texttt{[TO BE COMPUTED AT RELEASE TIME]}}\\[16pt]

{\large 版本 / Version: 1.0}\\[4pt]
{\large 日期 / Date: 2026-07-02}\\[4pt]
{\large 分类 / Classification: SCX-GOV-001-INTERNAL}\\[4pt]
{\large 密级 / Clearance: GOVERNANCE CORE TEAM ONLY}
\end{center}
