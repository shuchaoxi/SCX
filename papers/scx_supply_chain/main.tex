...
\begin{abstract}
Supply chain traceability () requires certifying that origin claims---organic certification (), fair-trade compliance (), conflict-free sourcing ()---propagate faithfully from farm to shelf. Current approaches rely on centralized auditors or blockchain-immutable ledgers, neither of which provides formal guarantees when intermediate nodes are adversarial. We apply the \SCX{} (SCX) multi-expert audit framework to supply chain traceability, treating each supply chain node as an independent auditor of upstream claims. We formalize the supply chain as a directed acyclic graph (DAG) of $K$ nodes, where node $k$ receives a claim vector $c_{k-1}$ from its immediate upstream neighbor and independently verifies it against physical inspection data $x_k$. The origin node publishes a certification claim (origin claim) that propagates downstream; every intermediate node audits the received claim against its local observations. We prove three core theorems. \textbf{Theorem~1 (Origin Certification Consensus ):} With $M$ independently verifying nodes, the probability that a false origin claim escapes detection decays exponentially as $\exp(-2M_{\mathrm{eff}} \Delta^2)$, where $\Delta$ is the per-node verification power and $M_{\mathrm{eff}}$ corrects for correlated verification methods. \textbf{Theorem~2 (Chain-of-Custody Unidentifiability ):} When a discrepancy () is detected at node $k$ (i.e., $\norm{c_{k-1} - x_k} > \varepsilon$), the cause among \{upstream fraud, measurement error at $k$, environmental degradation, label switching\} is structurally unidentifiable without declared assumptions, establishing the necessity of the SCX assumption registry for supply chain forensics. \textbf{Theorem~3 (Spring Hash-Gating for Counterfeit Prevention Spring):} We construct a cryptographic hash chain $\hashChain_k = \hashFunc(\hashChain_{k-1}, x_k, \tau_k)$ where $\Spring$'s gating function $M_t$ activates an incremental audit at node $k$ when the hash-chain discrepancy exceeds a dynamic threshold, enabling blockchain integration () over existing hash infrastructure. We define the \Cercis{} traceability score $S = Q + \eta \cdot N$ combining certification accuracy ($Q$) with supply chain regime coverage ($N$) including multi-hop origin chains, cold-chain integrity (), and conflict-zone provenance (). A full experimental protocol is specified on agricultural supply chains (coffee, cocoa, palm oil) with GPS-verified farm coordinates, spectral commodity fingerprinting, and IoT temperature logging, with explicit assumption-to-conclusion audit trails and Chinese/English bilingual terminology mappings.

\medskip
\noindent\textbf{Keywords:} SCX audit, supply chain traceability, , origin certification , counterfeit detection , organic certification , conflict minerals , hash chain , blockchain , fair trade , cold chain , \Spring{} gating, \Yajie{} consensus, \Cercis{} score, multi-node verification 
\end{abstract}
...
\begin{definition}[Node State and Claim ]
...
\subsection{The Origin Claim }

\begin{definition}[Origin Claim ]
...
\begin{definition}[Supply Chain Hash Chain ]
...
\begin{definition}[Node Verification Function ]
...
\begin{itemize}
    ...
    \item $v_k = 1$ indicates a \textbf{discrepancy} ().
\end{itemize}
\end{definition}

\begin{definition}[Verification Power ]
...
\begin{definition}[Adversarial Node ]
...
\begin{assumption}[A1: Linear Claims with Additive Noise ]
\label{ass:A1}
...
\begin{assumption}[A2: Conditionally Independent Verification ]
\label{ass:A2}
...
\begin{assumption}[A3: Bounded Adversarial Fraction ]
\label{ass:A3}
...
\begin{assumption}[A4: Measurement Map Identifiability ]
\label{ass:A4}
...
\begin{assumption}[A6: Honest Majority of Auditing Nodes ]
\label{ass:A6}
...
\begin{assumption}[A7: Verification Threshold Calibration ]
\label{ass:A7}
...
\begin{assumption}[A8: Upstream Honesty Propagation ]
\label{ass:A8}
...
\begin{assumption}[A9: Blockchain Anchoring Timeliness ]
\label{ass:A9}
...
\begin{assumption}[A10: Origin Fingerprint Stability ]
\label{ass:A10}
...
\begin{assumption}[A11: Consumer-Side Verification ]
\label{ass:A11}
...
%=====================================================================
\section{Theorem 1: Multi-Node Origin Certification Consensus }
\label{sec:theorem1}
%=====================================================================
...
\begin{lemma}[Single-Node Detection ]
...
\begin{definition}[Aggregate Detection Statistic ]
...
\begin{theorem}[Multi-Node Origin Certification Consensus ]
\label{thm:origin}
...
\begin{corollary}[Minimum Honest Verifiers ]
...
\begin{remark}[Comparison with Centralized Auditing ]
...
%=====================================================================
\section{Theorem 2: Chain-of-Custody Unidentifiability }
\label{sec:theorem2}
%=====================================================================
...
\begin{enumerate}[label=(C\arabic*)]
    \item \textbf{Upstream fraud C1 ():} The origin claim $c_0$ was false, and the fraud propagated undetected through nodes $1, \ldots, k-1$;
    \item \textbf{Intermediate substitution C2 ():} An honest node $j < k$ relayed truthfully, but an adversarial node $\ell \in (j, k]$ substituted counterfeit goods and fabricated a consistent claim;
    \item \textbf{Measurement error at node $k$ C3 ($k$):} The goods are authentic but node $k$'s physical measurement $x_k$ suffered a rare large deviation (false positive);
    \item \textbf{Environmental degradation C4 ():} The goods were authentic at origin but degraded during transit (temperature excursion degrading spectral fingerprint, moisture damage altering chemical markers);
    \item \textbf{Label switching C5 ():} The physical goods are authentic but the claim label $c_{k-1}$ was attached to a different batch at some upstream node.
\end{enumerate}

\begin{theorem}[Chain-of-Custody Unidentifiability ]
\label{thm:unident}
...
\begin{corollary}[Assumption Mandate for Supply Chain Forensics ]
...
\begin{remark}[Connection to the Honest Agent Theorem ]
...
%=====================================================================
\section{Theorem 3: Spring Hash-Gating for Counterfeit Prevention Spring}
\label{sec:theorem3}
%=====================================================================
...
\begin{definition}[Spring Gating Function Spring]
...
\begin{definition}[Extended Hash Chain with \Spring{} Audit Spring]
...
\begin{theorem}[Spring Hash-Gating Security Spring]
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Tamper evidence :} Any post-hoc alteration of any $c_0$, $x_k$, $\tilde{c}_k$, or $\tau_k$ for any $k$ changes $\hashChain_K$ with probability $1 - \nu(\lambda)$ (where $\nu$ is negligible), and this change is detectable by any party that stores the original $\hashChain_K$.
    
    \item \textbf{Adversarial evasion bound :} An adversary controlling $A$ consecutive nodes can suppress a discrepancy from the hash chain only if $v_k = 0$ for all $k$ in the adversarial segment. The probability that an adversarial segment of length $A$ successfully suppresses a true-origin discrepancy $\delta > 0$ is bounded by:
    \begin{equation}
    ...
    \item \textbf{Cumulative detection via \Spring{} gating Spring:} As $t \to \infty$ batches traverse the chain, the probability that a persistent origin fraud (same fraudulent $c_0$ across all batches) evades \Spring{} audit indefinitely decays super-exponentially:
    \begin{equation}
...
\begin{definition}[Blockchain-Anchored Traceability ]
...
\caption{\Spring{}-Gated Supply Chain Audit Protocol Spring}
...
\begin{corollary}[Batch-Level Guarantees ]
...
%=====================================================================
\section{Multi-Node \Cercis{} Score for Supply Chain Traceability Cercis}
\label{sec:cercis}
%=====================================================================

The \Cercis{} scoring framework ranks supply chains by traceability quality and regime coverage.

\begin{definition}[Traceability Quality Score ]
...
\begin{definition}[Traceability Regime Coverage ]
...
\begin{itemize}
    ...
    \item $d_2$: Cold-chain integrity () with temperature logging;
    \item $d_3$: Conflict-zone provenance ();
    \item $d_4$: Mixed-origin blending (blended commodities);
    \item $d_5$: Cross-border regulatory compliance ();
    \item $d_6$: Consumer-side terminal verification ().
\end{itemize}
The weight $\nu_d$ is inversely proportional to how commonly the regime appears in existing traceability deployments.
\end{definition}

\begin{definition}[\Cercis{} Traceability Score Cercis]
...
\toprule
\textbf{Supply Chain } & $\alpha$ & $\beta$ & Cost & $Q$ & $N$ & $S$ \\
\midrule
Organic coffee (3-node, EU)  & 0.02 & 0.05 & 0.03 & $-0.10$ & 2 & $+0.90$ \\
Conflict-free Ta (5-node)  & 0.01 & 0.08 & 0.05 & $-0.14$ & 4 & $+1.86$ \\
Cold-chain vaccine (4-node)  & 0.005 & 0.01 & 0.10 & $-0.115$ & 3 & $+1.385$ \\
Palm oil (7-node, SE Asia)  & 0.03 & 0.12 & 0.02 & $-0.17$ & 5 & $+2.33$ \\
\bottomrule
\end{tabular}
\end{table}

\begin{remark}[\Cercis{} and Supply Chain Selection Cercis]
...
\begin{table}[htbp]
\centering
\caption{Multi-node verification methods for coffee traceability .}
\label{tab:verification_methods}
\begin{tabular}{lp{3cm}p{3.5cm}p{3cm}}
\toprule
\textbf{Node } & \textbf{Physical Verification } & \textbf{Feature Extracted } & $\sigma$ \\
\midrule
Farm  & GPS coordinate recording & lat, lon, altitude & $\pm 3$m \\
Cooperative  & Wet chemistry pesticide panel & 15 pesticide residue levels & $\pm 0.1$ ppm \\
Exporter  & NIR spectroscopy & Spectral reflectance (400--2500 nm) & $\pm 0.5\%$ reflectance \\
Importer  & GC-MS volatile profile & 50 volatile compound peaks & $\pm 5\%$ peak area \\
Roaster  & Cupping + HPLC & Caffeine, chlorogenic acid, trigonelline & $\pm 2\%$ concentration \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{Origin fraud detection rate :} $\text{DR} = \frac{\text{true positives}}{\text{true positives} + \text{false negatives}}$, measured per batch with injected fraud.
    \item \textbf{False alarm rate :} $\text{FAR} = \frac{\text{false positives}}{\text{false positives} + \text{true negatives}}$, measured on known-authentic batches.
    \item \textbf{Discrepancy localization accuracy :} For batches where fraud is detected, the fraction where the first detecting node $k^*$ correctly identifies the earliest node in the chain where the fraud occurred.
    \item \textbf{Hash chain integrity :} $\text{HCI} = \frac{\text{verifiable hash chains}}{\text{total batches}}$, measuring the fraction of batches for which the consumer can successfully verify $\hashChain_K$ against the blockchain anchor.
    \item \textbf{\Spring{} audit efficiency Spring:} $\text{SAE} = \frac{\text{true frauds found by Spring audit}}{\text{Spring audits triggered}}$, measuring the precision of \Spring{}-triggered investigations.
    \item \textbf{Cost per verified batch :} Total verification cost (instrument time, reagents, personnel) per batch, normalized by batch value.
\end{enumerate}

\subsection{Baselines}

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{Centralized auditor ():} A single third-party certifier audits 10\% of nodes per batch.
    \item \textbf{Blockchain-only ():} Immutable ledger records all claims but performs no physical verification (GIGO baseline).
    \item \textbf{Random spot-check ():} Each node independently verifies with per-batch probability $p = 0.1$.
...
\subsection{Chinese/English Terminology Mapping }

\begin{table}[htbp]
\centering
\caption{Key terminology mapping .}
...
Chain of custody &  \\
Discrepancy & / \\
\Spring{} gating & Spring \\
\Yajie{} consensus & Yajie \\
\Cercis{} score & Cercis \\
Assumption registry &  \\
\bottomrule
\end{tabular}
