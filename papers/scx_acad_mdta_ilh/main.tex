...
%  超越隐喻：量子纠缠、数据虫洞与相对论不变性在SCX审计中的严格形式化
...
\title{
    \vspace{2cm}
    {\Huge \textbf{超越隐喻}}\\[0.2cm]
    {\LARGE 量子纠缠、数据虫洞与相对论不变性在SCX审计中的严格形式化}\\[0.5cm]
...
SCX的核心数学基础是离散Hodge理论与规范理论（$U(1)$-型平移规范 + $O(d)$ 格点规范）。这些工具源自量子场论和粒子物理。本文探索三个更深层的物理学概念——\textbf{量子纠缠}、\textbf{虫洞}（Einstein-Rosen桥）和\textbf{狭义相对论}（Lorentz变换的不变性）——并诚实地问：它们是否为SCX审计提供了离散Hodge理论尚未覆盖的视角？
...
\midrule
量子纠缠：非定域关联，Bell不等式 & 审计员之间的不可分离关联——篡改一处，全局可检测 \\
...
\bottomrule
\end{tabular}
\caption{核心直觉映射 / Core Intuition Mapping}
\label{tab:intuition}
\end{table}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{第3轮：严格形式化} —— 为三条修正路径建立显式的数学框架（定义、定理、证明）
...
\midrule
ACAD & 量子纠缠 $\rightarrow$ Bell审计不等式 & 审计相关性不对称检测（信息论安全+MMD检验） & \textbf{条件性有用} \\
...
\addlinespace
MDTA & 虫洞 $\rightarrow$ 数据虫洞捷径 & 流形密度拓扑分析（持久同调+捷径检测） & \textbf{实用主义的有用} \\
...
\textbf{第1轮} & ``量子纠缠参考态''和``Bell审计不等式'' —— 将量子概念类比到审计 \\
\textit{Round 1} & \textit{``Quantum entangled reference states'' and ``Bell audit inequality''} \\
\textbf{第2轮} & 识别裂缝：Bell不等式在经典概率中永远成立，纠缠需要Hilbert空间 \\
...
\textbf{第5轮} & 修正：Serfling不等式、降级为统计安全、哈希链约束方案 \\
...
$\square$ \textbf{注：} 原始版本（定理6.1 in Round 3）使用了独立Bernoulli近似的Hoeffding不等式。本修正版使用Serfling不等式处理无放回抽样的相关性。当$k \ll n$时，修正因子$1 - (k-1)/n \approx 1$，两版本等价。
\end{proof}
...
\begin{proof}
攻击者的信息状态由$\sigma(\tilde{D}_A, D_A)$（攻击者知道的$\sigma$-代数）决定。$\phi$在此$\sigma$-代数下是不可测的（攻击者不知道配对）。
...
$\square$ \textbf{重要修正：} 原始版本（定理6.2 in Round 3）声称``信息论安全''。本修正版降级为``统计安全''（statistical security），因为：(1) 约束$C$可能有结构可被利用；(2) 攻击者可通过修改大量数据点实现部分成功；(3) 检查是抽样的而非穷举的。ACAD不达到一次一密级别的完美安全。
\end{proof}
...
\begin{proof}
这是MMD的经典集中不等式。对于有界核$k(\cdot, \cdot) \leq K_{\max}$，经验MMD估计$\widehat{\MMD}_n$满足McDiarmid不等式。具体地，改变单个样本对MMD的影响最多为$2\sqrt{K_{\max}/n}$，因此：
\begin{equation}
...
\begin{proof}
对于方案(a)，约束不依赖专家预测，因此不存在``审计本身验证专家预测、约束又依赖专家预测''的循环性。攻击者需要知道流形的全局结构（$r_i$的邻域）才能伪造约束——这等价于知道$D_B$的分布，与攻击者知识限制一致。

对于方案(b)，攻击者在不知道$K$的情况下构造$\tilde{x}_i^A$使得$H(\tilde{x}_i^A \| K) = x_i^B$的概率等于对随机预言机的原像攻击成功概率，在标准密码学假设下为$O(2^{-|K|})$，即$\gamma = \text{negl}(|K|)$。
\end{proof}
...
\medskip
\begin{tabular}{p{3cm} p{11cm}}
\textbf{数学严格性} & $\star\star\star\star$ (4/5) —— 定理陈述清晰，证明结构正确，修正后安全性声明诚实 \\
\textit{Math Rigor} & \textit{Theorems clearly stated, proofs structurally correct, security claims honest after fixes} \\
\textbf{审计实用性} & $\star\star\star$ (3/5) —— 提供可操作的篡改检测协议，但真实部署面临密钥管理等挑战 \\
...
\medskip
\textbf{适用条件：} 当且仅当(1)约束可以被密码学方式实现（如哈希链），(2)审计员双方有安全信道时，ACAD提供一个可证明安全的篡改检测方案。
...
\begin{scxbox}[路径轨迹 / Path Trajectory]
\begin{tabular}{p{3cm} p{11cm}}
\textbf{第1轮} & ``数据虫洞''和``虫洞检测算法'' —— 将广义相对论虫洞类比到Situs流形 \\
...
等价地，如果$\cM$的reach（到达半径）$\tau_{\cM}$满足：
\begin{equation}
...
\subsection{定理3：审计风险分类及其Lipschitz一致性}
...
$\square$ \textbf{诚实地：} SAR的乘积组合方式缺乏理论依据——为什么是乘积？为什么是这三个因子？这是一种ad-hoc启发式。修正方案：使用SAR多指标向量（见第\ref{sec:mdta-fixes}节）。
\end{proof}
...
\subsection{定理5：捷径比率估计的一致性}

\begin{theorem}[捷径比率估计的一致性 / Consistency of Shortcut Ratio Estimation]
...
\begin{proof}
由k-NN图距离的Isomap一致性（Bernstein et al., 2000; Tenenbaum et al., 2000），在流形$\cM$是紧致且测地线凸的条件下，k-NN图最短路径一致地收敛到流形测地距离：
\begin{equation}
...
$\square$ \textbf{重要条件：} 一致性依赖于(1)流形紧致，(2)数据采样均匀或密度有下界，(3)度规有界曲率。在捷径发生的低密度区域，k-NN图连接可能不可靠——这是捷径检测的核心困境。
\end{proof}
...
\medskip
\textbf{MDTA是最``脚踏实地''的路径。} 不需要华丽的物理学包装——捷径检测、持久同调、SAR多指标分析在TDA工具箱中都有现成实现。核心贡献是将这些工具\textbf{定向到审计问题}上。
...
ILH将``相对论''类比修正为诚实的不变性结构文档。核心发现：SCX的不变性结构最接近\textbf{Galileo不变性}（空间平移+旋转），而非Lorentz不变性（含boost）。SCX的``相对论''是Galileo式的，不是Einstein式的。
...
$\square$ \textbf{注：} 原始版本（定理8.1 in Round 3）使用范畴论函子语言$\cF_\ell: \cG \to \cI_\ell$。该语言是装饰性的——$\cF_\ell$就是自然投影$\cG \to \cG/G_\ell$，不需要范畴论框架。本修正版使用直接的群论表述。
\end{proof}
...
差异向量集合$\{d_{mn}\}_{m<n}$构成平移群作用的\textbf{完备不变量集合}——即两个配置$\Gamma, \Gamma'$在平移下等价当且仅当它们的所有差异向量相等：
\begin{equation}
...
\begin{proof}
Betti数是同伦不变量，微分同胚诱导同伦等价。持久同调条码在等距嵌入下不变，微分同胚保持拓扑结构（但\textbf{不}一定保持几何/距离信息）。
\end{proof}
...
\begin{proof}
轨道空间$\cI_\ell = \cG / G_\ell$的``大小''随$G_\ell$增大而减小。对于连续群，信息损失等于商掉的群作用的自由度维数。由于$G_{\ell} / G_{\ell-1}$是$\dim(G_\ell) - \dim(G_{\ell-1})$维的李群，$\Delta H_\ell$如所述。

$\square$ \textbf{注：} 对于连续群，商空间的``熵''不是传统意义下的良定义量（差一个无穷常数）。本定理使用\textbf{自由度维数}作为信息损失的替代度量——这更严格且避免了微分熵的技术问题。
\end{proof}
...
$\square$ \textbf{核心澄清：} 这意味着当我们说``SCX的相对论''时，应该理解它是\textbf{Galileo式的}：没有绝对参考系，但也没有速度上限。Cercis的审计类比应该是``欧氏空间中相对距离的不变性''，而非``时空间隔的Lorentz不变性''。
\end{proof}
...
V1 & $(\RR^d, +)$ & Cercis, $d_{mn}$ & 相对一致性——专家间是否一致 \\
V2 & $O(d)$ & $\|\Gamma_m\|^2$, $\cos\theta_{mn}$, Cercis & 方向一致性——预测方向是否一致 \\
...
% ========================================================================
\part{路径协同与统一视角 / Cross-Path Synergies}
\label{part:synergy}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{MDTA $\times$ ILH：} ILH的竖直不变性确保捷径上的审计结论在gauge变换下稳定；MDTA的SAR评分的各维度在不同ILH层级上的行为不同（例如：预测方差在V1不变但V2可能变）。
...
这三个组件组装在一起，构成\textbf{基于纤维丛的审计安全框架}的原型。
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{穿上数学外衣不等于有数学实质：} 第3轮中，有些``定理''只是定义的重新陈述（ILH的许多定理），有些``证明''有技术漏洞（MDTA的流形假设）。
    
    \item \textbf{诚实比深刻更重要：} 承认ILH是``教学材料''而非``研究贡献''比声称它是``审计不变性的新理论''更诚实。
    
    \item \textbf{审计价值是最终准绳：} 无论形式化多么漂亮，如果它不产生可操作的审计改进，它就是学术体操而非审计工具。
\end{enumerate}
...
% ========================================================================
\section{未解决的开放问题 / Unresolved Open Questions}
\label{app:open}

\begin{enumerate}[leftmargin=*]
    \item \textbf{ACAD的实际安全界：} 在真实SCX数据分布下，审计相关性不对称检测的检测概率和假阳性率是多少？需要实证研究。
    ...
    \item \textbf{水平不变性的可行性：} 是否存在离散版本的微分同胚不变审计构造？Einstein的``广义协变性''概念能否指导SCX的数据表示无关审计？
...
%  REFERENCES
% ========================================================================
\section{参考文献与前置文档 / References and Prerequisites}
\label{app:refs}
