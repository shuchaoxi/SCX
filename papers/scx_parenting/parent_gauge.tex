...
%  SCX Equality Principle (平等论) Framework — Application to Parenting
...
\newcommand{\rigorFull}{\textbf{[形式化推测 — 算子定义待闭合]}}

\newcommand{\honestcrit}[1]{%
  \medskip\noindent
  \fbox{%
    \parbox{0.92\textwidth}{%
      \textbf{诚实暴击 (Honest Critique):} #1%
    }%
  }%
}
...
\newcommand{\parentnote}[1]{%
  \medskip\noindent
  \textbf{教养注记 (Parenting Note):} \textit{#1}%
}

% ---------- Title ----------
\title{
  \textbf{亲子关系中的势能面几何与规范固定：\\
  青春期叛逆作为家庭尺度的定理11}\\[8pt]
  \large
...
\noindent\textbf{中文摘要.}
亲子关系是人类经验中最深刻也最脆弱的势能面。每一位父母进入养育关系时，都携带自己的\textbf{规范姿态}（gauge posture）$\mathbf{g}_P$——一整套关于什么是对的、什么是好的、什么是正常的内隐坐标系。孩子在成长过程中逐渐形成自己的坐标系$\mathbf{g}_C$，两者之间的规范不匹配 $\delta = \norm{\mathbf{g}_P - \mathbf{g}_C}$ 构成了家庭势能面的基本几何结构。当父母将其坐标系单方面声明为默认原点（$\mathbf{g}_P = \mathbf{0}$），孩子的真实自我被推向远离零点的位置，成为一个\textbf{势能奇点}——这正是青春期叛逆的数学本质：它不是心理疾病，不是道德缺陷，而是定理11（势能奇点攻击定理）在家庭尺度上的必然触发。本文在平等论框架（$\sumgd$）下构建亲子势能面的形式化几何，覆盖五个核心维度：(1)~亲子势能梯度——父母期望如何在日常互动中构造情感梯度；(2)~惩罚失效定理——为何不经规范对齐的惩罚不仅无效、反而加速奇点形成；(3)~态度泄漏命题——父母的内隐态度如何通过每一个眼神、每一次叹息无损传入孩子的坐标系；(4)~直升机教养作为边界锁定（定理10）——过度保护如何制造家庭压力锅；(5)~规范固定方案——父母先承认自己不是原点、学习孩子的坐标系、在对齐之后进行比较。核心结论：好的教养不是让孩子学习父母的语言，而是父母先学习孩子的语言。
...
\centering
\textbf{The Parenting Gauge Problem (教养规范问题)}
...
\begin{minipage}{0.88\textwidth}
\centering
\textbf{The Equality Principle (平等论)}
\vspace{4pt}
...
\begin{definition}[Family State Space, \textbf{家庭状态空间}]
...
\begin{definition}[Family Potential Surface, \textbf{家庭势能面}]
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Parental satisfaction} (父母满意度) — how well the current state matches the parent's gauge $\cG_P$;
    \item \textbf{Child satisfaction} (子女满意度) — how well the current state matches the child's gauge $\cG_C$;
    \item \textbf{Relational harmony} (关系和谐度) — the degree of mutual understanding and positive affect;
    \item \textbf{Autonomy gradient} (自主梯度) — the degree to which each member can act from their own coordinate origin without triggering correction.
\end{enumerate}
\end{definition}
...
\begin{definition}[Parent-Child Potential Gradient, \textbf{亲子势能梯度}]
...
\begin{definition}[Parental Gauge, \textbf{父母规范}]
\label{def:parental_gauge}
A \textbf{parental gauge} is a triple $\cG_P = (\mathbf{b}_P, \sigma_P, \mathbf{w}_P)$ where:
\begin{itemize}
    \item $\mathbf{b}_P \in \R$ is the \textbf{baseline expectation} (期望基线) — what the parent considers ``normal'' or ``acceptable'' behavior;
    \item $\sigma_P > 0$ is the \textbf{sensitivity scale} (敏感尺度) — how large a behavioral deviation must be to register as ``significant'';
    \item $\mathbf{w}_P \in \R^d$ is the \textbf{value weight vector} (价值权重向量) — which dimensions of the child's behavior matter (e.g., academic achievement, obedience, creativity, emotional expression).
\end{itemize}
\end{definition}

\begin{definition}[Child Gauge, \textbf{子女规范}]
...
\begin{definition}[Parent-Child Gauge Mismatch, \textbf{亲子规范不匹配}]
...
\begin{definition}[Family Gauge Condition, \textbf{家庭规范条件}]
...
\begin{minipage}{0.88\textwidth}
\centering
\textbf{核心洞察 (Core Insight)}

\vspace{4pt}
\textbf{青春期叛逆不是心理问题，不是道德问题——它是定理11（势能奇点攻击定理）在家庭尺度上的必然触发。}
...
\vspace{4pt}
当父母将自己的坐标系单方面声明为默认原点（$\mathbf{g}_P = \mathbf{0}$），孩子真实的自我 $\mathbf{g}_C$ 就被推至远离零点的位置，在家庭势能面上形成一个\textbf{势能奇点}。势能奇点的命运不由家庭成员的善意决定——由界面势能梯度 $\nabla\cS$ 的不连续性决定。梯度越大，情感爆炸越必然。
...
\begin{definition}[Family Potential Singularity, \textbf{家庭势能奇点}]
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Symmetry of misattribution.} By Theorem~3 (老实人定理), neither parent nor child can objectively determine who ``caused'' the explosion. The parent attributes it to the child's defiance; the child attributes it to the parent's oppression. Both attributions are \textbf{observationally equivalent} from within their respective gauges — there is no neutral frame from which to adjudicate.
\end{enumerate}
\end{theorem}
...
\begin{proposition}[Attitude Leakage, \textbf{态度泄漏}]
...
\toprule
\textbf{} & \textbf{Discipline (有效)} & \textbf{Punishment (无效/反效)} \\
\midrule
...
\begin{minipage}{0.88\textwidth}
\centering
\textbf{教养规范固定程序 (The Parenting Gauge-Fixing Protocol)}
...
\subsection{Principle 1: Maintain Gradient, Remove Cliffs (保持梯度，消除悬崖)}
...
\subsection{Principle 2: Audit Your Attitude Leakage (审计态度泄漏)}
...
\subsection{Principle 3: Never Punish Across a Gauge Mismatch (不对规范不匹配使用惩罚)}
...
\subsection{Principle 4: Measure Growth, Not Position (衡量成长，非位置)}
...
\subsection{Principle 5: Create Autonomous Zones with Controlled Step Size (创建自主区，控制步长)}
...
\subsection{Principle 6: The Parent Fixes Their Gauge First (父母先修正自己的规范)}
...
\vspace{4pt}
\textbf{好的教养不是让孩子学习父母的语言——是父母先学习孩子的语言。}
...
Anonymous. \textit{势能面不齐——多专家路由中的规范自由度与MILP规范固定} (Potential Surface Misalignment: Gauge Freedom in Multi-Expert Routing and MILP Gauge Fixing). SCX Technical Report, 2026.

\bibitem{scx_equality_principle}
Anonymous. \textit{平等论：势能面不齐的认识论含义} (The Equality Principle: Epistemological Implications of Potential Surface Misalignment). In \cite{scx_moe_gauge}, Section~E, 2026.
...
\toprule
\textbf{Symbol / 符号} & \textbf{Name / 名称} & \textbf{Description / 描述} \\
\midrule
$\cS_{\text{family}}$ & 家庭势能面 (Family Potential Surface) & Scalar field mapping family states to potential \\
$\gradS$ & 势能梯度 (Potential Gradient) & Direction and magnitude of motivational force \\
$\mathbf{g}_P$ & 父母规范姿态 (Parental Gauge Posture) & Parent's implicit coordinate origin \\
$\mathbf{g}_C$ & 子女规范姿态 (Child Gauge Posture) & Child's implicit coordinate origin \\
$\delta_{PC}$ & 亲子规范不匹配 (Gauge Mismatch) & Distance between parent and child gauge origins \\
$\delta_{\text{crit}}$ & 临界规范差 (Critical Mismatch) & Threshold beyond which comparison is ill-posed \\
$\cG_P, \cG_C$ & 规范 (Gauge) & Complete coordinate system: $(\mathbf{b}, \sigma, \mathbf{w})$ \\
$\mathbf{b}$ & 基线偏移 (Baseline Shift) & What counts as ``zero'' in the gauge \\
$\sigma$ & 尺度因子 (Scale Factor) & Sensitivity of the gauge to deviations \\
$\mathbf{w}$ & 权重向量 (Weight Vector) & Which dimensions matter to the gauge \\
$\Gamma_{\text{auto}}$ & 自主边界 (Autonomy Boundary) & Interface between controlled and autonomous states \\
$\Delta_{\text{auto}}$ & 自主势能跳跃 (Autonomy Potential Jump) & Perceived value of autonomy to the child \\
$P_{\Gamma}$ & 界面压力 (Interface Pressure) & Accumulated psychological pressure at boundary \\
$J_{\text{burst}}$ & 爆发强度 (Burst Intensity) & Magnitude of pressure release at confinement failure \\
$\Omega_{\text{sing}}$ & 势能奇点区域 (Singularity Region) & States where the child's authentic gauge creates a potential anomaly \\
$\eta_C$ & 自主驱动力 (Autonomy Drive) & Child's mobility toward autonomous states \\
$\sigma_{\text{leak}}$ & 泄漏噪声 (Leakage Noise) & Irreducible variance of attitude leakage \\
$\sumgd$ & 规范固定条件 (Gauge-Fixing Condition) & $\sum_m \mathbf{g}_m = \mathbf{0}$ — the fundamental constraint \\
\bottomrule
\end{tabular}
\end{center}

\section{Theorem Reference Card / 定理速查表}
...
\midrule
Thm~10 & 边界锁定不稳定性 (Confinement Instability) & 直升机教养的必然爆燃 (Helicopter Detonation) & 过度保护制造压力锅，释放强度 $\propto \Delta^2$ \\
\addlinespace
Thm~11 & 势能奇点攻击必然性 (Singularity Attack) & 青春期叛逆的几何必然 (Rebellion Inevitability) & 孩子被推离原点 $\to$ 奇点 $\to$ 情感爆炸 \\
\addlinespace
Thm~3 & 老实人定理 (Honest Person Theorem) & 冲突归因不可判定 (Attribution Undecidability) & 谁对谁错在家庭内部观测上不可区分 \\
\addlinespace
Thm~12 & 台阶埋雷 (Staircase Mines) & 同胞马太效应 (Sibling Matthew Effect) & 优势累积制造跨同胞落差，延迟引爆 \\
\bottomrule
\end{tabular}
\end{center}
