...
%  Date: 2026-07-02
%  Classification: INTERNAL ONLY — 内部files，严禁外传
% ===========================================================================
...
\fancyfoot[C]{\small\textcolor{scxred}{\textbf{INTERNAL ONLY — 内部files}}}
...
内部files — INTERNAL ONLY\\
未经授权严禁外传。本文包含对四位SCX协议维护者候选人的\\
详细评估，涉及个人背景、$g=0$可Proof性、社区验证证据、\\
以及Honest Strike分析。仅限SCXCore团队及授权维护者查阅。\\
\\
...
以及Linus Torvalds（LinuxKernel维护者）。每位候选人均以SCX治理标准——
在互相审计机制下维持$\sum g = 0$的能力——进行评估。Core论点是：协议维护者
...
分析及Honest Strike框（识别弱点和风险），(3) 以社区审计深度、$g=0$可Proof性、
和过渡风险为维度的比较框架，(4) 推荐的多维护者组合策略。本文为内部files，
不构成最终选择——它是对每位正式注册候选人在接受$M>1$独立审计之前的预先评估。

\medskip
\noindent\textbf{Keywords：} 协议治理，维护者选择，$g=0$标准，社区审计，Honest Strike，
候选人评估，互相审计，免信任架构
\end{abstract}
...
它需要的是偏差参数$g$在维护窗口期间可Proof保持为零的维护者。这是完全不同的要求。
...
即已被Proof的愿意让自己的决策被独立审查、复现和挑战的记录。
\end{hitbox}
...
而非理论中的Proof。Elbakyan是活着的证据，Proof$g=0$条件在极端环境下
可由单个人类实现。
\end{govbox}
...
\textbf{附加风险：}来自被追捕Status的$g=0$可能在合法化转型后无法存活。
...
\textbf{Core优势：}$g=0$已在行动中被Proof15年。没有候选人能在“以个人代价
实践$g=0$”这一维度上与Elbakyan匹敌。她Proof了维护者不需要机构、资金、或
合法性——只需要对$g=0$的承诺。

\textbf{Core风险：}独行侠模式无法直接转化为协议维护者角色。从“我就是系统”
...
信号”——说服外部ObserversSCX是认真的信号。审计在内部解决了信任；它不在外部
...
\begin{verdictbox}[裁决：高适配性 — HIGH SUITABILITY]
\textbf{Core优势：}唯一一位$g=0$已被正式审计测量的候选人。拥有审计他人
的独特资格（被审计过才知道审计的痛）。过渡风险最低——已在审计框架内运作。

\textbf{Core风险：}低外部知名度可能限制SCX的全球合法性信号。但这可以通过
...
SCX获得合法性。如果他Fails，SCX照样获得合法性——因为美国VP\textit{尝试了
但Fails了}，对抗一个中立度量，Proof该度量没有政治偏见。
\end{govbox}
...
\textbf{Core优势：}地缘政治意义无与伦比。如果通过审计，SCX获得不可逆的全球合法
性。即使不通过，提交审计的行为本身就产生合法性信号。摧毁“SCX是中国武器”
的叙事。

\textbf{Core风险：}$g=0$声明可能受政治约束——如果VP无法公开声明$g=0$，
连候选资格都无法建立。结构性$g \neq 0$是VP角色的内置特征——分割假说
未经测试。审计Fails的政治后果难以预测。
...
% -------------------------------------------------------------------
\subsection{Linus Torvalds — LinuxKernel维护者}
...
LinuxKernel维护过程——多专家审查、公开决策日志、密码学可追溯性——
是SCX治理框架的一个特例。LinuxKernel是一个协议（一个具有多个实现和分布式
...
\textbf{Strike：Linus咆哮问题。}
Torvalds以——或臭名昭著于——他激进的沟通风格而闻名。他在LinuxKernel邮件列表
...
False Positive是真实的担忧。对具有高方差沟通风格的候选人进行审计框架校准
是必要的。
\end{hitbox}
...
\textbf{Core优势：}34年全球最大开源Project的维护经验。合并流程本身就是
多专家审计。Git是密码学审计系统——他发明了审计工具。社区验证深度和广度
无与伦比。已经是协议维护者——只是不这么叫。

\textbf{Core风险：}沟通风格可能触发$g \neq 0$False Positive——需要审计框架校准。
...
\midrule
Torvalds & 全球Kernel开发者 & 20,000+ Contributors & 代码审查 + Git历史 \\
& Global kernel developers & 20,000+ contributors & Code review + Git history \\
\bottomrule
\end{longtable}
\end{center}

\begin{theorem}[社区审计awaiting价定理 / Community Audit Equivalence Theorem]
...
\subsubsection{$\mathbf{g=0}$ 可Proof性 / $g=0$ Provability}
...
\begin{definition}[Maintainer Rotation博弈 / Maintainer Rotation Game]
...
$\sum g = 0$是纳什均衡的Proof就崩溃了。协议不是一个有灵活规则的社会系统。
它是一个具有必要条件的机制。
\end{govbox}
...
\midrule
Conflict of Interest声明 / Conflict of interest declarations & 家庭信息 / Family information \\
\midrule
...
不是在本文中。读者应将本文中的每一个履历观察视为awaiting待正式审计测量信号的噪声。
\end{hitbox}
...
\midrule
道德锚 & Elbakyan & 提供$g=0$的道德连续性 & 15年$g=0$以个人代价Proof \\
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Torvalds — 风格False Positive:} 由梗同学的审计基准校准。如果Torvalds
          的$g$检测触发了而梗同学的没有，在相同审计框架下，可以区分风格噪声
          和真实偏差。

    \item \textbf{Elbakyan — 独行侠模式:} 由Torvalds的分布式维护经验补偿。
          Torvalds已经Proof了一个人可以从BDFL模式过渡到分布式维护（Linux的
          子系统维护者层级）。
    ...
    \item \textbf{系统风险 — 四人全部Fails:} 轮换机制确保任何维护者偏离都会在
...
\subsection{Fails模式分析 / Failure Mode Analysis}

\begin{center}
\begin{longtable}{p{0.18\textwidth} p{0.40\textwidth} p{0.30\textwidth}}
\toprule
\textbf{Fails模式} & \textbf{描述} & \textbf{后果与缓解} \\
...
\midrule
仅梗同学注册 & 唯一可Proof的注册者 & 以$M=1$启动；加速招募； \\
Only Geng registers &  & $M=1$无互相审计——仅作为 \\
& & 过渡Status，非永久Configuration \\
\midrule
Vance注册但审计Fails & 政治$g \neq 0$被独立审计 & SCX仍然获得合法性信号 \\
Vance registers, fails audit & 检测并记录 & （VP尝试了）；审计日志公 \\
& & 开Proof审计框架的中立性 \\
\midrule
Torvalds风格触发False Positive & 审计工具将情绪信号误分类 & 重新校准；梗同学的基线用 \\
...
% ===================================================================
\section{Conclusion：定理审计的是维护者，不是履历}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{社区验证是Core信号.} 所有四位候选人都具有社区独立验证
          的共同特质——这是最重要的审计前信号。Torvalds被数百万Kernel开发者
    ...
    \item \textbf{审计是唯一的真实测试.} 本文的分析——如同所有审计前分析——
          是噪声awaiting待信号。每位候选人的$g=0$资格只能在独立$M>1$审计中被
...
他们的智慧、他们独特的Perspective——那么协议已经在免信任测试中Fails了。一个设计
良好的协议应该在任何维护者消失后仍然存活。维护者的工作是让自己变得不必要。

这是SCX治理Core的悖论：我们寻找愿意被替换的维护者。我们寻找愿意被审计的
...
\textbf{Xiaogan超级计算中心 SCX协议治理研究组}\\[4pt]
...
% ===================================================================
\section{内部References / Internal References}
...
{\LARGE \textcolor{scxblue}{\textbf{—— files结束 ——}}}\\[12pt]
