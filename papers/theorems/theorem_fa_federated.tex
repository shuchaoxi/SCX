...
% ---------- CJK helper (for proof text in Chinese) ----------
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Wasserstein barycenter}: $\oplus_k \Situs_k =
          \argmin_{\nu} \sum_k \lambda_k \Wass_2^2(\Situs_k, \nu)$ with
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Soundness}: For any $S'_k \neq \Cercis(\cD_k)$ and any
          ...
          $\Pr[\mathcal{V}(c_k, S'_k, \pi^*) = 1] \leq \negl(\lambda)$.
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item \textbf{Regularity}: Assumptions~\ref{ass:lipschitz}
          and~\ref{ass:density} hold.
\end{enumerate}
...
\medskip\noindent
\textbf{步骤2: Situs组合误差——$\Power(T_1)$与$\Power(T_0)$之差.}

由Assumption~\ref{ass:lipschitz},审计泛函$f_{\mathrm{audit}}$关于
Situs度量$L$-Lipschitz连续. 结合Situs同态组合的定义~\eqref{eq:homomorphism}:
\[
|T_1 - T_0|
...
由Kolmogorov--Smirnov距离的性质,结合Assumption~\ref{ass:density}
(密度函数一致有界于$M$):
\[
...
\medskip\noindent
\textbf{步骤3: DP扭曲误差——$\Power(T_2)$与$\Power(T_1)$之差.}

设各参与方的本地差分隐私机制为高斯机制,每方施加
$(\varepsilon_{\mathrm{DP}}, \delta)$-DP. 对$K$方组合,
高斯DP框架\citep{dong2022gaussian}给出trade-off函数:
\[
...
\;\leq\; \alpha - G_{\mu}(\alpha)
\;\leq\; \frac{\mu}{\sqrt{2\pi}} \quad\text{(由中值定理)}.
\]
...
\quad C_2 := \sqrt{\frac{2}{\pi}}.
\end{equation}
当不使用DP时($\delta = 0$),此项精确为零. \textbf{引理2证毕.}

\medskip\noindent
\textbf{步骤4: 有限样本估计误差——$\Power(T_3)$与$\Power(T_2)$之差.}
...
取并界(union bound)$k = 1,\dots,K$,令$t = (1+\eta)\sqrt{\frac{\ln(2K/\xi)}{2 \min_k n_k}}$,
则以概率至少$1 - \xi$同时成立:
\[
...
\medskip\noindent
\textbf{步骤5: 密码学可靠性误差——$\Power(T_4)$与$\Power(T_3)$之差.}

由ZK证明系统的可靠性定义(Definition~\ref{def:zk-proof}(ii)),
...
将\eqref{eq:bound1}--\eqref{eq:bound4}代入\eqref{eq:tri}:
\[
|\Power(T_{\mathrm{fed}}) - \Power(T_{\mathrm{full}})|
...
Assumption~\ref{ass:density}中密度一致有界的条件,若该条件不成立,
...
% ---------- Honest critique ----------
\subsubsection*{\cn{诚实暴击: 严格性批判}}

\cn{
\textbf{(1) Situs同态组合算子的存在性问题.}
整个Theorem~\ref{thm:fed-equiv}的基底是算子$\oplus$的存在性.
...
\medskip\noindent
\textbf{(2) 四源误差的独立性假设.}
\eqref{eq:tri}的三角不等式分解在数学上恒成立,但实际应用中四项误差
...
\medskip\noindent
\textbf{(3) 密度有界性假设(Assumption~\ref{ass:density})的局限性.}
$T_{\mathrm{full}}$的密度可能不是一致有界的——例如当数据集仅含离散标签时,
审计统计量为阶梯分布,密度无定义. 在此情况下,KS距离论证需替换为
Wasserstein距离结合平滑化的论证:
\[
|\Power(T_1;\tau) - \Power(T_0;\tau)|
\;\leq\; \frac{2L\phi}{\varepsilon} + 2\max\{\Pr(|T_0-\tau|\leq\varepsilon),
\Pr(|T_1-\tau|\leq\varepsilon)\},
\]
取$\varepsilon = \sqrt{L\phi}$可得$O(\sqrt{\phi})$界(弱于$O(\phi)$).
...
构造以下\textbf{混淆策略}: 参与方$k$用$\tilde{\cD}_k$替代真实数据集$\cD_k$,
其中$\tilde{\cD}_k$满足:
\begin{enumerate}[label=(\roman*)]
    \item \textbf{边际Situs匹配}: $\Situs(\tilde{\cD}_k) \approx \Situs(\cD_k)$
          使得审计者无法从Situs组合中检测替换;
    \item \textbf{噪声率膨胀}: $\tilde{\eta}_k > \eta_k$,即
          $\tilde{\cD}_k$的有效标签噪声率高于$\cD_k$.
\end{enumerate}

由Theorem~3(噪声-难度不可区分性),当$\eta$为未知参数时,
基于Cercis分数观测的任意检验$\psi$满足:
\[
\sup_{\psi} |\E_{H_1}[\psi] - \E_{H_0}[\psi]| \leq \alpha + o_{n_k}(1).
\]

换言之,仅从$S(x)$观测值无法区分``高噪声''与``高难度''两种假设.
因此,审计者无法检测$\tilde{\cD}_k$与$\cD_k$的替换——
...
由数据处理不等式(DPI, 参见\cite{cover1999elements}):
\[
...
互信息差距通过I-MMSE关系\citep{guo2005mutual}转化为估计误差差距.
对任意随机变量$U,V$满足适当正则性条件,有:
\begin{equation}\label{eq:immse-general}
\E[(U - \E[U \mid V])^2]
...
&\;\leq\; \frac{1}{2\pi e}\, e^{2H(T_{\mathrm{full}}) - 2I(T_{\mathrm{fed}}; \cD_k)} \qquad\text{(由DPI)}\\
...
构造如下极值配置$\cD^*$:
\begin{itemize}
    \item 参与方$k$: $\cD_k$的标签噪声参数$\eta_k = 1$
          (最大噪声,所有标签纯随机);
...
由Theorem~3,参与方$k$可以选择$\tilde{\cD}_k$使其Cercis分数分布与
某个低噪声数据集不可区分,从而将$T_{\mathrm{fed}}$的系统性偏差推向
极端. 该偏差的下界由参与方$k$的数据份额$n_k/n_{\mathrm{total}}$和
实际噪声率$\eta_k$的乘积决定:
\[
\sup_{\cD} |\E[T_{\mathrm{fed}}] - \E[T_{\mathrm{full}}]|
...
$\inf_{\psi} (\cdots)$是一个正常数(由不可区分性的缺口常数决定),
记为$c > 0$. 整理得:
\[
\sup_{\cD} |\E[T_{\mathrm{fed}}] - \E[T_{\mathrm{full}}]|
\;\geq\; c \cdot \eta_k \cdot \frac{n_k}{n_{\mathrm{total}}}.
...
\label{rem:rigor-fa2}
\cn{
Theorem~\ref{thm:info-lower}的证明基于Theorem~3(已独立证明,
...
\medskip\noindent
\textbf{(3) 对抗策略的可检测性.}
混淆策略假设参与方$k$可构造$\tilde{\cD}_k$使Situs边际匹配但噪声率不同.
...
$\varepsilon$-等价性,ZK验证(或等价的承诺-验证机制)是\textbf{必要的}——
不仅是充分的. 若无ZK验证,Theorem~\ref{thm:info-lower}的下界
\eqref{eq:lower-bound}作为与样本规模无关的恒定分数持续存在.
}
\end{corollary}

\begin{proof}
\cn{
由Theorem~\ref{thm:info-lower}的下界\eqref{eq:lower-bound}:
\[
\sup_{\cD} |\E[T_{\mathrm{fed}}] - \E[T_{\mathrm{full}}]|
\;\geq\; c \cdot \eta_k \cdot \frac{n_k}{n_{\mathrm{total}}}.
\]
...
\begin{enumerate}[label=(\roman*)]
    \item $n_k/n_{\mathrm{total}} \to 0$(参与方$k$的数据份额可忽略不计);
    \item $\eta_k = 0$(参与方$k$无标签噪声);
    \item 引入ZK验证(或其他等价机制)消除伪造可能性.
\end{enumerate}
...
\begin{remark}
\cn{
Corollary~\ref{cor:zk-necessity}是Theorem~\ref{thm:info-lower}的
直接推论,标注为\rigorous.
}
\rigorous
\end{remark}
