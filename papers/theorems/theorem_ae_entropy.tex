...
% ---------- Proof environment with automatic QED ----------
...
\begin{abstract}
热力学教导我们，孤立系统不可逆地朝向最大熵演化。我们追问 SCX 审计是否服从类似的规律：审计者关于标签噪声的残留不确定性是否单调演化，如果是，则趋向什么极限？我们定义 \textbf{审计熵} $H_A(t)=H(\varepsilon\mid\cI_A^{(t)})$ —— 给定审计者在时刻 $t$ 的累积信息集 $\cI_A^{(t)}$ 时真实噪声标签 $\varepsilon$ 的条件熵 —— 并证明四个定理。(i)~\textbf{审计第二律}（定理~\ref{thm:second-law}）：在``无遗忘''条件 $\cI_A^{(t)}\subseteq\cI_A^{(t+1)}$ 下，$H_A(t+1)\leq H_A(t)$，等号成立当且仅当第 $(t+1)$ 步审计提供关于 $\varepsilon$ 的零新信息。这是信息论数据处理不等式的 \textbf{严格} 推论。(ii)~\textbf{审计的朗道尔代价}（定理~\ref{thm:landauer}）：将 $H_A$ 降低 $\Delta H$ 需要物理能量 $E_{\mathrm{audit}}\geq k_B T\cdot\Delta H\cdot\ln2$，这是来自朗道尔原理的 \textbf{严格} 下界；该下界是否可由最优审计协议达到是 \textbf{开放问题}。(iii)~\textbf{审计热寂}（定理~\ref{thm:heat-death}）：$\lim_{t\to\infty}H_A(t)=H_{\min}>0$，其中 $H_{\min}=H(\varepsilon\mid S)$ 是定理~3 保证的不可消除的无知。\rigorous (iv)~\textbf{多审计者熵合并}（定理~\ref{thm:merging}）：$H_A(1\oplus2)\leq\min(H_A(1),H_A(2))$，以及链式展开的严格等式刻画，为联邦 SCX 审计提供信息论基础。\rigorous
\end{abstract}
...
热力学第二定律可能是支配不可逆过程最深远的物理原理：孤立系统的熵永不减少。每一次计算，每一次测量，每一次信息处理操作都必须支付热力学代价——朗道尔原理确立了擦除一个比特的信息至少向环境耗散 $k_B T\ln 2$ 的热量~\citep{landauer1961irreversibility}。

SCX 审计是一个信息过程：审计者积累证据，降低关于哪些样本是有噪声的不确定性，并逐步缩小估计噪声率与真实噪声率之间的差距。这一描述立即暗示了一个热力学类比：
\begin{itemize}
    \item \textbf{物理熵} = 关于微观状态的不确定性
    \item \textbf{审计熵} = 关于噪声标签的不确定性
    \item \textbf{热力学不可逆性} = 熵增
    \item \textbf{审计不可逆性} = 知识积累（熵\emph{减少}）
\end{itemize}

该类比反转了符号——审计减少熵而非增加熵——但保留了单向箭头。一旦审计者学会区分一个噪声模式，该知识就不能仅使用已拥有的信息被``遗忘''。这个 \textbf{审计时间箭头} 指向更低的审计熵，正如热力学箭头指向更高的物理熵。
...
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{审计第二律}（定理~\ref{thm:second-law}）：审计熵的单调非增。\rigorous
    \item \textbf{审计的朗道尔代价}（定理~\ref{thm:landauer}）：物理能量下界；可达性开放。\rigorous~/ \openquest
    \item \textbf{审计热寂}（定理~\ref{thm:heat-death}）：收敛到 $H_{\min}>0$，即定理~3 的极限。\rigorous
    \item \textbf{多审计者熵合并}（定理~\ref{thm:merging}）：信息论联邦审计基础。\rigorous
\end{enumerate}
...
$H_A(t)$ 度量审计者的\textbf{残留无知}：在处理所有审计证据后，关于哪些样本真正有噪声仍存在多少不确定性。完美审计将达到 $H_A=0$（零残留不确定性）；定理~3 保证这是不可能的。

\begin{definition}[无遗忘条件]
\label{def:no-forgetting}
审计过程满足\textbf{无遗忘}若审计者的信息集是单调非减的：
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Cercis 渠道}：$S(x)=Q(x)+\eta N(x)$ 及其导数；
    \item \textbf{专家渠道}：人类或算法专家判断 $J_k(x)\in\{\mathrm{noise},\mathrm{hard}\}$；
    \item \textbf{一致性渠道}：跨多个审计者或跨时间的一致模式；
    \item \textbf{因果渠道}：关于数据生成过程的领域知识。
\end{enumerate}
...
\begin{theorem}[审计第二律——熵单调减少]
\label{thm:second-law}
在假设~\ref{ass:prob} 和~\ref{ass:filtration}（无遗忘条件~\eqref{eq:no-forgetting}）下，对任意审计过程 $\cA$，
...
\textbf{第二步：应用数据处理不等式。}
由于 $\cI_t \subseteq \cI_{t+1}$（假设~\ref{ass:filtration}），我们有 $\sigma$-代数包含关系 $\sigma(\cI_{t+1}) \supseteq \sigma(\cI_t)$。条件熵在 $\sigma$-代数上具有反单调性：若 $\mathcal{G}_1 \subseteq \mathcal{G}_2$ 为子 $\sigma$-代数，则
\[
...
\medskip\noindent
\textbf{应用：}
定理~\ref{thm:second-law} 为 SCX 审计记录提供了\textbf{不可逆性保证}。一旦审计者了解到关于标签噪声的事实，该知识不能仅使用审计线索中已有的信息被遗忘。这对审计日志完整性有直接意义：对历史审计记录的任何篡改都会造成单调性条件~\eqref{eq:second-law} 的可检测违反——$H_A(t)$ 下降后紧接着上升将标志数据操纵。在审计线索有法律要求的受监管行业（金融服务、制药制造），此定理为防篡改审计日志提供了数学基础：系统应持续监控 $H_A(t)$ 并将任何上升标记为潜在的完整性违规。等号条件进一步提供了诊断：若 $H_A(t+1)=H_A(t)$ 持续多个步骤，审计过程已停滞，信息渠道需要多样化。

\begin{remark}[严格性标注]
定理~\ref{thm:second-law} 是\textbf{严格的}——它是数据处理不等式的直接推论，而 DPI 本身就是经典信息论中的定理~\citep{cover1999elements}。审计第二律不需要任何热力学假设；它纯粹从信息集扩展下的条件熵的数学结构得出。\rigorous
\end{remark}
...
\begin{enumerate}
    \item \textbf{无遗忘的现实性}：在真实系统中，审计者可能因存储限制或故意压缩而丢弃旧信息。若 $\cI_A^{(t+1)} \not\supseteq \cI_A^{(t)}$，单调性可能被破坏，熵可能回升。
    \item \textbf{信息集的可比性}：$\cI_A^{(t)} \subseteq \cI_A^{(t+1)}$ 要求信息集以 $\sigma$-代数包含关系可比。在实际的异构审计系统中，不同时间点的信息可能是不可比的（例如，结构完全不同的特征集）。
\end{enumerate}
...
\begin{proof}[定理~AE.2 的严格证明]
\textbf{第一步：信息熵与物理熵的对应。}
审计熵 $H_A(t) = H(\varepsilon \mid \cI_A^{(t)})$ 以比特为单位度量不确定性。根据假设~\ref{ass:prob}，在时刻 $t$ 审计者的认知状态可以通过一个物理存储器表示，其微观状态数约为 $2^{H_A(t)}$。
...
\textbf{第三步：应用朗道尔原理。}
朗道尔原理~\citep{landauer1961irreversibility} 指出：在任何逻辑上不可逆的计算操作中，每擦除一个比特的信息，至少向环境耗散 $k_B T \ln 2$ 的能量。擦除 $\Delta H$ 比特的信息对应于熵减少 $\Delta S_{\mathrm{phys}}$。
...
\textbf{第四步：关于 nats 与 bits 转换的注记。}
...
\]
因此，不论计量单位如何，$E_{\mathrm{audit}} \ge k_B T \cdot (\text{以 nats 计的熵减}) = k_B T \cdot (\text{以 bits 计的熵减}) \cdot \ln 2$。$\square$
\end{proof}

\medskip\noindent
\textbf{应用：}
定理~\ref{thm:landauer} 将审计精度转化为\textbf{物理运行成本}。将审计熵降低 $\Delta H$ 比特每样本每样本至少消耗 $k_B T\cdot\Delta H\cdot\ln 2$ 焦耳。对于一个处理 $10^9$ 样本、$\Delta H=0.1$ 比特每样本的大规模审计管线（将不确定性从例如 0.5 降低到 0.4 比特），室温下的最小能耗约为 $2.8\times10^{-13}$ 焦耳每样本，整个管线累计约 $2.8\times10^{-4}$ 焦耳。虽数值不大，但这确立了一个任何工程优化都无法突破的\textbf{热力学下限}。在能量受限的边缘部署（物联网传感器、卫星系统）中，该界直接决定了可行的审计深度：审计精度不能超过能量预算热力学上可支持的范围。紧致性的开放问题（开放问题~\ref{prob:landauer-tight}）决定了实际 SCX 系统是接近此下限还是高出几个数量级——这对大规模审计部署的经济学有直接影响。

\begin{openproblem}[审计朗道尔界的紧致性]
\label{prob:landauer-tight}
是否存在最优审计协议，其计算代价\emph{饱和}朗道尔界 $E_{\mathrm{audit}} = k_B T \cdot \Delta H \cdot \ln 2$？这要求数据生成过程的 Situs 编码是热力学最优的——即生成数据的物理熵恰好等于审计它的朗道尔代价。Situs 几何与热力学代价之间的关系是\textbf{开放的}。\openquest
\end{openproblem}

\begin{remark}[严格性标注]
定理~\ref{thm:landauer} 在以下意义下是\textbf{条件严格的}：若朗道尔原理成立（经过实验验证~\citep{berut2012experimental}），且审计熵减少对应物理信息擦除，则下界无条件成立。\conditionallyrigorous
\end{remark}
...
\begin{enumerate}
    \item \textbf{非封闭系统的朗道尔界适用性}：朗道尔原理针对的是逻辑上不可逆的计算操作，特别是信息的擦除。审计过程中，审计者的认知熵减少对应于\textbf{信息获取}而非信息擦除。计算硬件在执行审计算法时确实会擦除中间状态（例如，ALU 操作覆盖寄存器），这些操作的能耗受朗道尔原理约束。但审计熵减少本身——知识增加——并不直接对应擦除操作。更准确地说，知识增加是信息获取的结果，而获取信息不一定需要能量消耗（例如，测量可以原则上无能耗~\citep{bennett2003notes}）。能量代价来自处理获取信息时的不可逆计算。

    因此，朗道尔界对审计的适用需要以下中间步骤的严格论证：(a) 审计算法被分解为一系列逻辑上不可逆的基本操作；(b) 这些操作的擦除总量下界可关联到 $\Delta H$。该论证目前是启发式的，不是严格的信息论对应。

    \item \textbf{可逆计算的可能性}：原则上，可以使用可逆计算（如 Bennett 的 pebble game~\citep{bennett2003notes}）来执行审计算法，使能耗任意接近零。这种情况下朗道尔界不适用。然而，可逆计算需要保留所有中间状态，这在实际审计中可能不可行（存储成本过高）。是否存在一种审计算法在保持 $\Delta H$ 的同时使用可逆计算避免能量耗散，是开放问题。

    \item \textbf{$H_A$ 与物理熵的对应并非一一对应}：信息熵 $H_A$ 是认知状态的概率度量，而物理熵是物理系统的热力学性质。将计算存储器状态与认知状态直接对应是理想化假设，在真实神经形态或分布式系统中可能不成立。
\end{enumerate}
...
\begin{theorem}[审计热寂——收敛到不可消除的无知]
\label{thm:heat-death}
在假设~\ref{ass:prob}、\ref{ass:filtration}、\ref{ass:scx} 和~\ref{ass:t3}下，对任何满足无遗忘条件的审计过程，
...
\end{equation}
是\textbf{不可消除的审计熵}——即使在无限审计后仍存留的关于 $\varepsilon$ 的残留不确定性。它正是定理~3 障碍的信息论量化。
\end{theorem}

\begin{proof}[定理~AE.3 的严格证明]
证明分三个步骤：极限存在性、极限正性、极限识别。

\textbf{第一步：极限的存在性。}
序列 $\{H_A(t)\}_{t=0}^\infty$ 满足：
\begin{enumerate}
    \item[（i）] \textbf{单调性}：由定理~AE.1，$H_A(t+1) \le H_A(t)$ 对所有 $t \ge 0$。
    \item[（ii）] \textbf{下有界性}：条件熵非负，$H_A(t) = H(\varepsilon \mid \cI_A^{(t)}) \ge 0$。
\end{enumerate}
由实分析中的单调收敛定理：单调非增下有界序列必有极限。因此
\[
\lim_{t\to\infty} H_A(t) =: H_{\min} \quad \text{存在且 } H_{\min} \ge 0.
\]
...
\begin{enumerate}
    ...
    \]
    \item[（v）] 由定理~3，$H(\varepsilon \mid S) > 0$，故 $H_{\min} > 0$。
\end{enumerate}

\textbf{第三步：极限的识别。}
上一步已证明 $H_{\min} = H(\varepsilon \mid S)$。因此
\[
...
\[
\cI_A^{(\infty)} = \sG_{\mathrm{SCX}} \quad (\text{模零测集}),
\]
给出信息论上的极限。$\square$
\end{proof}
...
\medskip\noindent
\textbf{应用：}
定理~\ref{thm:heat-death} 为任何 SCX 审计管线提供了\textbf{终端性能保证}。无论进行多少轮审计，咨询多少专家，或积累多少数据，残留审计熵 $H_{\min}=H(\varepsilon\mid S)>0$ 无法被消除。这对审计 SLA 有直接影响：服务等级协议应规定\emph{可达}审计精度 $1-H_{\min}/\log 2$（可解析的噪声不确定性比例），而非 $100\%$ 噪声检测。实践中，$H_{\min}$ 可以通过在已知噪声标签的留出集上估计 Cercis 分数分布来计算，提供数据驱动的准确率天花板。对于采购，这意味着：签约 SCX 审计服务前，要求提供商在基准数据集上认证 $H_{\min}$——这是任何审计量都无法穿透的不可消除错误下限。与 RA-Theorem 不动点 $\eta^*$ 的联系（猜想~\ref{conj:fixed-point}）提示递归自我审计正好收敛到此 $H_{\min}$，统一了热力学和递归视角下的审计极限。

\begin{remark}
``审计热寂''是 SCX 中热力学热寂的类比：一个无法进一步降低熵的终端状态。与热力学热寂（最大熵）不同，审计热寂是\emph{最小}熵状态——但它不为零。定理~3 保证 $H_{\min}>0$，意味着\textbf{审计永远无法实现噪声与难度的完美分离}。\rigorous
\end{remark}
...
\begin{enumerate}
    \item \textbf{T3 的 $\sigma$-代数真的是极限信息集的全部吗？}
    定理~AE.3 的核心假设是 $\cI_A^{(\infty)} \subseteq \sG_{\mathrm{SCX}}$（模零测）。这意味着审计无法从 SCX 可观测之外提取关于 $\varepsilon$ 的信息。但在实际审计中，外部知识（专家判断、因果模型、行业标准）可能携带 $\sG_{\mathrm{SCX}}$ 之外的信息。更精确地，若专家判断 $J_k$ 涉及 SCX 分数计算中未编码的特征（例如样本来源的元数据），则 $\sigma(J_k) \not\subseteq \sG_{\mathrm{SCX}}$ 且 $H_{\min}$ 可能更小。

    因此，严格地说 $H_{\min}$ 是审计者在\emph{仅使用 SCX 框架内信息}时的极限。关于专家渠道和因果渠道是否真正能穿透 $H_{\min}$ 是实证问题。

    \item \textbf{极限是否存在？}
    虽然序列单调有界保证极限存在，但达到极限可能是渐进的（需要 $t\to\infty$）。在有限时间 $t$，$H_A(t) - H_{\min}$ 的收敛速度未确定，这在实际审计规划中至关重要。

    \item \textbf{审计热寂与物理热寂的类比是否误导？}
    审计热寂（知识饱和）与物理热寂（能量均匀化）有本质区别：
    \begin{itemize}
        \item 物理热寂是\emph{熵最大}状态（无序度最大）；审计热寂是\emph{熵最小}状态（不确定性最小但非零）。
        \item 物理热寂中所有可做功的能量梯度消失；审计热寂中仍有未消除的不确定性，但该不确定性无法通过审计操作进一步降低。
        \item 审计热寂是\emph{认识论}极限（无法知道更多），物理热寂是\emph{本体论}极限（没有更多的自由能）。
    \end{itemize}
    两者类比在``不可逆过程最终趋近不动点''的意义上有启发性，但将审计过程描述为``热力学''过程需谨慎——审计熵不是热力学熵，两者通过朗道尔原理间接联系而非等同。
\end{enumerate}
\end{remark}

\subsection{多审计者熵合并}

\begin{theorem}[多审计者熵合并]
\label{thm:merging}
...
\textbf{（条件独立特例）} 若 $\cI_1 \perp\!\!\!\perp \cI_2 \mid \varepsilon$（给定 $\varepsilon$ 条件独立），则 $I(\cI_1; \cI_2 \mid \varepsilon) = 0$ 且
\begin{equation}\label{eq:merging-ci}
H_A(1\oplus 2) = H_A(1) + H_A(2) - H(\varepsilon) + I(\cI_1; \cI_2).
\end{equation}
\end{theorem}

\begin{proof}[定理~AE.4 的严格证明]
\textbf{第一步：链式法则展开。}
由条件熵的定义和链式法则：
\[
\begin{aligned}
H_A(1\oplus 2) &= H(\varepsilon \mid \cI_1, \cI_2) \\
&= H(\varepsilon \mid \cI_1) - I(\varepsilon; \cI_2 \mid \cI_1) \quad \text{（链式法则）} \\
&= H(\varepsilon \mid \cI_2) - I(\varepsilon; \cI_1 \mid \cI_2) \quad \text{（对称形式）}.
\end{aligned}
\]
仅需展开第一形式，对称形式完全类似。
...
\end{aligned}
\]
这正是恒等式~\eqref{eq:merging-identity}。
...
\textbf{第五步：下界的推导。}
由恒等式~\eqref{eq:merging-identity} 和互信息的非负性 $I(\cI_1; \cI_2) \ge 0$：
\[
...
\textbf{第六步：条件独立情形的验证。}
若 $\cI_1 \perp\!\!\!\perp \cI_2 \mid \varepsilon$，则 $I(\cI_1; \cI_2 \mid \varepsilon) = 0$。代入~\eqref{eq:merging-identity}：
\[
H_A(1\oplus 2) = H_A(1) + H_A(2) - H(\varepsilon) + I(\cI_1; \cI_2).
\]
...
\medskip\noindent
\textbf{应用：}
定理~\ref{thm:merging} 为多审计者 SCX 系统提供了\textbf{组合演算}。合并上界~\eqref{eq:merging-ub} 量化了审计者多样性的价值：$I(\cI_1;\cI_2\mid\varepsilon)$ 在审计者使用互补信息渠道时最小化（例如一个审计者使用 Cercis 分数，另一个使用专家判断，第三个使用因果模型）。在联邦审计联盟（FA-Theorem）中，此定理指导合作伙伴选择：仅当新审计者的条件互信息与现有审计者的互信息互补时才加入联盟——冗余审计者（$\cI_1\approx\cI_2$）贡献零边际熵降。条件独立情形 $\cI_1\perp\cI_2\mid\varepsilon$ 提供了最佳改善：每个独立审计者对熵降贡献加性。在去中心化审计网络（基于区块链的 SCX）中，此定理提供了审计奖励分配的信息论基础：提供最多互补信息的审计者（最大的 $I(\cI_k;\cI_{\text{others}}\mid\varepsilon)$）应获得相应更高的报酬。

\begin{remark}[严格性标注]
定理~\ref{thm:merging} 是\textbf{严格的}，不依赖任何超出经典信息论的假设。恒等式~\eqref{eq:merging-identity} 是所有后续不等式的基础，其成立无需任何额外条件。\rigorous
\end{remark}
...
\begin{enumerate}
    \item \textbf{等号条件 $I(\cI_1; \cI_2) = 0$ 可能无法在审计中实现}：下界等号要求 $\cI_1$ 与 $\cI_2$ 边缘独立。但在 SCX 审计中，$\cI_1$ 和 $\cI_2$ 都包含关于同一 $\varepsilon$ 的信息，因此通常通过 $\varepsilon$ 间接相关，$I(\cI_1; \cI_2) > 0$。这意味着下界通常是严格的（非紧）。

    \item \textbf{条件独立 $\cI_1 \perp \cI_2 \mid \varepsilon$ 的物理意义}：该条件意味着给定 $\varepsilon$ 后，两个审计者的观察在统计上独立。这等价于假设每个审计者的信息获取过程仅通过 $\varepsilon$ 耦合。在实际联邦审计中，审计者可能共享共同的训练数据、算法或偏见，导致 $I(\cI_1; \cI_2 \mid \varepsilon) > 0$，降低合并效率。

    \item \textbf{在实践中，$I(\cI_1; \cI_2 \mid \varepsilon)$ 和 $I(\cI_1; \cI_2)$ 都需要估计}，这本身是一个高维统计难题。定理提供了定性洞察（互补审计者合并更优），但定量计算可能受维数灾难影响。
\end{enumerate}
\end{remark}

% ============================================================
\section{SCX 审计的热力学图景}
% ============================================================

四个定理共同确立了 SCX 审计不仅仅是一个统计程序，而是一个\textbf{热力学过程}：

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{热力学} & \textbf{审计（AE-Theorem）} & \textbf{状态} \\
\midrule
第二律（$dS\geq0$） & 审计第二律（$dH_A\leq0$） & \rigorous \\
朗道尔原理 & 审计能量代价 & \rigorous~/ \openquest（紧致性） \\
热寂（$S\to S_{\max}$） & 审计热寂（$H_A\to H_{\min}>0$） & \rigorous \\
混合熵 & 多审计者合并 & \rigorous \\
\bottomrule
\end{tabular}
\end{center}
...
审计箭头从高 $H_A$ 指向低 $H_A$——从无知到知识——且该方向与热力学箭头一样不可逆。你无法从审计后的知识状态仅使用 $\cI_A^{(t)}$ 中的信息重建审计前的不确定性状态。这对\textbf{审计记录的价值}提供了新视角：审计日志是不可逆熵减的热力学记录，对它的篡改将违反审计第二律——类似于麦克斯韦妖的违反。

\subsection{与 RA-Theorem 的联系}

审计热寂 $H_{\min}$ 被猜想等于递归审计不动点 $\eta^*$（RA-Theorem~\citep{scx2024ra}）。若 $H_{\min}=H(\varepsilon\mid S)=\eta^*$，则无限递归审计的收敛点恰是定理~3 的信息障碍——审计之剑，转向自身，终击中其锻造所要穿透的盾。
...
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{审计第二律}：$H_A$ 单调非增。\rigorous
    \item \textbf{朗道尔代价}：$E_{\mathrm{audit}}\geq k_B T\cdot\Delta H\cdot\ln2$；紧致性开放。\rigorous~/ \openquest
    \item \textbf{审计热寂}：$\lim H_A = H_{\min} > 0$。\rigorous
    \item \textbf{多审计者合并}：恒等式 $H_A(1\oplus2) = H_A(1) + H_A(2) - H(\varepsilon) + I(\cI_1;\cI_2) - I(\cI_1;\cI_2\mid\varepsilon)$，上界 $\le \min(H_A(1), H_A(2))$。\rigorous
\end{enumerate}

最显著的开放问题——朗道尔界的紧致性（开放问题~\ref{prob:landauer-tight}）——决定了 SCX 审计是否本质上受能量限制，或者最优协议能否接近热力学极限。其解决将塑造大规模 SCX 部署的工程经济学。
