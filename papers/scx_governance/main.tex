...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Multi-source data availability} The proliferation of independently collected data---satellite imagery (nighttime lights for economic activity, aerosol optical depth for pollution), household surveys, administrative records, social security databases, hospital birth registries, school enrollment records---enables independent estimation of governance statistics without relying on government-published aggregates.
    \item \textbf{Methodological pluralism} Different statistical agencies and research groups employ heterogeneous methodologies (frequentist vs. Bayesian, survey-based vs. administrative vs. remote sensing), creating a natural \textit{multi-expert} structure.
    \item \textbf{International monitoring infrastructure} Organizations such as the IMF, World Bank, OECD, and academic consortia routinely produce independent estimates of national statistics, forming a de facto auditor community.
\end{enumerate}
...
%=====================================================================
\section{Formalization: Governance as a Signaling Game with Audit}
\label{sec:formalization}
%=====================================================================
\subsection{The State Space Status}
\begin{definition}[Governance State Status]
\label{def:state}
...
\end{itemize}
\end{definition}
\begin{definition}[Government's Information ]
\label{def:gov_info}
...
\end{definition}
\subsection{The Signaling Game}
\begin{definition}[Government's Claim ]
\label{def:claim}
...
\end{definition}
\begin{definition}[Auditor Community ]
\label{def:auditors}
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Data source} Household surveys, satellite imagery, administrative records from independent agencies, social media data, commercial data providers, academic field studies.
    \item \textbf{Methodology} Frequentist sampling theory, Bayesian hierarchical models, machine learning estimators, structural equation models, difference-in-differences, synthetic control methods.
    \item \textbf{Institutional affiliation} National statistical agencies of other countries, international organizations (IMF, World Bank, OECD, UN), academic research groups, non-governmental organizations, independent think tanks.
\end{enumerate}
\end{definition}
\begin{definition}[Multi-Expert Consensus ]
\label{def:consensus}
...
\end{definition}
\subsection{The \Yajie{} Payoff Structure Yajie}
...
\begin{definition}[Government Payoff ]
\label{def:payoff}
...
\end{equation}
\begin{definition}[Honest Reporting ]
\label{def:honest}
...
\begin{assumption}[A1: Bounded State Space Status]
\label{ass:A1}
...
\end{assumption}
\begin{assumption}[A2: Bounded Benefit Function ]
\label{ass:A2}
...
\end{assumption}
\begin{assumption}[A3: Positive Detection Cost ]
\label{ass:A3}
...
\end{assumption}
\begin{assumption}[A4: Auditor Conditional Independence ]
\label{ass:A4}
...
\end{equation}
\end{assumption}
\begin{assumption}[A5: Auditor Detection Power ]
\label{ass:A5}
...
\end{assumption}
\begin{assumption}[A6: Unbiased Auditor Estimates ]
\label{ass:A6}
...
\end{assumption}
\begin{assumption}[A7: Government Rationality ]
\label{ass:A7}
...
\end{assumption}
\begin{assumption}[A8: Finite Publication Set ]
\label{ass:A8}
...
\end{assumption}
\begin{assumption}[A9: Policy Outcome Observability ]
\label{ass:A9}
...
\end{assumption}
\begin{assumption}[A10: Multi-Expert Causal Model Diversity ]
\label{ass:A10}
...
\end{assumption}
\begin{assumption}[A11: Lipschitz Payoff Sensitivity ]
\label{ass:A11}
...
\end{assumption}
\begin{assumption}[A12: Regime Transition Detectability ]
\label{ass:A12}
...
\end{assumption}
%=====================================================================
\section{Theorem 1: Transparency Dominance}
...
\subsection{The Detection Probability}
\begin{lemma}[Single-Component Detection Probability ]
...
\begin{theorem}[Transparency Dominance ]
...
\end{proof}
\begin{corollary}[Required Auditor Multiplicity ]
...
\end{remark}
%=====================================================================
\section{Theorem 2: Opacity Detection Bound}
\label{sec:opacity}
%=====================================================================
...
\begin{definition}[Publication Strategy ]
...
\end{definition}
\begin{definition}[Auditor Gap Detection ]
...
\begin{theorem}[Opacity Detection Bound ]
\label{thm:opacity}
...
\end{proof}
\begin{corollary}[Detection Probability for Partial Publication ]
...
\end{remark}
%=====================================================================
\section{Theorem 3: Policy Unidentifiability}
...
\end{itemize}
\end{definition}
\begin{definition}[Policy Outcome Decomposition ]
...
\end{definition}
\subsection{Theorem Statement and Proof}
\begin{theorem}[Policy Unidentifiability ]
...
\textbf{World 2 (Implementation-dominant}
\begin{align}
...
\textbf{World 4 (Measurement-dominant}
\begin{align}
...
\end{proof}
\begin{corollary}[Assumption Mandate for Policy Evaluation ]
...
\end{remark}
%=====================================================================
\section{Multi-Expert Policy Evaluation}
...
\subsection{Expert Configuration}
\begin{definition}[Policy Evaluation Expert ]
...
\end{itemize}
\end{definition}
\begin{definition}[Expert Estimate ]
...
\end{definition}
\subsection{\Yajie{} Consensus for Policy Effects Yajie}
\begin{definition}[Correlation-Adjusted \Yajie{} Weights Yajie]
...
\end{definition}
\begin{definition}[\Yajie{} Consensus Policy Effect Yajie]
...
\end{definition}
\begin{proposition}[Consensus Efficiency ]
...
\begin{table}[htbp]
\centering
\caption{Multi-expert configurations for governance metrics}
\label{tab:expert_configs}
\begin{tabular}{lp{2.5cm}p{3cm}p{3cm}p{2.5cm}}
\toprule
\textbf{Metric} & \textbf{Expert 1} & \textbf{Expert 2} & \textbf{Expert 3} & \textbf{Expert 4} \\
\midrule
Fertility  & National census  & Hospital birth records  & School enrollment  & UN Population Division  \\
\midrule
Employment  & Household survey  & Payroll data  & Social security records  & Satellite nightlights  \\
\midrule
Inflation  & CPI survey CPI & Online price scraping  & Producer price index  & IMF WEO estimates IMF \\
\midrule
GDP growth GDPGrowth & National accounts  & Electricity consumption  & Satellite nighttime lights  & VAT/tax receipts / \\
\midrule
Pollution  & Ground monitors  & Satellite AOD  & Industrial emissions reports  & Citizen science sensors  \\
\bottomrule
\end{tabular}
\end{table}
\begin{remark}[Methodological Diversity ]
...
\end{algorithm}
%=====================================================================
\section{\Cercis{} Score for Policy Cercis}
\label{sec:cercis}
%=====================================================================
...
\begin{definition}[Policy Quality Score ]
\label{def:policy_Q}
...
\end{definition}
\begin{definition}[Policy Novelty Score ]
\label{def:policy_N}
...
\end{definition}
\begin{definition}[\Cercis{} Policy Score \Cercis{}]
...
\end{definition}
\begin{remark}[Cercis{} and Policy Learning \Cercis{}]
...
\toprule
Policy Type  & Dimension  & $\varepsilon_{\mathrm{pred}}$ & $\varepsilon_{\mathrm{dir}}$ & $Q$ & $N$ & $S$ \\
\midrule
...
\end{table}
%=====================================================================
\section{\Spring{} Gating for Governance Regime Detection Spring}
\label{sec:spring}
%=====================================================================
...
\end{definition}
\begin{definition}[\Spring{} Detection Statistic \Spring{}]
...
\end{equation}
\end{definition}
\begin{proposition}[\Spring{} Regime Detection Properties \Spring{}]
...
%=====================================================================
\subsection{Fertility Rate Auditing}
...
\subsection{Employment Rate Auditing}
...
\subsection{Government Openness Index}
We formalize a \textbf{government openness index} as a directly measurable quantity from the publication record:
\begin{definition}[Openness Index ]
\label{def:openness}
\begin{equation}
    \Omega_t = \frac{K_{\mathrm{pub}, t}}{K_{\min}} \cdot \frac{1}{3}\left(
        \underbrace{\frac{g_{\mathrm{pub}, t}}{g_{\max}}}_{\text{granularity}} +
        \underbrace{\frac{1}{1 + \bar{\tau}_{\mathrm{lag}, t}}}_{\text{timeliness}} +
        \underbrace{\frac{f_{\mathrm{update}, t}}{f_{\max}}}_{\text{frequency}}
    \right),
    \label{eq:openness}
\end{equation}
where:
...
\subsection{Policy Evaluation: Causal Identification Strategies}
For policy evaluation, the multi-expert framework employs causally distinct identification strategies:
\begin{enumerate}[label=(\textbf{\arabic*}),leftmargin=*]
    \item \textbf{Difference-in-Differences (DiD)} Compares outcome trends in treated vs. untreated units before and after policy implementation. Identification assumption: parallel trends in the absence of treatment.
    \item \textbf{Synthetic Control (SC)} Constructs a weighted combination of untreated units that matches the treated unit's pre-treatment trajectory. Identification assumption: the synthetic control approximates the counterfactual.
    \item \textbf{Regression Discontinuity Design (RDD)} Exploits a threshold in policy assignment (e.g., population cutoff for program eligibility). Identification assumption: units just above and below the threshold are comparable.
    \item \textbf{Instrumental Variables (IV)} Uses an exogenous source of variation in policy exposure. Identification assumption: exclusion restriction (instrument affects outcome only through policy exposure).
    \item \textbf{Structural estimation} Specifies and estimates a fully parametric economic model. Identification assumption: the model structure correctly captures all relevant mechanisms.
\end{enumerate}
...
%=====================================================================
\section{M-Parameter as Management KPI MKPI}
\label{sec:m-kpi}
...
\end{enumerate}
\end{definition}
\begin{theorem}[Precise Blame Attribution  ( — Blame Shedding Theorem)]
...
\end{proof}
\begin{corollary}[Perfect Blame Shedding for Honest Managers ]
