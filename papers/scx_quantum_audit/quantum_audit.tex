...
%  量子通信在SCX防篡改审计中的严格形式化：M_t传输的信息论安全、
%  审计纠缠作为共识原语、量子信道容量。
% ===========================================================================
...
\title{
  ...
  {\normalsize\textit{量子安全的SCX审计：}}\\[2pt]
  {\normalsize\textit{BB84协议、审计纠缠与量子信道理论}}\\[4pt]
  {\footnotesize A Rigorous Formalization of Tamper-Proof Audit Communication}\\[2pt]
  {\footnotesize\textit{防篡改审计通信的严格形式化}}
}
...
via quantum mechanical principles. We develop three interconnected results:

\textbf{(1) BB84 Protocol for \Mt\ Transmission} (Section~\ref{sec:bb84}): We
formalize the Bennet--Brassard 1984 quantum key distribution protocol as the
...
introduces a detectable error rate $\varepsilon > 0$, bounded below by
$11\%$ for the intercept-resend attack — a guarantee that follows from the
no-cloning theorem and the uncertainty principle, not from unproven
...
Yajie) share entangled quantum states. Measuring one auditor's state instantly
determines the other's outcome. We prove an \textbf{Entanglement Audit Theorem}:
...
$C_Q = 1 - H_2(p) - p\log_2 3$, where $H_2$ is the binary entropy function.
...
\end{center}
\noindent
本文对SCX（位点共识专家）框架中的\textbf{量子安全审计通信}进行了严格形式化。
核心思想是：\Mt\ 参数传输和审计结果交付的安全性可以从计算复杂度提升到
\textbf{信息论安全}，通过量子力学原理实现。我们发展了三个相互关联的结果：

\textbf{(1) \Mt\ 传输的BB84协议}（第\ref{sec:bb84}节）：我们将Bennet--Brassard 1984
量子密钥分发协议形式化为从审计者（Alice）到审计账本的\Mt\ 参数安全传输通道。
我们证明任何恶意实体的窃听尝试都会引入可检测的错误率 $\varepsilon > 0$，
对于截获-重发攻击，其下界为 $11\%$——这一保证来自不可克隆定理和不确定性原理，
而非未经证明的计算假设。

\textbf{(2) 审计纠缠}（第\ref{sec:entanglement}节）：我们引入\emph{审计纠缠}概念，
即两个审计者（如Spring和Yajie）共享纠缠量子态。测量一个审计者的状态会立即决定
另一个审计者的结果。我们证明\textbf{纠缠审计定理}：如果两个审计者共享一个最大纠缠
Bell对且对手篡改其中一半，共享态的保真度下降为
$\cF \leq 1 - \delta_{\text{tamper}}$，使得篡改在经过
$\cO(1/\delta_{\text{tamper}})$ 次独立测量后以任意接近1的概率被检测到。

\textbf{(3) 量子审计信道}（第\ref{sec:channel}节）：我们正式定义了量子审计信道
$\cN_{\text{audit}}$并证明了其容量定理：可靠审计信息传输的最大速率由Holevo量
$\chi(\cN_{\text{audit}})$ 界定，对于噪声参数为 $p$ 的去极化信道，
$C_Q = 1 - H_2(p) - p\log_2 3$。

\textbf{(4) 实际可行性}（第\ref{sec:practical}节）：我们分析了硬件需求、现有
量子网络基础设施和分阶段部署路线图。我们的结论是，虽然完全量子安全的审计
尚未能大规模部署，但结合抗量子后量子密码学（CRYSTALS-Kyber）和基于纠缠的
时间戳的混合经典-量子方法今天已可行。

\vspace{4pt}
\noindent\textbf{关键词：} 量子密钥分发，BB84，审计安全，纠缠，量子信道容量，
SCX，防篡改审计，信息论安全
\end{abstract}
...
\par\smallskip
\noindent\textit{不可克隆定理：未知量子态不能被完美复制。}
\end{definition}
...
\par\smallskip
\noindent\textit{测量扰动：任何与系统状态不对易的测量必然扰动该系统状态。}
\end{definition}
...
\textbf{Output:} Shared key $k \in \{0,1\}^{\ell}$ ($\ell \leq n/2$ expected)
that encodes \Mt, or $\bot$ (abort on detected eavesdropping).
...
\begin{enumerate}[label=1.\arabic*]
  ...
  \item Privacy amplification: apply a 2-universal hash function to reduce
        any residual Eve information to negligible $\negl(\lambda)$.
...
function and $Q$ is the QBER. The protocol is secure (positive key rate)
whenever $Q < 11\%$ (since $1 - 2H_2(0.11) > 0$).
\end{theorem}
...
\begin{itemize}
  ...
  \item At 10 MHz photon source rate: transmission takes $< 1$ millisecond.
...
\subsubsection{Audit Trail with Quantum Timestamps}

Each $\Mt$ transmission generates a quantum-certified timestamp by
entangling a timestamp qubit with the transmitted state:

\[
\ket{\Psi_{\text{stamp}}} = \frac{1}{\sqrt{2}}
\left(\ket{0}_{\text{ts}} \otimes \ket{\Mt} + \ket{1}_{\text{ts}}
\otimes \ket{\Mt'}\right)
\]

where $\ket{\Mt}$ is the legitimate state and $\ket{\Mt'}$ is a
verification copy. Measuring the timestamp qubit later reveals whether
the $\Mt$ state has been disturbed, providing an immutable audit trail.
...
\]
Equivalently, the state is \emph{entangled} if its von Neumann entropy of
either reduced state exceeds the entropy of the total state:
$S(\rho_S) > S(\rho_{SY})$ or $S(\rho_Y) > S(\rho_{SY})$.
\par\smallskip
\noindent\textit{审计纠缠态是两个审计者之间的二分量子态，不能写成乘积态的凸组合。}
\end{definition}

\subsection{Entanglement as a Consensus Primitive}
\label{sec:ent_consensus}

The key insight: \textbf{entanglement provides a stronger consensus primitive
than classical communication}. In classical distributed consensus (e.g.,
PBFT, Raft), $n$ nodes must exchange $O(n^2)$ messages to reach agreement.
With pre-shared entanglement and local measurements, consensus can be
achieved with \emph{zero classical communication} in the ideal case.
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Perfect Correlation:}
        $\Pbb(\text{outcome}_S = \text{outcome}_Y) = 1$ for each copy.
  
  \item \textbf{Consensus without Communication:} The auditors can generate
        a shared random string of length $N$ with perfect agreement and
        zero classical communication.
...
fidelity under CPTP maps. For $k$ independent copies, the probability
that all $k$ copies pass a Bell test (each passing with probability $\cF$)
is $\cF^k$. The tamper detection probability is $1 - \cF^k \to 1$ as
$k \to \infty$, with sample complexity $k = \cO(1/(1-\cF))$.
\end{proof}
...
\begin{enumerate}[label=2.\arabic*]
  ...
  \item They compute the CHSH statistic:
        ...
        \]
  \item \textbf{Acceptance Criterion:} If $|S| \leq 2$ (classical bound),
        the entanglement has been broken — abort audit.
        If $|S| > 2$ (up to $2\sqrt{2}$ for maximally entangled),
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item \textbf{Instantaneous Collapse:} If an adversary tampers with
        Spring's audit state (e.g., by feeding Spring manipulated data),
        the shared entangled state collapses, immediately affecting
        Yajie's measurement statistics. This is not a causal signal —
        it is a correlation that \emph{reveals} the tampering, not one
        that \emph{transmits} information superluminally.
...
\end{itemize}
\par\smallskip
\noindent\textit{量子审计信道是一个CPTP映射，将审计者发送的编码$\Mt$的量子态
映射到审计账本接收到的量子态。}
\end{definition}
...
\textbf{Audit interpretation:} $p$ is the probability that an adversary
successfully randomizes a transmitted qubit without being detected.
The depolarizing channel is the ``worst-case'' noise model for BB84
security analysis.

\subsubsection{Phase Damping Channel}

Models decoherence without energy loss (relevant for fiber transmission):
\[
...
the classical capacity (equivalently, the Holevo quantity, which is additive
for this channel) is:
\[
...
over a set of mutually unbiased bases. The output entropy for the average
input $\id/2$ is:
\[
S(\cN_p(\id/2)) = S((1-p)\id/2 + \frac{p}{3}\sum_{\sigma} \sigma(\id/2)\sigma)
= S(\id/2) = 1
\]
since the depolarized state remains maximally mixed. The conditional output
entropy for a pure input $\ket{\psi}\bra{\psi}$ is:
\[
S(\cN_p(\ketbra{\psi}{\psi})) = H_2(p) + p\log_2 3
\]
because the output is $(1-p)\ketbra{\psi}{\psi} + p\id/2$ and the
eigenvalues are $\{1-p, p/3, p/3, p/3\}$ in a 4-dimensional space
(but restricted to the qubit subspace, the entropy of a state with
eigenvalues $\{1 - 3p/4, p/4, p/4, p/4\}$ simplifies to the
expression above). By the HSW theorem, taking the difference:
\[
\chi = 1 - [H_2(p) + p\log_2 3] = 1 - H_2(p) - p\log_2 3.
\]
\end{proof}
...
\[
R_{\text{audit}} \leq C_Q(p) = 1 - H_2(p) - p\log_2 3.
\]
For a noiseless channel ($p=0$): $R_{\text{audit}} \leq 1$ bit/qubit
(the maximum possible). For the security threshold ($p=11\%$):
$C_Q(0.11) \approx 0.384$ bits/qubit. Above $p \approx 0.189$,
$C_Q(p) \leq 0$, and the channel cannot reliably transmit audit information.
\end{corollary}
...
\[
Q(p) \geq 1 - H_2(p) - p\log_2 3 - H_2(p) \quad \text{(coherent information lower bound)}.
\]
The exact quantum capacity is not known for all $p$, but it is zero
for $p \geq 1/3$ (when the channel becomes entanglement-breaking).
\end{theorem}
...
\begin{tikzpicture}[node distance=3cm, auto]
  ...
  \node [draw, rectangle, minimum width=3cm, minimum height=1cm,
         below of=S, yshift=-2.5cm, xshift=1cm] (L) {Audit Ledger\\审计账本};
...
\begin{table}[H]
\centering
\caption{Deployment Feasibility by Scenario / 各场景部署可行性}
\label{tab:feasibility}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Scenario / 场景} & \textbf{BB84} & \textbf{Ent.} & \textbf{Chan.} & \textbf{Overall} \\
\midrule
Single data center ($<1$ km) & \checkmark & \checkmark & \checkmark & \textbf{Ready} \\
单个数据中心（$<1$ 公里）& 可行 & 可行 & 可行 & \textbf{就绪} \\
\midrule
Metro area ($<50$ km) & \checkmark & \checkmark & $\sim$ & \textbf{Feasible} \\
城域（$<50$ 公里）& 可行 & 可行 & 可尝试 & \textbf{可行} \\
\midrule
Regional ($<500$ km) & \checkmark & $\times$* & $\times$* & \textbf{Challenging} \\
区域（$<500$ 公里）& 可行 & 需中继 & 需中继 & \textbf{有挑战} \\
\midrule
Global ($>1000$ km) & $\times$ & $\times$ & $\times$ & \textbf{Not Yet} \\
全球（$>1000$ 公里）& 不可行 & 不可行 & 不可行 & \textbf{暂不行} \\
\midrule
\multicolumn{5}{l}{\footnotesize * Without quantum repeaters / 无量子中继器时} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Cost-Benefit of Quantum Audit / 量子审计成本效益分析}
\label{tab:cost}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Component / 组件} & \textbf{Unit Cost} & \textbf{Years to} & \textbf{Security} \\
& \textbf{单位成本 (USD)} & \textbf{Deploy / 部署年限} & \textbf{Gain / 安全增益} \\
\midrule
...
\end{itemize}
\par\smallskip
\noindent\textit{量子审计对手模型：对手拥有无限经典和量子计算能力，可访问量子信道，
但不能违反量子力学定律。}
\end{definition}
...
security: any eavesdropping introduces $Q \geq Q_{\max} \approx 11\%$.
With $\lambda$ photons, the detection probability is $1 - e^{-\Omega(\lambda)}$
...
\begin{table}[H]
\centering
\caption{Classical vs. Quantum Audit Security / 经典与量子审计安全对比}
\label{tab:comparison}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Property / 属性} & \textbf{Classical / 经典} & \textbf{Quantum / 量子} \\
\midrule
...
前向安全性 & 依赖密钥擦除 & 有保证 \\
\midrule
Channel capacity & Shannon: $C = B\log_2(1+\text{SNR})$ & HSW: $C_Q = \chi(\cN)$ \\
信道容量 & 香农定理 & HSW定理 \\
\midrule
Hardware cost & $\sim \$0$ (software) & $\$10\text{K}--\$100\text{K}$ \\
硬件成本 & $\sim \$0$（软件） & $\$1\text{万}--\$10\text{万}$ \\
\midrule
...
\begin{enumerate}[label=\textbf{Phase \arabic*:}, leftmargin=*]
  ...
  \item \textbf{Medium-term (2029--2032):} Regional deployment with
        first-generation quantum repeaters. Full BB84 audit channel
        with decoy states. Quantum-secured multi-party audit
        ($>2$ auditors).

  \item \textbf{Long-term (2032--2035):} Global quantum audit network
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item \textbf{Quantum Audit Channel:} We formally defined the quantum
        ...
        audit information rate is bounded by $C_Q = 1 - H_2(p) -
        p\log_2 3$ for the depolarizing channel model.
\end{enumerate}
...
\begin{center}
\small
\textit{量子力学是现有最强的安全基础。它将审计安全问题从}\\
\textit{``对手能解决这个难题吗？''转变为``对手能违反海森堡不确定性原理吗？''}\\
\textit{后者的答案是一个响亮的"不能"——在量子力学是物理现实的正确描述范围内，}\\
\textit{量子安全审计是可证明不可伪造的。}
\end{center}
...
However, in the full 4-dimensional operator space (considering the
Pauli basis), the output entropy requires tracing over the qubit's
Hilbert space. A more careful analysis gives:
\[
S(\cN_p(\ketbra{\psi}{\psi})) = H_2(p) + p\log_2 3.
\]
This can be verified by computing the eigenvalues of the Choi matrix
of the channel: the Choi state of $\cN_p$ is
\[
J(\cN_p) = (1-p)\bell\bra{\Phi^+} + \frac{p}{4}\id \otimes \id,
\]
whose entropy yields the capacity expression.

\textbf{Step 4: Holevo quantity.} The Holevo quantity is:
\[
...
$\chi(\cN_p^{\otimes n}) = n\chi(\cN_p)$. This was proven by King~\cite{king2003}
using the multiplicativity of the maximum output $p$-norm for the
depolarizing channel. Hence:
\[
C(\cN_p) = \chi(\cN_p) = 1 - H_2(p) - p\log_2 3.
\]

\textbf{Numerical check:}
\begin{itemize}
  \item $p = 0$: $C_Q = 1 - 0 - 0 = 1.000$ (noiseless, 1 bit/qubit).
  \item $p = 0.01$: $C_Q = 1 - 0.0808 - 0.0158 = 0.903$ bits/qubit.
  \item $p = 0.11$: $C_Q = 1 - 0.4999 - 0.1743 = 0.326$ bits/qubit.
  \item $p = 0.189$: $C_Q = 1 - 0.7015 - 0.2994 = 0.000$ (capacity zero).
\end{itemize}
\end{proof}
...
\begin{table}[H]
\centering
\caption{Recommended Security Parameters / 推荐安全参数}
\label{tab:params}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Parameter / 参数} & \textbf{Value / 值} & \textbf{Rationale / 理由} \\
\midrule
