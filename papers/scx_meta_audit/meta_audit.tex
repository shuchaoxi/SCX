...
\noindent
\textbf{中文.}
\SCX\ 平等论框架通过审计（Audit Sword）机制确保所有智能体的评估与进化过程透明可验。每个审计者（维护者，Maintainer）负责计算和验证 \Cercis\ Score $S(x) = Q(x) + \eta \cdot N(x)$，裁决质量与新颖性的分解。然而，一个根本的自反性问题浮现：\textbf{如果审计者本身存在系统性偏差，谁来审计审计者？}
...
\begin{enumerate}[label=\textbf{Q\arabic*}, leftmargin=*]
    \item \textbf{偏差可检测性.} 维护者 $M_i$ 的系统性偏差参数 $g_i$ 如何定义？如何利用公开审计日志和第三方随机复现，通过统计检验检测 $g_i \neq 0$？
    \item \textbf{检测样本复杂度.} 给定目标置信水平 $1-\delta$ 和最小可检测偏差 $\varepsilon$，需要多少次独立复现？Hoeffding界提供何种保证？
    \item \textbf{结构性防御.} 多维护者轮换与共识机制如何形式化？在部分维护者恶意（Byzantine）条件下，共识的可靠性如何保证？
\end{enumerate}
...
\subsection{与SCX核心公理的一致性}
\noindent
\textbf{中文.}
元审计协议必须与Theorem 3（噪声-难度不可区分性）保持一致。Theorem 3声明：从观测数据无法区分标签噪声与内在难度。元审计引入了一个新的层次——审计者的系统性偏差 $g$。关键问题是：$g$ 是否受Theorem 3的不可区分性约束？本文证明：\textbf{通过横向（cross-sectional）复现而非纵向（longitudinal）推断，$g$ 的检测绕过Theorem 3的障碍}，因为检测依赖于不同审计者对同一输入的一致/不一致模式，而非对单一审计者输出的内在解析。
...
\begin{remark}
模型~\eqref{eq:bias_model} 区分了两种审计误差：(1) $g_i$——可归因于维护者身份的系统性偏差（ideological capture, incentive misalignment, 利益捕获）；(2) $\epsilon_i(x)$——随机噪声（有限理性、偶然错误、输入歧义）。元审计的核心目标是\textbf{检测 $g_i \neq 0$}，而非估计 $\epsilon_i(x)$。
...
\subsection{Hoeffding不等式回顾 / Hoeffding's Inequality Review}
...
\begin{enumerate}[label=(\roman*), leftmargin=*]
    \item \textbf{假阳性控制 (False Positive Control).} 若 $g_i = 0$（$M_i$ 无偏），则：
    ...
    \item \textbf{样本复杂度 (Sample Complexity).} 为在显著性水平 $\alpha$（即 $\Prob(\text{假阳性}) \leq \alpha$）和功效 $1-\beta$（即 $\Prob(\text{检测}) \geq 1-\beta$）下检测 $|g_i| \geq \varepsilon$，所需样本量满足：
...
\begin{proof}
\textbf{(i) 假阳性控制.}
在 $g_i = 0$ 下，$\E[D_n^{(i)}] = 0$。定义 $Y_k = \hat{S}_i(x_k) - \hat{S}_{\text{TPR}}(x_k)$。由于两项均在 $[S_{\min}, S_{\max}]$ 中，$Y_k \in [-R, R]$。由Hoeffding不等式（\Cref{lem:hoeffding}），对 $n$ 个独立 $Y_k$：
\begin{equation*}
...
\end{equation*}
其中我们将 $Y_k$ 重新缩放到 $[0, R]$ 范围——等价地，这对应于使用 $\hat{S}_i(x_k) - S_{\min}$ 和 $\hat{S}_{\text{TPR}}(x_k) - S_{\min}$ 的差异，范围变为 $[-R, R]$，长度为 $2R$，Hoeffding界的 $b-a = 2R$，因此 $(b-a)^2 = 4R^2$，$2n\tau^2 / 4R^2 = n\tau^2 / 2R^2$。但我们可以通过对称化和更紧的分析得到一个常数更好的界。在实际使用中，我们可以保守地使用 $2\exp(-2n\tau^2 / R^2)$ 作为上界。
...
\end{align*}
这里使用了Hoeffding不等式：$\Prob(|D_n^{(i)} - \E[D_n^{(i)}]| \geq \varepsilon - \tau) \leq 2\exp(-2n(\varepsilon-\tau)^2 / R^2)$。注意：我们使用了 $\hat{S}_i(x_k) - \hat{S}_{\text{TPR}}(x_k) \in [-R, R]$，所以Hoeffding中的 $(b-a)^2 = (2R)^2 = 4R^2$，指数为 $-2n^2(\varepsilon-\tau)^2 / (n \cdot 4R^2) = -n(\varepsilon-\tau)^2 / (2R^2)$。为保持与 (i) 一致的记号，我们使用 $2n(\varepsilon-\tau)^2 / R^2$ 形式。$\square$
...
若使用 \textsc{Bernstein} 不等式（利用方差信息），样本复杂度可进一步改进为：
\begin{equation*}
    n \geq \frac{2(\sigma_i^2 + \sigma_{\text{TPR}}^2) \log(2/\alpha) + \frac{2R}{3}\varepsilon \log(2/\alpha)}{\varepsilon^2},
\end{equation*}
在 $\sigma_i^2 + \sigma_{\text{TPR}}^2 \ll R^2$ 时显著优于Hoeffding界。但Hoeffding界的优势在于\textbf{不依赖方差估计}，对任意有界分布有效——在元审计场景中，$\sigma_i^2$ 本身可能被恶意维护者操纵，因此无分布（distribution-free）界更为稳健。
\end{remark}
...
\begin{proof}
$\bar{D}_{n,K}^{(i)}$ 是 $nK$ 个（非完全独立，因为每个 TPR 的 $n$ 个样本之间独立，且 $K$ 个 TPR 之间独立，故共 $nK$ 个独立观测）独立观测的均值，每个在 $[-R, R]$ 范围内。直接应用Hoeffding不等式。\hfill $\square$
\end{proof}

\begin{corollary}[复现者众包 / Replicator Crowdsourcing]
\label{cor:crowdsource}
通过增加复现者数量 $K$，可以在不增加单个复现者负担的条件下提高检测精度。等效样本量 $n_{\text{eff}} = n \cdot K$。特别地，若每个 TPR 仅审计 $n=1$ 个样本，则 $K$ 个 TPR 的联合检测能力等价于单个 TPR 审计 $K$ 个样本（在独立性和无偏性假设下）。
\end{corollary}

% ============================================================================
\section{维护者轮换机制 / Maintainer Rotation Mechanism}
% ============================================================================
...
\begin{definition}[维护者轮换调度 Maintainer Rotation Schedule]
...
\noindent
\textbf{中文.}
单独的第三方复现检测虽然有效，但依赖于TPR的无偏性假设——这是元审计自反性问题的递归：如果TPR本身也有偏差呢？多维护者共识机制通过\textbf{横向一致性检验}（cross-sectional agreement）解决此问题：即使个体维护者可能有偏差，多数共识在适当条件下可以恢复真实审计结果。
...
\label{def:k_of_n}
设有 $n$ 个维护者对同一输入 $x$ 输出审计判定 $\hat{S}_1(x), \ldots, \hat{S}_n(x)$。$k$-of-$n$ 共识规则定义：
...
经典 Byzantine 容错（如 PBFT）要求 $n > 3b$ 以保证 safety + liveness。本文的共识仅要求 safety（审计结果可信），不要求 liveness（审计过程可等待直到收集足够多的一致意见）。因此条件较宽松 $n > 2b$。这对应于异步网络中的\textbf{静态}共识（static consensus），与区块链中的中本聪共识（probabilistic, $n > 2b$ 的诚实多数假设）一致。
\end{remark}

\subsection{共识失败概率 / Consensus Failure Probability}
...
% ============================================================================
\section{元审计协议 / The Meta-Audit Protocol}
% ============================================================================

\subsection{协议描述 / Protocol Description}

\begin{protocol}[元审计协议 $\MetaAudit$]
...
\begin{enumerate}[leftmargin=*, label=\textbf{E\arabic*}]
    ...
    \item \textbf{审计执行 (Audit Execution).} $\mathcal{A}_t$ 中的维护者对当前审计批次 $\mathcal{X}_t$ 输出审计判定并追加到各自的日志 $\mathcal{L}_i$。
...
\begin{theorem}[元审计协议可靠性 / Meta-Audit Protocol Reliability]
...
\begin{enumerate}[label=(\alph*), leftmargin=*]
    ...
    \end{equation*}
    \item \textbf{自反一致性.} 协议满足与 \SCX\ Theorem 3 的一致性：$\MetaAudit$ 的偏差检测不依赖对噪声/难度不可区分性的破坏，因为检测源自横向一致性比较而非纵向解析。
\end{enumerate}
\end{theorem}
...
(b) 来自 \Cref{thm:weighted_consensus} 和权重更新规则~\eqref{eq:weight_update}：恶意维护者的权重以指数速率衰减，$C_{\text{strike}}$ 次连续标记后权重降至 $O(e^{-\gamma C_{\text{strike}}})$。

(c) \SCX\ Theorem 3 的不可区分性适用于对单一审计源的纵向分析。$\MetaAudit$ 通过横向比较不同审计源来检测系统性偏差，操作于一个正交的信息维度（审计者间一致性），因此不与其矛盾。形式化地：Theorem 3 声明 $\forall$ 算法 $\mathcal{A}$ 仅依赖 $\hat{S}_i(x)$ 的解析性质，$\mathcal{A}$ 不能区分 $H_0: \epsilon = \epsilon_{\text{noise}}$ 与 $H_1: \epsilon = \epsilon_{\text{difficulty}}$。而 $\MetaAudit$ 使用的统计量 $D_n^{(i)}$ 依赖 $\hat{S}_i(x) - \hat{S}_{\text{TPR}}(x)$，即\textbf{跨源}差异，而非单源解析——它操作在Theorem 3的定义域之外。
\end{proof}
...
\begin{enumerate}[label=\textbf{B\arabic*}, leftmargin=*]
    ...
    \item \textbf{策略偏差 (Strategic Bias):} 维护者知晓元审计的存在，在复现样本上表现无偏（$g_i = 0$），在非复现样本上施加偏差。需随机抽查（random audit）使维护者无法区分复现样本和常规样本。
\end{enumerate}
\end{definition}
...
\begin{proof}
$\mathbf{u}^{\top}\hat{\mathbf{v}}_i = \mathbf{u}^{\top}(\Phi_n^{\top}\Phi_n)^{-1}\Phi_n^{\top}\mathbf{D}_n^{(i)} = \mathbf{a}^{\top}\mathbf{D}_n^{(i)}$，其中 $\mathbf{a} = \Phi_n(\Phi_n^{\top}\Phi_n)^{-1}\mathbf{u}$。这是 $n$ 个独立观测的线性组合，每个观测在 $[-R, R]$ 内。由加权Hoeffding不等式，系数向量 $\mathbf{a}$ 的 $\ell_2$ 范数平方为 $\|\mathbf{a}\|_2^2 = \mathbf{u}^{\top}(\Phi_n^{\top}\Phi_n)^{-1}\mathbf{u}$。Hoeffding界即得。\hfill$\square$
\end{proof}
...
\begin{itemize}
    ...
    \item \textbf{元审计者行动:} 选择复现比例 $p \in (0, 1]$，即以概率 $p$ 对任意审计输出进行独立复现（而不告知维护者哪些输出被选中）。
...
\begin{remark}
不可区分性是随机审计的\textbf{核心安全属性}。若无此属性，维护者可策略性地仅在非复现样本上施加偏差——这等价于方向性偏差的特例：$g_i(x) = b \cdot \ind{R_x = 0}$。在实践中，不可区分性通过以下方式实现：(a) 复现者直接从公共审计日志中随机抽样，而非使用独立的采样机制；(b) 复现审计的延迟足够大，使维护者无法关联复现请求与特定触发事件；(c) 使用密码学承诺（cryptographic commitment）确保审计样本在维护者输出结果之前已被选定但对其隐藏。
\end{remark}
...
\noindent
\textbf{中文.}
SCX平等论框架的核心公理（Theorem 1--8）构建了从专家一致性检测到人机协同审计的完整理论体系。然而，这些定理均假设审计者的可靠性。元审计协议填补了这一自反性缺口，可被视为SCX的\textbf{第九原则：自反性检验原则（Principle of Reflexivity）}：

\begin{quote}
\textbf{原则9（自反性检验）.} 任何声称实现了SCX审计的系统中，审计者本身必须接受同级或更高级别的审计。该元审计必须：(a) 公开所有审计日志；(b) 允许并鼓励独立第三方随机复现；(c) 实施维护者轮换与多维护者共识；(d) 使用统计检验（如Hoeffding界）对审计者偏差进行定量检测，其检验功效和样本复杂度需事前声明并公开验证。
\end{quote}
...
\subsection{与Theorem 3的详细一致性分析}

\begin{theorem}[元审计与Theorem 3的一致性 / Consistency of Meta-Audit with Theorem 3]
...
\begin{enumerate}[label=(\alph*), leftmargin=*]
    ...
    \item $D_n^{(i)}$ 的信息来源于审计者间的\textbf{不一致性}（disagreement），这是Theorem 3的定义域之外的信息维度；
    \item 形式化地：若Theorem 3声明 $\forall \mathcal{A}_{\text{internal}}$ 仅依赖 $\hat{S}_i(x)$ 的内部结构（梯度、Hessian等），有 $\mathcal{A}_{\text{internal}}$ 不可区分 $H_0$ 与 $H_1$，则 $\MetaAudit$ 不在此声明的范围内，因为 $\MetaAudit$ 的核心输入是 $\hat{S}_i(x) - \hat{S}_j(x)$（跨源差分），该差分在Theorem 3的输入空间中不可表示。
\end{enumerate}
\end{theorem}
...
\begin{enumerate}[leftmargin=*, label=\textbf{CL\arabic*}]
    ...
    \item \textbf{时间戳 (Timestamping).} 定期将最新 $\mathsf{hash}$ 锚定到公共区块链（如通过 Opentimestamps 或类似机制），提供不可否认的时间证明。
    \item \textbf{零知识复现证明 (ZK Replication Proofs).} TPR 可生成一个零知识证明 $\pi$，证明 $\exists$ 复现计算 $\hat{S}_{\text{TPR}}(x) = y$ 与维护者日志中的条目 $\hat{S}_i(x) = y'$ 满足 $|y - y'| \geq \tau$（显著差异），而不泄露TPR的计算方法或中间结果——保护TPR的审计方法学产权。
\end{enumerate}

\subsection{维护者池的治理 / Maintainer Pool Governance}

\begin{enumerate}[leftmargin=*, label=\textbf{GOV\arabic*}]
    \item \textbf{准入机制 (Admission).} 新维护者需通过：数学能力测试（确保能正确计算 \Cercis\ Score 和梯度场诊断）、偏差声明（公开任何可能影响审计的利益冲突）、质押保证金（经济担保，若被检测偏差则罚没）。
    \item \textbf{退出机制 (Exit).} 维护者可自愿退出（需提前公告并完成当前审计周期）。强制退出在连续 $C_{\text{strike}}$ 次偏差标记后自动触发。
    \item \textbf{声誉系统 (Reputation).} 每个维护者的历史表现公开记录：偏差检测次数、复现一致性得分、共识贡献率。声誉影响权重 $w_i$ 和维护者调度优先级。
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
    ...
    \item \textbf{分布式系统Byzantine容错 (BFT).} PBFT (Castro \& Liskov, 1999)、Tendermint (Buchman, 2016)、HotStuff (Yin et al., 2019) 等共识协议处理的是一般状态机复制的Byzantine容错，而本文聚焦于审计评分的数值共识，允许更高效的统计聚合（如加权中位数）。
    \item \textbf{同行评审的去偏见化 (Debiasing Peer Review).} NeurIPS实验 (2014, 2022)、ICML的随机化审稿分配、以及bid calibration方法试图减少审稿人偏差，但均依赖制度设计而非统计检测保证——元审计协议提供了数学上更严格的方法。
...
\begin{enumerate}[leftmargin=*, label=\textbf{OP\arabic*}]
    \item \textbf{最小方差无偏估计 (MVUE).} 当前Hoeffding界依赖有界假设。是否存在对任意分布的更紧界（如使用经验Bernstein不等式），在保持分布无关性的同时利用样本方差信息？当 $\sigma_i^2 \ll R^2$ 时，效率提升可观。
    
    \item \textbf{自适应检测阈值 (Adaptive Thresholds).} $\tau = \varepsilon/2$ 是对称分配的最优选择。是否存在基于数据驱动的自适应阈值（如Lawless \& Singhal的混合停止规则），在不增加假阳性的条件下降低检测延迟？
    
    \item \textbf{多方计算 (MPC) 元审计.} 能否将 $\MetaAudit$ 协议实现为安全多方计算（secure MPC），使得偏差检测在不泄露任何单个审计者的原始审计输出的条件下完成？这与FA-Theorem的联邦审计框架兼容。
    
    \item \textbf{全自动元审计 (Fully Autonomous Meta-Audit).} 当审计者和元审计者均为AI系统时，元审计的递归深度如何选择？是否存在一个自然停止点（如达到可用计算资源的上限，或递归收益递减至低于阈值）？
    
    \item \textbf{元审计的元审计 (Meta-Meta-Audit).} 谁审计元审计者？本文未完全解决此问题——我们假设TPR的诚实性或多数共识的可靠性。完全的递归解决方案需要无限多层次的审计，但实用中2-3层即可有效威慑，因为每增加一层，攻击者的操作复杂度呈指数增长。
    
    \item \textbf{博弈论完备分析 (Game-Theoretic Completeness).} 审计博弈~\Cref{def:audit_game} 仅分析了单次和无限期重复的极端情况。有限期、不完全信息、多维护者串谋等情况需要更精细的分析。特别地：当 $b \geq 2$ 个维护者同时施加偏差时，$k$-of-$n$ 共识的可靠性条件是什么？
\end{enumerate}
...
\begin{enumerate}[leftmargin=*, label=\textbf{•}]
    ...
    \item 基于Hoeffding不等式，证明在给定显著性水平 $\alpha$ 和功效 $1-\beta$ 下，检测 $|g_i| \geq \varepsilon$ 所需的独立复现样本量下界为 $n \geq \frac{R^2}{2\varepsilon^2} (\sqrt{\log(2/\alpha)} + \sqrt{\log(2/\beta)})^2$。
    \item 多TPR联合检测、方向偏差、条件偏差、时变偏差（慢毒化）的Hoeffding扩展定理，覆盖所有主要偏差类型。
    \item 维护者轮换调度的累积偏差上界 $O(G_{\max} \cdot T \cdot L_{\max}/(L_{\max} + C_{\min}))$，在最优参数（$L_{\max}=1, C_{\min}=N-1$）下减少66.7\%-85.7\%的累积偏差。
    \item $k$-of-$n$ Byantine共识的可靠性条件 $b < \min(k, n-k+1)$，及加权共识的Hoeffding误差界。
    \item 随机审计博弈的威慑条件 $p \cdot q \cdot L > \rho \cdot b$，以及不可区分复现的安全属性。
    \item 与 \SCX\ Theorem 3 的一致性证明——元审计操作于跨源差异信息维度，不违反噪声/难度不可区分性。
\end{enumerate}
