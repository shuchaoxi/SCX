...
我们构造\textbf{审计群}（Audit Group）$\AuditGrp$——所有保持共识等价类（CEC）不变的专家重排所构成的群，
...
\textbf{关键词：} Galois理论；审计群；不可解性；正规化子；固定域；多专家审计；SCX框架
\end{abstract}
...
SCX框架的核心问题可以简洁地表述为：\textbf{给定一组专家的判断，在什么条件下我们可以确信一个声称被审计了？}
...
然而，上述定理的证明依赖概率和信息论工具，其底层\textbf{结构}——为何有些声称可审计而另一些不可审计，
为何某些专家组合独立而另一些相关——尚未获得纯代数刻画。
本文的主旨就是填补这一空白。

Galois理论是代数中最优美的结构理论之一。
它的核心陈述是：给定一个域扩张$L/K$，其中间域格与$\Gal(L/K)$的子群格之间存在\textbf{反序一一对应}。
这一对应的威力在于：它把一个难以直接分析的连续对象（域扩张）转化为一个可用群论工具处理的离散对象（子群格）。

我们注意到SCX框架中存在一个完全平行的结构：
\begin{itemize}[itemsep=4pt]
    \item \textbf{共识等价类（CEC）} $\longleftrightarrow$ Galois扩张中的\textbf{基域}$K$；
...
\begin{itemize}[itemsep=3pt]
    ...
    \item $\CEC$：共识等价类（Consensus Equivalence Class），定义为所有在专家重排下保持一致的声称构成的等价类。
...
\end{equation}
其中$x \sim_{\CEC} y$表示$x$和$y$属于同一个共识等价类（即所有专家对$x$和$y$的判断在共识度量下一致）。
\end{definition}
...
等价地：\textbf{噪声-困难不可区分性 $\Longleftrightarrow$ 审计群的合成因子中存在非交换单群。}
\end{theorem}
...
\begin{corollary}[老实人定理的群论等价形式]
以下陈述等价：
\begin{enumerate}[label=(\arabic*),itemsep=3pt]
    \item 声称$x$可被审计（老实人定理的条件满足）；
    \item $\Stab(\{x\})$（$x$在$\AuditGrp$中的稳定化子群）是\textbf{可解群}；
...
\begin{remark}
这一等价性意味着$\CEC$不再是一个模糊的共识概念，而是一个\textbf{精确的代数对象}。
...
强人所难定理（Thm~2）的结论是：\textbf{相关专家的存在破坏了独立共识的因式分解结构。}
...
本文的核心贡献是建立了SCX审计框架与Galois理论之间的\textbf{精确范畴等价}。
具体而言：

\begin{enumerate}[label=(\arabic*),itemsep=3pt]
    \item \textbf{审计群} $\AuditGrp$ 被定义为保留共识等价类的专家置换群，它是Galois群的精确类似物。
    \item \textbf{审计Galois对应}（定理~\ref{thm:galois_correspondence}）建立了子群格与可审计子问题格之间的一一反序对应。
    \item \textbf{不可解性定理}（定理~\ref{thm:unsolvability}）证明：$A_5 \leq \AuditGrp$ $\implies$ 存在不可审计的声称，这是老实人定理的群论形式。
    \item \textbf{正规化子独立定理}（定理~\ref{thm:normalizer}）证明互相正规、审计可分、独立共识三者的等价性。
    \item \textbf{固定域定理}（定理~\ref{thm:fixed_domain}）证明$\CEC = \Fix(\AuditGrp)$是极大不变量。
...
\begin{itemize}[itemsep=3pt]
    ...
    \item \textbf{动态演化}的（专家集合随时间增长或衰减）；
...
\begin{enumerate}[label=(\arabic*),itemsep=3pt]
    ...
    \item \textbf{profinite群：}若将有限专家逼近视为profinite完备化，则审计群是profinite群，Galois对应的profinite版本仍然成立。
