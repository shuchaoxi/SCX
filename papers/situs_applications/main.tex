...
Table~\ref{tab:twelve_scenarios} provides the deployment decision for twelve canonical scientific scenarios organized into six domain groups, with genomics designated as the flagship verification field. One scenario (enzyme active sites) is explicitly excluded in \S\ref{sec:excluded} as a failure mode where the framework's preconditions cannot be met. Oceanography is conditionally included (\S\ref{sec:excluded}): feasible in well-sampled regions, infeasible in the deep ocean interior.
...
\midrule
\rowcolor{gray!15}
\textbf{F} & \textbf{Genomics variant auditing} \newline{\scriptsize \textbf{FLAGSHIP — Life Sciences}} & $\uparrow$ & \textbf{Large} & $\star\star$ & \textbf{The strongest verification field for SCX+Situs.} Three sequencing platforms $\times$ four variant callers $\to$ nominal $3 \times 4$ grid, but \textbf{honest $M_{\text{eff}} \in [3,6]$}: platforms $\approx 3$ (distinct measurement physics---Illumina sequencing-by-synthesis, PacBio ZMW, Nanopore ionic current---but all align to the same reference genome), algorithms $\approx 2$ (DeepVariant is architecturally distinct; GATK/FreeBayes/Strelka share Smith-Waterman realignment and BQSR heuristics). Reference genome bias is the common blindfold. Base-pair 1D coordinate = Situs's most precise encoding (single-nucleotide resolution, zero interpolation error). Rare variant vs.\ sequencing error = \textbf{exact instance of Theorem~3}: genuine low-frequency allele and platform-specific noise are indistinguishable from a single caller's output, separable only through multi-expert consensus at fixed $p$. Three additional audit targets: somatic/germline confusion, structural variant breakpoint discordance, pseudogene discrimination (identical $S$, different $P$). \textcolor{warning}{$\blacksquare$ Caveat}: Reference genome bias—all platforms align to the same reference, creating shared blindness in repetitive/GC-rich regions. \\
\midrule
...
\midrule
\textbf{M} & Medical imaging \newline{\scriptsize Medical Imaging} & $\approx 0 / \downarrow$ & Near zero to negative & $\star\star$ & CT/MRI/PET multi-modal at same $(x,y,z)$ voxel coordinates. \textbf{Without sub-voxel registration, $\delta_s^{\mathrm{PE}}$ is near zero at best, negative at worst.} Multi-modal registration inconsistency, tumor segmentation disagreement across modalities, and radiomics feature cross-modal instability as audit targets. \textcolor{warning}{$\blacksquare$ Honest assessment}: Registration error is not a caveat---it is the dominant term. Without sub-voxel deformable registration with propagated uncertainty, Situs encodes physically mismatched positions as the same $p$, misattributing registration error to label noise. $\delta_s^{\mathrm{PE}}$ turns negative because position disagreement is systematically confounded with modality disagreement. The Small--Med gain previously projected is aspirational, not physically grounded. \\
\bottomrule
\end{tabular}
\end{table}
...
\textcolor{warning}{$\blacksquare$ Honest assessment}: OPC engines \textbf{share fundamental optical models}—Hopkins imaging theory, thin-mask (Kirchhoff) approximation, and resist chemistry parameterizations (Dill ABC model). All three engines can agree on an incorrect contour because they inherit the same thin-mask approximation error at a given feature. This is shared-model redundancy, not multi-expert independence. \textbf{Effective $M_{\text{eff}} \approx 1.2$}: three nominal engines collapse to near-one effective expert because the shared Hopkins + thin-mask + Dill backbone dominates all outputs; the ``disagreement'' is discretization noise, not genuinely independent physical measurement. The $<1$\,nm encoding precision, while enabling the finest-grained Situs deployment, makes the system maximally sensitive to wafer-grid alignment error—a 0.5\,nm stage misalignment turns Situs from signal into noise. \textbf{Net assessment}: The economic value proposition (CD-SEM cost reduction) is real, but the expert-independence premise underpinning the Chernoff bound is structurally violated. Deploy with calibrated skepticism.
...
A framework earns credibility not by claiming universal applicability but by clearly stating where it \emph{cannot} work. We document one domain where Situs's preconditions fail structurally, one domain where they are conditionally satisfied, and one class of applications whose real-time constraints place them outside SCX's architectural scope.
...
\textbf{Oceanographic multi-platform auditing (conditional).} \textcolor{projected}{$\blacksquare$ Conditionally included — physically reasoned, not experimentally verified.} Argo floats (in-situ T/S profiles), satellite altimetry (sea surface height), satellite salinity (SMOS/Aquarius), and ocean reanalysis products (GLORYS12, ECCO, HYCOM) observe the same ocean through fundamentally different physical channels, creating a natural multi-expert matrix at 4D spatiotemporal coordinates $(lat,lon,depth,t)$. \textbf{Feasible regions}: The North Atlantic ($>20$ Argo profiles per $1^\circ \times 1^\circ$ per decade) and North Pacific have sufficient in-situ density for meaningful multi-platform comparison; when augmented with satellite altimetry and reanalysis as additional ``experts,'' the effective $M_{\text{eff}}$ reaches 4--5. \textbf{Infeasible regions}: The deep ocean interior ($>2000$\,m), Southern Ocean, and most of the Indian Ocean remain below five Argo profiles per $1^\circ \times 1^\circ$ per decade---insufficient for Theorem~1's multi-expert consistency analysis. Satellite-only experts (altimetry, salinity) provide surface constraints but cannot audit the interior. Reanalysis products add expert diversity but introduce model-structure correlation (many share NEMO/MOM6 ocean cores)---the same genealogy problem that plagues CMIP6.
