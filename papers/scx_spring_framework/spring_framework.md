...
>     \exp\left(-2 M_t^{eff} \cdot \frac{\Delta^2}{\bar{\sigma}^2}\right)
> $$
> 
...
> 在物理层专家预测的校准方差为$\bar{\sigma}^2$、声称偏差为$\delta = |claimed(v) - true(v)|$的条件下，
> 跨模态审计检测$\delta > \Delta$的概率满足：
> 
> $$<!-- label: eq:CM1_bound -->
>     \Pbb(detect \mid \delta > \Delta) \geq 1 - \exp\left(-2 M_t^{eff} \cdot \frac{(\delta - \Delta)^2}{\bar{\sigma}^2}\right)
> $$

> **Proof:** 跨模态审计等价于对物理量$v$的$M_t$个独立测量的假设检验：
> $H_0: v = v_{claimed}$ vs $H_1: v \neq v_{claimed}$。
> 在$M_t$个独立专家的条件下，每个专家提供$v$的一个独立估计$\hat{v}_m$。
> 检验统计量$T = \frac{1}{M_t}\sum_m (\hat{v}_m - v_{claimed})^2 / \bar{\sigma}^2$。
> 在$H_0$下，$T \sim \chi^2_{M_t^{eff}}$（经相关性校正）。
> 在$H_1$下（$\delta > \Delta$），$T$的非中心参数$\lambda = M_t^{eff} \cdot \delta^2 / \bar{\sigma}^2$。
