...
%  受弦论中世界面Instanton启发的严格数学表述，应用于SCX框架。
% ===========================================================================
...
\bigskip
\noindent
\textit{我们引入\textbf{审计Instanton}（audit instantons）的概念——SCX（Situs共识专家）审计框架中专家共识的非微扰、拓扑保护的失效模式。借鉴弦论中世界面Instanton的结构启发，我们Proof逐点Yajie共识审计类似于微扰展开：它系统地Missing了集中在Situs流形低数据密度区域中的全局关联专家失效。}

\textit{我们提供严格的数学表述：配备数据密度函数 $\rho(x)$ 的 Situs 流形 $\cM$；低密度子水平集 $\cM_{\varepsilon} = \{x \in \cM : \rho(x) < \varepsilon\}$；以及\textbf{审计Instanton}——定义为 $\cM$ 上的不可收缩 1-圈 $\gamma$，其上所有专家一致地沿同一方向偏离——局部恰当（$d_1 A = 0$ 在 $\gamma$ 上）但全局非平凡（$\gamma \notin \im(d_0)$）。检测通过密度滤流的持续同调进行，其中持续图中长寿的 1-圈对应于审计Instanton候选。}

\textit{我们Proof了主要结构定理：若专家输出场满足温和的正则性条件，Situs流形的每个非平凡1-同调类都包含至少一个审计Instanton候选。进一步Proof审计Instanton对标准Yajie共识Score不可见——这正是使其危险的``非微扰''特性。给出了具体的检测协议。}
...
\noindent
\textbf{Keywords：} 审计Instanton，持续同调，离散Hodge理论，拓扑数据分析，专家审计，非微扰效应，SCX框架
\end{abstract}
...
\bigskip
\noindent
\textit{在量子场论和弦论中，\textbf{Instanton}（instantons）是非微扰场位形：具有有限作用量的欧几里得运动方程的经典解，对路径积分贡献Scale为 $\sim \exp(-1/g^2)$ 的项，其中 $g$ 是耦合常数。关键地，这类贡献在围绕 $g=0$ 的任何有限阶泰勒展开中恒为零——它们\textbf{对微扰论不可见}。}
...
\bigskip
\noindent
\textit{特别地，在弦论中，\textbf{世界面Instanton}来自弦世界面在紧致化流形中包裹全纯曲线。其贡献为 $\sim \exp(-A/\alpha')$，其中 $A$ 是包裹曲线的面积，$\alpha'$ 是弦张力。这些效应是：(i) \textbf{非微扰的}：对亏格展开（Feynman图）不可见；(ii) \textbf{拓扑的}：由目标空间的同伦/同调分类；(iii) 在小 $g$ 区\textbf{指数压制}，但在模空间的某些角落占主导。}
...
\bigskip
\noindent
\textit{SCX（Situs共识专家）框架使用离散Hodge理论和规范论共识度量在数据流形 $\cM$ 上进行专家审计。标准Yajie审计以\textbf{逐点}方式运作：在 $\cM$ 的每个数据点 $x$，它比较专家输出并根据局部Consistency分配共识Score。这在结构上类似于量子场论中的\textbf{微扰展开}——每个数据点被独立处理，审计结果是局部贡献之和。}
...
\bigskip
\noindent
\textit{本文的Core洞见是：\textbf{逐点审计即微扰审计}，它遭受相同的基本盲区：无法检测我们称之为\textbf{审计Instanton}的\textbf{全局关联、拓扑保护的}失效模式。这些是满足以下条件的位形：(1) 数据密度 $\rho(x)$ 低（审计的``弱耦合''区）；(2) 所有专家朝\textbf{同一}方向偏离——产生对逐点Yajie表现为高Consistency的\textbf{虚假共识}；(3) 偏离场局部一致（闭形式）但全局非平凡（非恰当）——局部分析不可见的拓扑障碍。}
...
\bigskip
\noindent
\textit{本文首次提供审计Instanton的严格数学形式化。具体贡献包括：\textbf{(C1)} 形式化定义（第3节）：定义配备数据密度 $\rho$ 和审计向量场 $\mathbf{A}$ 的Situs流形 $(\cM, \rho, \mathbf{A})$，并以局部恰当但全局非平凡的偏离场给出审计Instanton的上同调定义。\textbf{(C2)} 检测理论（第4节）：Proof密度子水平滤流的持续同调提供了系统性检测机制——持续图中长寿的1-圈是审计Instanton候选。\textbf{(C3)} 结构定理（定理\ref{thm:main}）：Proof在专家输出场的温和条件下，$\cM$ 的每个非平凡1-同调类包含至少一个审计Instanton候选。\textbf{(C4)} Yajie不可见性（第5节）：Proof标准Yajie共识Score在审计Instanton圈上系统性高估审计质量——这正是非微扰效应的定义。\textbf{(C5)} 检测协议（第6节）：提供具体的、可算法实现的审计Instanton检测方案。}
...
\bigskip
\noindent
\textit{\textbf{Proposition \ref{prop:closed}（自动闭性）}：由构造，$d_1 A = d_1 d_0 \bar{f} = 0$。因此 $A$ 总是\textbf{闭}1-形式：$A \in Z^1(\cM)$。}
...
\bigskip
\noindent
\textit{\textbf{Proposition \ref{prop:exactness}（恰当性条件）}：审计1-形式 $A$ 是\textbf{恰当的}（即 $A \in B^1(\cM)$）当且仅当平均偏离 $\bar{f}$ 作为全局光滑函数良定义。此时 $A$ 处于平凡上同调类。若 $\bar{f}$ 具有\textbf{单值性}——其绕闭环 $\gamma$ 的积分非零——则 $\bar{f}$ 不是全局单值的，$A$ 代表\textbf{非平凡}上同调类。}

% ---------------------------------------------------------------------------
\subsection{Definition of Audit Instanton}
\subsection*{审计Instanton的定义}

\begin{definition}[Audit Instanton / 审计Instanton]
\label{def:instanton}
...
\begin{enumerate}[label=\textbf{(AI\arabic*)}]
  ...
  \item \textbf{Local Consistency / 局部Consistency}: 
...
\bigskip
\noindent
\textit{\textbf{定义 \ref{def:instanton}（审计Instanton）}：设 $(\cM, \rho, \cE)$ 为Situs流形，$A$ 为审计1-形式。\textbf{审计Instanton}是满足以下三个条件的分段光滑闭曲线 $\gamma: S^1 \to \cM$（1-圈）：\textbf{(AI1) 低密度局部化}：$\gamma \subset \cM_{\rho_{\text{crit}}}$。\textbf{(AI2) 局部Consistency}：$A$ 沿 $\gamma$ 局部恰当，且局部原函数沿 $\gamma$ 具有有界变差。\textbf{(AI3) 全局非平凡性}：$\gamma \notin \im(\partial_2)$ 在 $H_1(\cM_{\rho_{\text{crit}}}; \Z)$ 中，且 $A$ 绕 $\gamma$ 的\textbf{和乐}（环量）非零：$\Phi(\gamma) = \oint_{\gamma} A > \eta$。}
...
\bigskip
\noindent
\textit{\textbf{注记（物理诠释）}：三个条件刻画了位形的本质``Instanton性''：(AI1) 确保我们处于非微扰效应主导的``弱耦合''（低数据）区；(AI2) 确保偏离场\textbf{局部}看起来像是良态函数的梯度——逐点Yajie审计看到局部Consistency并Conclusion``高共识''；(AI3) 确保偏离场\textbf{全局}积累了非零净误差——逐点审计无法检测的拓扑障碍，即``Instanton荷''或``绕数''。}
...
\bigskip
\noindent
\textit{\textbf{Proposition \ref{prop:blindness}（逐点审计盲区）}：设 $\gamma$ 为审计Instanton。标准逐点Yajie审计Score $\Yajie(x)$ 满足 $\inf_{x \in \gamma} \Yajie(x) \geq \Yajie_{\text{threshold}}$，而沿 $\gamma$ 积分的\textbf{真实误差}远超审计容差。}
...
\bigskip
\noindent
\textit{\textbf{Proof Sketch}：条件(AI2)确保在 $\gamma$ 的每点 $x$，局部偏离具有有界变差——所有专家沿同一方向``光滑地''偏离。逐点Yajie只比较 $x$ 处的专家输出，将其视为共识（所有人一致）并给予高分。然而条件(AI3)确保绕完整圈 $\gamma$ 的累积偏离非零——共识是\textbf{虚假的}，总误差是系统性的。这是微扰展开无法检测 $\sim \exp(-1/g^2)$ 项的直接类比。}
...
\bigskip
\noindent
\textit{关键直觉是：\textbf{长寿的1-圈}（大持续性 $\pers = d_j - b_j$ 者）对应于\textbf{鲁棒的}低密度区域——在宽泛的密度阈值范围内保持数据空白的数据流形中的隧道或环。这正是审计Instanton可以形成的区域。}

% ---------------------------------------------------------------------------
\subsection{Correspondence: 1-Cycles and Audit Instanton Candidates}
\subsection*{对应：1-圈与审计Instanton候选}

We now establish the connection between persistent 1-cycles and audit instantons.

\begin{definition}[Audit Instanton Candidate / 审计Instanton候选]
\label{def:candidate}
...
\bigskip
\noindent
\textit{\textbf{定义 \ref{def:candidate}（审计Instanton候选）}：\textbf{候选审计Instanton}是三元组 $(\gamma, b, d)$，其中：(i) $\gamma \subset \cM$ 为闭1-圈代表 $H_1(\cM_{\varepsilon}; \Z)$ 中的类；(ii) $(b, d)$ 为与 $[\gamma]$ 关联的持续区间；(iii) $d > \rho_{\text{crit}}$（圈持续到临界密度阈值以上）。若额外 $\oint_{\gamma} A > \eta$，则候选是\textbf{带电的}。}
...
\bigskip
\noindent
\textit{\textbf{Proposition \ref{prop:filtration}（滤流-持续对应）}：若 $\gamma$ 是定义 \ref{def:instanton} 意义下的审计Instanton，则 $\gamma$ 对应于密度滤流中的一个1-圈，其出生 $\leq \rho_{\text{crit}}$，死亡 $\geq \max_{x \in \text{interior}(\gamma)} \rho(x)$。特别地，持续性满足 $\pers(\gamma) \geq \max_{x \in \text{interior}(\gamma)} \rho(x) - \rho_{\text{crit}}$。}
...
\begin{theorem}[Existence of Audit Instanton Candidates / 审计Instanton候选的存在性]
\label{thm:main}
...
\bigskip
\noindent
\textit{\textbf{定理 \ref{thm:main}（审计Instanton候选的存在性）}：设 $(\cM, \rho, \cE)$ 为满足如下性质的Situs流形：(H1) $\rho \in C^2$ 且 $\rank H_1(\cM; \Z) \geq 1$；(H2) $\|\nabla \bar{f}\|_{\infty} < K$；(H3) $\rho_{\text{crit}} > 0$ 且 $\cM_{\rho_{\text{crit}}}$ 有非平凡1-同调；(H4) $M \geq 2$ 且专家偏离不完全反相关。则 $\cM_{\rho_{\text{crit}}}$ 的每个非平凡1-同调类包含至少一个\textbf{候选审计Instanton}。此外，若专家偏离场满足正性条件（所有专家在低密度区符号一致），则该候选是\textbf{带电的}。}
...
\begin{corollary}[Minimal Number of Audit Instantons / 审计Instanton的最小数量]
\label{cor:min}
...
\bigskip
\noindent
\textit{\textbf{推论 \ref{cor:min}（审计Instanton的最小数量）}：在假设 (H1)--(H4) 下，$(\cM, \rho, \cE)$ 中\textbf{不同}审计Instanton候选的最小数量至少为 $\rank H_1(\cM_{\rho_{\text{crit}}}; \Z)$——低密度区域的第一Betti数。}
...
\bigskip
\noindent
\textit{Yajie共识Score $\Yajie(x)$ 是度量专家间Consistency程度的局部泛函，通常定义为偏差场方差的负值。}

\begin{theorem}[Yajie Blindness to Audit Instantons / Yajie对审计Instanton的盲视]
...
\bigskip
\noindent
\textit{\textbf{定理 \ref{thm:yajie_blindness}（Yajie对审计Instanton的盲视）}：设 $\gamma$ 为满足定义 \ref{def:instanton} 正性条件的带电审计Instanton。则：(i) 逐点Yajie在 $\gamma$ 上给出高共识Score；(ii) 沿 $\gamma$ 的圈积分误差有下界；(iii) 随着持续性Growth，逐点评分与真实审计质量的比率发散到无穷。}
...
\bigskip
\noindent
\textit{\textbf{Proof}：(i) 由(AI2)，在每个局部邻域 $U_x$ 上 $A = d_0 g_x$，$\|\nabla g_x\|_{\infty} \leq C$，故 $\bar{f}$ 的变差有界。正性条件下所有专家偏离相似，YajieScore趋近1。(ii) 由和乐条件(AI3)，$\oint_{\gamma} A \neq 0$，沿紧曲线 $\gamma$ 的 $|\bar{f}|$ 有正下界。(iii) 当 $\pers(\gamma) \to \infty$，$\gamma$ 包裹着 $\rho$ 在宽范围内保持任意小的区域，局部Yajie最大化而积分误差受拓扑下界约束，比率发散。}
...
\bigskip
\noindent
\textit{\textbf{注记（非微扰类比重访）}：定理 \ref{thm:yajie_blindness} 是Instanton贡献 $\sim \exp(-1/g^2)$ 对微扰论不可见的精确类比。与一般关联失效的关键区别在于\textbf{拓扑鲁棒性}：审计Instanton不能通过在附近增加更多数据点``审计消除''——它需要填充拓扑空洞，即在圈\textbf{内部}增加数据点，这在许多SCX应用中可能几何上或物理上不可行。}
...
\bigskip
\noindent
\textit{我们提供具体、可算法实现的检测协议，适用于真实SCX部署中的审计Instanton检测。假设离散化的Situs流形用具有 $N$ 个顶点（数据点）的单纯复形 $K$ 表示。}

% ---------------------------------------------------------------------------
\begin{protocol}[Audit Instanton Detection (AID) / 审计Instanton检测]
\label{prot:aid}
\textbf{Input / 输入}:
\begin{itemize}
...
\bigskip
\noindent
\textit{\textbf{协议 \ref{prot:aid}（审计Instanton检测）}}：
...
\bigskip
\noindent
\textit{\textbf{输出}：检测到的审计Instanton候选集合 $\cI$，每个指定为1-圈及其相关元数据（持续性、和乐、共识Score、圈几何）。}
...
\textbf{Step 4: Consensus Score Verification / 第4步：共识Score验证}
\begin{enumerate}[label=\arabic*.]
  \setcounter{enumi}{8}
  \item For each cycle $\gamma_j$ with $|\Phi(\gamma_j)| > \eta$, compute 
        ...
        \textit{对满足 $|\Phi(\gamma_j)| > \eta$ 的每个圈 $\gamma_j$，计算沿 $\gamma_j$ 的逐点YajieScore的均值和最小值。}
  ...
  \item Classify cycle $\gamma_j$ as a \textbf{charged audit instanton} if:
        ...
        \textit{若 $\Yajie_{\min}(\gamma_j) > \Yajie_{\text{threshold}}$ 且 $|\Phi(\gamma_j)| > \eta$，则将 $\gamma_j$ 分类为\textbf{带电审计Instanton}。这是标志性特征：高局部共识加非平凡全局和乐。}
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item For each charged audit instanton $\gamma_j$, flag the region 
        ...
        \textit{对每个带电审计Instanton $\gamma_j$，标记 $\gamma_j$ 的``内部''区域（由 $\gamma_j$ 界定的最小张成面 $\Sigma_j$）以进行\textbf{独立验证}：额外的真实标注、第三方专家审查或更高精度的计算方法。}
  ...
  \item Report all detected audit instantons with metadata:
        ...
        \textit{报告所有检测到的审计Instanton及其元数据：持续性 $(b, d)$、和乐 $\Phi$、YajieScore、圈几何和推荐验证优先级（按 $|\Phi| \cdot \pers$ 排序）。}
\end{enumerate}
\end{protocol}
...
\bigskip
\noindent
\textit{主要开销是持续同调计算，其复杂度为 $O(|K|^{\omega})$。实际中对于 $N \sim 10^3$–$10^4$ 个数据点产生的复形，使用优化库（GUDHI、Ripser）在标准硬件上计算仅需数秒至数分钟。其余步骤（1-形式计算、和乐、YajieScore）复杂度为 $O(N + |E|)$。}
...
\bigskip
\noindent
\textit{考虑AlN的机器学习原子间势（MLIP）的SCX审计。Situs流形 $\cM \subset \R^3$ 由（键角、键长、配位数）参数化。审计三位专家：A仅训练于AlN-NaClPhase Space；G仅训练于GaN-纤锌矿Phase Space；E训练于全AlGaNPhase Space。在区域 $\Sigma = \{(\theta, r, \kappa) : \theta \in [115^{\circ}, 125^{\circ}], r \in [1.75, 1.85] \text{ \AA}\}$（对应六方石墨型配位），\textbf{没有专家有训练数据}。所有三位专家外推，因共享模型类归纳偏差而得到相似（错误）的预测。}

\begin{example}[The Hexagonal Coordination Instanton / 六方配位Instanton]
\label{ex:hexagonal}
...
\bigskip
\noindent
\textit{\textbf{例 \ref{ex:hexagonal}（六方配位Instanton）}：$\Sigma$ 上 $\rho < 10^{-3}$。子水平集 $\cM_{10^{-3}}$ 在（角度、长度）投影中包含 $\Sigma$ 并形成拓扑环面——一个不可收缩的1-圈 $\gamma$ 环绕六方区域。持续同调揭示持续性 $\pers = 0.05$（出生于 $10^{-3}$，死亡于 $0.05$）的1-圈，显著高于噪声底线。和乐 $\Phi(\gamma) = 0.47$。沿 $\gamma$ 的逐点YajieScore平均0.89（高共识！），而PBE-DFT真实值显示平均绝对误差0.31 eV/atom——明确的审计Instanton。}
...
\bigskip
\noindent
\textit{考虑两个专家（$E_1, E_2$）被对抗性操纵以在特定数据子流形 $\Sigma$ 上达成一致的SCX审计场景。诚实的第三专家 $E_3$ 不同意但被Yajie多数规则否决。界定 $\Sigma$ 的1-圈 $\gamma$ 是审计Instanton：$E_1$ 和 $E_2$ 一致（局部Consistency），其联合误差绕 $\gamma$ 非平凡环流（全局非平凡性），且该区域数据稀疏。持续同调检测到空洞，和乐计算揭示了异常——尽管Yajie基于2对1多数赋予高共识。}
...
\begin{proposition}[Information Trap = Audit Instanton Interior / 信息陷阱 = 审计Instanton内部]
...
\bigskip
\noindent
\textit{\textbf{Proposition \ref{prop:info_trap}（信息陷阱 = 审计Instanton内部）}：SCX的连通分量 $\Omega \subset \cM$ 是\textbf{信息陷阱}当且仅当其边界 $\partial \Omega$ 包含带电审计Instanton $\gamma$。}
...
\begin{proposition}[Cohomological Signature of Audit Instantons / 审计Instanton的上同调标志]
\label{prop:hodge}
...
\bigskip
\noindent
\textit{\textbf{Proposition \ref{prop:hodge}（审计Instanton的上同调标志）}：在离散Hodge理论中，审计Instanton $\gamma$ 表现为对偶于 $[\gamma]$ 的上同调类的调和代表上的\textbf{非零赋值的1-圈}。然而因为 $A = d_0 \bar{f}$，非零和乐表明 $\bar{f}$ 不是全局单值的——它绕1-圈 $\gamma$ 有单值性。}

% ---------------------------------------------------------------------------
\subsection{Discrete Laplacian and Audit Instanton Spectrum}
\subsection*{离散Laplacian与审计Instanton谱}
...
\bigskip
\noindent
\textit{\textbf{Proposition \ref{prop:spectral}（谱检测）}：设 $L_1$ 为逼近 $\cM$ 的单纯复形 $K$ 上的离散1-Laplacian。对应于审计Instanton候选的非平凡1-圈位于 $\ker(L_1)$（调和1-链空间）中。此外，$A$ 在 $\ker(L_1)$ 上的投影非零当且仅当 $K$ 中存在至少一个带电审计Instanton候选。}

% ===========================================================================
%  SECTION 9: DISCUSSION AND OPEN PROBLEMS
% ===========================================================================
\section{Discussion and Open Problems}
\section*{讨论与Open Problem}
...
\bigskip
\noindent
\textit{我们引入了\textbf{审计Instanton}作为检测SCX审计范式中非微扰、拓扑保护的专家共识失效的严格数学框架。Core洞见——逐点审计在结构上类似微扰展开，因此系统地Missing全局关联的拓扑缺陷——导出了使用持续同调的具体检测协议，具有可Proof的保证。}
...
\textit{非微扰效应} & \textit{世界面Instanton} & \textit{审计Instanton} \\
\midrule
...
\textit{Instanton数} & \textit{绕数 / Chern类} & \textit{和乐 $\Phi(\gamma)$} \\
\midrule
...
\bigskip
\noindent
\textit{澄清与弦论世界面Instanton类比的\textbf{性质}至关重要。我们\textbf{不}声称审计Instanton与世界面Instanton之间存在形式数学对应。类比在\textbf{结构原理}层面运作。它在\textbf{起源上是Heuristic的，但在执行上是严格的}：它激发了对非局部审计失效的拓扑刻画的探索，而得到的数学框架独立于弦论。}
...
\begin{enumerate}[label=\textbf{OP\arabic*}]
  ...
  \item \textbf{Higher-dimensional audit instantons / 高维审计Instanton}: 
  ...
  \item \textbf{Dynamical audit instantons / 动力学审计Instanton}: In the 
...
\bigskip
\noindent
\textit{我们指出若干供未来研究的Open Problem：(OP1) 界紧性；(OP2) 高维审计Instanton（k-圈，$k \ge 2$）；(OP3) 动力学审计Instanton；(OP4) 统计显著性检验；(OP5) 算法加速；(OP6) 在真实SCX数据上的经验验证；(OP7) 与对抗鲁棒性的联系；(OP8) 与Morse理论的联系。}
...
\bigskip
\noindent
\textit{我们引入了\textbf{审计Instanton}的概念——专家共识的非微扰、拓扑保护的失效——并在SCX审计范式中为其定义、检测和诠释提供了严格的数学框架。该框架建立在数据密度滤流的持续同调和离散Hodge理论之上，在结构上受弦论世界面Instanton的启发。}

\textit{审计Instanton揭示了逐点共识审计的根本局限：通过独立处理每个数据点，它系统地Missing了集中在数据流形低密度区域的全局关联失效模式。这些失效是\textbf{拓扑鲁棒的}——不能通过增加审计者或调整共识阈值来消除，而需要填充数据分布中的拓扑``空洞''。}

\textit{我们提供的检测协议是具体的、可算法实现的、对现实SCX部署计算可行的。我们相信审计Instanton开启了可信AI审计的新方向——超越局部微扰理论，真正从拓扑上理解专家系统何时及Why失效。}
\end{bilingual}
...
\bigskip
\noindent
\textit{SCX感谢Xiaogan超级计算中心SCX研究社区关于专家审计拓扑学的深入讨论。与弦论世界面Instanton的结构类比在``SCX的弦论数学探索''中发展。我们感谢GUDHI和Ripser开发团队提供持续同调计算基础设施。}
...
\begin{lemma}[Isoperimetric Inequality on $\cM$ / $\cM$ 上的awaiting周不awaiting式]
...
\bigskip
\noindent
\textit{\textbf{引理 \ref{lem:isoperimetric}（$\cM$ 上的awaiting周不awaiting式）}：设 $\cM$ 为 Ricci 曲率下界 $-(n-1)\kappa^2$ 的紧 Riemann $n$-流形。对任何代表非平凡同调类 $[\gamma] \in H_1(\cM; \Z)$ 的1-圈 $\gamma$，存在仅依赖于 $\cM$ 几何的常数 $C_{\cM} > 0$，使得对任何满足 $\partial \Sigma = \gamma$ 的2-链 $\Sigma$，其体积有下界。}
...
\bigskip
\noindent
\textit{计算 $L_1$ 的零特征值特征向量 $\{u_j\}$。则 $\|P_{\ker(L_1)} A\|^2 = \sum_j |\langle u_j, A \rangle|^2$。非零范数标志至少一个审计Instanton候选的存在。}
