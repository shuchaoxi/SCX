...
我们证明：在温和条件下，Spring训练循环满足$\Lyap_t \to 0$（$t \to \infty$）的收敛保证（定理P3/猜想）；
自动生成的$M_t$是数据复杂度$\Sigma_0(\D)$的单调函数，实现了参数-数据的结构绑定（命题）；
Yajie{}共识在多专家框架下的噪声检测概率随$M_t$呈指数增长（定理P1/P2）。
...
> **Heuristic:** [收敛速率估计]<!-- label: heur:convergence_rate -->
> 在条件(C1)-(C4)下，Spring训练的收敛速率经验估计为：
> 
> $$<!-- label: eq:convergence_rate -->
>     \Lyap_t \leq \Lyap_0 \cdot \exp(-\lambda t) + \mathcal{O}\left(\frac{1}{t}
ight)
> $$
> 
> **注意：此公式为启发式估计，非严格证明。** 指数衰减项 $\exp(-\lambda t)$ 在温和条件下是合理的猜想，但当前缺乏完整的 Lyapunov 漂移证明来严格确立指数收敛率。
> 
