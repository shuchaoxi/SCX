...
\item \textbf{"审计保障"威慑的无需行使性：} 威慑由\textit{能力}维持，而非由\textit{行动}维持。在均衡路径上，可能\textit{永不}发生实际审计——所有实体均选择 $P$ 并维持充分的数据质量标准，使得审计成为不必要的。但此均衡恰由审计能力的存在所支撑。形式化地：存在均衡 $s^* = (P, \ldots, P)$，其维持不依赖任何实体实际执行审计；偏离至 $\neg P$ 将面临 $\mathcal{V}(\mathcal{D}_i, s^*) > 0$ 的预期审计概率，使偏离无利可图。
\end{enumerate}
\end{theorem}
...
\item \textbf{审计概率的单调性：} $q: [0,1] \to (0, 1]$ 是连续、严格递增函数，$q(0) > 0$（即便仅有一个采纳者，其也可能审计竞争对手）。\hfill\textit{经济学含义：采纳者越多，非采纳者被审计的概率越大——审计保障的集体行动维度。}
...
\item \textbf{混同均衡的威慑效率：} 在 $\mu^* = 1$（完全采纳）的均衡中，所有组织采纳，但威慑由非均衡路径的偏离维持——即"审计保障不须挥舞"的博弈论基础。
