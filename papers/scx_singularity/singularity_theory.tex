...
\newcommand{\claimGR}[1]{\textcolor{orange}{\textbf{[GR类比 — 非推导结果]}} #1}
\newcommand{\claimRigorous}[1]{\textcolor{teal}{\textbf{[独立于GR的可证明结果]}} #1}
...
\color{red}
\centering
\textbf{\Large 诚实声明 — Honesty Disclaimer (R3)}\\[6pt]
{\normalsize
本文对广义相对论(GR)的引用是\textbf{结构性类比(structural analogy)}，\\
而非从SCX公理出发的严格推导。\\[4pt]
\textbf{以下声明适用于全文所有声称与GR\"对应\"的定理：}\\[4pt]
\begin{enumerate}[leftmargin=*,label=(\arabic*)]
    \item $h_{ij} = \partial_i\partial_j\mathcal{S}$ 是Hessian——\textbf{不是}Lorentz度规。无零锥，无因果结构。
    \item 审计视界$\delta_{\mathrm{crit}}$的推导是Newton逃逸速度类比——非测地线方程的严格解。
    \item Raychaudhuri方程、Hawking辐射、Bogoliubov变换均\textbf{假定}了类Schwarzschild度规的存在，该度规\textbf{并非}从$h_{ij}$导出。
    \item 本文真正的五条\textbf{独立于GR}的洞见见下框。
\end{enumerate}
\textbf{读者应当：}将GR部分读作启发性比喻(heuristic metaphor)，\\
...
\begin{tcolorbox}[
    ...
    title=\textbf{五条可挽回的洞见 (5 Salvageable Insights — 独立于GR框架)}]
\begin{enumerate}[leftmargin=*,label=\textbf{S\arabic*}]
    \item \textbf{势能面Hessian分析:} 负定区域=特权集中区域。这是$h_{ij} = \partial_i\partial_j\mathcal{S}$直接给出的良定义几何概念，不依赖GR。
    \item \textbf{规范曲率作为态度扭曲:} $F_{ij} \neq 0 \implies$ 不同观察者无法就``零方向''达成一致。这是从SCX公理直接可证的。
    \item \textbf{Fisher信息衰减:} $\mathbf{\Sigma}_N = \mathbf{\Sigma}_0(\mathbf{I} + N\mathbf{A})^{-1}$ 是标准贝叶斯更新——不需要任何GR概念。
    \item \textbf{临界慢化预警:} $\tau_{\mathrm{relax}} \propto |T-T_c|^{-\zeta}$ 是复杂系统已知现象——独立于SCX也成立。
    \item \textbf{不稳定性诊断框架:} 条件(G1-G4)（去掉循环的G5）构成有用的系统不稳定性诊断标准。
\end{enumerate}
\end{tcolorbox}
