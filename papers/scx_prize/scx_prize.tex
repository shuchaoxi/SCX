...
\hypersetup{
    ...
    citecolor=blue,
    urlcolor=blue,
    pdftitle={SCX Prize Declaration——审计替代诺贝尔},
    pdfauthor={SCX},
...
% ================================================================
\title{
    \textbf{\LARGE SCX奖宣言\\[6pt]
    ——审计替代诺贝尔：在可审计标准下重新定义科学卓越}\\[14pt]
    \large
...
\begin{quotation}
\noindent\textit{``如果一项声称无法被审计，那么它甚至不是一项声称。它是一种噪音——一种在通信信道中传输了能量的声波，但携带的信息量为零。诺贝尔奖声称它在颁发卓越。SCX奖不声称——它声明标准，然后执行计算。区别在于前者用权威替代了审计，后者用审计替代了权威。"}
...
\centering
\textbf{公理 0 (卓越的可审计性公理)}\\[4pt]
\textbf{Axiom 0 (The Auditability of Excellence)}

\vspace{6pt}
设 $\mathcal{E}$ 为卓越性声明的集合。设 $\auditOp: \mathcal{E} \to \{\text{真}, \text{假}, \bot\}$ 为一个审计函数，其中 $\bot$ 表示``无法判定''。

\medskip
\textit{Chinese:} 若 $\auditOp(e) = \bot$，则 $e$ 不是一项卓越性声明——它是一项卓越性断言。断言与声明的区别是：声明接受审计；断言拒绝审计。只有声明具有数学意义。
...
% ================================================================
\section{审计真空：奖项颁发中的$M=1$非声明性}
\label{sec:audit_vacuum}

\subsection{奖项作为声明}

\noindent
\textbf{1.1 奖项作为声明 (Prizes as Claims)}

当一个机构颁发一项科学卓越奖——诺贝尔奖、菲尔兹奖、图灵奖——它在做什么？从信息论的角度看，它在做出一个\textbf{二元声明}：在候选者集合 $\candidateSet$ 中，获奖者 $w \in \candidateSet$ 满足

\begin{equation}
    w = \argmax_{c \in \candidateSet} \; \text{卓越性}(c),
    \label{eq:prize_claim}
\end{equation}

其中卓越性$(\cdot)$是某个函数，它将候选者映射到一个全序集上的标量值。问题不在于这个形式化——所有奖项都隐含地做出这种声明。问题在于：\textbf{卓越性$(\cdot)$是什么？}

诺贝尔奖的答案是：卓越性$(\cdot)$由瑞典皇家科学院的一个委员会决定。委员会进行审议（deliberation），然后投票。投票结果\textit{就是}卓越性的值。

这是一个合法的方法——\textbf{当且仅当}委员会的审议过程是可审计的。但事实并非如此。

\begin{honestbox}
    \item 诺贝尔奖的审议记录保密50年。这意味着在50年内，没有人——包括获奖者本人——知道他们为何获奖。他们只知道一个委员会\textit{说}他们卓越。
...
\subsection{$M=1$ 非声明性的数学形式化}

\noindent
\textbf{1.2 $M=1$ 非声明性的形式化 (Formalizing $M=1$ Undeclared-ness)}

\begin{definition}[奖项函数, Prize Function]
...
其中：
\begin{itemize}
    \item $\candidateSet$：候选者集合（可能是隐式的）；
    \item $\scoreOp: \candidateSet \to \R$：评分函数，将候选者映射到标量评分；
    \item $\gaugeVec = (\mathbf{g}_1, \dots, \mathbf{g}_M)$：评审委员会成员的规范参数向量，每人一个；
    \item $\text{procedure}$：评分聚合程序；
    \item $\text{declaration}$：是否公开声明上述所有参数的布尔标志。
\end{itemize}
\end{definition}

\begin{definition}[$M=1$ 非声明奖项, $M=1$ Undeclared Prize]
\label{def:M1_undeclared}
若 $\prizeSet$ 满足以下条件之一，则称其为 \textbf{$M=1$ 非声明奖项}：
\begin{enumerate}[label=(\roman*)]
    \item $\gaugeVec$ 未公开声明；
    \item $\scoreOp$ 未被定义为一个从 $\candidateSet$ 到 $\R$ 的可计算函数；
...
\begin{theorem}[$M=1$ 非声明奖项的信息论无效性, \\
    Information-Theoretic Voidness of $M=1$ Undeclared Prizes]
\label{thm:M1_void}
设 $\prizeSet$ 是一个 $M=1$ 非声明奖项，颁发给候选者 $w$。则声明``$w$ 是最佳候选者''所携带的信息量 $I(w; \text{truth})$ 的上界为零：
\begin{equation}
...
信息 $I(w; \text{truth})$ 衡量获奖者身份在多大程度上减少了我们对``真正最佳候选者''的不确定性。当 $\gaugeVec$ 未声明时，评分函数 $\scoreOp_g(c)$ 对每个 $c$ 的值在 $\gaugeVec$ 的可能取值空间上是一个随机变量。由于 $\gaugeVec$ 的任何分布都可以与任何候选者排序兼容（只需适当选择 $\gaugeVec$），因此 $\scoreOp_g$ 的排序不提供关于``真正最佳候选者''的任何信息——它是 $\gaugeVec$ 选择的纯函数。形式上，$\scoreOp_g(c)$ 与真实卓越性 $\text{truth}(c)$ 之间在 $\gaugeVec$ 未固定时互信息为零。
\end{proof}

\begin{corollary}[诺贝尔奖的信息论地位]
\label{cor:nobel_info}
诺贝尔奖、菲尔兹奖、图灵奖以及所有其他依赖非声明委员会评审的奖项，其关于``获奖者是最佳候选者''的声称所携带的信息量为零。这并非一个关于这些奖项\textit{是否}选出了最佳候选者的经验性声明——这是一个关于\textit{声明本身是否可验证}的逻辑声明。不可验证的声明携带零信息。
\end{corollary}

\subsection{委员会并非审计结构}

\noindent
\textbf{1.3 委员会并非审计结构 (Committees Are Not Audit Structures)}

委员会的存在给公众一种``多人审查''的错觉。但$M$名委员的存在本身并不构成审计。审计要求：

\begin{enumerate}[label=(\alph*)]
    \item \textbf{标准独立性}：审计标准必须独立于审计者。委员会\textit{是}标准的制定者，因此不能\textit{同时}是审计者。
    \item \textbf{可复现性}：另一位审计者使用相同标准必须得出相同结论。委员会审议不可复现——换一组委员可能得出不同结论。
    \item \textbf{可追溯性}：审计结论的每一条推理链必须可追溯至原始证据。委员会审议产生一个最终投票，没有推理链。
\end{enumerate}
...
\begin{honestbox}
    \item 诺贝尔奖委员会的审议是\textbf{定性商议}（qualitative deliberation），不是定量审计。没有评分表，没有加权方案，没有可复现的排序算法。
    \item 委员会成员的选择标准是什么？他们的$g$参数由谁校准？答案是：委员会选择委员会。这是一个自指系统（self-referential system），其审计链长度为0。
    \item 诺贝尔奖的权威性完全来自其历史声望——即来自\textit{过去的非审计声称}的积累。这是声望的复利，不是审计的积累。
\end{honestbox}

\subsection{历史注记：傲慢作为规范力的缺失}

\noindent
\textbf{1.4 历史注记：傲慢作为规范力的缺失 (Historical Note: Arrogance as Gauge Freedom)}

诺贝尔奖自1901年颁发以来，其评选方式的核心——委员会秘密审议——从未改变。这不是一个被忽视的缺陷；这是一个被制度化维护的特征。保密性被宣称为保护审议独立性的必要手段。

但SCX平等论揭示了一个更深层的结构：\textbf{保密性不是保护独立性的手段，而是保护规范自由度的手段。}当委员会保持其审议秘密时，它保持了在任何方向上调谐其$g$参数的自由——而不必对任何人负责。规范固定意味着失去这种自由。委员会不愿放弃它。
...
% ================================================================
\section{SCX奖机制：算法化卓越评定的架构}
\label{sec:mechanism}

\subsection{核心理念：没有人类的评审委员}

\noindent
\textbf{2.1 核心理念：没有人类的评审委员 (Core Idea: No Human Judges)}
...
\begin{minipage}{0.88\textwidth}
\centering
\textbf{SCX奖第一原则 (SCX Prize First Principle)}

\vspace{4pt}
没有评审委员会。没有人类判断。没有审议。没有投票。没有协商。没有共识。没有多数意见。没有少数意见。没有保密档案。没有50年解密期。

\medskip
\textit{No评审 committee. No human judgment. No deliberation. No voting. No negotiation. No consensus. No majority opinion. No minority opinion. No sealed archives. No 50-year declassification period.}
...
\noindent
\textbf{2.2 奖项架构 (Prize Architecture)}

SCX奖由以下组件构成：

\begin{enumerate}[label=\textbf{P\arabic*}.]
    \item \textbf{候选者空间} $\candidateSet$：所有公开发表的科学论文、数据集、复现研究、审计报告以及相关科学产出。候选者空间是开放的——任何人可以被提名，任何人可以自荐。提名不是特权，而是将候选者添加到待评估队列中的操作。
    
    \item \textbf{评分函数} $\scoreOp_k: \candidateSet \to \R$：对于每个奖项类别 $k \in \{1, \dots, 8\}$，存在一个明确定义的、可计算的评分函数。评分函数接受候选者的公开元数据（引文、复现状态、数据可用性、审计分数等）作为输入，输出一个标量分数。
    
    \item \textbf{规范固定条件} $\sumgd$：所有评分函数的参数 $\gaugeVec$ 是公开的、固定的。不存在隐藏的权重。不存在审美的判断。不存在``感觉''。每项输入数据的来源和计算方法在评分规范中完整记录。
    
    \item \textbf{排名引擎}：对每个类别，评分函数对候选者空间中的所有候选者进行评估。排名是确定性的。平局由预定义的决胜规则解决（最早发表日期优先）。
    
    \item \textbf{审计日志}：每次评分的完整计算路径被记录并公开。任何人可以追溯从原始数据到最终排名的推理链。
\end{enumerate}

\subsection{算法化评审的形式化}

\noindent
\textbf{2.3 算法化评审的形式化 (Formalizing Algorithmic Judgment)}

\begin{definition}[可审计奖项, Auditable Prize]
\label{def:auditable_prize}
一个\textbf{可审计奖项} $\prizeSet_{\text{auditable}}$ 满足：
\begin{enumerate}[label=(\roman*)]
    \item $\scoreOp$ 是一个明确定义的、可计算的函数；
    \item $\gaugeVec$ 已完全声明并公开；
    \item $\text{declaration} = \text{true}$，即所有参数、权重、数据源和处理步骤均被记录；
    \item 对任意候选者 $c \in \candidateSet$，$\scoreOp(c)$ 可由任何第三方独立重新计算并得出完全相同的结果。
\end{enumerate}
\end{definition}

\begin{proposition}[可审计奖项的信息论非平凡性]
\label{prop:auditable_info}
设 $\prizeSet_{\text{auditable}}$ 为一个可审计奖项。则声明``$w$ 是类别 $k$ 中的最佳候选者''携带非零信息：
\begin{equation}
...
\begin{honestbox}
    \item SCX奖并不声称其评分函数是``正确''的。它声称其评分函数是\textbf{公开的}。``正确性''是公开辩论的产物；``公开性''是审计的前提。
...
\noindent
\textbf{3.1 类别概述 (Category Overview)}

SCX奖设有八个类别，每个类别对应科学知识生产中一个可审计的卓越性维度。维度的选择遵循一个指导原则：\textbf{每个维度必须可以通过公开可得的数据进行客观量化和独立验证。}

\begin{enumerate}[label=\textbf{C\arabic*}., leftmargin=*]
    \item \textbf{最具可审计性论文奖 (Most Auditable Paper)}：奖励其方法、数据和推理链最易于外部验证的论文。
    \item \textbf{最佳势函数奖 (Best Potential Function)}：奖励提出了新的、有用的科学评估函数的论文或工具。
    \item \textbf{最佳开放数据奖 (Best Open Data)}：奖励生产了最高质量、最广泛使用的开放科学数据集的个人或团队。
    \item \textbf{复现冠军奖 (Replication Champion)}：奖励完成了最大数量高质量独立复现研究的个人或团队。
    \item \textbf{审计贡献奖 (Audit Contribution Award)}：奖励开发了新的审计工具、方法或基础设施的个人或团队。
    \item \textbf{终身审计分奖 (Lifetime Audit Score)}：奖励其全部已发表作品中累积审计分最高的研究者。
    \item \textbf{跨域创新奖 (Cross-Domain Innovation)}：奖励其工作在三个或以上学科中产生了可验证影响的个人。
    \item \textbf{自审计奖 (Self-Audit Award)}：奖励对其自身过去已发表工作进行了最彻底公开审计的个人。
\end{enumerate}

\subsection{详细判据表}

\noindent
\textbf{3.2 详细判据表 (Detailed Criteria Table)}

\begin{longtable}{p{0.14\textwidth} p{0.26\textwidth} p{0.26\textwidth} p{0.26\textwidth}}
\caption{SCX奖类别、判据、评分函数与审计方法} \\
\hline
\textbf{类别} & \textbf{判据 (Criterion)} & \textbf{评分函数 $\scoreOp_k$ (Scoring Function)} & \textbf{可审计性方法 (Auditability Method)} \\
\hline
\endfirsthead
\hline
\textbf{类别} & \textbf{判据 (Criterion)} & \textbf{评分函数 $\scoreOp_k$ (Scoring Function)} & \textbf{可审计性方法 (Auditability Method)} \\
...
\textbf{C1: 最具可审计性论文} & 论文的完整推理链、数据、代码和分析脚本的可验证性程度。 & $\scoreOp_1(p) = w_a \cdot A(p) + w_d \cdot D(p) + w_r \cdot R(p) + w_c \cdot C(p)$，其中 $A(p)$ = 代码可用性分数，$D(p)$ = 数据可用性分数，$R(p)$ = 复现文档分数，$C(p)$ = 计算环境可复现性分数。权重 $w_a, w_d, w_r, w_c$ 公开。 & 每个子分数由自动化检查确定：代码仓库可访问性（二进制）、数据DOI有效性（二进制）、复现脚本执行成功（二进制）、容器/环境文件完备性（二进制）。所有检查可独立重跑。 \\
\hline

\textbf{C2: 最佳势函数} & 候选者提出的评估函数/度量/基准的质量、采用范围与区分能力。 & $\scoreOp_2(f) = w_u \cdot U(f) + w_i \cdot I(f) + w_d \cdot D_{\text{disc}}(f)$，其中 $U(f)$ = 采用度（使用该函数的论文数），$I(f)$ = 影响力（引用该函数原始提出论文的引用数），$D_{\text{disc}}(f)$ = 区分度（函数在标准测试集上的信息增益）。 & 采用度和影响力通过公开引文数据库计算。区分度通过公开基准测试结果计算。所有数据源公开，计算脚本公开。 \\
\hline

\textbf{C3: 最佳开放数据} & 数据集的质量、覆盖度、文档完备性、可访问性和下游采用率。 & $\scoreOp_3(d) = w_s \cdot S(d) + w_q \cdot Q(d) + w_a \cdot A_{\text{adopt}}(d) + w_d \cdot D_{\text{doc}}(d)$，其中 $S(d)$ = 规模分数（样本数/变量数），$Q(d)$ = 质量分数（完整性、一致性自动检查），$A_{\text{adopt}}(d)$ = 采用分数（使用该数据集的论文数），$D_{\text{doc}}(d)$ = 文档完备性分数。 & 规模和采用率通过公开元数据计算。质量由自动化数据验证流水线评估。文档分数由结构化检查清单确定。全部可独立复现。 \\
\hline

\textbf{C4: 复现冠军} & 完成的高质量独立复现研究的数量和成功率。 & $\scoreOp_4(r) = \sum_{i=1}^{N_r} q_i \cdot s_i$，其中 $N_r$ = 复现者完成的复现研究数量，$q_i$ = 第 $i$ 项复现的质量分数（基于复现报告的完整性），$s_i$ = 复现状态（成功=1，部分成功=0.5，失败但报告完整=0.3）。 & 每项复现研究以结构化复现报告的形式发布在公开注册库中。质量分数由自动化检查清单评估。状态分类标准预先定义。注册库公开可查询。 \\
\hline

\textbf{C5: 审计贡献奖} & 开发的审计工具、方法或基础设施的实际采用和有效性。 & $\scoreOp_5(t) = w_u \cdot U(t) + w_e \cdot E(t) + w_n \cdot N(t)$，其中 $U(t)$ = 采用度（使用该工具/方法的项目数），$E(t)$ = 有效性（该工具检测到的可验证问题的数量），$N(t)$ = 新颖性（该方法与先前方法的语义距离——由自动化文献分析确定）。 & 采用度通过公开仓库星标/依赖图计算。有效性通过工具输出的公开审计报告追踪。新颖性由预定义的语义分析流水线计算。 \\
\hline

\textbf{C6: 终身审计分} & 研究者全部已发表工作中累积的可审计性得分。 & $\scoreOp_6(r) = \sum_{p \in \text{pubs}(r)} \scoreOp_1(p) \cdot t(p)$，其中 $\text{pubs}(r)$ = 研究者 $r$ 的全部已发表论文，$t(p)$ = 时间衰减因子（近期论文权重更高）。注：使用 $\scoreOp_1$ 作为每篇论文的可审计性分数。 & 全部出版物列表通过公开数据库（ORCID, DBLP, PubMed等）获取。每篇论文的 $\scoreOp_1$ 分数独立计算。所有计算可追溯至单篇论文。时间衰减参数公开。 \\
\hline

\textbf{C7: 跨域创新} & 其工作在三个或以上学科中产生了可验证影响的程度。 & $\scoreOp_7(r) = \sum_{d \in \text{domains}(r)} w_d \cdot \text{Impact}_d(r) + B(r)$，其中 $\text{domains}(r)$ = 研究者工作涉及的学科集合（由引文来源的学科分类确定），$\text{Impact}_d(r)$ = 在学科 $d$ 中的影响度量，$B(r)$ = 跨域奖励（当 $|\text{domains}(r)| \geq 3$ 时激活）。 & 学科分类使用公开的学术分类法（如Scopus或OpenAlex分类）。影响度量通过引文计数和各学科的标准化引用指标计算。跨域验证要求引用来源至少属于三个不同的一级学科分类。 \\
\hline

\textbf{C8: 自审计奖} & 研究者对其自身过去已发表工作进行了公开、彻底审计的程度。 & $\scoreOp_8(r) = \sum_{s \in \text{self\_audits}(r)} q_s \cdot d_s \cdot c_s$，其中 $\text{self\_audits}(r)$ = 研究者发布的自审计报告集合，$q_s$ = 审计质量分数，$d_s$ = 审计深度分数（被审计论文的年份跨度），$c_s$ = 纠正性分数（审计是否导致更正或撤回）。 & 自审计报告必须在公开注册库中发布。质量分数由自动化检查清单评估。深度和纠正性通过公开记录验证。自我审计的诚实性是其评分的内在组成部分——表面化的自审计得分低。 \\
\hline

\end{longtable}

\subsection{评分函数的规范固定}

\noindent
\textbf{3.3 评分函数的规范固定 (Gauge-Fixing the Scoring Functions)}

每个评分函数 $\scoreOp_k$ 中的权重向量 $\mathbf{w}^{(k)} = (w_1^{(k)}, \dots, w_{n_k}^{(k)})$ 是公开声明的。权重的选择本身接受审计：权重的合理性由独立的审计小组定期审查，审查结果公开。

但是——这里有一个关键的规范理论洞察——\textbf{权重的选择是任意的}。任何固定的权重向量都定义了一个合法的评分函数。关键不在于权重``正确''，而在于权重\textit{固定}。固定的权重意味着排序是可复现的。这意味着对``这个候选者应该排在那个候选者前面吗？''的问题，存在一个确定的、非协商的答案。
...
\noindent
\textbf{3.4 年度周期 (Annual Cycle)}

SCX奖的年度周期是自动化的：

\begin{enumerate}[label=\textbf{阶段 \arabic*}.]
    \item \textbf{提名开放 (Nominations Open)}：每年1月1日--3月31日。任何人可以通过公开注册库提名任何候选者（包括自己）。提名需要提供候选者与类别判据相关的公开证据。
    \item \textbf{数据采集 (Data Collection)}：每年4月1日--6月30日。自动化流水线收集所有候选者的公开数据（引文、代码仓库状态、数据集元数据、复现报告等）。数据源和采集方法公开。
    \item \textbf{评分计算 (Scoring)}：每年7月1日。评分引擎对所有候选者运行全部8个评分函数。计算完全自动化。结果公开。
    \item \textbf{审计窗口 (Audit Window)}：每年7月1日--8月31日。公开评分结果。任何人可以挑战任何分数，前提是提供可复现的证据（例如，指出数据采集错误或计算错误）。挑战通过公开问题追踪系统处理。
    \item \textbf{最终排名 (Final Ranking)}：每年9月1日。在审计窗口结束后，所有有效挑战已解决，最终排名锁定。
    \item \textbf{颁奖 (Award Ceremony)}：每年12月10日（与诺贝尔奖颁奖同日）。SCX奖颁奖典礼在线直播。获奖者公布。奖金分发。
\end{enumerate}

\begin{honestbox}
    \item 提名不是特权。它是将候选者添加到计算队列中的操作。任何人都可以添加任何人。评分的``民主化''不在于谁投票，而在于谁可以被计算。
    \item 审计窗口是SCX奖与所有其他奖项的关键区别。在审计窗口中，公众的反馈是\textbf{计算性}的（指出数据错误），而不是\textbf{意见性}的（``我觉得他不配''）。意见被排除在系统之外。
...
\subsection{规范不等价性：诺贝尔奖的结构性缺陷}

\noindent
\textbf{4.1 规范不等价性：诺贝尔奖的结构性缺陷 (Gauge Inequivalence: The Nobel's Structural Defect)}

SCX平等论揭示了诺贝尔奖——以及所有委员会制奖项——的一个根本性缺陷：\textbf{委员会与被其评判的领域之间是规范不等价的。}

\begin{definition}[规范等价性, Gauge Equivalence]
\label{def:gauge_equivalence}
两个评估系统 $A$ 和 $B$ 是\textbf{规范等价的}，当且仅当存在一个规范变换 $\mathcal{T}: \gaugeVec_A \to \gaugeVec_B$，使得对所有被评估对象 $x$：
\begin{equation}
    \scoreOp_A(x; \gaugeVec_A) = \scoreOp_B(x; \mathcal{T}(\gaugeVec_A)).
    \label{eq:gauge_equiv}
\end{equation}
换句话说，若 $A$ 通过调整自身的规范参数可以重现 $B$ 的所有评估结果，则 $A$ 和 $B$ 是规范等价的。规范等价性意味着两个系统\textit{覆盖相同的评估空间}。
\end{definition}
...
\begin{proof}[证明概要 (Proof Sketch)]
$\mathcal{F}$ 的评估规范参数 $\gaugeVec_{\mathcal{F}}$ 是可以被估计的——它是该领域科学家集体行为的统计特性（谁被引用、谁的工作被复现、谁的数据被使用）。$\mathcal{N}$ 的评估规范参数 $\gaugeVec_{\mathcal{N}}$ \textbf{无法被估计}，因为委员会的审议过程保密。由于 $\gaugeVec_{\mathcal{N}}$ 的取值空间无法被约束，不存在一个将（已知的）$\gaugeVec_{\mathcal{F}}$ 映射到（未知的）$\gaugeVec_{\mathcal{N}}$ 的变换。映射的存在需要两个空间都已被定义——但 $\gaugeVec_{\mathcal{N}}$ 的空间无法被定义，因为它被保密。
\end{proof}

\begin{corollary}[诺贝尔奖声称的非传递性]
\label{cor:nobel_nontransitive}
诺贝尔奖的声称——``获奖者是该领域最卓越的贡献者''——无法被转化为该领域的任何内部评估语言。这是一种\textbf{不可翻译的声称}。它属于诺贝尔奖的系统，但不属于科学的系统。它是一枚无法在科学市场兑换的货币。
\end{corollary}

\subsection{SCX奖的规范固定：$\sumgd$ 作为定义性条件}

\noindent
\textbf{4.2 SCX奖的规范固定 (Gauge-Fixing in the SCX Prize)}
...
这个条件的含义是明确的：\textbf{所有评估参数的总和为零。}这不是一个自然条件——它是被\textit{施加}的。在杨-米尔斯理论中，洛伦兹规范 $\partial_\mu A^\mu = 0$ 是一个选择，不是自然定律。同样，$\sumgd$ 是SCX奖的选择，不是评估的本然属性。

施加这个条件的结果是：\textbf{SCX奖的评分是规范不变的。}这意味着：
\begin{itemize}
    \item 评分函数 $\scoreOp_k$ 对任何规范变换 $\gaugeVec \to \gaugeVec + \delta\gaugeVec$ 是不变的，只要 $\sum_m \delta\mathbf{g}_m = \mathbf{0}$。
    \item 换言之，只有满足 $\sumgd$ 的规范参数集合才被允许。这固定了规范自由度，使评分成为定义明确的操作。
\end{itemize}

\begin{proposition}[SCX奖的规范等价性]
\label{prop:scx_gauge_equiv}
SCX奖的评估系统 $\mathcal{S}$ 与科学领域的集体评估系统 $\mathcal{F}$ 是\textbf{局部规范等价的}：在 $\mathcal{F}$ 的评估可以被公开数据充分近似的子领域内，存在一个明确定义的映射 $\mathcal{T}$，使得 $\scoreOp_{\mathcal{S}}$ 与 $\scoreOp_{\mathcal{F}}$ 产生一致排序。
\end{proposition}

\begin{proof}[证明概要 (Proof Sketch)]
$\mathcal{S}$ 的评分函数使用公开可得的指标（引文、数据可用性、复现等），这些指标本身就是 $\mathcal{F}$ 集体评估的近似。由于 $\mathcal{S}$ 的 $\gaugeVec$ 已声明，我们可以计算 $\scoreOp_{\mathcal{S}}$ 与 $\scoreOp_{\mathcal{F}}$ 在已知排序上的相关性。这种相关性是可检验的、可改进的。这不是等价性证明——它是等价性\textit{检验}。而这正是诺贝尔奖做不到的。
\end{proof}

\subsection{委员会作为奇点}

\noindent
\textbf{4.3 委员会作为奇点 (The Committee as Singularity)}

在规范理论的视角下，诺贝尔奖委员会是一个\textbf{规范奇点}（gauge singularity）：一个无法被固定或变换的评估源。它类似于广义相对论中的黑洞——在事件视界（保密协议）之内，物理定律（评估逻辑）是不可知的。
...
\begin{honestbox}
    \item 诺贝尔奖的声望是一种\textbf{社会规范惯性}（social gauge inertia）——系统持续运行，不是因为参数正确，而是因为动量太大无法停止。
...
\subsection{范式转移：从``相信''到``验证''}

\noindent
\textbf{4.4 范式转移：从``相信''到``验证'' (Paradigm Shift: From ``Trust'' to ``Verify'')}

诺贝尔奖要求公众\textit{相信}委员会做出了正确的选择。SCX奖要求公众\textit{验证}算法产生了最高分。这种从信任到验证的转变是整个现代审计学的基石——也是整个SCX框架的基石。
...
\noindent
\textbf{5.1 正式声明 (Formal Declaration)}

以下声明不仅是一份立场陈述——它是公理0和定理\ref{thm:M1_void}的直接推论。它被表述为一项声明而非论证，因为论证已在前面各节中完成。
...
\bigskip
\noindent
\textbf{声明四 (Declaration IV):}

SCX奖不声称其评分函数是``正确的''。它声称其评分函数是\textbf{公开的、固定的、可复现的}。对评分函数``正确性''的判断被委托给科学界的公开辩论——而非任何委员会。评分函数的改进是科学界的集体任务，通过公开提案、同行审议和共识机制完成。
...
\subsection{宣言的法律与哲学地位}

\noindent
\textbf{5.2 宣言的法律与哲学地位 (Legal and Philosophical Status of the Declaration)}

本宣言不寻求任何政府或机构的认可。它不依赖任何现有法律框架。它是一项\textbf{数学-伦理声明}：从公理0（卓越的可审计性公理）出发，通过定理\ref{thm:M1_void}和推论\ref{cor:nobel_info}的逻辑，推导出的规范结论。
...
\noindent
\textbf{5.3 宣言的不可逆性 (Irreversibility of the Declaration)}

一旦一份公开声明敲定了评分的规范参数，它就不能被私下更改而不被察觉。这是因为：

\begin{proposition}[公开发表的不可逆性, Irreversibility of Public Declaration]
...
\subsection{资金来源：雅捷API收入模型}

\noindent
\textbf{6.1 资金来源：雅捷API收入模型 (Funding Source: The Yajie API Revenue Model)}

SCX奖的奖金来源于雅捷（Yajie）API的商业收入。具体而言：

\begin{itemize}
    \item 雅捷API为科学审计和复现工作提供计算基础设施。其收入来自API调用费用。
    \item 雅捷API总收入的\textbf{固定百分比}（初始设定为 $5\%$，由SCX审计委员会每年审查）被划拨至SCX奖金池。
    \item 此比例的变更需要公开提案、社区讨论和公开投票——与评分函数权重的变更机制相同。
\end{itemize}
...
\noindent
\textbf{6.2 圣经-教皇模型 (The Bible-and-Pope Model)}
...
\begin{minipage}{0.88\textwidth}
\centering
\textbf{圣经-教皇模型 (The Bible-and-Pope Model)}

\vspace{4pt}
\textbf{圣经 (The Bible)} = SCX奖的公开规范——评分函数、权重、数据源、计算程序。这是不可变的宗教文本：每次更改必须通过公开提案和社区批准。规范的历史版本永远保留，任何人都可以引用任何历史版本。

\medskip
\textbf{教皇 (The Pope)} = SCX审计委员会——一个轮值的技术机构，负责数据流水线的维护、计算的执行、审计窗口的管理以及争议的裁决。教皇的权力是有限的：它可以解释圣经，但不能改变圣经。教皇的人选是公开的、轮换的，其决定是可上诉的（上诉到圣经本身，即重新运行计算）。
...
\noindent
\textbf{6.3 奖金分配 (Prize Distribution)}

\begin{itemize}
    \item 奖金的\textbf{70\%} 分配给八个类别（每类别 $8.75\%$），具体分配：类别内获奖者平分该类别奖金。
    \item 奖金的\textbf{20\%} 用于运营成本：计算基础设施、数据存储、审计小组津贴。
    \item 奖金的\textbf{10\%} 作为储备金，用于应对API收入波动和长期资金稳定。
    \item 如果某类别在当年没有合格的候选者（即最高评分低于预设的最低阈值），该类别奖金转入储备金。
\end{itemize}

\subsection{自审计机制}

\noindent
\textbf{6.4 自审计机制 (The Self-Audit Mechanism)}

SCX奖本身是C8类（自审计奖）的一个永久候选者。这意味着：

\begin{enumerate}[label=(\arabic*)]
    \item SCX奖的规范、代码、数据和决策每年接受一次正式的公开审计。
    \item 审计报告由独立审计小组撰写（审计小组成员不得是SCX审计委员会成员，也不得是当前奖项周期的获奖者）。
    \item 审计报告公开。任何发现的问题通过版本控制系统中的补丁解决。
    \item SCX奖在C8类别中的得分——即其自审计分数——每年公开。如果SCX奖本身获得了足够高的分数，它将（讽刺地）有资格获得C8奖。如果它获奖，奖金转入储备金。
\end{enumerate}
...
\begin{honestbox}
    \item 自审计不是自我表扬。C8的评分函数奖励\textit{诚实的}自审计——它惩罚掩盖错误。发现并公开自己的错误比宣称自己完美得分更高。这是SCX奖与所有其他奖项的本质区别：错误是积分，不是污点。
...
\noindent
\textbf{6.5 可持续性分析 (Sustainability Analysis)}

SCX奖的财务模型基于一个简单的假设：科学审计的需求将增长。如果这个假设正确：
\begin{itemize}
    \item 雅捷API收入增长 $\to$ 奖金池增长 $\to$ 更高奖金吸引更多关注 $\to$ 更多科学家采用可审计实践 $\to$ 对审计工具的需求增加 $\to$ 雅捷API收入增长。
\end{itemize}

如果这个假设错误：
\begin{itemize}
    \item 雅捷API收入不增长或下降 $\to$ 奖金池缩小 $\to$ 奖金减少，但奖项机制不变（评分函数不依赖奖金规模）。
\end{itemize}
...
\begin{quotation}
\noindent\textit{``在SCX奖的第一天，什么都没有发生。算法运行。排名生成。没有人鼓掌，因为没有人知道应该鼓掌。没有人抗议，因为没有人知道应该抗议什么。排名就在那里——一份公开文件，在互联网上，任何人都可以阅读、复现和挑战。}}

\medskip

\noindent\textit{第二天，有人重新运行了算法。结果一致。第一个人说：'我验证了。'第二个人说：'我也验证了。'第三个人发现了一个数据错误，提交了补丁。错误被修复。排名更新了。}}

\medskip

\noindent\textit{第一百天，没有人再谈论'相信'这个奖项。他们谈论它的评分函数是否应该调整，权重是否合理，数据源是否全面。}}

\medskip

\noindent\textit{这是SCX奖的胜利。不是因为它被相信，而是因为它被质疑。不是因为它完美，而是因为它可改进。不是因为它是一个更好的诺贝尔奖——而是因为它是一个完全不同的东西。}}

\medskip

\noindent\textit{一个真正的审计系统不要求信任。它要求验证。而在验证完成的那一刻，信任不再是必要的。"}
...
\noindent
\textbf{A.1 评分引擎架构 (Scoring Engine Architecture)}

SCX奖评分引擎是一个确定性的数据流水线：

\begin{enumerate}[label=\textbf{步骤 \arabic*}.]
    \item \textbf{候选者发现}：从公开数据库（OpenAlex, Crossref, DBLP, PubMed, arXiv, GitHub, Zenodo等）采集候选者和提名。
    \item \textbf{元数据提取}：对每个候选者，提取论文元数据、引文计数、代码仓库URL、数据DOI、许可证信息。
    \item \textbf{可审计性验证}：自动化检查代码仓库可访问性、数据可用性、复现文档和计算环境完备性。
    \item \textbf{评分计算}：对每个候选者运行8个评分函数。所有中间结果被缓存。
    \item \textbf{排名生成}：按类别排名。平局通过发表日期（优先更早发表）和字母顺序（最终决胜）解决。
    \item \textbf{审计日志生成}：每项评分决策的可追溯记录。
\end{enumerate}

\subsection{数据源规范}

\noindent
\textbf{A.2 数据源规范 (Data Source Specification)}

\begin{itemize}
    \item OpenAlex: 用于论文元数据和引文计数。API: \texttt{https://api.openalex.org}。免费且开放。
    \item Crossref: 用于DOI验证和元数据补充。API: \texttt{https://api.crossref.org}。
    \item GitHub API: 用于代码可用性检查、星标计数和活跃度评估。
    \item Zenodo API: 用于数据DOI验证和数据集元数据。
    \item ORCID API: 用于研究者身份消歧和出版物列表。
\end{itemize}
...
\noindent
\textbf{A.3 审计挑战协议 (Audit Challenge Protocol)}

\begin{enumerate}[label=(\arabic*)]
    \item 挑战者通过公开GitHub仓库提交issue，指出具体的计算错误或数据采集错误。
    \item 挑战必须包含可复现的证据（例如，错误的数据值、遗漏的候选者、错误的分类）。
    \item 审计委员会在14天内回应。回应必须是：接受（附带修复补丁）、拒绝（附带拒绝理由）、或请求更多信息。
    \item 如果挑战被拒绝，挑战者可以上诉。上诉由独立的元审计小组裁决。
    \item 所有挑战、回应和裁决公开记录。
\end{enumerate}
...
\noindent
\textbf{C.1 信息论完备性证明 (Information-Theoretic Completeness Proof)}

以下给出定理\ref{thm:M1_void}的完整证明。

\begin{proof}[定理1的完整证明]
设 $\prizeSet$ 为一个奖项，其委员会具有规范参数 $\gaugeVec = (g_1, \dots, g_M)$，其中每个 $g_i \in \R^d$ 是一个 $d$ 维向量，代表第 $i$ 名委员的评估偏差。委员会通过一个聚合函数 $F: \R^{M} \to \R$ 产生最终评分：
...
\begin{equation}
    P(\scoreOp \text{ 的正确排序} \mid \gaugeVec \text{ undeclared}) = \frac{1}{|\candidateSet|!}.
...
其中 $H(\text{truth}) = \log |\candidateSet|$（无先验知识时的熵），而

\begin{equation}
    H(\text{truth} \mid w) = H(\text{truth}) \quad \text{（由方程 \ref{eq:uniform_prob} 中的均匀后验）}。
    \label{eq:conditional_entropy}
\end{equation}

因此 $I(w; \text{truth}) = 0$。$\square$
\end{proof}

\subsection{规范固定与杨-米尔斯类比的细节}

\noindent
\textbf{C.2 规范固定与杨-米尔斯类比 (Gauge-Fixing and the Yang-Mills Analogy)}

在SCX评估的形式体系中，候选者在评估空间中的位置由坐标 $x^\mu(c)$（$\mu = 1, \dots, d$）给出。评估势 $A_\mu$ 是一个规范场——它受规范变换的影响：
...
其中 $\Lambda$ 是任意标量函数（规范参数）。物理量必须是规范不变的——即它必须在变换 \eqref{eq:gauge_transform} 下不变。对于奖项评估，``物理量''是候选者的\textbf{排名}——即评分的序数关系，而非绝对评分值。

诺贝尔奖的问题是：它声称产生物理量（排名），但保留了对其规范（$\Lambda$）的完全控制。这意味着：\textbf{它可以产生任何排名，同时声称这是唯一合法的排名。}这是规范自由的滥用。
