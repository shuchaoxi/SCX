...
% ============================================================
%  势能面几何与Educational Equity：评分的规范固定与势能阶梯管理
...
% ---------- Title ----------
\title{
  \textbf{势能面几何与Educational Equity：\\评分的规范固定与势能阶梯管理}\\[8pt]
  \large
  Potential Surface Geometry and Educational Equity:\\
  Gauge Fixing of Grading and Potential Staircase Management\\[6pt]
  \normalsize
  --- 基于 SCX Equality Principle框架的教育系统分析 ---\\
...
评分制度创造了一个教育势能面 $\cS_{\text{edu}}(x)$：每个学生通过Score获得一个势能位置。势能梯度 $\gradS$ 是学习动机的数学表达——学生沿梯度上行、追求更高Score。然而，当评分产生的势能台阶过大时，梯度退化为\textbf{悬崖}（cliff）：底部的学生永远无法看到攀登路径，顶部的学生被锁定为\textbf{势能奇点}（Thm~10--11）。马太效应（Thm~12）使高势能学生获得更高的势能Growth率，将平滑梯度变为不可逆的阶梯函数——每一级台阶都是一个延迟引爆的不稳定界面。本文在Equality Principle框架（$\sumgd$）下构建教育势能面的形式化几何，提出三个Core定理：(1)~梯度必要性定理——$|\gradS|>0$ 是学习动机存在的必要条件；(2)~悬崖定理——当相邻势能台阶 $\Delta_k$ 超过临界值 $\Delta_{\text{crit}}$ 时，跨台阶的学习迁移在数学上不可行；(3)~态度对齐定理——教师与学生的规范姿态 $\mathbf{g}_T, \mathbf{g}_S$ 必须满足 $\mathbf{g}_T + \sum_{i} \mathbf{g}_{S_i}/N = \mathbf{0}$，否则评分比较失去合法性。我们分析排名系统、标准化考试、分轨/分流制作为教育势能面管理的不同策略，并给出保留动机梯度同时避免悬崖的操作准则。
...
\subsection{Principle 2: Gauge-Fixed Rubrics (规范固定的Rubric)}
...
\vspace{4pt}
不是应该平awaiting——是不平awaiting的教育系统在数学上注定分层，分层在数学上注定引爆。
...
This work builds on the SCX Equality Principle (Equality Principle) framework developed in~\cite{scx_moe_gauge}. We acknowledge the intellectual debt to the勢能面几何 formulation and the Theorem 7--12 chain. The application to education is novel to this paper; any errors in domain translation are ours alone.
...
Anonymous. \textit{Potential Surface Misalignment——多专家路由中的规范自由度与MILP规范固定} (Potential Surface Misalignment: Gauge Freedom in Multi-Expert Routing and MILP Gauge Fixing). SCX Technical Report, 2026.

\bibitem{scx_equality_principle}
Anonymous. \textit{Equality Principle：Potential Surface Misalignment的认识论含义} (The Equality Principle: Epistemological Implications of Potential Surface Misalignment). In~\cite{scx_moe_gauge}, Section~E, 2026.
...
$\sumgd$ & Equality Principle gauge condition & Equality Principle规范条件 \\
\bottomrule
\end{tabular}
\end{table}
