...
\newcommand{\edunote}[1]{%
  \medskip\noindent
  \textbf{教育注记 (Education Note):} \textit{#1}%
}

% ---------- Title ----------
\title{
  \textbf{势能面几何与教育公正：\\评分的规范固定与势能阶梯管理}\\[8pt]
  \large
  Potential Surface Geometry and Educational Equity:\\
...
\noindent\textbf{中文摘要.}
评分制度创造了一个教育势能面 $\cS_{\text{edu}}(x)$：每个学生通过分数获得一个势能位置。势能梯度 $\gradS$ 是学习动机的数学表达——学生沿梯度上行、追求更高分数。然而，当评分产生的势能台阶过大时，梯度退化为\textbf{悬崖}（cliff）：底部的学生永远无法看到攀登路径，顶部的学生被锁定为\textbf{势能奇点}（Thm~10--11）。马太效应（Thm~12）使高势能学生获得更高的势能增长率，将平滑梯度变为不可逆的阶梯函数——每一级台阶都是一个延迟引爆的不稳定界面。本文在平等论框架（$\sumgd$）下构建教育势能面的形式化几何，提出三个核心定理：(1)~梯度必要性定理——$|\gradS|>0$ 是学习动机存在的必要条件；(2)~悬崖定理——当相邻势能台阶 $\Delta_k$ 超过临界值 $\Delta_{\text{crit}}$ 时，跨台阶的学习迁移在数学上不可行；(3)~态度对齐定理——教师与学生的规范姿态 $\mathbf{g}_T, \mathbf{g}_S$ 必须满足 $\mathbf{g}_T + \sum_{i} \mathbf{g}_{S_i}/N = \mathbf{0}$，否则评分比较失去合法性。我们分析排名系统、标准化考试、分轨/分流制作为教育势能面管理的不同策略，并给出保留动机梯度同时避免悬崖的操作准则。
...
This paper formalizes this tension within the SCX Equality Principle (\textit{平等论}) framework~\cite{scx_equality_principle}. We model education as a potential surface geometry problem, where the core question is:

\begin{center}
\fbox{%
\begin{minipage}{0.88\textwidth}
\centering
\textbf{How to preserve $\gradS$ — the motivational gradient — without creating cliffs (悬崖) where students can never catch up?}
\end{minipage}%
}
\end{center}
...
\begin{definition}[Educational Potential Surface, \textbf{教育势能面}]
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Assessed achievement} (分数, grades, test scores) — the direct observable;
    \item \textbf{Teacher perceived competence} (教师感知能力) — transmitted through feedback valence, attention allocation, and implicit expectations;
    \item \textbf{Peer-relative standing} (同伴相对位置) — rank within reference group;
    \item \textbf{Institutional labels} (制度标签) — track placement, honors status, remedial classification.
\end{enumerate}
\end{definition}
...
\begin{definition}[Educational Potential Gradient, \textbf{教育势能梯度}]
...
\begin{definition}[Potential Step, \textbf{势能台阶}]
...
\begin{definition}[Cliff, \textbf{悬崖}]
\label{def:cliff}
...
\begin{definition}[Grading Gauge, \textbf{评分规范}]
\label{def:grading_gauge}
A \textbf{grading gauge} is a triple $\cG = (\mathbf{b}, \sigma, \mathbf{w})$ where:
\begin{itemize}
    \item $\mathbf{b} \in \R$ is the \textbf{baseline shift} (零刻度偏移) — what score counts as ``zero'' or ``passing'';
    \item $\sigma > 0$ is the \textbf{scale factor} (尺度因子) — how wide a score difference counts as ``one grade step'';
    \item $\mathbf{w} \in \R^d$ is the \textbf{weight vector} (权重向量) — which dimensions of student state matter for the grade.
\end{itemize}
...
\begin{definition}[Teacher and Student Gauge Postures, \textbf{师生规范姿态}]
...
\subsection{Theorem 1: Gradient Necessity (梯度必要性定理)}
...
\textbf{教育含义 (Educational Implication).}
...
\subsection{Theorem 2: The Cliff Theorem (悬崖定理)}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Cliff-propagation.} Students clustered below a cliff form a \textbf{confinement basin} (受限盆地). By Thm~10 (Confinement Instability), pressure accumulates at rate proportional to $\Delta_k$, and the basin inevitably destabilizes on time scale $O(1/\Delta_k^2)$.
\end{enumerate}
\end{theorem}
...
\subsection{Theorem 3: Attitude Alignment and Grading Legitimacy (态度对齐与评分合法性)}
...
\subsection{Theorem 4: Matthew Effect in Education (教育马太效应阶梯)}
...
\textbf{排名系统 (Ranking Systems).} Consider a system that assigns each student an integer rank $r_i \in \{1, \dots, N\}$ based on a scalar exam score. The potential surface is
\begin{equation}
...
\textbf{标准化考试 (Standardized Testing).} Standardized tests attempt to fix a common gauge $\cG_{\text{std}}$ across all classrooms, schools, and regions. The test score provides a potential value:
\begin{equation}
...
\textbf{分轨/分流制 (Tracking / Streaming).} Tracking assigns students to different instructional tracks (e.g., remedial, standard, honors, gifted) based on prior performance. Formally, it partitions the student state space:
\begin{equation}
...
\begin{definition}[Teacher Attitude, \textbf{教师态度}]
...
\begin{definition}[Student Attitude, \textbf{学生态度}]
...
\subsection{Principle 1: Continuous Gradient with Bounded Steps (连续梯度，有界步长)}
...
\subsection{Principle 2: Gauge-Fixed Rubrics (规范固定的评分标准)}
...
\subsection{Principle 3: Dynamic Regrouping with Controlled Step Size (动态重组的受控步长)}
...
\subsection{Principle 4: Achievement Measured in Gain, Not Level (增量衡量，非水平衡量)}
...
\subsection{Principle 5: Interface Smoothing Through Peer Teaching (同伴教学的界面平滑)}
...
This work builds on the SCX Equality Principle (平等论) framework developed in~\cite{scx_moe_gauge}. We acknowledge the intellectual debt to the勢能面几何 formulation and the Theorem 7--12 chain. The application to education is novel to this paper; any errors in domain translation are ours alone.
...
\toprule
\textbf{Symbol} & \textbf{Meaning (English)} & \textbf{含义 (中文)} \\
\midrule
$\cS_{\text{edu}}(x)$ & Educational potential surface & 教育势能面 \\
$\gradS$ & Educational potential gradient & 教育势能梯度 \\
$\Delta_k$ & Potential step between strata $k$ and $k+1$ & 第 $k$ 级与 $k+1$ 级之间的势能台阶 \\
$\Delta_{\text{crit}}$ & Cliff threshold & 悬崖临界值 \\
$\mathbf{g}_m$ & Gauge posture of subsystem $m$ & 子系统 $m$ 的规范姿态 \\
$\mathbf{g}_T$ & Teacher gauge posture & 教师规范姿态 \\
$\mathbf{g}_S$ & Student gauge posture & 学生规范姿态 \\
$\delta_i$ & Gauge mismatch $\norm{\mathbf{g}_T - \mathbf{g}_{S_i}}$ & 规范错配 \\
$\eta$ & Learning rate (mobility) & 学习率（迁移率） \\
$\tau$ & Available time window & 可用时间窗口 \\
$\beta$ & Cumulative advantage coefficient & 累积优势系数 \\
$\kappa_k$ & Interface permeability & 界面渗透率 \\
$\Omega_k$ & $k$-th potential stratum & 第 $k$ 个势能层级 \\
$\sumgd$ & Equality Principle gauge condition & 平等论规范条件 \\
\bottomrule
\end{tabular}
\end{table}
