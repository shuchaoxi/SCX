...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Observer access Observers访问.} International and domestic election observers, party poll-watchers, and civil society organizations form a verification community that independently witnesses vote counting. When observers have access to multiple counting method outputs, they become an additional layer of independent verification.
...
\subsection{Observer Community Observers社区}

\begin{definition}[Observer Community Observers社区]
...
\begin{assumption}[A4: Observer Independence Observers独立性]
\label{ass:A4}
...
\begin{assumption}[A5: Positive Observer Detection Rate 正Observers检测率]
\label{ass:A5}
...
\begin{corollary}[Required Observer Multiplicity 所需Observers数量]
...
%=====================================================================
\section{Theorem 3: Observer Community Detection Power Observers社区检测力}
\label{sec:thm3}
%=====================================================================
...
\begin{definition}[Observer Detection Event Observers检测事件]
...
\begin{theorem}[Observer Community Detection Power Observers社区检测力]
...
\begin{corollary}[Minimum Observer Count for VERIFIED Certification 认证所需最少Observers数量]
...
\EndIf
\State \textbf{Phase 3: Observer Verification Observers验证}
\For{$j = 1$ to $K$}
...
\begin{table}[htbp]
\centering
\caption{Illustrative \Cercis{} scores for electoral systems 选举系统Cercis评分Example. $\eta = 0.3$.}
...
\subsection{Observer Access as a Continuous Variable Observers访问作为连续变量}
...
\begin{enumerate}[label=\limitationTag{\arabic*},leftmargin=*]
    ...
    \item \textbf{\limitationTag{5} Observer collusion reduces detection power Observers串通降低检测力.} Assumption~\ref{ass:A4} (observer independence) fails if observers collude. A coordinated observer community that agrees to report ``no discrepancy'' regardless of what they witness has $\alpha_{\min} \to 0$, collapsing the observer detection bound. Institutional diversity of observer organizations is the operational defense.
