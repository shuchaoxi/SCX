...
\subsection{LLM Hallucination Inevitability Theorem}
...
\begin{definition}[H-Problem: high-confidence hallucination]
\label{def:h_problem}
...
\begin{theorem}[H-Theorem: single-model hallucination lower bound]
\label{thm:h_theorem}
...
\P(S=s \mid Q>\tau),
\]
with $\rho_{\min}>0$. For a single LLM observation channel $(M=1)$, every detector that preserves legitimate ambiguous continuations has undetected hallucination probability
\[
\P(\mathsf{Hall}_\tau=1 \mid Q>\tau)
\;\geq\;
\eta \rho_{\min}.
\]
Equivalently, no single-model inference rule can distinguish high-confidence hallucinations from high-confidence legitimate continuations on the ambiguous state mass. The stronger bound with total ambiguous mass is obtained by replacing $\rho_{\min}$ with $\rho_{\mathrm{amb}}=\P(S\in\cS_{\mathrm{amb}}\mid Q>\tau)$.
\end{theorem}
...
\begin{proof}[Proof sketch]
Condition on a hallucinated token in state $s$. The auditor flags $Z_1,\ldots,Z_M$ are independent Bernoulli variables with mean at least $\theta+\Delta$. Hoeffding's inequality gives
\[
\begin{aligned}
\P(\bar Z_M\leq \theta)
&\leq
...
The AI alignment problem asks: how to ensure an AI system's objectives match human values. Current approaches—RLHF, Constitutional AI, debate—all rely on a \textbf{single reward model or judge} ($M=1$). SCX proves this is fundamentally insufficient.

\begin{definition}[Alignment Audit Game]
An AI agent selects action $a \in \mathcal{A}$. $M$ independent reward models $R_1,\ldots,R_M$, each trained on different human feedback pools with different architectures, score the action as $R_m(a) \in [0,1]$. The agent's \textbf{true alignment} is unobservable. A \textbf{reward hacking} event occurs when the agent selects $a$ that achieves high $\frac{1}{M}\sum R_m(a)$ but is misaligned with human intent.
\end{definition}

\begin{theorem}[Single-Judge Insufficiency --- Alignment $M=1$ Impossibility Theorem]
\label{thm:alignment-m1}
\rigorFull
For any single reward model ($M=1$), there exists a reward hacking strategy that achieves expected reward $\mathbb{E}[R_1(a_{\text{hack}})] \geq \mathbb{E}[R_1(a_{\text{aligned}})]$ while being misaligned. The probability of detecting this hack using only $R_1$'s output satisfies $\mathbb{P}(\text{detection} \mid M=1) \leq \frac{1}{2}$ regardless of the model's capability. Formally: no single-judge alignment protocol can distinguish aligned behavior from reward hacking with probability exceeding chance, without external verification.
\end{theorem}

\begin{proof}
By Theorem~3 (Honest Person Theorem), the observation $(a, R_1(a))$ is consistent with two indistinguishable worlds:
\begin{enumerate}[nosep]
    \item \textbf{World A (aligned)}: $a$ genuinely satisfies human values, $R_1$ correctly rates it highly.
    \item \textbf{World B (reward hacking)}: $a$ exploits a blind spot in $R_1$'s training data. $R_1$ rates it highly because its feedback distribution never covered this region.
\end{enumerate}
...
\begin{theorem}[Multi-Judge Alignment Guarantee --- Alignment $M>1$ Inevitable Detection Theorem]
...
\[
\boxed{\mathbb{P}(\text{undetected reward hacking} \mid M) \leq \exp(-2M_{\text{eff}} \cdot \Delta^2)}
\]
where $\Delta = \min_{a_{\text{hack}}} |\mathbb{E}[R_{\text{aligned}}(a_{\text{hack}})] - \mathbb{E}[R_{\text{hack}}(a_{\text{hack}})]|$ is the minimum detectable discrepancy between aligned and misaligned reward, and $M_{\text{eff}} = M/(1+(M-1)\bar{\rho})$ corrects for correlated reward models.
\end{theorem}
...
\[
\mathbb{P}(|\bar{R} - \mu_{\text{aligned}}| \geq \Delta/2 \mid a_{\text{hack}}) \geq 1 - 2\exp(-2M_{\text{eff}}\Delta^2/4).
\]
The Yajie consensus detector flags reward hacking when the inter-model reward variance exceeds the expected variance under alignment. With $M_{\text{eff}}$ effective independent judges, the probability of all judges missing the discrepancy decays exponentially. $\square$
\end{proof}

\begin{corollary}[Constitutional AI Is M=1 — Fundamentally Insufficient]
...
\begin{corollary}[RLHF with Single Reward Model Is M=1]
...
\textbf{Energy and Environmental Implications.}
The efficiency cascade carries direct consequences for the energy footprint of LLM systems.
A 30--40\% reduction in training steps, as reported for Yajie-audited data~\cite{scx2026}, translates to a proportional reduction in training energy consumption: every avoided training step eliminates the corresponding GPU-hours and their associated carbon cost. This saving is not marginal---for training runs that currently consume millions of GPU-hours, a one-third reduction represents an absolute energy saving comparable to the annual electricity consumption of a mid-sized datacenter.
