...
% ── Packages ────────────────────────────────────────────────────────────────
% Compile with XeLaTeX for Chinese (CJK) support
\usepackage{fontspec}
\usepackage{xeCJK}
...
\author{
  SCX Research Collective \\[4pt]
  \small SCX Technical Report SCX-TR-2026-009 \\
  \small June 2026
}
\date{}
...
(sim-to-real distribution shift, \emph{sim-to-real}~\textsuperscript{仿真到现实})
...
during deployment; the \textbf{Cercis Score}---a composite metric $Q \cdot N$
where $Q$ quantifies physical fidelity (conservation-law adherence, collision
validity) and $N$ captures scenario novelty (out-of-distribution environment
deviation); and \textbf{Yajie Consensus}---a Byzantine-robust protocol for
...
transfer validation on manipulation and locomotion tasks.  Throughout, we
interleave the English exposition with key Chinese-domain terminology
(\textbf{世界模型}~world model,~\textbf{物理仿真}~physical simulation,
\textbf{sim-to-real}~sim-to-real transfer) to anchor the work in the broader
...
(\textbf{世界模型}).  Systems such as NVIDIA Cosmos~\cite{cosmos2025},
...
(\textbf{sim-to-real}~\textsuperscript{仿真到现实})---the systematic
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item \textbf{Spring Online Monitoring} (Section~\ref{sec:spring}): A
    ...
    drift during deployment, with controlled false-alarm rate.
...
We formalize a learned world model (\textbf{世界模型}) as a composition of
$K$ expert modules (\textbf{物理仿真}~physical simulation modules), each
...
(\textbf{sim-to-real}~\textsuperscript{仿真到现实}).
...
(\textbf{雅捷共识}) resolves these conflicts through a Byzantine-resilient
...
(\textbf{世界模型}), establishing formal guarantees (Theorems~\ref{thm:cascade}
and~\ref{thm:unident}), runtime monitoring (Spring), a composite trust metric
(Cercis Score), and a Byzantine-resilient consensus protocol (Yajie,
\textbf{雅捷共识}).  The framework addresses a critical gap in the current
...
\paragraph{Sim-to-Real Transfer} (\textbf{sim-to-real}~\textsuperscript{仿真到现实}).
...
(\textbf{世界模型}, \textbf{物理仿真}, \textbf{仿真到现实},
\textbf{雅捷共识}) to facilitate adoption in the broader international
simulation-verification community.

\subsection{Conclusion}

As learned world models (\textbf{世界模型}) become the backbone of
...
simulation} (\textbf{物理仿真}), where every sim-to-real transfer
...
sim-to-real transfer community (\textbf{仿真到现实}) and the formal
