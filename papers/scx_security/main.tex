...
\medskip
\normalsize\textbf{SCX审计网络安全：多传感器共识认证入侵检测}}
...
\medskip
\noindent\textbf{Keywords:} intrusion detection 入侵检测, false positive reduction 误报降低, zero-day attacks 零日漏洞, multi-sensor consensus 多传感器共识, SCX auditing, \Cercis{} scoring, \Yajie{} consensus, \Spring{} gating, SIEM aggregation, cybersecurity certification 网络安全认证
\end{abstract}

% ======================================================================
\section{Introduction 引言 入侵检测的认证鸿沟}
% ======================================================================
...
\subsection{The Cybersecurity Audit Gap 网络安全审计空白}
...
\midrule
IDS/IPS sensor & Expert $m \in [M]$ & 专家/传感器 \\
Alert & Claim about traffic state & 告警声明 \\
False positive & Noise (sensor error) & 误报/噪声 \\
True attack & Signal (genuine anomaly) & 真实攻击/信号 \\
Multi-sensor correlation & \Yajie{} consensus & 雅洁共识 \\
Zero-day detection & \Spring{} regime shift & 零日漏洞检测 \\
SIEM aggregation & Audit layer & 审计层 \\
Benign traffic baseline & Historical regime $r_0$ & 良性流量基线 \\
Attack campaign & Anomalous regime $r_a$ & 攻击活动 \\
\bottomrule
\end{tabular}
\end{center}
...
% ======================================================================
\section{Formalization: Network Intrusion Detection as State-Conditioned Expert Prediction 网络入侵检测作为状态条件专家预测的形式化}
\label{sec:formalization}
% ======================================================================

\subsection{Network Traffic State 网络流量状态}

\begin{definition}[Network Traffic State Space 网络流量状态空间]
\label{def:state}
...
\begin{definition}[False Positive — 误报]
...
\begin{definition}[False Negative — 漏报]
...
\subsection{IDS/IPS Sensors as Experts 传感器作为专家}

\begin{definition}[Intrusion Detection Expert 入侵检测专家]
\label{def:expert}
...
\assumptionTag{2} \textbf{(Bounded Alert Signal 有界告警信号)}: The alert space $\{0, 1\}^{M}$ is compact with diameter $B = \sqrt{M}$, bounded by construction since each alert is binary.

\assumptionTag{3} \textbf{(Attack Base Rate 攻击基础率)}: The prior probability of any attack in a given time window satisfies $\Pbb(r_t \neq r_{\mathrm{benign}}) = \pi_a \in (0, 1)$. For operational networks, $\pi_a \ll 1$ (typically $\pi_a < 10^{-3}$), which drives the false-positive problem.
...
% ======================================================================
\section{Theorem 1: Multi-Sensor Consensus Detection 多传感器共识检测定理}
\label{sec:theorem1}
% ======================================================================
...
\begin{definition}[Effective Sensor Multiplicity 有效传感器多重性]
\label{def:Meff}
...
\begin{theorem}[Multi-Sensor Attack Miss Bound 多传感器攻击漏报界]
...
\begin{corollary}[Required Sensor Multiplicity 所需传感器多重性]
...
\begin{corollary}[Family Diversity Tradeoff 传感器族多样性的权衡]
\label{cor:diversity}
...
% ======================================================================
\section{Theorem 2: False Alarm vs.\ Novel Attack Unidentifiability 误报与新型攻击的不可辨识性}
\label{sec:theorem2}
% ======================================================================
...
\subsection{The Alert Ambiguity Problem 告警歧义问题}
...
\begin{theorem}[False Positive vs.\ Zero-Day Attack Unidentifiability 误报与零日漏洞攻击不可辨识性]
...
\begin{corollary}[Multi-Sensor Resolution of Ambiguity 多传感器消解歧义]
...
\subsection{Assumption Mandate for Alert Attribution 告警归因的假设声明要求}

\begin{corollary}[Assumption Mandate for Alert Attribution 告警归因的假设声明要求]
...
\begin{remark}[The Alert Triage Paradox 告警分诊悖论]
\label{rem:triage}
...
% ======================================================================
\section{\Cercis{} Score for Intrusion Detection 入侵检测的Cercis评分}
\label{sec:cercis}
% ======================================================================
...
\subsection{Quality Score $Q$: Detection Accuracy 检测准确度}

\begin{definition}[Cybersecurity Quality Score 网络安全质量评分]
\label{def:Q_cyber}
...
\begin{definition}[Attack-Aware Quality 针对攻击的质量]
\label{def:Q_attack}
...
\subsection{Novelty Score $N$: Traffic Pattern Coverage 流量模式覆盖}

\begin{definition}[Traffic Novelty Score 流量新颖度评分]
\label{def:N_cyber}
...
\begin{definition}[\Cercis{} Score for Intrusion Detection 入侵检测的Cercis评分]
...
\begin{proposition}[\Cercis{} Monotonicity with Detection Diversity \Cercis评分与检测多样性的单调性]
...
% ======================================================================
\section{\Yajie{} Multi-Sensor Consensus 雅洁多传感器共识}
\label{sec:yajie}
% ======================================================================
...
\begin{definition}[\Yajie{} Consensus Vector 雅洁共识向量]
...
\begin{definition}[Dynamic \Yajie{} Weighting 动态雅洁加权]
...
\begin{theorem}[\Yajie{} Consensus False-Positive Bound 雅洁共识误报界]
\label{thm:yajie_fp}
\rigorFull
...
\begin{corollary}[Certified Intrusion Verdict 认证入侵裁决]
...
\begin{remark}[Why Weighted Consensus Beats Majority Vote 为何加权共识优于多数投票]
...
% ======================================================================
\section{\Spring{} Gating: Zero-Day Detection as Regime-Shift Testing 零日漏洞检测作为制度变更检验}
\label{sec:spring}
% ======================================================================
...
\subsection{Sequential Alert Process 序贯告警过程}

\begin{definition}[Alert Count Process 告警计数过程]
...
\begin{definition}[\Spring{} CUSUM Statistic Spring CUSUM统计量]
...
\begin{theorem}[\Spring{} Detection Delay Bound Spring检测延迟界]
...
\begin{definition}[Zero-Day Detection Protocol 零日漏洞检测协议]
...
\begin{proposition}[Zero-Day Detection Guarantee 零日漏洞检测保证]
...
% ======================================================================
\section{SIEM Aggregation as the SCX Audit Layer SIEM聚合作为SCX审计层}
\label{sec:siem}
% ======================================================================
...
\begin{definition}[Cybersecurity Audit Layer 网络安全审计层]
...
\begin{proposition}[SIEM Completeness Guarantee SIEM完备性保证]
...
\begin{remark}[The Cost of Certification 认证的成本]
\label{rem:cost}
...
\subsection{Sensor Configuration 传感器配置}
...
\subsection{Experimental Design 实验设计}
...
\subsection{Contributions Summary 贡献总结}
...
\midrule
IDS/IPS sensor & 入侵检测/防御传感器 & Expert $m$ \\
Alert & 告警 & Claim \\
False positive & 误报 & Noise \\
Multi-sensor consensus & 多传感器共识 & \Yajie{} \\
Zero-day detection & 零日漏洞检测 & \Spring{} gating \\
Intrusion certification & 入侵认证 & Audit layer \\
Sensor quality score & 传感器质量评分 & \Cercis{} Score \\
Attack regime & 攻击制度 & Regime $r_t$ \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Implications for SOC Operations 对安全运营中心的影响}
...
\subsection{Honest Uncertainty Communication 诚实的不确定性沟通}
