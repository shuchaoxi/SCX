...
\begin{abstract}
We formalize news verification as a multi-expert audit problem under the \SCX{} (Structured Causal eXamination) framework. News sources are modeled as an expert community $\sourceSet = \{S_1, \ldots, S_M\}$; each published claim $c \in \claimSpace$ is an auditable assertion about a ground-truth event $\theta \in \newsState$. Multi-source corroboration is operationalized through the \Yajie{} consensus mechanism: when $M$ independent sources converge on the same factual claim, their agreement provides an exponentially decaying upper bound on the probability of collective error. We prove four core theorems. \textbf{Theorem~1 (Multi-Source Corroboration Bound 多源印证界):} Under conditional source independence, the probability that the \Yajie{} consensus deviates from the true event by more than $\varepsilon$ satisfies $\Pbb(\norm{c - \theta}_\infty > \varepsilon) \leq \exp(-2M_{\mathrm{eff}} \cdot \varepsilon^2 / \bar{\sigma}^2)$, establishing that verification confidence grows exponentially with the number of corroborating sources. \textbf{Theorem~2 (Single-Source Unverifiability 单源不可验证性):} When $M = 1$, the detection probability for any misreporting is bounded above by a constant $p_{\max} < 1$ that depends on the source's precision but is independent of claim magnitude---formalizing why fake news (单源假新闻) is epistemically indistinguishable from truth under single-source conditions. \textbf{Theorem~3 (Anonymity-Preserving Consensus 匿名性保护共识):} The \Yajie{} consensus computed under \VISprivate{} source visibility---where auditor identities are hidden but estimates are shared---preserves the same asymptotic convergence rate as the fully public \VISpublic{} mode, proving that source protection does not degrade verification power. \textbf{Theorem~4 (Fact-Checker Network Resilience 事实核查网络韧性):} When a fraction $f$ of sources in a fact-checker network are adversarial (deliberately propagating false claims), the \Yajie{} consensus remains within $\varepsilon$ of truth provided $f < \frac{1}{2} - \frac{\bar{\sigma}^2 \log(1/\delta)}{2M\varepsilon^2}$, establishing a breakdown threshold for coordinated disinformation campaigns. We develop the \Cercis{} veracity score $S(c) = Q(c) + \eta \cdot N(c)$ for ranking claims by corroboration quality and source novelty, the \Situs{} encoding for representing source-claim attribution graphs with privacy-preserving visibility modes, and an experimental protocol with specific benchmark datasets (LIAR, FEVER, FakeNewsNet, CHEF). All theorems carry full proofs. Eleven explicit assumptions are stated and verified.

\medskip
\noindent\textbf{Keywords:} news verification 新闻核查, fact-checking 事实核查, multi-source corroboration 多源验证, fake news detection 假新闻检测, \Yajie{} consensus, \SCX{} auditing, source anonymity 来源匿名, certified fact-checking 认证式事实核查, expert ensemble, disinformation resilience
\end{abstract}
...
$M = 1$ & Single-source claim (potential fake news 潜在假新闻) \\
Auditor community $\cA$ & Fact-checker network 事实核查网络 \\
...
\begin{enumerate}[label=(\roman*)]
    ...
    \begin{itemize}
        \item Theorem~\ref{thm:corroboration}: Multi-Source Corroboration Bound (多源印证界)---exponential confidence growth with source count.
        \item Theorem~\ref{thm:single_source}: Single-Source Unverifiability (单源不可验证性)---formal bound on fake news detectability at $M = 1$.
        \item Theorem~\ref{thm:anonymity}: Anonymity-Preserving Consensus (匿名性保护共识)---\VISprivate{} visibility preserves verification power.
        \item Theorem~\ref{thm:resilience}: Fact-Checker Network Resilience (事实核查网络韧性)---breakdown threshold for adversarial source contamination.
    \end{itemize}
...
%=====================================================================
\section{Formalization: News as a Multi-Source Assertion System 新闻作为多源断言系统}
\label{sec:formalization}
%=====================================================================

\subsection{The Event Space 事件空间}

\begin{definition}[News Event 新闻事件]
\label{def:event}
...
\begin{definition}[News Claim 新闻声明]
\label{def:claim}
...
\begin{equation}
    ...
    \label{eq:claim}
\end{equation}
where $c$ may or may not equal $\theta_t$. A claim is \textbf{true} if $\norm{c - \theta_t}_\infty \leq \varepsilon_{\mathrm{tol}}$ for a tolerance $\varepsilon_{\mathrm{tol}} > 0$ representing practical equivalence, and \textbf{false} (fake news 假新闻) otherwise.
\end{definition}

\subsection{The Source Community as Expert Ensemble 来源社区作为专家集成}

\begin{definition}[News Source 新闻来源]
\label{def:source}
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Access modality 获取方式:} Direct eyewitness, secondary reporting, document analysis, data journalism, satellite imagery, leaked documents, official press releases.
    \item \textbf{Institutional affiliation 机构隶属:} Wire services (Reuters, AP, AFP, Xinhua), national broadcasters, independent investigative outlets, citizen journalism platforms, government press offices, NGO reports.
    \item \textbf{Methodological approach 方法论:} Investigative interviewing, document triangulation, data forensics, open-source intelligence (OSINT), geolocation verification, metadata analysis.
    \item \textbf{Epistemic stance 认识论立场:} Neutral reporting, advocacy journalism, adversarial interviewing, collaborative verification.
\end{enumerate}

\begin{definition}[Multi-Source Corroboration 多源印证]
...
\begin{remark}[Yajie Consensus as Epistemic Aggregation Yajie共识作为认识论聚合]
...
\subsection{Visibility Modes and Source Anonymity 可见性模式与来源匿名}

\begin{definition}[Visibility Modes 可见性模式]
\label{def:visibility}
Each source operates under one of three visibility modes:
\begin{itemize}
    \item \textbf{\VISpublic{} (公开):} Both the source identity and its claim are publicly attributed. Example: a named journalist publishing under a byline in a major newspaper.
    \item \textbf{\VISprivate{} (匿名保护):} The claim is published but the source identity is cryptographically protected. The system can verify that the source is a member of the authorized source set $\sourceSet$ (via zero-knowledge membership proof) without revealing which member. Example: a whistleblower providing documents through SecureDrop; a confidential informant in an authoritarian regime.
    \item \textbf{\VISblind{} (盲审):} Neither the source identity nor the raw claim is visible to other sources. Sources submit encrypted claims; the consensus is computed via secure multi-party computation (MPC). This prevents herding behavior where later sources converge on earlier-published claims.
\end{itemize}
\end{definition}
...
\subsection{The Fact-Checker Network as Auditor Community 事实核查网络作为审计者社区}

\begin{definition}[Fact-Checker Network 事实核查网络]
...
\begin{assumption}[A8: Temporal Coherence 时间Consistency]
\label{ass:A8}
...
%=====================================================================
\section{Theorem 1: Multi-Source Corroboration Bound 多源印证界}
...
\begin{lemma}[Consensus Concentration 共识集中不awaiting式]
...
\begin{remark}[Comparison with Classical Hoeffding Hoeffding经典比较]
...
\begin{theorem}[Multi-Source Corroboration Bound 多源印证界]
...
\begin{corollary}[Exponential Confidence Growth 指数置信Growth]
...
%=====================================================================
\section{Theorem 2: Single-Source Unverifiability 单源不可验证性}
...
\begin{theorem}[Single-Source Unverifiability 单源不可验证性]
...
\begin{remark}[Fake News as the $M=1$ Regime 假新闻作为$M=1$Status]
...
%=====================================================================
\section{Theorem 3: Anonymity-Preserving Consensus 匿名性保护共识}
\label{sec:anonymity}
%=====================================================================
...
\begin{theorem}[Anonymity-Preserving Consensus 匿名性保护共识]
\label{thm:anonymity}
...
\begin{corollary}[Whistleblower Protection Without Verification Loss 举报人保护不损失验证力]
...
%=====================================================================
\section{Theorem 4: Fact-Checker Network Resilience 事实核查网络韧性}
...
\begin{definition}[Adversarial Source Contamination 对抗性来源污染]
...
\begin{theorem}[Fact-Checker Network Resilience 事实核查网络韧性]
...
\begin{remark}[Relationship to Byzantine Fault Tolerance 与拜占庭容错的关系]
...
%=====================================================================
\section{The Cercis Veracity Score 新闻真实性的Cercis评分}
\label{sec:cercis}
%=====================================================================

\begin{definition}[Cercis Veracity Score Cercis真实性评分]
\label{def:cercis}
...
\begin{proposition}[Properties of the Cercis Score Cercis评分的性质]
...
%=====================================================================
\section{The Situs Source-Claim Encoding Situs来源-声明编码}
\label{sec:situs}
%=====================================================================
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{CHEF}~\cite{hu2023}: Chinese fact extraction and verification dataset (中文事实抽取与验证). We evaluate specifically on the multi-source corroboration scenario where multiple Chinese-language news outlets report on the same event.
\end{enumerate}
...
%=====================================================================
\section{The Spring Gating for Regime Detection Spring门控的制度检测}
\label{sec:spring}
%=====================================================================

\begin{definition}[Reporting Regime 报道制度]
\label{def:regime}
The \Spring{} gating mechanism detects transitions between two reporting regimes:
\begin{itemize}
    \item \textbf{Normal regime 正常制度 ($R_0$)}: Sources independently report with bounded errors; $\Gamma(M) \approx 1$ for sufficient $M$; adversarial fraction $f \approx 0$.
    \item \textbf{Disinformation regime 虚假信息制度 ($R_1$)}: A coordinated fraction $f > f_{\mathrm{break}}$ of sources publish coherently false claims; $\Gamma(M)$ degrades; the \Yajie{} consensus is biased.
\end{itemize}
\end{definition}
...
\begin{proposition}[Spring Gate False Alarm Rate Spring门控误报率]
...
\subsection{Relationship to Journalism Ethics 与新闻伦理的关系}
...
\subsection{Relationship to Automated Fact-Checking 与自动事实核查的关系}
