...
\small SCX审计法律证据：多证人认证与交叉质询的数学基础}
...
\medskip
\noindent\textbf{Keywords:} SCX auditing, legal evidence 法律证据, witness certification 证人认证, cross-examination 交叉质询, evidence chain 证据链, hearsay rule 传闻证据规则, perjury detection 伪证检测, multi-expert verification 多专家验证, cryptographic hash chain 加密哈希链, \Yajie{} consensus, \Cercis{} scoring, adversarial audit 对抗审计
\end{abstract}
...
\midrule
Witness & Expert & 证人 $\leftrightarrow$ 专家 \\
Testimony & Claim with declared assumptions & 证言 $\leftrightarrow$ 声明+假设 \\
Cross-examination & Audit procedure $\crossexam$ & 交叉质询 $\leftrightarrow$ 审计 \\
Corroboration & \Yajie{} consensus & 佐证 $\leftrightarrow$ 共识 \\
Contradiction & Discrepancy signal $\discrepancy$ & 矛盾 $\leftrightarrow$ 差异 \\
Evidence chain & Cryptographic hash chain & 证据链 $\leftrightarrow$ 哈希链 \\
Hearsay & Self-audit loop (Theorem~2) & 传闻 $\leftrightarrow$ 自审计 \\
Perjury & Active evil / fabrication regime (Theorem~3) & 伪证 $\leftrightarrow$ 主动作恶 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \begin{itemize}
        \item \textbf{Theorem~\ref{thm:corroboration}} (Multi-Witness Corroboration 多证人佐证): With $M$ independent witnesses, the probability of a false fact surviving unanimous corroboration decays exponentially in $M$.
        \item \textbf{Theorem~\ref{thm:hearsay}} (Hearsay as Self-Audit 传闻自审计): A hearsay claim provides zero independent corroboration and is mathematically equivalent to a self-audit loop.
        \item \textbf{Theorem~\ref{thm:perjury}} (Perjury Detection via Adversarial Audit 伪证检测): Adversarial cross-examination detects active perjury with probability exponentially close to 1 in the number of independent questions $Q$.
    \end{itemize}
...
%=====================================================================
\section{Formalization: Legal Evidence as Multi-Expert Claims 法律证据作为多专家声明系统}
\label{sec:formalization}
%=====================================================================

\subsection{The Fact Space 事实空间}

\begin{definition}[Fact 法律事实]
\label{def:fact}
...
\begin{definition}[Compound Fact 复合事实]
\label{def:compound}
...
\subsection{Witnesses as Experts 证人即专家}

\begin{definition}[Witness 证人]
\label{def:witness}
...
\begin{definition}[Witness Independence 证人独立性]
...
\subsection{Corroboration and Discrepancy 佐证与差异}

\begin{definition}[Corroboration 佐证]
...
\begin{definition}[Discrepancy 差异/矛盾]
...
\subsection{Evidence Chain as Hash Chain 证据链即哈希链}

\begin{definition}[Evidence Chain 证据链]
...
\begin{definition}[Tamper Evidence 篡改检测]
\label{def:tamper}
...
\subsection{Cross-Examination as Audit 交叉质询即审计}

\begin{definition}[Cross-Examination Operator 交叉质询算子]
\label{def:crossexam}
...
\subsection{Perjury: The Active Evil Regime 伪证：主动作恶制度}

\begin{definition}[Perjury Regime 伪证制度]
\label{def:perjury}
...
\begin{remark}[Perjury vs. Error 伪证与错误的区别]
...
%=====================================================================
\section{Theorem 1: Multi-Witness Corroboration Detection 多证人佐证检测}
\label{sec:corroboration}
%=====================================================================

\subsection{Statement}

\begin{theorem}[Multi-Witness Corroboration Bound 多证人佐证界]
...
\begin{corollary}[Corroboration Threshold 佐证阈值]
\label{cor:threshold}
...
\begin{corollary}[Partial Corroboration 部分佐证]
\label{cor:partial}
...
%=====================================================================
\section{Theorem 2: Hearsay as Self-Audit 传闻证据的自审计不可辨识性}
\label{sec:hearsay}
%=====================================================================

\subsection{Statement}

\begin{theorem}[Hearsay Self-Audit Unidentifiability 传闻自审计不可辨识性]
\label{thm:hearsay}
...
\begin{corollary}[Hearsay Exclusion Justification 传闻排除的数学理由]
...
\begin{corollary}[Multiple Hearsay 多重传闻]
...
%=====================================================================
\section{Theorem 3: Perjury Detection via Adversarial Cross-Examination 伪证检测与对抗性质询}
\label{sec:perjury}
%=====================================================================

\subsection{Statement}

\begin{theorem}[Perjury Detection via Adversarial Audit 对抗审计伪证检测]
\label{thm:perjury}
...
\begin{corollary}[Evidence Chain Integrity 证据链完整性]
...
\begin{corollary}[Optimal Cross-Examination Strategy 最优质询策略]
...
%=====================================================================
\section{Multi-Witness \Yajie{} Consensus Protocol 多证人Yajie共识协议}
\label{sec:yajie}
%=====================================================================

\subsection{Correlated Witnesses and Effective Sample Size 相关证人与有效样本量}
...
\begin{definition}[Witness Correlation Matrix 证人相关矩阵]
\label{def:corrmat}
...
\begin{definition}[\\Yajie{} Consensus for Witnesses Yajie证人共识]
...
\begin{proposition}[Consensus Accuracy 共识准确性]
...
%=====================================================================
\section{\Cercis{} Evidence Score Cercis证据评分}
\label{sec:cercis}
%=====================================================================

\begin{definition}[\\Cercis{} Evidence Score Cercis证据评分]
\label{def:cercis}
...
%=====================================================================
\section{\Spring{} Gating for Perjury Regime Detection Spring门控伪证制度检测}
\label{sec:spring}
%=====================================================================
...
\begin{definition}[Testimony Regime 证言制度]
\label{def:regime}
A witness operates in one of two regimes:
\begin{itemize}
    \item \textbf{Regime 0 (Credible 可信):} $r_k \sim P_{\mathrm{true}}(\cdot \mid \fact)$, centered at the true answer with variance $\sigma_w^2$.
    \item \textbf{Regime 1 (Perjurious 伪证):} $r_k \sim P_{\mathrm{fab}}(\cdot)$, inconsistent with $\fact$ and with prior responses (Definition~\ref{def:perjury}).
\end{itemize}
...
\begin{proposition}[\\Spring{} Detection of Perjury Transition Spring伪证转换检测]
...
\subsection{Evidence Chain Verification Protocol 证据链验证协议}
...
\subsection{Cross-Examination Strategy Optimization 交叉质询策略优化}

\begin{proposition}[Optimal Question Allocation 最优问题分配]
...
\subsection{Hearsay-Contaminated Corroboration 传闻污染的佐证}

\begin{proposition}[Hearsay Contamination Adjustment 传闻污染调整]
...
\midrule
Thm.~1 & Honest noise (error) & Multi-witness consensus & 多证人佐证 \\
Thm.~2 & Information laundering (hearsay) & Self-audit identification & 传闻自审计 \\
Thm.~3 & Active evil (perjury) & Adversarial cross-examination & 伪证对抗检测 \\
\bottomrule
\end{tabular}
\end{table}
...
The SCX Research Collective acknowledges the legal scholars, forensic scientists, and cryptographers whose work informed this synthesis. The framework builds on foundational results in hypothesis testing~\cite{Hoeffding1963}, cryptographic hash functions~\cite{Merkle1980}, and change-point detection~\cite{Page1954}.
