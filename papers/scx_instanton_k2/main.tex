...
\title{
    ...
    \normalsize SCX  · C8  \\ SCX Gauge Theory · C8 Extension
}
\author{SCX}
\date{20267}
...
\noindent
\textbf{.}
SCXSitusC71-cycleholonomy——C8Situs$k>1$2-``''Audit Flux2-

(1) Situs$k$-Hodge$A = A_{\mathrm{exact}} + A_{\mathrm{coexact}} + A_{\mathrm{harmonic}}$2-$F = \dd_1 A$co-exact part(2) Situs2-2-``''circular inconsistency——(3) 2-2-skeleton$F$$\norm{\int_\Sigma F} > \tau$2-(4) persistent homology——density filtration$H_2$$\texttt{verify\_instanton\_k2.py}$
...
C7``1-Cycle Audit Instanton''$E_i$$E_j$1-$A_{ij}$$\hol_\gamma(A) = \oint_\gamma A$$\gamma$C7SCX1-——\textbf{$k=1$}

C8``Higher-Dimensional Audit Instantons''$k > 1$Situs2-2-$F = \dd_1 A$1-——``''circular inconsistency2-$F$$\int_\Sigma F$
...
\begin{enumerate}[label=\textbf{C\arabic*}, leftmargin=*]
    \item \textbf{Hodge.} Situs$k$-Hodge1-$A$exact partco-exact partharmonic part$F = \dd_1 A$
    \item \textbf{2-.} $F \neq 0$2-1-cocycle condition
    \item \textbf{.} 2-$\int_\Sigma F \neq 0$``''——pairwise agreement
    \item \textbf{.} 2-$O(N^3)$$N$
    \item \textbf{.} 2$H_2$
\end{enumerate}
...
\noindent
\textbf{.}
Situs$C^k(\SSS; \R)$$k$-coboundary operator$\dd_k: C^k \to C^{k+1}$
...
\end{equation}
 ...
 $A$$A$$A_{(E_i, E_j)} = (\alpha_{ij}, \beta_{ij})$$A_{(E_j, E_i)} = (-\alpha_{ij}/\beta_{ij}, 1/\beta_{ij})$$\beta_{ij} \neq 0$
...
\noindent
\textbf{.}
Situs$\{E_i\}$$(E_i, E_j)$$A$$(\alpha_{ij}, \beta_{ij})$2-$(E_i, E_j, E_k)$$F_{ijk}$
...
\begin{equation}\label{eq:eval_density}
    \rho(\sigma) = \frac{\#\{\text{}\sigma\text{}\}}{\#\{\text{}\}}.
\end{equation}
...
\begin{enumerate}[label=(\arabic*), leftmargin=*]
    \item \textbf{1-2-.} C71-1-SCXgauge-able``''$k=2$——1-$A$$\dd A = 0$2-$F$
    \item \textbf{Hodge.} Hodge$A$co-exact part$F$$A_{\mathrm{coexact}}$
    \item \textbf{.} $H_2$$N$
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*), leftmargin=*]
    \item \textbf{$k \geq 3$.} $k=2$$k \geq 3$$k$-$F^{(k)} = \dd_{k-1} A^{(k-1)}$Hodge$F^{(k)}$``''
    \item \textbf{.} ~\ref{thm:info_bound}HoeffdingBernstein
    \item \textbf{.} $O(N^3 M)$$N$$M$sketchingcoreset
    \item \textbf{SCX.} AR-TheoremTS-TheoremRA-TheoremSCX
    \item \textbf{.} ``''``''``''
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*), leftmargin=*]
    \item \textbf{.} 1-$A$$S_i(x)-S_j(x)$$F$$(\alpha_{ij}, \beta_{ij})$$F_{ijk}$
    \item \textbf{Hodge.} $\alpha_{ij}$1-co-exact part$F$$F \neq 0$——$c_i - c_j$
    \item \textbf{2-.} $K_N$2-$F_{ijk}$——
    \item \textbf{.} $O(N^2 M)$$O(N^3)$
    \item \textbf{.} $H_2$
\end{enumerate}
...
\noindent
\textbf{.}
 \texttt{verify\_instanton\_k2.py} 
\begin{itemize}
