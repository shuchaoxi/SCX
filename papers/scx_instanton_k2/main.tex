% ============================================================================
%  SCX C8 Extension: Higher-Dimensional Audit Instantons
...
\title{
    \textbf{Higher-Dimensional Audit Instantons: 2-Form Flux on the Situs Complex\\and Topological Detection of Circular Inconsistency}\\[0.5em]
    \normalsize SCX Gauge Theory \textperiodcentered C8 Extension
}
...
\subsection{From C7 to C8: The Dimensional Leap of Audit Instantons}
...
% ============================================================================
\section{Preliminaries: Situs Complex and Differential Forms}
\label{sec:prelim}
% ============================================================================

\subsection{Construction of the Situs Complex}

\begin{definition}[Situs Complex]
...
% -------------------------------------------------------------------
\subsection{Cochain Complex and Coboundary Operator}
\label{sec:cochain}
...
% -------------------------------------------------------------------
\subsection{Audit Instanton Field: Formal Definition}
\label{sec:instanton_def}

\begin{definition}[Audit Calibration Connection]
\label{def:instanton}
Unlike the conventional approach (defining $A$ directly as raw score discrepancies $S_i(x)-S_j(x)$), we define the audit instanton field as the \textbf{calibration model} between experts.

Let $\mathcal{E} = \{E_1, \ldots, E_N\}$ be the set of experts. The 0-simplices of the Situs complex are experts $E_i$, and the 1-simplices are ordered expert pairs $(E_i, E_j)$. For each expert pair $(E_i, E_j)$, estimate the calibration model via linear regression:
...
\textbf{Key distinction:} $A$ is the \textbf{model parameter} of the score mapping between experts, not the raw score discrepancy. Under reversal, $A_{(E_i, E_j)} = (\alpha_{ij}, \beta_{ij})$ and its inverse is $A_{(E_j, E_i)} = (-\alpha_{ij}/\beta_{ij}, 1/\beta_{ij})$ (when $\beta_{ij} \neq 0$).

The 2-form field strength (audit flux) $F = \dd_1 A$ on a 2-simplex $\Sigma = (E_i, E_j, E_k)$ is defined as the \textbf{non-transitivity} of calibration model composition:
...
\textbf{Physical meaning:} $F_{ijk}$ measures the inconsistency between the indirect calibration $E_i \to E_k$ via $E_j$ and the direct calibration $E_i \to E_k$. If all pairwise calibration models are transitive (i.e., $F_{ijk}=0$ for all $(i,j,k)$), then a globally consistent reference frame exists.
...
% ============================================================================
\section{Death of the 1-Cycle Audit Instanton}
\label{sec:death}
% ============================================================================

\begin{theorem}[1-Cycle Instanton Death Theorem]
\label{thm:C7}
Let $S_i(x) = S^*(x) + c_i$ hold for all experts $E_i$ and data points $x$ (i.e., all experts share the same underlying scoring function, differing only by constant offsets). Then the pairwise calibration model is $A_{(E_i, E_j)} = (c_i - c_j, 1)$, and:

\begin{enumerate}[label=(\roman*), leftmargin=*]
    \item $F_{ijk} = 0$ for all expert triples (field strength zero, flat connection);
    \item Calibration composition is transitive: $A_{(E_i, E_j)} \circ A_{(E_j, E_k)} = A_{(E_i, E_k)}^{-1}$;
    \item A global reference frame exists: all expert scores can be normalized to the same basis via constant offsets $c_i$.
\end{enumerate}
...
If $F_\Sigma = 0$ for all 2-simplices $\Sigma$, then there exists a global 0-cochain $\phi$ (the ``true Cercis scoring function'') such that all expert scores can be expressed as $S_i(x) = \phi(x) + c_i$, where $c_i$ are expert-dependent constant offsets. This means all inconsistencies are \textbf{gauge-able}.
\end{corollary}

% ============================================================================
\section{Higher-Dimensional Audit Instantons: Non-Flat Configurations at $k>1$}
\label{sec:nonflat}
% ============================================================================

\subsection{Beyond Linear Frameworks: Emergence of Non-Flat Connections}
...
\label{thm:hodge}
For a finite simplicial complex $\SSS$, the $k$-cochain space $C^k(\SSS; \R)$ admits an orthogonal direct sum decomposition:
...
\begin{itemize}
    \item $\im \dd_{k-1}$: \textbf{exact $k$-cochains} --- expressible as the coboundary of a lower-dimensional cochain;
    \item $\im \delta_k$: \textbf{co-exact $k$-cochains} --- expressible as the codifferential of a higher-dimensional cochain;
    \item $\ker \Delta_k$: \textbf{harmonic $k$-cochains} --- satisfying $\Delta_k \omega = 0$, equivalently $\dd_k\omega = 0$ and $\delta_{k-1}\omega = 0$. Isomorphic to the cohomology group $H^k(\SSS; \R)$.
\end{itemize}
\end{theorem}
...
% -------------------------------------------------------------------
\subsection{Relationship Between Field Strength and Co-Exact Part}
\label{sec:F_coexact}

\begin{theorem}[Field-Strength Co-Exact Relation]
\label{thm:F_coexact}
Let $A \in C^1(\SSS; \R)$ be a 1-form audit connection with Hodge decomposition:

\begin{equation}\label{eq:A_hodge}
    A = A_{\mathrm{exact}} + A_{\mathrm{coexact}} + A_{\mathrm{harmonic}},
\end{equation}

where $A_{\mathrm{exact}} \in \im \dd_0$, $A_{\mathrm{coexact}} \in \im \delta_1$, $A_{\mathrm{harmonic}} \in \ker \Delta_1$.

Then the 2-form field strength $F = \dd_1 A$ satisfies:
...
In particular:
\begin{enumerate}[label=(\roman*), leftmargin=*]
    \item $F = 0$ if and only if $A_{\mathrm{coexact}} = 0$ (on the subspace where $\dd_1$ is injective);
    \item $A_{\mathrm{exact}}$ and $A_{\mathrm{harmonic}}$ contribute nothing to $F$;
    \item $F_{\mathrm{norm}} \equiv \norm{F}_2 = \norm{\dd_1 A_{\mathrm{coexact}}}_2$, which can serve as a measure of the size of the co-exact component.
\end{enumerate}
\end{theorem}
...
(iii) $F_{\mathrm{norm}}$ is the operator norm of $\dd_1$ restricted to $\im \delta_1$; since $\dd_1$ is a bounded operator on a finite-dimensional space, $\norm{F}_2$ is proportional to $\norm{A_{\mathrm{coexact}}}_2$ (via the discrete Poincar\'e inequality on compact complexes). $\hfill\square$
\end{proof}

\begin{corollary}[Spectral Signature of Audit Inconsistency]
\label{cor:spectral}
Regard $F_{\mathrm{norm}}$ as a global measure of audit inconsistency. For $N$ experts across $M$ data points, define the inconsistency matrix $\mathbf{D} \in \R^{N \times N}$:
...
\begin{enumerate}[label=(\roman*), leftmargin=*]
    \item All inconsistencies are gauge-able, i.e., there exist offsets $\{c_i\}$ such that $S_i(x) - c_i = S_j(x) - c_j$ for all $i,j,x$;
    \item $F_\Sigma = 0$ for all 2-simplices $\Sigma$;
    \item $\mathbf{D}$ satisfies the triangle inequality $\mathbf{D}_{ik} \leq \mathbf{D}_{ij} + \mathbf{D}_{jk}$ and $\mathbf{D}_{ii} = 0$, i.e., $\mathbf{D}$ is a metric;
    \item $A_{\mathrm{coexact}} = 0$ in the Hodge decomposition.
\end{enumerate}
\end{corollary}

% ============================================================================
\section{2-Cycles on the Situs Complex and Audit Flux}
\label{sec:2cycles}
% ============================================================================

\subsection{2-Cycle Structure on the Expert Graph}
\label{sec:2cycle_construction}

\begin{definition}[2-Cycle and Flux on Expert Graph]
\label{def:2cycle_flux}
Let $K_N$ be the complete graph on $N$ experts. A 2-chain $\Sigma$ is an integer-coefficient sum of triangles:
\begin{equation}\label{eq:2chain}
    \Sigma = \sum_{1 \leq i < j < k \leq N} c_{ijk} \cdot \Delta_{ijk},
\end{equation}
where $\Delta_{ijk}$ is the triangle $(E_i, E_j, E_k)$ and $c_{ijk} \in \Z$. $\Sigma$ is a 2-cycle iff $\bd_2 \Sigma = 0$.
...
\subsection{Algebraic Criterion for Calibration Non-Transitivity}
\label{sec:algebraic_criterion}

\begin{theorem}[Calibration Non-Transitivity Criterion]
\label{thm:2cycle_computation}
For three experts $E_i, E_j, E_k$, the calibration non-transitivity $F_{ijk}$ involves the following components:
...
\textbf{Geometric interpretation:} $F_{ijk}^\beta$ measures scale non-transitivity ($\beta$ product deviating from 1), $F_{ijk}^\alpha$ measures offset non-transitivity (calibrated intercept inconsistency). When all $\beta = 1$ and $\alpha_{ij} + \alpha_{jk} = \alpha_{ik}$, the flat connection is recovered.
\end{theorem}

\begin{proof}
Direct computation: the composed calibration $(E_i \to E_j) \circ (E_j \to E_k)$ gives $S_i = (\alpha_{ij} + \beta_{ij}\alpha_{jk}) + (\beta_{ij}\beta_{jk}) S_k$. The inverse of $(E_k \to E_i)$ gives $S_i = -\alpha_{ki}/\beta_{ki} + (1/\beta_{ki}) S_k$. The difference norm is precisely $F_{ijk}$. $\hfill\square$
\end{proof}

\subsection{Information-Theoretic Measure of Non-Transitivity}
\label{sec:info_measure}

\begin{definition}[Circular Inconsistency SNR]
\label{def:circ_snr}
For a triangle $(E_i, E_j, E_k)$, define the signal-to-noise ratio of circular inconsistency:

\begin{equation}\label{eq:circ_snr_def}
    \mathrm{SNR}_{ijk} = \frac{F_{ijk}}{\sigma_{\mathrm{noise}}},
\end{equation}

where $\sigma_{\mathrm{noise}}$ is the mean standard deviation of pairwise calibration residuals. High SNR indicates that the detected non-transitivity significantly exceeds estimated noise, constituting a statistically significant circular inconsistency.
\end{definition}
...
\subsection{Information-Theoretic Model of Triangular Inconsistency}
...
\begin{theorem}[Circular Inconsistency $\iff$ Non-Zero Audit Flux]
\label{thm:circular_flux}
For $N \geq 3$ experts, the following conditions are equivalent:

\begin{enumerate}[label=(\roman*), leftmargin=*]
    \item Circular inconsistency exists;
    \item There exists a 2-cycle $\Sigma$ such that audit flux $\Phi(\Sigma) \neq 0$;
    \item The offset matrix $\mathbf{C} = (c_{ij})$ does not satisfy the cocycle condition: there is no vector $\mathbf{c} = (c_1, \ldots, c_N)$ such that $c_{ij} = c_j - c_i$ for all $i,j$;
    \item $\dd_1 \mathbf{C} \neq 0$, where $\dd_1$ acts on 1-cochains on the complete graph with experts as vertices.
\end{enumerate}
\end{theorem}
...
Let the pairwise Cercis score discrepancies of three experts $A, B, C$ have variances $\sigma_{AB}^2, \sigma_{BC}^2, \sigma_{CA}^2$. Define the signal-to-noise ratio of circular inconsistency:

\begin{equation}\label{eq:SNR}
    \mathrm{SNR}_{\mathrm{circ}} = \frac{|\E[F_{ABC}]|^2}{\Var[F_{ABC}]},
\end{equation}

where $F_{ABC} = (S_A - S_B) + (S_B - S_C) + (S_C - S_A)$ evaluated at a single data point.

If the score discrepancies are independent across pairs, then:

\begin{equation}\label{eq:SNR_bound}
    \Var[F_{ABC}] = \sigma_{AB}^2 + \sigma_{BC}^2 + \sigma_{CA}^2.
\end{equation}

To detect circular inconsistency at significance level $\alpha$ (rejecting the null hypothesis $F_{ABC} = 0$), the required number of data points $M$ satisfies:

\begin{equation}\label{eq:M_bound}
    M \geq \frac{(\sigma_{AB}^2 + \sigma_{BC}^2 + \sigma_{CA}^2) \cdot (z_{1-\alpha/2})^2}{\delta^2},
\end{equation}

where $\delta = |\E[F_{ABC}]|$ is the expected circular inconsistency strength and $z_{1-\alpha/2}$ is the standard normal quantile.
\end{theorem}
...
\begin{algorithmic}[1]
\Require Expert score matrix $\mathbf{S} \in \R^{N \times M}$ ($N$ experts, $M$ data points)
\Require Detection threshold $\tau > 0$
\Ensure Labeled set $\mathcal{F}_{\mathrm{bad}}$ of inconsistent 2-cycles

\State \textbf{Step 1:} Construct the 2-skeleton of the Situs complex
\For{$x = 1$ \textbf{to} $M$}
    \For{each triple $(i,j,k)$ with $1 \leq i < j < k \leq N$}
        \State Add 2-simplex $\sigma_{x,ijk}$ to 2-skeleton $K_2$
    \EndFor
\EndFor

\State \textbf{Step 2:} Compute 2-form field strength
\For{each 2-simplex $\sigma = (x, i, j, k) \in K_2$}
    \State $F_{x,ijk} \gets S_i(x) - S_j(x) + S_j(x) - S_k(x) + S_k(x) - S_i(x)$
    \Comment{Identically zero on single-point triangles; non-zero only on cross-point 2-cycles}
\EndFor

\State \textbf{Step 3:} Enumerate cross-point 2-cycles
\State Initialize $\mathcal{F}_{\mathrm{bad}} \gets \emptyset$
\For{each expert triple $(i,j,k)$}
    \For{each triple of data points $(x_1, x_2, x_3)$ with $x_1 < x_2 < x_3$}
        \State Construct cross-point 2-cycle $\Sigma = \sigma_{x_1,ijk} - \sigma_{x_2,ijk} + \sigma_{x_3,ijk}$
        \State Verify $\bd_2 \Sigma = 0$ (check boundary)
        \If{$\bd_2 \Sigma = 0$}
...
\State \textbf{Step 4:} Compute alternative cross-point flux measure (more efficient)
\State Construct difference matrix $\mathbf{D} \in \R^{M \times N \times N}$:
\For{$x = 1$ \textbf{to} $M$}
...
\State For each expert triple $(i,j,k)$, compute:
\State $\mathrm{FluxScore}_{ijk} \gets \frac{1}{M}\sum_{x=1}^M \left|(S_i(x) - S_j(x)) + (S_j(x) - S_k(x)) + (S_k(x) - S_i(x))\right|$
\State \textbf{Note:} This is the mean absolute value of single-point triangle flux. When it is zero, all single-point triangle fluxes are zero (but cross-point flux can still be non-zero --- see Step 3).

\State \textbf{Step 5:} Report results
...
\subsection{Density Filtration and $H_2$}
...
\begin{theorem}[Audit Flux and $H_2$ Persistence Diagram]
\label{thm:persistent_H2}
Under the density filtration $\{\SSS_t\}$, let $\mathrm{Dgm}_2$ be the persistence diagram of $H_2$. Then:

\begin{enumerate}[label=(\roman*), leftmargin=*]
    \item Each point $(b, d) \in \mathrm{Dgm}_2$ (birth time $b$, death time $d$) corresponds to a 2-dimensional topological feature of the Situs complex --- it is ``born'' (first appears) at filtration value $b$ and ``dies'' (gets filled in) at $d$.
    \item Within the filtration interval $[b, d)$, the supporting 2-cycle $\Sigma_{(b,d)}$ of this topological feature can carry non-zero audit flux.
    \item The audit inconsistency intensity $\Phi(\Sigma_{(b,d)})$ is statistically positively correlated with the feature's persistence $\mathrm{pers}(b,d) = d - b$: stronger inconsistency corresponds to more ``persistent'' topological features.
    \item If all expert evaluations are perfectly consistent, then $\mathrm{Dgm}_2 = \emptyset$ (no 2-dimensional features) and $F \equiv 0$.
\end{enumerate}
\end{theorem}
...
% -------------------------------------------------------------------
\subsection{Audit Flux Spectrum and Persistence Barcode}
\label{sec:barcode}
...
\begin{itemize}
    \item Experiment 1 (Globally Consistent): $N=5$ experts share the same underlying scoring function (constant offsets only), verifying all calibrations are transitive $\rightarrow$ $F \approx 0$, co-exact part $\approx$ 0.
    \item Experiment 2 (Circular Inconsistency): $N=3$ experts use different nonlinear scoring scales ($\sqrt{f}$, $f^2$, etc.), verifying calibration non-transitivity $\rightarrow$ $F \neq 0$, co-exact part non-zero.
...
Key insight: Audit connection A_ij = pairwise calibration model (alpha,beta)
...
\subsection{Existence and Uniqueness of Discrete Hodge Decomposition}
\label{sec:hodge_existence}

\begin{lemma}[Existence and Uniqueness of Discrete Hodge Decomposition]
\label{lem:hodge_unique}
For any finite simplicial complex $\SSS$ and any $k$-cochain $\omega \in C^k(\SSS; \R)$, the Hodge decomposition $\omega = \dd_{k-1}\alpha + \delta_k\beta + \gamma$ (where $\gamma \in \ker \Delta_k$) exists and is unique.

\noindent
\textbf{Existence:} By the standard finite-dimensional version of the Hodge theorem, $C^k = \im \Delta_k \oplus \ker \Delta_k$ (since $\Delta_k$ is self-adjoint). Moreover, $\im \Delta_k = \im \dd_{k-1} \oplus \im \delta_k$ (orthogonal direct sum).

\noindent
\textbf{Uniqueness:} If two decompositions exist, their difference belongs to $\im \dd_{k-1} \cap \im \delta_k = \{0\}$ and $\im \dd_{k-1} \cap \ker \Delta_k = \{0\}$, etc., and orthogonality forces the difference to be zero.
\end{lemma}

\subsection{Non-Zero Lower Bound on Cross-Point Flux}
\label{sec:flux_lowerbound}

\begin{proposition}[Lower Bound on Cross-Point Flux]
\label{prop:flux_lowerbound}
In the circular inconsistency scenario of Experiment 2, if $\delta_1(x), \delta_2(x)$ are $x$-dependent non-trivial functions satisfying $\Var_x[\delta_1] = \sigma_1^2 > 0$ and $\Var_x[\delta_2] = \sigma_2^2 > 0$ with $\delta_1 \perp \delta_2$, then the expected flux on a cross-point 2-cycle $\Sigma_{\mathrm{cross}}$ is bounded below by:
...
\begin{proof}
In the setting of Experiment 2, $S_1 = f$, $S_2 = f + \delta_1$, $S_3 = f - \delta_1 + \delta_2$. At a single data point:
\begin{align*}
...
This shows that single-point triangle fluxes on the \textbf{same expert triple} are always zero, and alternating sums are also zero --- because cancellation occurs independently at each point.

\textbf{Genuine non-triviality} appears when the 2-cycle involves \textbf{different expert subsets}. Consider the following 2-cycle:
\[
...
Thus a more precise non-triviality condition requires at least four experts distributed across multiple data points forming a 2-cycle. This qualitative observation has been integrated into the design of the detection algorithm (Algorithm~\ref{alg:flux_detect}).

For the variance metric lower bound: $V_{ijk} = \Var_x[F_{ijk}(x)]$. In the circular inconsistency scenario of Experiment 2, although $F_{ijk}(x) = 0$ holds pointwise (at a single data point), the \textbf{score discrepancy pattern} (the covariance structure of $S_i(x) - S_j(x)$) varies across data points, producing a non-zero variance metric. The explicit computation yields~\eqref{eq:flux_lower}. $\hfill\square$
\end{proof}
