...
## 第一部分：数学根源完整追溯<!-- label: ux7b2cux4e00ux90e8ux5206ux6570ux5b66ux6839ux6e90ux5b8cux6574ux8ffdux6eaf -->
...
**定义 A.1 (Kolmogorov, 1933).** 设 \((\Omega, \mathcal{F}, P)\)
...
&= \underbrace{\mathbb{E}[ (f_m(X) - r(X))^2 \mid X \in s ]}_{近似误差} + \underbrace{\mathbb{E}[ (r(X) - f^*(X))^2 \mid X \in s ]}_{目标噪声} 

&\quad + 2\underbrace{\mathbb{E}[ (f_m(X) - r(X))(r(X) - f^*(X)) \mid X \in s ]}_{交叉项}.
$$
\]
...
\[R_m(s) = \mathbb{E}[ (f_m(X) - r(X))^2 \mid X \in s ] + \mathbb{E}[ Var(Y \mid X) \mid X \in s ].\]

这正是**偏差-方差分解**的条件版本。第一项是状态 \(s\)
...
定义贝叶斯风险为
\(R^* = \inf_{f  可测} \mathbb{E}[\ell(f(X), Y)]\)。对于每个状态
\(s\)，可以定义**状态条件贝叶斯风险**：

\[R^*(s) = \inf_{f  可测} \mathbb{E}[\ell(f(X), Y) \mid X \in s].\]

那么 SCX 中的 \(R_m(s)\) 衡量的是专家 \(f_m\) 在状态 \(s\)
中相对于**不可知最优** \(f^*\) 的表现，而非相对于 \(R^*(s)\)。这在
\(f^* \neq \arg\inf_f \mathbb{E}[\ell(f(X),Y)]\) 时产生差异。

**定理 A.1 (条件风险分解).** 若 \(f^*\)
为目标函数（可能是人标注者或理想模型），则：

\[R_m(s) = \underbrace{\mathbb{E}[\ell(f_m(X), r(X)) \mid X \in s]}_{可减少误差} + \underbrace{\mathbb{E}[\ell(r(X), f^*(X)) \mid X \in s]}_{不可减少的状态偏差}.\]
...
\[\mathbb{E}_{Q}[\mathcal{R}(h)] \leq \mathbb{E}_{Q}[\hat{\mathcal{R}}(h)] + \sqrt{\frac{KL(Q \| P) + \log(n/\delta)}{2n-1}}.\]

将其扩展到**状态条件化 PAC-Bayes**：

\[\mathbb{E}_{Q_s}[\mathcal{R}(h \mid s)] \leq \mathbb{E}_{Q_s}[\hat{\mathcal{R}}(h \mid s)] + \sqrt{\frac{KL(Q_s \| P_s) + \log(n_s/\delta)}{2n_s - 1}},\]
...
#### A.5 与 Domain Adaptation / Covariate Shift
的关系<!-- label: a.5-ux4e0e-domain-adaptation-covariate-shift-ux7684ux5173ux7cfb -->

在协变量偏移下，训练分布 \(P_{train}\) 与测试分布
\(P_{test}\) 不同。条件风险 \(R_m(s)\) 的定义天然适用于此：

\[R_m^{test}(s) = \mathbb{E}_{x \sim P_{test}(\cdot|s)}[\ell(f_m(x), f^*(x))] = \frac{\mathbb{E}_{x \sim P_{train}} [\ell(f_m(x), f^*(x)) \cdot \mathbf{1}\{x \in s\} \cdot w(x)]}{\mathbb{E}_{x \sim P_{train}} [\mathbf{1}\{x \in s\} \cdot w(x)]}.\]

其中 \(w(x) = dP_{test} / dP_{train}(x)\) 为似然比。这表明
SCX 的 **状态级重要性权重** 本质上是对 covariate shift
...
\(\bigcup_{k \in I} s_k\)
的集合（\(I \subseteq \{1,...,K\}\)）的族。那么状态条件期望
...
SCX 的状态划分 \(\{s_k\}\) 可以视为**离散化充分统计量**：我们希望
\(Y \perp X \mid s(X)\)，即给定状态后，输入不再提供关于输出的额外信息。这等价于：

\[P(Y \mid X) = P(Y \mid s(X)), \quad a.s.\]

**信息瓶颈 (Tishby et al., 1999).**
状态划分是信息瓶颈问题的最优解：
...
直接控制假设类的容量 - SCX
中的状态发现等价于在**表达力**（更多状态）与**泛化**（更少参数）之间做权衡
...
**核 k-means 状态发现**：给定核 \(K\)，在 RKHS 嵌入
...
当数据嵌入低维流形时，最优状态划分应**沿着流形的固有几何**进行。谱聚类在数据图的
Laplacian 特征空间中进行 \(K\)-means：

**谱状态发现**： 1. 构建亲和图
\(W_{ij} = \exp(-\|x_i - x_j\|^2 / \sigma^2)\) 2. 计算归一化 Laplacian
\(L = I - D^{-1/2} W D^{-1/2}\) 3. 提取 \(L\) 的 \(K\) 个最小特征向量
\(v_1, ..., v_K\) 4. 在 \([v_1, ..., v_K]\) 的行上进行 \(K\)-means

这等价于求解流形上的**切比雪夫切割 (Cheeger cut)**。SCX
在此框架下的**状态发现**可理解为：找到在条件风险意义上尽可能均匀的子流形区域。
...
**定义 C.2 (半值 / Semivalue).** 半值是 Shapley
值的推广，使用一般化的概率权重：
...
\[\phi_i^{B}(v) = \frac{1}{2^{n-1}} \sum_{S \subseteq N \setminus \{i\}} [v(S \cup \{i\}) - v(S)].\]

在 SCX 中，噪声分数
\(NoiseScore(x_i) = r_i / (\rho(s_i) + \varepsilon) \cdot [1 - C(s_i)]\)
可以解释为一种**近似 Banzhaf
影响力指标**------它衡量移除该点（或将其标记为噪声）对损失的影响。

#### C.3 与 Influence Function
的关系<!-- label: c.3-ux4e0e-influence-function-ux7684ux5173ux7cfb -->

**定义 C.3 (Hampel, 1974).** 给定统计泛函 \(T(F)\)（其中 \(F\)
为分布），在点 \(x\) 处的 influence function 为：

\[IF(x; T, F) = \lim_{\varepsilon \to 0^+} \frac{T((1-\varepsilon)F + \varepsilon\delta_x) - T(F)}.\]

在经验分布 \(\hat{F}_n\) 下，leave-one-out 影响近似为：

\[\hat{IF}(x_i) \approx (n-1)[T(\hat{F}_n) - T(\hat{F}_{-i})].\]
...
\[\Pi^* = \arg\max_ \sum_{k=1}^K \log P(\{y_i: x_i \in s_k\} \mid \{x_i: x_i \in s_k\}, 朴素模型),\]
...
#### D.1 Dawid-Skene 模型<!-- label: d.1-dawid-skene-ux6a21ux578b -->

**定义 D.1 (Dawid \& Skene, 1979).** 设 \(N\) 个任务，\(M\)
个标注者。每个任务有真实标签
\(z_j \in \{1, ..., C\}\)，每个标注者提供标签 \(y_{ij}\)。标注者 \(i\)
...
#### D.4 与 Calibration / ECE
的关系<!-- label: d.4-ux4e0e-calibration-ece-ux7684ux5173ux7cfb -->

**期望校准误差 (Expected Calibration Error)**：

\[ECE = \mathbb{E}_{\hat{P}}[|P(Y = 1 \mid \hat{P} = p) - p|].\]

SCX 的 \(SCX_m(s)\) 本质上是对专家 \(f_m\) 在状态 \(s\)
上的条件校准评估：

\[ECE_m(s) = \left| \frac{1}{|s|} \sum_{x_i \in s} \mathbf{1}\{\ell(f_m(x_i), y_i) < \tau\} - \mathbb{E}_{x \in s}[SCX_m(s)] \right|.\]

当 \(\tau \to 0\) 时，\(SCX_m(s)\) 趋近于在状态 \(s\)
上的精确条件准确率。

#### D.5 与 Conformal Prediction
的关系<!-- label: d.5-ux4e0e-conformal-prediction-ux7684ux5173ux7cfb -->

**共形预测** (Vovk et al., 2005) 在给定置信度 \(1-\alpha\)
下构造预测集：
...
Tsybakov 条件给出了**最小最大最优收敛率**
\(\tilde{O}(n^{-\frac{1+\kappa}{2+\kappa}})\)，比无假设时的
\(\tilde{O}(n^{-1/2})\) 更快。

#### E.2 Mammen-Tsybakov Margin
Condition<!-- label: e.2-mammen-tsybakov-margin-condition -->

**定义 E.2 (Mammen \& Tsybakov, 1999).** 称分布 \(P\) 满足 Margin
...
\[NoiseScore(x_i) > \tau_{noise} \Rightarrow x_i  可能来自污染组分.\]

状态 \(s\) 中的污染参数为
\(\varepsilon(s) = \frac{|\{x_i \in s: NoiseScore(x_i) > \tau\}|}{|s|}\)。

#### E.4 与 EM
算法在众包学习中的关系<!-- label: e.4-ux4e0e-em-ux7b97ux6cd5ux5728ux4f17ux5305ux5b66ux4e60ux4e2dux7684ux5173ux7cfb -->

众包学习中，EM 算法（Dawid-Skene）交替进行： - **E 步**:
基于当前标注者可靠性估计真实标签 - **M 步**:
基于估计的真实标签更新标注者可靠性

SCX 的状态发现与噪声检测可以嵌入类似的 EM 框架： 1. **状态 E 步**:
给定当前状态划分 \(\Pi^{(t)}\) 和专家可靠性
\(SCX_m^{(t)}(s)\)，估计每个点的''可学习性''状态 2. **状态 M 步**:
基于当前估计，更新状态边界和专家可靠性

**定理 E.1 (收敛性).** 若状态发现的损失函数是划分的连续函数，则上述
EM 过程收敛到局部最优状态划分。
...
下，采样策略应**在状态内**满足 D-最优性：

\[V(s) \propto \det(\mathbb{E}_{x \in s}[xx^\top]),\]

在广义线性模型下，这等价于最大化状态内 Fisher 信息矩阵的行列式。

#### F.3 与 Bandit Theory (UCB, Thompson Sampling)
的关系<!-- label: f.3-ux4e0e-bandit-theory-ucb-thompson-sampling-ux7684ux5173ux7cfb -->

**UCB (Upper Confidence Bound)** (Auer et al., 2002)：

\[a_t = \arg\max_{a \in \mathcal{A}} \hat_a(t-1) + \sqrt{\frac{2\log t}{n_a(t-1)}}.\]

SCX 的状态获取函数 \(s^* = \arg\max_s V(s)\) 可以理解为**状态臂
(state-arm) bandit**：

\$\$V(s) =
\underbrace{\bar{r}(s) \cdot \rho(s)}*\{exploitation\}
\cdot \underbrace{L(s) \cdot [1-D(s)]}*\{exploration\}
\cdot \underbrace{\max_m SCX_m(s)}\_\{expertise\}.{]}
...
#### F.4 与贝叶斯优化 (Acquisition Functions)
...
\[EI(x) = \mathbb{E}[\max(f(x) - f(x^*), 0)] = (\mu(x) - f(x^*))\Phi(\frac{\mu(x) - f(x^*)}{\sigma(x)}) + \sigma(x)\phi(\frac{\mu(x) - f(x^*)}{\sigma(x)}).\]

**知识梯度 (Knowledge Gradient)** (Frazier et al., 2008)：

\[KG(x) = \mathbb{E}[\max_{x'} \mu_{n+1}(x') \mid x] - \max_{x'} \mu_n(x').\]

SCX 的 \(V(s)\) 与 KG 有深层联系：\(V(s)\) 衡量在状态 \(s\)
中采样将**提升最佳专家在该状态下性能**的期望增益。\(\max_m SCX_m(s)\)
...
## 第二部分：三个命题的严格证明<!-- label: ux7b2cux4e8cux90e8ux5206ux4e09ux4e2aux547dux9898ux7684ux4e25ux683cux8bc1ux660e -->
...
#### 数学陈述<!-- label: ux6570ux5b66ux9648ux8ff0 -->

**命题 1.** 设 \(\mathcal{F} = \{f_1, ..., f_M\}\)
为一个专家集合，\(\Pi: \mathcal{X} \to \{s_1, ..., s_K\}\)
为输入空间的一个可分划。则存在分布 \(P\) 和损失函数
\(\ell\)，使得以下同时成立：

\[\forall m, m': \quad R(f_m) < R(f_{m'}) \quad (全局排序)\]

但存在状态 \(s_k\) 使得：

\[R_{m'}(s_k) < R_m(s_k) \quad (状态级排序反转).\]

其中 \(R(f_m) = \mathbb{E}_{x \sim P}[\ell(f_m(x), f^*(x))]\)
为全局风险，\(R_m(s_k) = \mathbb{E}_{x \sim P(\cdot|s_k)}[\ell(f_m(x), f^*(x))]\)
为条件风险。

#### 证明<!-- label: ux8bc1ux660e -->

**构造 (反例).** 设输入空间 \(\mathcal{X} = [0, 1]\)，真实函数
...
#### 全局排序充分的条件<!-- label: ux5168ux5c40ux6392ux5e8fux5145ux5206ux7684ux6761ux4ef6 -->

**定理 1.1 (充分条件).** 全局专家排序 \(R_m \leq R_{m'}\)
...
表明：**全局排序不能一致地反映状态级排序**，正如 Arrow
定理中社会偏好不能同时满足所有理性条件。

具体类比： |{} Arrow 定理 |{} SCX |{}
|-----------|-----|{} |{} 投票者 \(i\)
|{} 状态 \(s\) |{} |{} 候选方案 \(a\) |{}
专家 \(f_m\) |{} |{} 社会偏好排序 |{} 全局风险排序
\(R_m\) |{} |{} 帕累托最优 |{} 状态级排序一致性
|{} |{} 无关选项独立性 |{} 引入新专家的独立性
|{}
...
#### 数学陈述<!-- label: ux6570ux5b66ux9648ux8ff0-1 -->
...
若 \(P(H_{noise}) > 0\)，则纯 residual-based 采样策略
\(\pi_{res}(x) \propto r_i = \ell(f(x_i), y_i)\)
的期望预算浪费存在严格正下界。

**命题 2.** 设 \(P_X\) 为输入分布，\(\ell\) 为有界损失
\(0 \leq \ell \leq L_\)。令采样预算为 \(B\)。定义： -
**Max-Residual Sampling**: 选取 \(r_i\) 最大的 \(B\) 个点 -
**State-Level Sampling (SCX)**: 选取 \(V(s)\) 最大的状态 \(s\)
内的所有点

则存在数据分布使得：

\[\mathbb{E}[增益_{res}] \leq \mathbb{E}[增益_{state}] - \delta(B),\]

其中 \(\delta(B) > 0\) 为 \(B\) 的增函数。

#### 证明<!-- label: ux8bc1ux660e-1 -->

**构造.** 考虑两个状态 \(s_1, s_2\)，各含 \(n\) 个点。

状态 \(s_1\) (可学习)： - \(y_i = f^*(x_i) + \epsilon_i\)，其中
\(\epsilon_i \sim N(0, \sigma^2_{low})\) - 当前模型 \(f\) 在
\(s_1\)
上有系统偏差：\(\mathbb{E}[f(x) \mid x \in s_1] = f^*(x) + \mu\)，其中
\(\mu \gg 0\)

状态 \(s_2\) (噪声)： - \(y_i = 常数 + \eta_i\)，其中
\(\eta_i \sim N(0, \sigma^2_{high})\)，\(\sigma^2_{high} \gg \sigma^2_{low}\)
- 当前模型 \(f\) 在 \(s_2\)
上已最优：\(\mathbb{E}[f(x) \mid x \in s_2] = 常数\)
...
\[\mathbb{E}[r^{(1)}] = \mu^2 + \sigma^2_{low}.\]

在 \(s_2\) 中： \[r_i^{(2)} = (f(x_i) - y_i)^2 = \eta_i^2.\]
\[\mathbb{E}[r^{(2)}] = \sigma^2_{high}.\]

**参数条件.** 令
\(\mu^2 + \sigma^2_{low} < \sigma^2_{high}\)。那么 \(s_2\)
中的平均残差大于 \(s_1\) 中的平均残差。

**Max-Residual Sampling 行为.** 选取全局残差最大的 \(B\) 个点。由于
\(\sigma^2_{high} \gg \mu^2 + \sigma^2_{low}\)，几乎所有被选中的点都来自
\(s_2\)（噪声状态）。

**增益分析.** 在 \(s_2\)
中采样额外的点无助于减少预测误差------模型已经最优。在 \(s_1\)
中每采样一个点，可将偏差从 \(\mu^2\) 降低到
\(O(\sigma^2_{low} / n_{s_1})\)。

**期望增益下界.** 若 Max-Residual 采了 \(B\) 个 \(s_2\) 中的点：

\[\mathbb{E}[增益_{res}] = O\left(\frac{\sigma^2_{low}}{n_{s_1}}\right) \cdot \left(\frac{n_{s_1}}{n_{s_1} + n_{s_2}}\right)^B \to 0 \quad as  B \to \infty\]

（因为 \(\frac{n_{s_1}}{n_{s_1}+n_{s_2}} < 1\)，指数增长地小）。

**差异分析.** 设 \(B_{res}\) 为 Max-Residual 分配在 \(s_1\)
的预算，\(B_{state}\) 为 State-Level Sampling 分配在 \(s_1\)
的预算。由于 Max-Residual 偏向
\(s_2\)，\(B_{res} \ll B_{state}\)。

更新后的期望损失： - Max-Residual:
\(\mathbb{E}[\mathcal{L}_{res}] \approx \frac{\sigma^2_{low}}{n_{s_1} + B_{res}} + \sigma^2_{high}\)
- State-Level:
\(\mathbb{E}[\mathcal{L}_{state}] \approx \frac{\sigma^2_{low}}{n_{s_1} + B_{state}} + \sigma^2_{high}\)

差值：\(\mathbb{E}[\mathcal{L}_{res}] - \mathbb{E}[\mathcal{L}_{state}] \approx \sigma^2_{low}\left(\frac{1}{n_{s_1} + B_{res}} - \frac{1}{n_{s_1} + B_{state}}\right) > 0\)。

因此
\(\mathbb{E}[增益_{state}] - \mathbb{E}[增益_{res}] > 0\)。□

#### SCX
状态级采样的上界<!-- label: scx-ux72b6ux6001ux7ea7ux91c7ux6837ux7684ux4e0aux754c -->

**定理 2.1 (改进上界).** 若 SCX 的状态获取函数
\(V(s) = \bar{r}(s) \cdot \rho(s) \cdot L(s) \cdot [1 - D(s)]\)
能够正确区分可学习状态和噪声状态（即 \(L(s)\) 是 \(s\)
的可学习性的精确度量），则 SCX 采样的期望预算浪费至多为：

\[\mathbb{E}[浪费_{SCX}] \leq B \cdot \epsilon_{est},\]

其中
\(\epsilon_{est} = P(L(s)  估计错误)\)。相比之下，Max-Residual
的期望浪费下界为：

\[\mathbb{E}[浪费_{res}] \geq B \cdot P(x \in H_{noise} \mid 残差最高).\]

当
\(P(x \in H_{noise} \mid 残差最高) \gg \epsilon_{est}\)
...
#### 数学陈述<!-- label: ux6570ux5b66ux9648ux8ff0-2 -->
...
\[w_m^{global} \propto \exp(-\alpha \hat{R}_m).\]

**命题 3.** 在以下意义下，状态条件权重 \(w_m(x)\) 优于全局权重
\(w_m^{global}\)：

\[\mathbb{E}_{x \sim P}\left[\sum_m w_m(x) \ell(f_m(x), y)\right] \leq \mathbb{E}_{x \sim P}\left[\sum_m w_m^{global} \ell(f_m(x), y)\right],\]
...
\[w_m^{global} = \frac{\exp(-\alpha \hat{R}_m)}{\sum_{m'} \exp(-\alpha \hat{R}_{m'})}.\]

**Step 1: 使用 Gibbs 不等式.** 对于固定的 \(x \in s\)，在状态 \(s\)
内，权重 \(w_m(s)\) 最小化以下目标：

\[J_s(w) = \sum_m w_m \hat{R}_m(s) + \frac{1} \sum_m w_m \log w_m.\]

这是概率单纯形上的凸优化，其 KKT 条件给出 \(w_m(s)\)
为唯一解。因此对于任意其他权重向量 \(w'\)（包括
\(w_m^{global}\)）：

\[\sum_m w_m(s) \hat{R}_m(s) + \frac{1} \sum_m w_m(s) \log w_m(s) \leq \sum_m w_m^{global} \hat{R}_m(s) + \frac{1} \sum_m w_m^{global} \log w_m^{global}.\]

**Step 2: 两边取条件期望.** 给定 \(x \in s\)：

\[\mathbb{E}\left[\sum_m w_m(s) \ell(f_m(X), Y) \mid X \in s\right] + \frac{1} H(w(s)) \leq \mathbb{E}\left[\sum_m w_m^{global} \ell(f_m(X), Y) \mid X \in s\right] + \frac{1} H(w^{global}),\]

其中 \(H(w) = -\sum_m w_m \log w_m\) 为 Shannon 熵。

**Step 3: 利用熵差.** 由 Jensen
不等式，全局权重的熵小于等于状态条件权重的期望熵：

\[H(w^{global}) \leq \mathbb{E}_{s}[H(w(s))],\]

这是因为凸函数 \(-H\)
满足：\(H(\mathbb{E}[w(s)]) \geq \mathbb{E}[H(w(s))]\)。因此：

\[\mathbb{E}\left[\sum_m w_m(s) \ell(f_m(X), Y) \right] \leq \mathbb{E}\left[\sum_m w_m^{global} \ell(f_m(X), Y) \right].\]

**严格性.** 当且仅当所有状态有相同的风险估计（即
\(\hat{R}_m(s) = \hat{R}_m\) 对所有 \(s\)
成立），或者熵差正好抵消风险差时，等号成立。否则不等式严格。

□

#### 风险度量讨论<!-- label: ux98ceux9669ux5ea6ux91cfux8ba8ux8bba -->

**定理 3.1 (Bound 的显式化).** 若 \(\ell\) 有界于
\([0, L_]\)，则：

\[\mathbb{E}\left[\sum_m w_m(s) \ell(f_m(X), Y)\right] \leq \mathbb{E}\left[\sum_m w_m^{global} \ell(f_m(X), Y)\right] - \frac{1} \cdot I(s; w),\]

其中 \(I(s; w) = \mathbb{E}_s[KL(w(s) \| w^{global})]\)
为状态与权重的互信息。
...
\[
$$
&\mathbb{E}\left[\sum_m w_m^{global} \ell(f_m(X), Y) - \sum_m w_m(s) \ell(f_m(X), Y)\right] 

&= \mathbb{E}\left[\sum_m (w_m^{global} - w_m(s)) \ell(f_m(X), Y)\right] 

&= \mathbb{E}\left[\sum_m (w_m^{global} - w_m(s)) (\ell(f_m(X), Y) - \hat{R}_m(s))\right] + \mathbb{E}\left[\sum_m (w_m^{global} - w_m(s)) \hat{R}_m(s)\right] 

&\geq \mathbb{E}\left[\sum_m (w_m^{global} - w_m(s)) \hat{R}_m(s)\right] \quad (由于 $\ell - \hat{R}$ 是零均值噪声)
$$
\]

由 Gibbs 不等式：

\[\sum_m (w_m^{global} - w_m(s)) \hat{R}_m(s) = \frac{1} \sum_m w_m(s) \log \frac{w_m(s)}{w_m^{global}} = \frac{1} KL(w(s) \| w^{global}).\]
...
## 第三部分：理论界限分析<!-- label: ux7b2cux4e09ux90e8ux5206ux7406ux8bbaux754cux9650ux5206ux6790 -->
...
### \texorpdfstring{3.1 \(R_m(s)\)
的样本复杂度{3.1 R\_m(s) 的样本复杂度}}<!-- label: r_ms-ux7684ux6837ux672cux590dux6742ux5ea6 -->

**问题.** 给定 \(n\) 个样本，需要多少样本才能可靠估计
\(\hat{R}_m(s)\)？

**定理 3.1.1 (条件风险估计的一致收敛).** 设
\(\ell \in [0, L_]\)，状态 \(s\) 有 \(n_s\) 个样本。则对于任意
\(\delta > 0\)，以概率至少 \(1 - \delta\)：

\[|\hat{R}_m(s) - R_m(s)| \leq L_ \sqrt{\frac{\log(2/\delta)}{2n_s}}.\]

**证明.** 由 Hoeffding 不等式。

**推论.** 要使 \(|\hat{R}_m(s) - R_m(s)| \leq \epsilon\) 以概率
\(1-\delta\) 成立，需要的样本数为：

\[n_s \geq \frac{L_^2 \log(2/\delta)}{2\epsilon^2}.\]

这意味着： - 若状态 \(s\) 太小（\(n_s\) 小），\(R_m(s)\) 的估计不可靠 -
SCX 的状态发现算法需要在**状态粒度**（更多状态 →
更精细）与**估计精度**（更多状态 → 更少每状态样本）间做权衡

**考虑多个专家和多重比较.** 若有 \(M\) 个专家，使用 Bonferroni
校正：

\[n_s \geq \frac{L_^2 \log(2M/\delta)}{2\epsilon^2}.\]

**定理 3.1.2 (最小最大下界).** 存在分布使得任意估计器
\(\hat{R}_m(s)\) 满足：

\[\mathbb{E}[|\hat{R}_m(s) - R_m(s)|] \geq \frac{\sigma(s)}{2\sqrt{n_s}},\]

其中 \(\sigma^2(s) = Var(\ell(f_m(X), f^*(X)) \mid X \in s)\)。
...
**定义 3.2.1 (真实状态).** 真实状态是输入空间 \(\mathcal{X}\)
的划分 \(\Pi^* = \{s_1^*, ..., s_K^*\}\)，使得在每个 \(s_k^*\)
内，条件分布 \(P_{Y|X}\)
是**常值**：\(P(Y \mid X) = P(Y \mid s_k^*)\) 对所有
\(x \in s_k^*\) 成立。

**定理 3.2.1 (谱聚类的相合性).** 假设： 1. \(\mathcal{X}\)
...
\[d_{Hausdorff}(\partial \hat_n, \mathcal{J}) \xrightarrow{P} 0, \quad n \to \infty,\]

其中 \(d_{Hausdorff}\) 为 Hausdorff
距离，\(\partial \hat_n\) 为估计划分的边界。

**证明思路.** 遵循 von Luxburg et al.~(2008)
...
**定理 3.2.2 (状态数的可识别性).** 若 \(K^*\)
是真实状态数，则对于充分大的 \(n\)，以高概率有 \(\hat{K}_n = K^*\)，只要
\(\ell\) 在每个状态内是次高斯分布且 \(\lambda\) 满足
\(\lambda \gg \sigma^2 \log(n)/n\)。

### 3.3 State-wise vs.~Point-wise Regret
Bound<!-- label: state-wise-vs.-point-wise-regret-bound -->

**问题.** SCX 的状态级路由 \(m^*(x) = \arg\min_m R_m(s(x))\)
与逐点最优 \(m^{**}(x) = \arg\min_m \mathbb{E}[\ell(f_m(x), y) \mid x]\)
之间的差距。

**定义.** 状态级路由的 regret 为：

\[Regret_{state}(n) = \mathbb{E}\left[\sum_{m=1}^M \mathbf{1}\{m^*(X) = m\} \ell(f_m(X), Y)\right] - \mathbb{E}\left[\min_m \ell(f_m(X), Y)\right].\]

**定理 3.3.1 (Regret Bound).** 若损失有界于
\([0, L_]\)，且状态划分满足 \(\epsilon\)-近似充分性：
...
\[Regret_{state} \leq 2L_ \cdot \epsilon.\]

**证明.**

\[
$$
Regret_{state} &= \mathbb{E}\left[\min_m \mathbb{E}[\ell(f_m(X), Y) \mid X \in s] - \min_m \ell(f_m(X), Y)\right] 

&\leq \mathbb{E}\left[\max_m |\mathbb{E}[\ell(f_m(X), Y) \mid X \in s] - \ell(f_m(X), Y)|\right] 

&\leq \mathbb{E}\left[|\mathbb{E}[Y \mid X \in s] - Y|\right] \cdot L_' \quad (由 Lipschitz) 

&\leq 2L_ \cdot \epsilon.
$$
\]

**推论.** 当
\(\epsilon \to 0\)（状态内条件分布完全齐次）时，状态级路由的 regret
趋于零。这意味着**充分细的划分可达到逐点最优**。

**收敛率.** 若状态估计误差 \(\epsilon_n\) 随 \(n\) 衰减（如
\(\epsilon_n = O(n^{-1/(d+2)})\) 对 \(d\) 维流形），则：

\[Regret_{state}(n) = O(n^{-1/(d+2)}).\]

### 3.4
噪声状态的可识别性条件<!-- label: ux566aux58f0ux72b6ux6001ux7684ux53efux8bc6ux522bux6027ux6761ux4ef6 -->

**问题.**
什么条件下噪声状态（\(H_{noise}\)）可从可学习状态中分离？

**定义 3.4.1 (可识别性条件).** 称噪声状态 \(s\) 是
\(\Delta\)-可识别的，若：

\[|\bar{r}(s) - \bar{r}(s')| > \Delta \quad 且 \quad |L(s) - L(s')| > \Delta, \quad \forall s' \neq s,\]
...
\[P(\mathcal{A}  正确识别了所有噪声状态) \geq 1 - \delta.\]

**证明概要.**
基于似然比检验的指数收敛性。每个状态的噪声检验可视为二元假设检验
\(H_0: s \in H_{learnable}\)
vs.~\(H_1: s \in H_{noise}\)。由 Chernoff-Stein 引理，错误概率以
\(\exp(-n_s \cdot KL(P_1 \| P_0))\) 速率衰减。
...
## 第四部分：与现有理论的连接<!-- label: ux7b2cux56dbux90e8ux5206ux4e0eux73b0ux6709ux7406ux8bbaux7684ux8fdeux63a5 -->
...
### 4.1 SCX 嵌入 PAC-Bayes<!-- label: scx-ux5d4cux5165-pac-bayes -->

**构造.** 在每个状态 \(s\) 中，定义一个''先验分布''
\(P_s\)（通常为所有专家的均匀分布）和''后验分布'' \(Q_s\)：

\[Q_s(m) = \frac{\exp(-\alpha R_m(s))}{\sum_{m'} \exp(-\alpha R_{m'}(s))}.\]

**定理 4.1 (PAC-Bayes 泛化界).** 对于每个状态 \(s\)，后验 \(Q_s\)
的期望风险以概率 \(1-\delta\) 满足：

\[\mathbb{E}_{m \sim Q_s}[R_m(s)] \leq \mathbb{E}_{m \sim Q_s}[\hat{R}_m(s)] + \sqrt{\frac{KL(Q_s \| P_s) + \log(n_s/\delta)}{2n_s - 1}}.\]

**SCX 的特殊性.** 当 \(\alpha \to \infty\) 时，\(Q_s\) 退化为
...
\[R(Q) \leq \hat{R}(Q) + \sqrt{\frac{KL(Q \| P) + \log(n/\delta)}{2n - 1}},\]

其中 \(KL(Q \| P)\) 需覆盖所有状态间的差异。由于
\(KL(Q \| P) \geq \mathbb{E}_s[KL(Q_s \| P_s)]\)（数据处理不等式），状态
PAC-Bayes 的界比全局 PAC-Bayes 更紧。

### 4.2 与 Information Bottleneck Principle
的关系<!-- label: ux4e0e-information-bottleneck-principle-ux7684ux5173ux7cfb -->

**信息瓶颈 (Tishby, 1999).** 寻找压缩表示 \(\tilde{X}\) 最大化
\(I(\tilde{X}; Y)\) 同时最小化 \(I(\tilde{X}; X)\)：

\[\min_{P(\tilde{x}|x)} I(\tilde{X}; X) - \beta I(\tilde{X}; Y).\]

**SCX
的状态发现**对应信息瓶颈的**硬聚类版本**：\(\tilde{X} = s(X)\)
是一个确定性函数，\(\tilde{X}\) 的取值是离散的状态索引。

**定理 4.2 (等价性).** 假设 \(|\tilde{\mathcal{X}}| = K\)。则 SCX
的最优状态划分 \(\Pi^*\) 满足：
...
**证明.** 注意到 \(I(X; \tilde{X}) = H(\tilde{X})\)（因为
\(\tilde{X}\) 是 \(X\) 的确定性函数），而
\(H(\tilde{X}) \leq \log K\)。最大化 \(I(\tilde{X}; Y)\) 等价于最小化
\(\mathbb{E}_s[H(Y \mid X \in s)]\)，这正是 SCX 中''状态内 \(Y\)
的方差最小化''目标。□

**SCX 的扩展.** 标准信息瓶颈没有考虑''专家''维度。SCX
将其扩展为**条件信息瓶颈**：

\[\min_ I(X; \tilde{X} \mid \mathcal{F}) - \beta I(\tilde{X}; Y \mid \mathcal{F}),\]

其中 \(\mathcal{F} = \{f_1, ..., f_M\}\)
为专家集合。这意味着状态划分应条件于可用专家------不同的专家集合诱导不同的最优划分。

**推论.** SCX
的状态发现算法本质上是**专家条件信息瓶颈最优化**。

### 4.3 决策论推导<!-- label: ux51b3ux7b56ux8bbaux63a8ux5bfc -->

**框架.** SCX 可以完全从**统计决策理论** (Wald, 1950)
推导出来。

**决策论三要素：** 1. **参数空间**
\(\Theta\)：专家的潜在性能参数 2. **行动空间**
\(\mathcal{A}\)：选择哪个专家 \(f_m\)，或权重分配 3. **损失函数**
\(L(\theta, a)\)：选择的专家在真实状态下的风险

**决策规则.** 一个决策规则 \(d: \mathcal{X} \to \mathcal{A}\)
是从输入到行动的函数。SCX 使用的是**状态-决策规则**：

\[d(x) = \arg\min_m \mathbb{E}[\ell(f_m(x), f^*(x)) \mid x \in s(x)].\]

**定理 4.3 (最小最大性).** 若状态划分 \(\Pi\)
是决策论意义下的完备统计量（即
\(Y \perp X \mid s(X)\)），则状态-决策规则 \(d(x)\)
是**可采纳的**（admissible）------不存在其他决策规则在所有状态下均不劣于它且在某些状态下严格更优。

**证明.** 由完备统计量的决策论性质（Lehmann-Scheffe 定理推广）：若
\(T(X)\) 是完备充分统计量，则任何基于 \(T(X)\) 的决策规则都是可采纳的。

由于 \(s(X)\) 是 \(X\)
的粗粒化，完备性条件在一般情况下不自动满足。但若状态划分接近充分统计量，则状态-决策规则近似可采纳。

**SCX 作为贝叶斯决策规则.** 设先验 \(\pi(\theta)\)
刻画专家的可靠性。贝叶斯决策规则为：

\[d_{Bayes}(x) = \arg\min_a \mathbb{E}_{\theta \sim \pi(\cdot|x)}[L(\theta, a)].\]

SCX 的状态条件权重 \(w_m(x)\) 正是**贝叶斯后验权重的变分近似**：

\[w_m(x) \approx P(\theta_m  是最优专家 \mid x).\]

**Neyman-Pearson 视角.** SCX 的噪声检测可以框定为 Neyman-Pearson
假设检验：在每个状态 \(s\) 中，检验 \(H_0: x\) 由可学习机制生成
vs.~\(H_1: x\) 为纯噪声点。最优检验由似然比给出：

\[\Lambda(x) = \frac{P_{learnable}(x)}{P_{noise}(x)} \lessgtr \tau.\]

SCX 的噪声分数 \(NoiseScore(x_i)\) 是 \(\Lambda(x_i)\)
的变分近似。
...
### 附录：符号表<!-- label: ux9644ux5f55ux7b26ux53f7ux8868 -->
