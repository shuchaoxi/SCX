# SCX (State-Conditioned eXpertise) 核心数学框架深度分析

> 本文档对 SCX 状态条件专家可靠性框架进行逐层解构，追溯每个数学对象的统计/信息论/博弈论源头，证伪/证成三条核心命题，并给出理论边界。
...
1. [数学根源追溯](#1-数学根源追溯)
   - 1.1 条件风险与 Kolmogorov 体系
   - 1.2 统计决策理论与贝叶斯风险
   - 1.3 状态划分：充分统计量与信息瓶颈
   - 1.4 数据价值：Shapley 值与合作博弈论
   - 1.5 专家可靠性：Dawid-Skene 与 IRT
   - 1.6 噪声与可学习性：Tsybakov 与 Huber
   - 1.7 主动采样：OED、信息论与 Bandit
   - 1.8 SCX-Compress：Coreset 与蒸馏
   - 1.9 专家路由：MoE 谱系
2. [三个命题的完整证明框架](#2-三个命题的完整证明框架)
   - 2.1 命题一：全局专家排序不足
   - 2.2 命题二：高误差采样在噪声下非最优
   - 2.3 命题三：状态条件专家权重优于全局固定权重
3. [状态空间构造方法](#3-状态空间构造方法)
4. [子模块数据流与协调](#4-子模块数据流与协调)
5. [SCX 理论边界](#5-scx-理论边界)

---

## 1. 数学根源追溯

### 1.1 条件风险与 Kolmogorov 公理体系
...
这一定义看似平凡，实则嵌套了 Kolmogorov 概率公理体系中三个最深刻的概念：**条件期望**、**正则条件概率** 和 **Radon-Nikodym 导数**。

#### 1.1.2 Kolmogorov 条件期望的正式定义

设 $(\Omega, \mathcal{F}, P)$ 为概率空间，$X$ 为随机变量满足 $\mathbb{E}[|X|] < \infty$，$\mathcal{G} \subseteq \mathcal{F}$ 为子 $\sigma$-代数。则存在 $P$-几乎必然唯一的 $\mathcal{G}$-可测函数 $\mathbb{E}[X|\mathcal{G}]$，满足：

$$\forall G \in \mathcal{G}: \int_G \mathbb{E}[X|\mathcal{G}] dP = \int_G X dP$$

**存在性**由 Radon-Nikodym 定理保证：定义有限符号测度 $\nu(G) = \int_G X dP$，则 $\nu \ll P|_{\mathcal{G}}$，其 RN 导数即为 $\mathbb{E}[X|\mathcal{G}]$。

SCX 中的 $R_m(s)$ 本质上是 $\mathbb{E}[\ell(f_m(X), f^*(X)) | X \in s]$。设 $s$ 为 $\mathcal{X}$ 中的一个可测子集，则状态 $s$ 对应条件事件，$R_m(s) = \mathbb{E}[L_m | \mathbf{1}_s(X)=1]$，其中 $L_m = \ell(f_m(X), f^*(X))$。

#### 1.1.3 正则条件概率

严格处理 $P(\cdot|s)$ 需要正则条件概率（Regular Conditional Probability）。给定随机元 $X: \Omega \to \mathcal{X}$ 和子 $\sigma$-代数 $\mathcal{G} \subset \mathcal{F}$，若存在映射 $P(\cdot|\mathcal{G})(\cdot): \mathcal{F} \times \Omega \to [0,1]$ 满足：

1. 对 $P$-a.e. $\omega$，$P(\cdot|\mathcal{G})(\omega)$ 是 $(\Omega, \mathcal{F})$ 上的概率测度；
2. 对每个 $F \in \mathcal{F}$，$P(F|\mathcal{G})(\cdot)$ 是 $\mathbb{E}[\mathbf{1}_F|\mathcal{G}]$ 的一个版本；

则称 $P(\cdot|\mathcal{G})(\cdot)$ 为正则条件概率。

在 SCX 中，状态 $s$ 诱导出条件分布 $P_{X|s}$。当 $\mathcal{X} = \mathbb{R}^d$ 为 Polish 空间时，正则条件概率存在（这由 Blackwell 定理保证）。因此 $P(x|s)$ 在测度论意义下是良定义的。

#### 1.1.4 损失函数 $\ell$ 的可积性条件

$R_m(s)$ 的良定义需要 $\ell(f_m(x), f^*(x))$ 对 $P(\cdot|s)$ 可积。对于回归问题 ($\ell_2$ 损失) 或分类问题 (0-1 损失)，只要 $f_m$ 和 $f^*$ 是平方可积函数，条件期望存在。更一般地，若 $\ell$ 是凸的且 $f^*(x) = \arg\min_y \mathbb{E}[\ell(Y, y)|X=x]$ (贝叶斯最优)，则相关测度论条件自动满足。
...
Abraham Wald 在 1950 年代建立的统计决策理论框架为 SCX 的条件风险提供了上层结构：

- **决策空间** $\mathcal{D}$：专家选择（从 $M$ 个专家中选择路由），或标签分配（acquire/relabel）
- **损失函数** $L(d, \theta)$：选择专家 $d$ 而真实状态为 $\theta$ 的代价
- **风险函数** $R(\pi, d) = \int_\Theta R(\theta, d) d\pi(\theta)$：先验 $\pi$ 下的贝叶斯风险
...
#### 1.2.2 Berger 的广义贝叶斯分析

Berger (1985) 指出，任意合理的决策规则都是某个（可能为 impropriate）先验下的广义贝叶斯规则。SCX 状态条件路由 $m^*(x) = \arg\min_m \sum_s \gamma_s(x) R_m(s)$ 可理解为：将输入 $x$ 的不确定性通过状态的软赋值 $\gamma_s(x)$ 分散到多个"局部先验"上，然后取后验期望损失最小的专家。

#### 1.2.3 与 PAC-Bayes 的联系

状态条件风险 $R_m(s)$ 与 PAC-Bayes 边界也有深层联系。令 $\mathcal{Q}$ 为专家假设空间上的后验分布，标准 PAC-Bayes 边界为：

$$\mathbb{E}_{h \sim \mathcal{Q}}[R(h)] \leq \mathbb{E}_{h \sim \mathcal{Q}}[\hat{R}(h)] + \sqrt{\frac{KL(\mathcal{Q}||\mathcal{P}) + \log(n/\delta)}{2n}}$$

SCX 中，状态条件权重 $w_m(x) \propto \exp(-\alpha \sum_s \gamma_s(x) \hat{R}_m(s))$ 可以看作一个**状态依赖的后验** $\mathcal{Q}_x$，其 KL 散度相对于均匀先验的惩罚项通过 $\alpha$ 参数控制，实现了状态水平的 PAC-Bayes 边界。
...
SCX 中的"状态" $s$ 是输入空间 $\mathcal{X}$ 的一个子集或一个区域。状态划分的数学本质是找到一个映射 $\phi: \mathcal{X} \to \mathcal{S}$，使得：
...
Fisher (1922) 提出：统计量 $T(X)$ 对参数 $\theta$ 是充分的，当且仅当：
...
在 SCX 中，状态 $s$ 的角色类似于一个离散的充分统计量：给定 $x \in s$，专家的条件风险 $R_m(s)$ 应能充分反映专家在该区域的性能，而不需要更细粒度的 $x$ 信息。
...
Tishby, Pereira 与 Bialek (1999) 提出的信息瓶颈 (IB) 原理为状态划分提供了变分框架：

$$\min_{p(\tilde{X}|X)} I(X; \tilde{X}) - \beta I(\tilde{X}; Y)$$

其中 $\tilde{X}$ 是 $X$ 的压缩表示。在 SCX 中，状态变量 $S = \phi(X)$ 扮演了 $\tilde{X}$ 的角色。IB 拉格朗日量：

$$\mathcal{L}_{IB} = I(X; S) - \beta I(S; Y)$$

- $I(X; S)$：状态划分的复杂度（需要多少信息来指定状态）
- $I(S; Y)$：状态对标签的预测能力
- $\beta$：压缩-预测的权衡参数

SCX 的状态划分隐式地解决了 IB 目标：它寻找一个状态空间 $\mathcal{S}$，使得：
1. 同一状态内的样本有相似的专家表现（高 $I(S; R_m)$）
2. 状态数量可控（低 $I(X; S)$）

当 $\beta \to \infty$ 时，IB 退化为每个 $x$ 对应唯一状态（完全不压缩），$M^* = \text{专家数量}$；当 $\beta \to 0$ 时，$|S| = 1$（全局池化），退化为全局专家排序。

#### 1.3.4 率失真理论视角

状态划分也是率失真理论 (Rate-Distortion Theory) 的一个实例。定义失真度量：

$$d(x, x') = |R_m(x) - R_m(x')|$$

其中 $R_m(x) = \mathbb{E}[\ell(f_m(X), f^*(X)) | X = x]$ 为点态条件风险。状态划分等价于对 $(\mathcal{X}, d)$ 进行量化（quantization），率失真函数为：
...
这是五个因子的乘积。其根源可追溯到合作博弈论中的**Shapley 公理体系**和**数据 Shapley** (Ghorbani & Zou, 2019)。

#### 1.4.2 Shapley 值的公理

Shapley (1953) 定义了合作博弈 $v: 2^N \to \mathbb{R}$ 中单个玩家 $i$ 的价值：

$$\phi_i(v) = \frac{1}{|N|!} \sum_{\pi \in \Pi(N)} [v(S_\pi^i \cup \{i\}) - v(S_\pi^i)]$$

其中 $S_\pi^i$ 是排列 $\pi$ 中 $i$ 之前玩家的集合。Shapley 值是唯一满足以下公理的分配方案：

1. **有效性 (Efficiency)**：$\sum_{i \in N} \phi_i(v) = v(N)$
2. **对称性 (Symmetry)**：若 $v(S \cup \{i\}) = v(S \cup \{j\})$ 对所有 $S \subseteq N \setminus \{i,j\}$，则 $\phi_i = \phi_j$
3. **虚拟性 (Dummy)**：若 $v(S \cup \{i\}) = v(S)$ 对所有 $S$，则 $\phi_i = 0$
4. **可加性 (Additivity)**：$\phi_i(v + w) = \phi_i(v) + \phi_i(w)$

#### 1.4.3 SCX 价值函数与 Shapley 的偏离

SCX 的 $V(s)$ 不是精确的 Shapley 值，而是其**可分解代理**。真正计算数据点级别的 Shapley 值是 $O(2^N)$ 的。SCX 做了关键简化：

- **状态级而非样本级**：先聚合成状态 $s$，再计算状态价值，复杂度 $O(2^{|S|})$ 其中 $|S| \ll N$
- **乘法分解**：将边际贡献分解为五个可理解的因子
...
#### 1.4.4 与 Data Shapley 的精确关系

Ghorbani & Zou (2019) 的数据 Shapley 将每个训练点 $z_i$ 的价值定义为：
...
#### 1.5.1 Dawid-Skene 标注者模型

Dawid & Skene (1979) 提出了著名的标注者（专家）可靠性模型。设 $J$ 个标注者对 $N$ 个样本进行分类，每个样本的真实类别为 $y_i \in \{1,\ldots,K\}$。标注者 $j$ 的混淆矩阵为：

$$\pi^{(j)}_{kl} = P(\text{标注者 }j\text{ 将类别 }k\text{ 标为 }l)$$

观测似然为：

$$P(\text{数据} | \pi, y) = \prod_{i=1}^N \prod_{j=1}^J \pi^{(j)}_{y_i, z_{ij}}$$

EM 算法交替估计 $\pi^{(j)}$ 和 $y_i$ 的后验分布。

SCX 的 $SCX_m(s) = P(\ell(f_m(x), y) < \tau | x \in s)$ 是 Dawid-Skene 框架的推广：

- Dawid-Skene 假设标注者可靠性是全局常数（$\pi^{(j)}_{kl}$ 不依赖 $x$）
- SCX 允许可靠性随状态 $s$ 变化，即 $\pi^{(j,s)}_{kl}(x) = P(\ell(f_j(x), y) < \tau | x \in s)$

这一推广的关键数学结果：当存在状态 $s_1, s_2$ 使得 Dawid-Skene 全局混淆矩阵 $\pi^{(j)}$ 无法同时预测 $s_1$ 和 $s_2$ 上的表现时，状态条件建模是必要的。这正是命题一的本质。
...
其中 $\theta_i$ 是样本 $i$ 的"能力"，$b_j$ 是标注者 $j$ 的"难度阈值"。

SCX 的 $SCX_m(s)$ 可以映射到 Rasch 模型：

$$\log\left(\frac{SCX_m(s)}{1 - SCX_m(s)}\right) = \theta_s - b_m$$

其中 $\theta_s$ 是状态 $s$ 的"可处理性"参数（容易与否），$b_m$ 是专家 $m$ 的能力阈值。

**更丰富的 IRT 模型**如 2-PL (两参数逻辑模型)：

$$P(Y_{ij}=1|\theta_i, a_j, b_j) = \frac{\exp(a_j(\theta_i - b_j))}{1 + \exp(a_j(\theta_i - b_j))}$$

对应 SCX 中引入专家确度斜率 $a_j$，允许不同专家对状态变化的敏感度不同——这正是 SCX 中 $\text{max}_m SCX_m(s)$ 的核心假设。

#### 1.5.3 与贝叶斯众包的联系

Raykar et al. (2010) 的贝叶斯众包模型进一步考虑了标注者的敏感度和特异度。SCX 在状态水平上的 $SCX_m(s)$ 可以看作在状态 $s$ 上的标注者性能矩阵的奇异值分解的第一个分量——即最能区分"可靠/不可靠"维度的投影。
...
Tsybakov (2004) 提出了分类问题中噪声条件的关键定义。对于二分类问题，设 $\eta(x) = P(Y=1|X=x)$，则 Tsybakov 噪声条件为：存在常数 $C > 0$ 和 $\alpha \geq 0$，使得：
...
在 Tsybakov 条件下，分类器的收敛速率为 $n^{-\frac{1+\alpha}{2+\alpha}}$。
...
当 $C(s) \to 1$ 时，状态内部近似为贝叶斯最优决策（$\alpha \to \infty$）；当 $C(s) \to 0$ 时，状态内标签接近随机（$\alpha \to 0$）。

具体地，若在状态 $s$ 内有 $P_X(|\eta(x)-1/2| \leq t | x \in s) \leq C_s t^{\alpha_s}$，则 $C(s) = \Phi(\alpha_s)$，其中 $\Phi$ 是单调增函数（如 $\Phi(\alpha) = 1 - e^{-\alpha}$ 或 $\Phi(\alpha) = \alpha/(1+\alpha)$）。
...
#### 1.6.3 Natarajan 维度与噪声

在噪声条件下学习的样本复杂度由 Natarajan 维度刻画。对于多分类问题，Natarajan (1989) 给出了噪声存在的泛化边界：

$$R(f) \leq \hat{R}_\gamma(f) + \tilde{O}\left(\sqrt{\frac{d}{n\gamma^2}}\right)$$

其中 $d$ 是 Natarajan 维度。SCX 的 $L(s)$ 隐式编码了 $\frac{d_s}{n_s}$ 的比值——状态 $s$ 的"有效复杂度"。

---

### 1.7 主动采样：OED、信息论与 Bandit

#### 1.7.1 MacKay 信息论主动学习

MacKay (1992) 的主动学习框架基于信息增益：

$$x^* = \arg\max_x H[\theta | \mathcal{D}] - H[\theta | \mathcal{D} \cup \{x, y\}]$$

其中 $H[\cdot]$ 是 Shannon 熵。在 SCX 的数据四分类中，**valuable** 数据点正是具有高信息增益的点。
...
在经典 OED 中，目标是选择实验 $x$ 使得 Fisher 信息矩阵的某个标量函数最优：

- **A-optimality**：$\min \text{tr}(I(x)^{-1})$
- **D-optimality**：$\min \det(I(x)^{-1})$
- **E-optimality**：$\min \lambda_{\max}(I(x)^{-1})$

SCX 的 $V(s)$ 中的 $\bar{r}(s) \cdot L(s)$ 因子对应**与参数估计的不确定性成比例**的采样准则。当 $\bar{r}(s)$ 大且 $L(s)$ 高时，标注 $(s, y)$ 对 Fisher 信息矩阵的更新贡献大。

#### 1.7.3 Bandit 视角：探索-利用权衡

SCX 的 acquire 动作本质上是**多臂 Bandit 的变体**：

- 臂 (arm)：每个状态 $s$ 是一个臂
- 收益 (reward)：标注后 $Y \sim P(Y|s)$ 带来的信息增益
- 目标：在有限标注预算 $B$ 下最大化总信息增益

SCX 的 $V(s)$ 是 Upper Confidence Bound (UCB) 启发式的推广：

$$V_{\text{UCB}}(s) = \underbrace{\bar{r}(s) \cdot L(s)}_{\text{利用项}} + \underbrace{\rho(s) \cdot [1 - D(s)]}_{\text{探索项}} \cdot \max_m SCX_m(s)$$

其中 $\rho(s)$ 类比于 UCB 中的 $\sqrt{\frac{\log t}{n_s}}$，当状态样本少时 $\rho(s)$ 被低估，需手动校准 $\frac{1}{\rho(s) + \varepsilon}$ 因子。

#### 1.7.4 Thompson Sampling 联系

V(s) 中出现的 $\max_m SCX_m(s)$ 也可以理解为 Thompson Sampling 式的后验概率：对状态 $s$，最佳专家 $m^*(s) = \arg\max_m SCX_m(s)$ 是后验下最可能的"最佳动作"。
...
Coreset 理论的目标是找到一个加权子集 $C \subseteq \mathcal{D}$，使得对任意 $f \in \mathcal{F}$：
...
| Redundant | 可压缩 (冗余合并) | $w_i < 1/\text{密度}$ |
| Noisy | 异常值 (排除或降权) | $w_i = 0$ 或 $\ll 1$ |
| Expert-dependent | 需要路由缓存 | $w_i$ 取决于选中的专家 |

#### 1.8.2 敏感度采样

Chen et al. (2018) 的 Coreset 构造通过敏感度采样实现。每个点 $x_i$ 的敏感度为：
...
数据集蒸馏 (Wang et al. 2018) 寻求合成样本 $(x^*, y^*)$ 使得在其上训练的模型近似等价于在全数据集上训练的模型：

$$(x^*, y^*) = \arg\min_{(\tilde{x}, \tilde{y})} \mathcal{L}(\theta(\tilde{x}, \tilde{y}), \mathcal{D}_{\text{val}})$$

SCX-Compress 的 Redundant 类别就是蒸馏目标：同一状态内的冗余样本可以用少量代表性样本替代。

---

### 1.9 专家路由：MoE 谱系

#### 1.9.1 Jacob et al. (1991) 自适应混合专家

这是 MoE 的开山之作。模型使用门控网络 $g(x)$ 来加权各个专家 $f_m(x)$ 的输出：
...
#### 1.9.2 Shazeer et al. (2017) 稀疏门控 MoE

Shazeer 引入 top-$k$ 门控实现稀疏性：

$$g(x) = \text{softmax}(\text{KeepTopK}(x \cdot W_g, k))$$

其中 $\text{KeepTopK}(v, k)_i = v_i$ 若 $v_i$ 在前 $k$ 大，否则 $-\infty$。
...
> **命题陈述**：若存在状态 $s_1, s_2 \in \mathcal{S}$ 和专家 $a, b \in \{1,\ldots,M\}$，使得 $R_a(s_1) < R_b(s_1)$ 但 $R_a(s_2) > R_b(s_2)$，则不存在全局最优专家。

#### 2.1.1 形式化定义

**定义 1（全局专家排序）**：全局专家排序是一个全序关系 $\preceq$ 在专家集合 $\{1,\ldots,M\}$ 上，满足：若 $a \preceq b$，则认为专家 $a$ 整体上不劣于专家 $b$。
...
但在状态 $s_1$ 处：$R_{m^*}(s_1) \leq R_a(s_1) < R_b(s_1)$，故 $m^*$ 在 $s_1$ 上优于 $b$。记 $\Delta_1 = R_b(s_1) - R_{m^*}(s_1) > 0$。

在状态 $s_2$ 处：$R_{m^*}(s_2) \leq R_b(s_2) < R_a(s_2)$，故 $m^*$ 在 $s_2$ 上也优于 $a$。记 $\Delta_2 = R_a(s_2) - R_{m^*}(s_2) > 0$。
...
**修正的证明**：我们需要证明不存在单一的 $m^*$ 能在两个状态上**同时**最优，而非逐状态最优。

设 $\mathcal{M} = \{1,\ldots,M\}$。定义状态条件最优集：
...
当 $\mathcal{S}$ 连续时（如 $\mathcal{S} = [0,1]$），命题退化为：若 $\exists s_1, s_2$ 使得 $R_a(s_i)$ 与 $R_b(s_i)$ 交叉，则不存在 Lebesgue-几乎必然的全局最优专家。这用测度论语言表述为：
...
> **命题陈述**：在噪声存在时，单纯基于残差（max-residual sampling）的主动采样策略不是最优的。

#### 2.2.1 问题形式化

设 $\mathcal{D} = \{(x_i, y_i)\}_{i=1}^N$ 为当前标注数据集，未标注池为 $\mathcal{U}$。对于 $x \in \mathcal{U}$，定义残差：
...
其中 $\varepsilon(x) \sim P_{\text{noise}}(x)$ 是状态依赖的噪声过程。
...
**定理**（命题二的正式陈述）：若存在至少一个pair $(i,j)$ 使得 $\hat{r}_i > \hat{r}_j$ 但 $\mathbb{E}[\Delta R(f|x_i)] < \mathbb{E}[\Delta R(f|x_j)]$，则 $\pi_{\text{MR}} \neq \pi^*$。
...
假设 $\mathcal{U} = \{x_1, x_2\}$：
- $x_1$: $r_{\text{true}}(x_1) = 0.1$, $\sigma_\varepsilon^2(x_1) = 0.5$, $\tilde{r}(x_1) \approx 0.71$
- $x_2$: $r_{\text{true}}(x_2) = 0.5$, $\sigma_\varepsilon^2(x_2) = 0.01$, $\tilde{r}(x_2) \approx 0.50$

则 max-residual 选择 $x_1$，但 $\mathbb{E}[\Delta R(f|x_1)] \approx 0.01 + \sigma_y^2(x_1) \ll \mathbb{E}[\Delta R(f|x_2)] \approx 0.25 + \sigma_y^2(x_2)$。
...
> **命题陈述**：对于专家融合（ensemble）问题，状态条件权重 $w_m(x)$ 的期望风险不高于任何全局固定权重 $w_m$。
...
若状态条件权重 $w_m(X)$ 与 $\nabla \ell(\bar{f}_{\text{ens}}) \cdot f_m(X)$ 负相关（这正是 SCX 设计目标——降低高损失区域的权重），则 $\Delta R > 0$。

#### 2.3.5 极端情况：当状态条件权重退化为全局权重
...
### 3.1 嵌入映射 $\phi: \mathcal{X} \to \mathbb{R}^d$

#### 3.1.1 嵌入选择的原则

状态构造的第一步是将原始输入 $x$ 映射到低维连续空间 $\mathbb{R}^d$。选择准则：

1. **保真性**：$\phi(x)$ 应保留与专家性能 $R_m(x)$ 相关的信息
2. **平滑性**：相邻点 $\phi(x_i) \approx \phi(x_j)$ 应有相似的风险 $R_m(x_i) \approx R_m(x_j)$
3. **计算效率**：$\phi$ 的评估应是 $O(d)$ 或 $O(\log d)$ 的

#### 3.1.2 候选嵌入方法

| 方法 | 维度 $d$ | 特性 | 适用场景 |
|------|---------|------|---------|
| 原始特征 | $\dim(\mathcal{X})$ | 无信息损失 | $\mathcal{X}$ 维度低 ($<100$) |
...
| 专家特征 $[f_1(x),\ldots,f_M(x)]$ | $M$ | 专家性能的直接表征 | 当专家数量少时 |
...
其中 $\phi_{\text{feat}}(x)$ 是原始特征的降维版本，$\phi_{\text{exp}}(x) = [f_1(x), \ldots, f_M(x)]$ 是专家输出向量。

### 3.2 降维方法对比分析

#### 3.2.1 PCA（主成分分析）

**目标**：最大化投影后方差 $\max_{V} \text{tr}(V^\top \Sigma V)$，其中 $\Sigma = \text{Cov}(X)$。

**代价**：$O(n d^2 + d^3)$ 或 $O(n d \log k)$ (随机化 SVD)。

**SCX 适配性**：PCA 保持全局方差，但不保证局部风险同质性。在 $R_m(x)$ 变化剧烈的区域，PCA 可能将不同风险的区域投影到相邻位置。

**线性限制**：$R_m(x)$ 对 $x$ 的函数可能高度非线性，PCA 的线性投影会破坏保真性。

#### 3.2.2 t-SNE (t-distributed Stochastic Neighbor Embedding)

**目标**：最小化 $KL(P||Q)$，其中 $p_{ij}$ 是高维高斯相似度，$q_{ij}$ 是低维 t-分布相似度。
...
**目标**：最小化模糊集交叉熵 $CE(F, G)$，其中 $F$ 是高维模糊拓扑表示，$G$ 是低维表示。
...
**SCX 适配性**：PaCMAP 对参数不敏感，聚类鲁棒性优于 UMAP。当 SCX 需要自动化管线时，PaCMAP 减少了调参需求。
...
**SCX 适配性**：
- 天然支持软状态赋值 $\gamma_s(x) = \gamma_{ik}$！这是 SCX 中 $\gamma_s(x)$ 的精确实现
- 协方差矩阵 $\Sigma_k$ 捕捉状态内部方差
- 适用于凸形状的簇

**局限性**：
- 需要指定 $K$
- 假设高斯分布（不适用于高度非凸簇）
- 高维数据时协方差矩阵估计困难（可通过 $\Sigma_k = \sigma_k^2 I$ 或对角矩阵简化）

#### 3.3.2 DBSCAN（Density-Based Spatial Clustering of Applications with Noise）

**核心概念**：
- $\varepsilon$-邻域：$N_\varepsilon(p) = \{q \in D | \text{dist}(p,q) \leq \varepsilon\}$
...
**局限性**：
- $\varepsilon$ 和 MinPts 敏感
- 密度变化剧烈的数据效果差
- 边界点 $\gamma_s(x)$ 是硬赋值（0 或 1），不支持软状态

#### 3.3.3 HDBSCAN (Hierarchical DBSCAN)

**改进**：基于层次聚类，消除 $\varepsilon$ 参数。

1. 构建最小生成树（MST）
2. 对 MST 按边长度降序构建层次树
3. 使用 $\lambda = 1/\text{distance}$ 作为稳定性度量
4. 从层次树中提取最稳定的簇
...
**推荐**：对 $\mathcal{X}$ 空间复杂时，HDBSCAN 是比 GMM 更好的选择。

#### 3.3.4 Spectral Clustering

**步骤**：
1. 构建相似度图 $W_{ij} = \exp(-\|x_i - x_j\|^2 / 2\sigma^2)$
2. 计算图拉普拉斯 $L = D - W$ 或归一化拉普拉斯 $L_{\text{sym}} = I - D^{-1/2} W D^{-1/2}$
3. 取 $L$ 的 $K$ 个最小特征向量构成 $U \in \mathbb{R}^{n \times K}$
4. 对 $U$ 的行进行 $K$-means
...
**局限性**：
- $O(n^3)$ 复杂度（可近似为 $O(n^2)$）
- $K$ 需预设
- 对新样本无法直接预测（需要 Nystroem 逼近）
...
其中 $\kappa(x,y) = \exp(-\|x - y\|^2 / 2\sigma^2)$，$\mu_s$ 是簇中心。
...
SCX 主框架使用软赋值，因为 $\gamma_s(x)$ 在专家路由 $$w_m(x) \propto \exp(-\alpha \sum_s \gamma_s(x) \hat{R}_m(s))$$ 中的使用需要微分连续性。
...
其中 $a_i$ 是 $i$ 与同一簇内点的平均距离，$b_i$ 是与最近的其他簇的平均距离。

整体 silhouette 得分：$S(K) = \frac{1}{n} \sum_i s_i$。

**SCX 选择准则**：$K^* = \arg\max_K S(K)$。但 silhouette 仅基于几何距离，不考虑风险 $R_m(s)$ 的内部方差。

#### 3.5.2 Gap Statistic (Tibshirani et al., 2001)

比较实际数据的聚类紧凑度 $W_K$ 与零参考分布下的期望值 $E_n^*[W_K]$：

$$\text{Gap}_n(K) = E_n^*[\log W_K] - \log W_K$$

最优 $K$ 是使得 $\text{Gap}(K) \geq \text{Gap}(K+1) - s_{K+1}$ 的最小值。

**SCX 适配性**：可替换 $W_K$ 为聚类内风险方差之和 $\sum_{s \in \mathcal{S}} \text{Var}_{x \in s}[R_m(x)]$。
...
1. 对新样本 $x_{\text{new}}$ 计算嵌入 $\phi(x_{\text{new}})$
2. 用现有聚类模型计算 $\gamma_s(x_{\text{new}})$，分配状态
3. 轻量更新状态统计量：$|s|_{\text{new}} = |s|_{\text{old}} + \gamma_s(x_{\text{new}})$
4. 增量更新 $R_m(s)$：使用 Welford 在线算法
5. 重新评估 $V(s)$，若变化超过阈值则触发动作

**当动作触发时**：

- **discard**：从数据集中移除样本，更新状态统计
- **split**：在状态内重新运行聚类（$K=2$），更新全局状态索引
- **relabel**：用新标签覆盖旧标签，重新计算风险
- **acquire**：将新标注样本加入数据集，触发新数据到达流程

### 4.4 计算复杂度分析

| 模块 | 初始构建 | 增量更新 |
|------|---------|---------|
| UMAP 嵌入 | $O(n \log n)$ | $O(n_{\text{new}} \log n)$ |
| GMM 聚类 | $O(n K d)$ | $O(K d)$ (E-step only) |
| 风险估计 | $O(n M)$ | $O(n_{\text{new}} M)$ |
| 价值评估 | $O(K M)$ | $O(K M)$ |
| 四分类 | $O(K)$ | $O(1)$ |
| SCX-Compress | $O(n \log n)$ | $O(n_{\text{new}} \log K)$ |

**总复杂度**：初始构建 $O(n \log n + n M)$，增量更新 $O(n_{\text{new}} M)$，适合流式场景。
...
给定 $n_s$ 个来自状态 $s$ 的独立同分布样本，经验风险 $\hat{R}_m(s)$ 的收敛速度由 Hoeffding 不等式和 Bernstein 不等式控制。
...
对所有 $K$ 个状态和 $M$ 个专家应用 Bonferroni 校正，以概率 $1 - \delta$：

$$\max_{m,s} |\hat{R}_m(s) - R_m(s)| \leq L \sqrt{\frac{\log(2MK/\delta)}{2 \min_s n_s}}$$

**警告**：当某个状态 $s$ 的 $n_s$ 很小时（如样本稀疏状态），该边界失去意义。这是 SCX 的固有局限——**状态划分越细，每个状态内的样本越少，风险估计的方差越大**。

### 5.2 泛化边界：状态条件专家路由

对状态条件路由 $m^*(x) = \arg\min_m \sum_s \gamma_s(x) \hat{R}_m(s)$，其期望风险为：

$$R(m^*) = \mathbb{E}[\ell(f_{m^*(X)}(X), f^*(X))]$$

#### 5.2.1 基于 VC 维的边界

设专家函数类 $\mathcal{F}_m$ 的 VC 维为 $d_m$（二分类）或伪维数为 $P_{\dim}(\mathcal{F}_m)$（回归）。则对任意 $\delta > 0$，以概率 $1 - \delta$：
...
如果状态 $\mathcal{S}$ 和嵌入 $\phi$ 是通过数据学习的（通常如此），则需要额外惩罚：

$$R(m^*) \leq \hat{R}(m^*) + \tilde{O}\left(\sqrt{\frac{\max_m d_m + d_{\phi} + K \log M}{n}}\right)$$

其中 $d_{\phi}$ 是嵌入函数的 Rademacher 复杂度或 VC 维（如 UMAP 的参数自由度）。
...
当 $n/K < \log M$ 时，不可能以高概率识别每个状态的最优专家。这为 SCX 设定了一个**必要条件的样本复杂度**——每个状态至少需要 $O(\log M)$ 个样本来判断最优专家。
...
噪声分数 $\text{NoiseScore}(x_i) = r_i \cdot \frac{1}{\rho(s_i)+\varepsilon} \cdot [1 - C(s_i)]$ 作为噪声检测器，其 Type-I/II 错误率由以下边界控制：
...
若 $\Delta I_{\text{split}} < \log(2)/n$（每个子状态的样本少于 2 个的等效信息量），则分裂不应执行。

#### 5.5.2 最优状态数的信息论界

记 $R_{\text{gen}}(\mathcal{S})$ 为状态划分 $\mathcal{S}$ 下的期望泛化风险。由率失真理论：

$$R_{\text{gen}}(\mathcal{S}) \geq R^* + \exp(-I(S; R_m))$$

其中 $R^* = \min_{m^*(x)} \mathbb{E}[\ell(f_{m^*(X)}(X), f^*(X))]$ 是已知点态最优专家的极限风险。

因此，状态粒度选择是在两个目标间的权衡：
- 减小 $R_{\text{gen}}(\mathcal{S})$：需要更多状态（高 $I(S; R_m)$）
...
若攻击者可以操纵 $\gamma_s(x)$（如通过中毒攻击），则状态条件路由可能被劫持。

在 $\ell_\infty$ 攻击下，$\tilde{\gamma}_s(x) = \gamma_s(x) + \delta_s(x)$，$\|\delta\|_\infty \leq \varepsilon$。路由决策的变化上界为：
...
| $K=n/\log n$ | 低（$n_s=\log n$） | 最高 | 极高 | 专家性能高度异质 |

**经验法则**：对 $n=10^5$ 样本，$K \in [10, 100]$ 是合理范围，对应 $n_s \in [10^3, 10^4]$。

### 5.8 边界汇总表

| 边界类型 | 表达式 | 条件 |
|---------|-------|------|
| 风险估计误差 | $O(L\sqrt{\log(MK/\delta)/n_{\min}})$ | 有界损失, $\ell \leq L$ |
| 路由泛化 | $\tilde{O}(\sqrt{(d_{\max} + K\log M)/n})$ | VC 维 $d_{\max}$, 状态数 $K$ |
| 信息论下界 | $\Omega(\sqrt{KM/n})$ | 最坏情况分布 |
| 噪声检测 | $O(n_s^{-(1+\alpha)/(2+\alpha)})$ | Tsybakov 条件 $\alpha \geq 0$ |
| 对抗鲁棒性 | $O(\varepsilon / (\gamma_{\min} + \varepsilon))$ | $\ell_\infty$ 攻击 $\varepsilon$ |
| 分裂增益 | $\Delta I_{\text{split}} \geq \log(2)/n$ | 最小可检测增益 |
| 率失真极限 | $R_{\text{gen}} \geq R^* + \exp(-I(S;R_m))$ | 任意状态划分 |
