...
\tightlist
\item
  \hyperref[1-ux6570ux5b66ux6839ux6e90ux8ffdux6eaf]{数学根源追溯}
  ...
  \end{itemize}
\item
  \hyperref[2-ux4e09ux4e2aux547dux9898ux7684ux5b8cux6574ux8bc1ux660eux6846ux67b6]{三个命题的完整证明框架}
  ...
  \end{itemize}
\item
  \hyperref[3-ux72b6ux6001ux7a7aux95f4ux6784ux9020ux65b9ux6cd5]{状态空间构造方法}
\item
  \hyperref[4-ux5b50ux6a21ux5757ux6570ux636eux6d41ux4e0eux534fux8c03]{子模块数据流与协调}
\item
  \hyperref[5-scx-ux7406ux8bbaux8fb9ux754c]{SCX 理论边界}
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{1. 数学根源追溯}\label{ux6570ux5b66ux6839ux6e90ux8ffdux6eaf}
...
概率公理体系中三个最深刻的概念：\textbf{条件期望}、\textbf{正则条件概率}
和 \textbf{Radon-Nikodym 导数}。
...
\textbf{存在性}由 Radon-Nikodym 定理保证：定义有限符号测度
...
\tightlist
\item
  对 \(P\)-a.e. \(\omega\)，\(P(\cdot|\mathcal{G})(\omega)\) 是
  \((\Omega, \mathcal{F})\) 上的概率测度；
\item
  对每个 \(F \in \mathcal{F}\)，\(P(F|\mathcal{G})(\cdot)\) 是
  \(\mathbb{E}[\mathbf{1}_F|\mathcal{G}]\) 的一个版本；
\end{enumerate}
...
的可积性条件}{1.1.4 损失函数 \textbackslash ell 的可积性条件}}\label{ux635fux5931ux51fdux6570-ell-ux7684ux53efux79efux6027ux6761ux4ef6}
...
\paragraph{1.2.1 从 Wald 到 SCX}\label{ux4ece-wald-ux5230-scx}
...
\tightlist
\item
  \textbf{决策空间} \(\mathcal{D}\)：专家选择（从 \(M\)
  个专家中选择路由），或标签分配（acquire/relabel）
\item
  \textbf{损失函数} \(L(d, \theta)\)：选择专家 \(d\) 而真实状态为
  \(\theta\) 的代价
\item
  \textbf{风险函数}
  \(R(\pi, d) = \int_\Theta R(\theta, d) d\pi(\theta)\)：先验 \(\pi\)
  下的贝叶斯风险
\end{itemize}

SCX 的创新在于将这一框架\textbf{局部化}到每个状态 \(s\)：

\[R_m(s) = \int_{\mathcal{X}} \ell(f_m(x), f^*(x)) dP_{X|s}(x)\]

这正是\textbf{状态条件}的贝叶斯风险，其先验由状态 \(s\) 的指示函数给出。
...
可以看作一个\textbf{状态依赖的后验} \(\mathcal{Q}_x\)，其 KL
...
\paragraph{1.3.1 状态的定义}\label{ux72b6ux6001ux7684ux5b9aux4e49}
...
SCX 定义状态 \(s\) 的\textbf{数据价值}为：

\[V(s) = \bar{r}(s) \cdot \rho(s) \cdot L(s) \cdot [1 - D(s)] \cdot \max_m SCX_m(s)\]

这是五个因子的乘积。其根源可追溯到合作博弈论中的\textbf{Shapley
公理体系}和\textbf{数据 Shapley} (Ghorbani \& Zou, 2019)。
...
\tightlist
\item
  \textbf{有效性 (Efficiency)}：\(\sum_{i \in N} \phi_i(v) = v(N)\)
\item
  \textbf{对称性 (Symmetry)}：若 \(v(S \cup \{i\}) = v(S \cup \{j\})\)
  对所有 \(S \subseteq N \setminus \{i,j\}\)，则 \(\phi_i = \phi_j\)
\item
  \textbf{虚拟性 (Dummy)}：若 \(v(S \cup \{i\}) = v(S)\) 对所有
  \(S\)，则 \(\phi_i = 0\)
\item
  \textbf{可加性
...
值，而是其\textbf{可分解代理}。真正计算数据点级别的 Shapley 值是
\(O(2^N)\) 的。SCX 做了关键简化：

\begin{itemize}
\tightlist
\item
  \textbf{状态级而非样本级}：先聚合成状态 \(s\)，再计算状态价值，复杂度
  \(O(2^{|S|})\) 其中 \(|S| \ll N\)
\item
  \textbf{乘法分解}：将边际贡献分解为五个可理解的因子
\end{itemize}
...
这种分解是\textbf{启发式但可解释的}，其代价是放弃 Shapley 的加法公理。
...
SCX 的 \(V(s)\) 可以看作一个\textbf{代理函数} \(\tilde{V}(s)\)，它满足：

\[V(s) \approx \mathbb{E}_{i \in s}[\phi_i] \cdot |s|\]

但 \(V(s)\) 的乘法分解意味着 SCX 假设价值函数具有\textbf{因子可分离性}：
...
\tightlist
\item
  Dawid-Skene 假设标注者可靠性是全局常数（\(\pi^{(j)}_{kl}\) 不依赖
  \(x\)）
\item
  SCX 允许可靠性随状态 \(s\) 变化，即
...
\paragraph{1.6.2 Huber 污染模型}\label{huber-ux6c61ux67d3ux6a21ux578b}
...
其中 \(P_{X|s}^0\) 是干净数据的条件分布，\(P_{X|s}^{\text{noise}}\)
是噪声分布。

\textbf{可学习性} \(L(s) = C(s) \cdot [1 - N(s)]\) 可以严谨定义为：
...
的数据四分类中，\textbf{valuable} 数据点正是具有高信息增益的点。
...
因子对应\textbf{与参数估计的不确定性成比例}的采样准则。当 \(\bar{r}(s)\)
...
SCX 的 acquire 动作本质上是\textbf{多臂 Bandit 的变体}：
...
\paragraph{1.8.2 敏感度采样}\label{ux654fux611fux5ea6ux91c7ux6837}
...
\paragraph{1.8.3 数据集蒸馏}\label{ux6570ux636eux96c6ux84b8ux998f}
...
其中 \(\text{KeepTopK}(v, k)_i = v_i\) 若 \(v_i\) 在前 \(k\) 大，否则
\(-\infty\)。

SCX 的路由不是基于 \(x\) 本身，而是基于\textbf{状态条件风险}
\(\sum_s \gamma_s(x) R_m(s)\)。这是 MoE
门控的一种\textbf{元学习变体}：门控决策不是直接学习
\(x \to \text{expert}\) 的映射，而是通过估计条件风险来间接确定路由。
...
\begin{quote}
\textbf{命题陈述}：若存在状态 \(s_1, s_2 \in \mathcal{S}\) 和专家
\(a, b \in \{1,\ldots,M\}\)，使得 \(R_a(s_1) < R_b(s_1)\) 但
\(R_a(s_2) > R_b(s_2)\)，则不存在全局最优专家。
\end{quote}

\paragraph{2.1.1 形式化定义}\label{ux5f62ux5f0fux5316ux5b9aux4e49}

\textbf{定义 1（全局专家排序）}：全局专家排序是一个全序关系 \(\preceq\)
在专家集合 \(\{1,\ldots,M\}\) 上，满足：若 \(a \preceq b\)，则认为专家
\(a\) 整体上不劣于专家 \(b\)。
...
\paragraph{2.1.2 证明}\label{ux8bc1ux660e}

\textbf{证法 1（反证法）}： 假设存在全局最优专家 \(m^*\)。
...
\textbf{修正的证明}：我们需要证明不存在单一的 \(m^*\)
能在两个状态上\textbf{同时}最优，而非逐状态最优。
...
\paragraph{2.1.4 实证含义}\label{ux5b9eux8bc1ux542bux4e49}

这一命题直接论证了 SCX 框架的\textbf{必要性}而非充分性。它说明： -
...
\begin{quote}
\textbf{命题陈述}：在噪声存在时，单纯基于残差（max-residual
sampling）的主动采样策略不是最优的。
\end{quote}

\paragraph{2.2.1 问题形式化}\label{ux95eeux9898ux5f62ux5f0fux5316}
...
\paragraph{2.2.3 证明}\label{ux8bc1ux660e-1}
...
\textbf{定理}（命题二的正式陈述）：若存在至少一个pair \((i,j)\) 使得
\(\hat{r}_i > \hat{r}_j\) 但
...
因此 max-residual 是\textbf{次优}的。\(\square\)
...
\begin{quote}
\textbf{命题陈述}：对于专家融合（ensemble）问题，状态条件权重 \(w_m(x)\)
的期望风险不高于任何全局固定权重 \(w_m\)。
\end{quote}

\paragraph{2.3.1 形式化设定}\label{ux5f62ux5f0fux5316ux8bbeux5b9a}
...
\paragraph{2.3.2 凸性假设}\label{ux51f8ux6027ux5047ux8bbe}

损失函数 \(\ell(\cdot, \cdot)\) 对第一个参数是凸的。常见损失（MSE,
交叉熵, Hinge）均满足。

\paragraph{2.3.3 证明}\label{ux8bc1ux660e-2}

\textbf{证法 1（利用 Jensen 不等式和条件期望的塔性质）}：

步骤 1：定义状态条件风险差。

对任意固定的全局权重
\(\bar{w} = (\bar{w}_1, \ldots, \bar{w}_M)\)，全局固定融合的风险为：
...
若状态条件权重 \(w_m(X)\) 与
\(\nabla \ell(\bar{f}_{\text{ens}}) \cdot f_m(X)\) 负相关（这正是 SCX
...
当
\(\mathcal{S} = \{\text{all}\}\)（仅一个状态）时，\(\gamma_s(x) \equiv 1\)，\(w_m(x) = \frac{\exp(-\alpha \hat{R}_m)}{\sum_{m'} \exp(-\alpha \hat{R}_{m'})}\)，此时
...
\(\phi: \mathcal{X} \to \mathbb{R}^d\)}{3.1 嵌入映射 \textbackslash phi: \textbackslash mathcal\{X\} \textbackslash to \textbackslash mathbb\{R\}\^{}d}}\label{ux5d4cux5165ux6620ux5c04-phi-mathcalx-to-mathbbrd}
...
\tightlist
\item
  \textbf{保真性}：\(\phi(x)\) 应保留与专家性能 \(R_m(x)\) 相关的信息
\item
  \textbf{平滑性}：相邻点 \(\phi(x_i) \approx \phi(x_j)\) 应有相似的风险
  \(R_m(x_i) \approx R_m(x_j)\)
\item
  \textbf{计算效率}：\(\phi\) 的评估应是 \(O(d)\) 或 \(O(\log d)\) 的
\end{enumerate}
...
专家特征 \([f_1(x),\ldots,f_M(x)]\) & \(M\) & 专家性能的直接表征 &
当专家数量少时 \\
\end{longtable}
...
其中 \(\phi_{\text{feat}}(x)\)
是原始特征的降维版本，\(\phi_{\text{exp}}(x) = [f_1(x), \ldots, f_M(x)]\)
是专家输出向量。
...
\textbf{SCX 适配性}： - 自动识别噪声状态（Noise 分类） - 不需要预指定
\(K\) - 可发现任意形状的簇

\textbf{局限性}： - \(\varepsilon\) 和 MinPts 敏感 -
...
\textbf{改进}：基于层次聚类，消除 \(\varepsilon\) 参数。
...
\item
  对 MST 按边长度降序构建层次树
\item
  使用 \(\lambda = 1/\text{distance}\) 作为稳定性度量
\item
  从层次树中提取最稳定的簇
\end{enumerate}

\textbf{SCX 适配性}： - 优于 DBSCAN：可处理变密度数据 -
提供样本水平的状态归属概率（通过 \(\lambda\)-稳定性导出） -
获取开箱即用的噪声点检测

\textbf{推荐}：对 \(\mathcal{X}\) 空间复杂时，HDBSCAN 是比 GMM
更好的选择。
...
\(L = D - W\) 或归一化拉普拉斯
\(L_{\text{sym}} = I - D^{-1/2} W D^{-1/2}\) 3. 取 \(L\) 的 \(K\)
个最小特征向量构成 \(U \in \mathbb{R}^{n \times K}\) 4. 对 \(U\)
的行进行 \(K\)-means

\textbf{SCX 适配性}： - 擅长发现非凸簇（环状、螺旋状） -
对专家性能局部分布复杂的场景有效 - 可导出类似软赋值的谱嵌入空间

\textbf{局限性}： - \(O(n^3)\) 复杂度（可近似为 \(O(n^2)\)） - \(K\)
需预设 - 对新样本无法直接预测（需要 Nystroem 逼近）

\subsubsection{\texorpdfstring{3.4 软状态赋值
\(\gamma_s(x)\)}{3.4 软状态赋值 \textbackslash gamma\_s(x)}}\label{ux8f6fux72b6ux6001ux8d4bux503c-gamma_sx}

\paragraph{3.4.1 定义}\label{ux5b9aux4e49}
...
\textbf{方法 3：t-SNE/UMAP 嵌入上的 Softmax}
...
\textbf{SCX 选择准则}：\(K^* = \arg\max_K S(K)\)。但 silhouette
...
最优 \(K\) 是使得 \(\text{Gap}(K) \geq \text{Gap}(K+1) - s_{K+1}\)
的最小值。

\textbf{SCX 适配性}：可替换 \(W_K\) 为聚类内风险方差之和
...
\subsubsection{4.1 整体架构}\label{ux6574ux4f53ux67b6ux6784}
...
\textbf{数据流方向}：单向，状态构造仅在数据新增或状态分裂时触发。
...
\tightlist
\item
  对新样本 \(x_{\text{new}}\) 计算嵌入 \(\phi(x_{\text{new}})\)
\item
  用现有聚类模型计算 \(\gamma_s(x_{\text{new}})\)，分配状态
\item
  轻量更新状态统计量：\(|s|_{\text{new}} = |s|_{\text{old}} + \gamma_s(x_{\text{new}})\)
\item
  增量更新 \(R_m(s)\)：使用 Welford 在线算法
\item
  重新评估 \(V(s)\)，若变化超过阈值则触发动作
\end{enumerate}
...
\tightlist
\item
  \textbf{discard}：从数据集中移除样本，更新状态统计
\item
  \textbf{split}：在状态内重新运行聚类（\(K=2\)），更新全局状态索引
\item
  \textbf{relabel}：用新标签覆盖旧标签，重新计算风险
\item
  \textbf{acquire}：将新标注样本加入数据集，触发新数据到达流程
\end{itemize}
...
\textbf{总复杂度}：初始构建 \(O(n \log n + n M)\)，增量更新
\(O(n_{\text{new}} M)\)，适合流式场景。

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{5. SCX 理论边界}\label{scx-ux7406ux8bbaux8fb9ux754c}
...
\paragraph{5.1.1 有界损失}\label{ux6709ux754cux635fux5931}

设 \(\ell \in [0, L]\)，对任意 \(\delta > 0\)，以概率 \(1 - \delta\)：

\[|\hat{R}_m(s) - R_m(s)| \leq L \sqrt{\frac{\log(2/\delta)}{2 n_s}}\]

\paragraph{5.1.2 亚高斯损失}\label{ux4e9aux9ad8ux65afux635fux5931}
...
\textbf{警告}：当某个状态 \(s\) 的 \(n_s\)
很小时（如样本稀疏状态），该边界失去意义。这是 SCX
的固有局限------\textbf{状态划分越细，每个状态内的样本越少，风险估计的方差越大}。
...
\(P_{\dim}(\mathcal{F}_m)\)（回归）。则对任意 \(\delta > 0\)，以概率
\(1 - \delta\)：
...
其中 \(d_{\phi}\) 是嵌入函数的 Rademacher 复杂度或 VC 维（如 UMAP
的参数自由度）。

\subsubsection{5.3 信息论下界}\label{ux4fe1ux606fux8bbaux4e0bux754c}
...
设定了一个\textbf{必要条件的样本复杂度}------每个状态至少需要
\(O(\log M)\) 个样本来判断最优专家。
...
若 \(\Delta I_{\text{split}} < \log(2)/n\)（每个子状态的样本少于 2
个的等效信息量），则分裂不应执行。

\paragraph{5.5.2
最优状态数的信息论界}\label{ux6700ux4f18ux72b6ux6001ux6570ux7684ux4fe1ux606fux8bbaux754c}

记 \(R_{\text{gen}}(\mathcal{S})\) 为状态划分 \(\mathcal{S}\)
下的期望泛化风险。由率失真理论：
...
\(R_{\text{gen}}(\mathcal{S})\)：需要更多状态（高 \(I(S; R_m)\)） -
...
在 \(\ell_\infty\)
攻击下，\(\tilde{\gamma}_s(x) = \gamma_s(x) + \delta_s(x)\)，\(\|\delta\|_\infty \leq \varepsilon\)。路由决策的变化上界为：
...
防御：\(\gamma\)-平滑}{5.6.2 防御：\textbackslash gamma-平滑}}\label{ux9632ux5fa1gamma-ux5e73ux6ed1}
...
\textbf{经验法则}：对 \(n=10^5\) 样本，\(K \in [10, 100]\)
是合理范围，对应 \(n_s \in [10^3, 10^4]\)。

\subsubsection{5.8 边界汇总表}\label{ux8fb9ux754cux6c47ux603bux8868}
...
\bottomrule\noalign{}
\endlastfoot
风险估计误差 & \(O(L\sqrt{\log(MK/\delta)/n_{\min}})\) & 有界损失,
\(\ell \leq L\) \\
路由泛化 & \(\tilde{O}(\sqrt{(d_{\max} + K\log M)/n})\) & VC 维
\(d_{\max}\), 状态数 \(K\) \\
信息论下界 & \(\Omega(\sqrt{KM/n})\) & 最坏情况分布 \\
噪声检测 & \(O(n_s^{-(1+\alpha)/(2+\alpha)})\) & Tsybakov 条件
\(\alpha \geq 0\) \\
对抗鲁棒性 & \(O(\varepsilon / (\gamma_{\min} + \varepsilon))\) &
\(\ell_\infty\) 攻击 \(\varepsilon\) \\
分裂增益 & \(\Delta I_{\text{split}} \geq \log(2)/n\) &
最小可检测增益 \\
率失真极限 & \(R_{\text{gen}} \geq R^* + \exp(-I(S;R_m))\) &
任意状态划分 \\
\end{longtable}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{附录：符号对照表}\label{ux9644ux5f55ux7b26ux53f7ux5bf9ux7167ux8868}
