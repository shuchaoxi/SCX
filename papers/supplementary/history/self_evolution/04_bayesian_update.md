...
> self-evolution loop **Notation note**: In this document, \(S\)
...
\(S_t(x,y)\) estimates \(P(clean \mid x, y)\) 
...
**Goal of this document**: Interpret the \(S_t\) evolution as a
...
**Definition 4.1 (Bayesian Gatekeeper).** At time \(t\), the
...
\(S^*(x,y) = P(clean \mid x, y)\) is the (unknowable) oracle
reliability.
...
\[
P_0(S) = Prior belief about the gatekeeper function
\]
...
raw data but a **filtered collection**: each sample was accepted by
a previous gatekeeper \(S_i\). We must account for this selection bias.

**Definition 4.3 (Selection-Aware Likelihood).** Given a candidate
gatekeeper function \(S\), the likelihood of observing a memory bank
\(\mathcal{M}_t\) is:

\[
P(\mathcal{M}_t \mid S) = \prod_{i=1}^{N_t} P(x_i, y_i \mid S, accepted at time  i)
\]

Applying Bayes' rule to the acceptance event:

\[
P(x, y \mid S, accepted) \propto P(accepted \mid x, y, S) \cdot P(x, y)
\]
...
\[
P(accepted \mid x, y, S) = S(x, y)
\]

**Proposition 4.1 (Likelihood Form).** Under the assumption that
...
\[
\log P(\mathcal{M}_t \mid S) = \sum_{i=1}^{N_t} \log S(x_i, y_i) + \sum_{i=1}^{N_t} \log P(x_i, y_i) + const
\]

where the constant accounts for the normalization of the selection
distribution.

**Remark.** The second term \(\sum \log P(x_i, y_i)\) does not
depend on \(S\) and can be absorbed into the constant. The essential
likelihood is:

\[
\log P(\mathcal{M}_t \mid S) = \sum_{i=1}^{N_t} \log S(x_i, y_i) + const(S-independent)
\]

*Proof sketch.* By the acceptance mechanism: each sample enters
...
**Assumption SE-A3 (Memory Bank Conditional Independence).**
...
**Equivalent incremental form.** Writing the non-accepted data as
\(\tilde{\mathcal{D}}_{t+1}\) (samples that were evaluated but
rejected), the posterior becomes:

\[
P_{t+1}(S) \propto P_t(S) \cdot \underbrace{\left[\prod_{(x,y) \in \mathcal{D}_{t+1}} S(x,y)\right]}_{accepted} \cdot \underbrace{\left[\prod_{(x,y) \in \tilde{\mathcal{D}}_{t+1}} (1 - S(x,y))\right]}_{rejected}
\]

*Interpretation.* The gatekeeper update is a Bayesian binary
...
**Corollary 4.1 (Posterior Recursion).** The sequence of posterior
...
**Proposition 4.2 (Optimality of Posterior Mean).** The posterior
...
*Proof.* Standard result: the posterior mean minimizes Bayes risk
under squared error loss. See {[}Berger, 1985, Section 4.4{]}.

**Connection to SCX consensus.** When the prior is flat and only
...
**Assumption SE-A4 (Logit-Normal Likelihood).** The acceptance
...
**Theorem 4.2 (GP Posterior Update).** Under the GP prior and
...
likelihood), the **Laplace approximation** yields a Gaussian
posterior:
...
**Conjugate special case: Gaussian likelihood.** If we use a
...
**Proposition 4.3 (Conjugate Sequential GP Update).** Under the
...
*Proof.* Standard sequential GP update formulas. See {[}Rasmussen
...
\(\mathcal{F}_t = \sigma(\mathcal{M}_1, ..., \mathcal{M}_t)\):
...
&= \mathbb{E}[S(x,y) \mid \mathcal{M}_{1:t}] \quad (tower property) 
...
**Corollary 4.2 (Pointwise Martingale).** For any fixed \((x,y)\),
the sequence \(\{S_t(x,y)\}_{t \geq 0}\) is a martingale bounded in
\([0,1]\).

**Corollary 4.3 (Vector Martingale).** If
...
*Proof.* The martingale convergence theorem (Doob, 1953) states
...
**Proposition 4.4 (Structure of the Limit).** The limit
...
**Proposition 4.5 (Convergence Rate).** Under the GP prior with
...
*Proof sketch.* For GP regression with i.i.d. Gaussian noise, the
...
**Theorem 4.5 (Behavior Under Prior Misspecification).** Let
...
**(a) Posterior inconsistency.** The posterior distribution \(P_t\)
does **not** concentrate on \(S^*\):
...
*Proof sketch.* Part (a) follows from the definition of support: if
...
\[
D_{KL}(P^* \parallel P_t) = \mathbb{E}_{(x,y) \sim P^*} \left[ \log \frac{P^*(x,y)}{P_t(x,y)} \right]
\]

**Theorem 4.6 (KL Contraction).** Under the Bayesian update
framework with correctly specified prior (\(S^* \in supp(P_0)\))
and regularity conditions (identifiability, integrability, and Doob's
consistency conditions):

\[
D_{KL}(P^* \parallel P_t) \xrightarrow{a.s.} 0 \quad as  t \to \infty
\]

Moreover, the contraction rate satisfies:

\[
\mathbb{E}[D_{KL}(P^* \parallel P_t)] \leq \frac{D_{KL}(P^* \parallel P_0)}{t} \quad (under i.i.d. sampling)
\]

*Proof sketch.* This follows from Doob's consistency theorem for
...
**Proposition 4.6 (KL Decomposition).** The KL divergence
decomposes into prior mismatch and learning components:

\[
D_{KL}(P^* \parallel P_t) = \underbrace{D_{KL}(P^* \parallel P_0)}_{initial gap} - \underbrace{\sum_{i=1}^t \mathbb{E}_{P^*}[\log \ell_i(S) - \log \mathbb{E}_{P_{i-1}}[\ell_i(S)]]}_{cumulative learning}
\]

where \(\ell_i(S) = P(\mathcal{D}_i \mid S)\) is the likelihood of batch
\(i\).

*Proof.* Write the posterior
...
*Proof sketch.* The Bernstein-von Mises theorem (van der Vaart,
...
**Corollary 4.5 (Asymptotic Normality of Gatekeeper).** For any
...
**Corollary 4.6 (Asymptotic Credible Intervals).** Under the
Bernstein-von Mises theorem, the \(1-\alpha\) credible interval for
\(S^*(x,y)\) is asymptotically:

\[
S_t(x,y) \pm z_{\alpha/2} \cdot \sqrt{\frac{\hat^2_t(x,y)}{N_t}}
\]

where \(\hat^2_t(x,y)\) is the estimated posterior variance and
\(z_{\alpha/2}\) is the standard normal quantile.

**Important caveat.** The Bernstein-von Mises theorem requires
...
**misspecified Bernstein-von Mises** phenomenon (Kleijn \& van der
Vaart, 2012).
...
\bottomrule\noalign{}
\endlastfoot
Theorem 4.1: Bayesian update recursion & **Proven** & Direct
application of Bayes' rule 

Proposition 4.1: Likelihood form & **Proven** & Derived from
acceptance mechanism 

Proposition 4.2: Posterior mean optimality & **Proven** & Standard
Bayes risk minimization 

Theorem 4.3: Posterior mean martingale & **Proven** & Tower
property + conditional expectation 

Corollary 4.2: Pointwise martingale & **Proven** & Direct
consequence of Theorem 4.3 

Theorem 4.4: Martingale convergence & **Proven** & Doob's
martingale convergence theorem 

Proposition 4.3: Conjugate GP update & **Proven** & Standard GP
regression formulas 

Theorem 4.5: Prior misspecification & **Proven** & KL projection +
posterior concentration theory 

Theorem 4.6: KL contraction & **Proven under regularity
conditions** & Doob's consistency + LAN conditions 

Proposition 4.4: Structure of the limit & **Proven** & Limit of
Bayes estimator under squared error 

Proposition 4.5: Convergence rate & **Conjectured** & Rate
\(O_p(t^{-1/2})\) holds for GP regression with finite-trace kernel;
general case depends on kernel eigen-decay 

Theorem 4.7: Bernstein-von Mises & **Proven under regularity
conditions** & Standard BvM + infinite-dimensional extension (Castillo \&
Nickl, 2013) 

Proposition 4.6: KL decomposition & **Proven** & Algebraic
manipulation of log posterior 

Corollary 4.4: Misspecification diagnostics & **Heuristic** & The
drift diagnostic is empirically motivated 

\end{longtable}

**Key uncertainty**: The convergence rate (Proposition 4.5) depends
...
### 15. References<!-- label: references -->
...
*End of 04\_bayesian\_update.md -- Bayesian update interpretation
