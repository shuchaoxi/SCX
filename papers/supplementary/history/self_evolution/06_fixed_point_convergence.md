# Fixed Point and Convergence Theorem of SCX
Self-Evolution

**Author:** SCX

> **Version**: 2026-06-28 |{} **Status**: Theoretical
> framework (partial proof) |{} **Scope**: Central theorem
> establishing convergence regimes for the self-evolution loop
> **Notation note**: \(\mathcal{S}\) denotes the state space
...
$$
\mathcal{M}_t &\gets AcceptOrReject(\mathcal{D}_t, S_t) \quad &(gatekeeping) 

\theta_{t+1} &\gets \theta_t - \alpha_t \nabla_\theta \ell(f_{\theta_t}, \mathcal{M}_t) \quad &(student update) 

S_{t+1} &\gets UpdateSCX(S_t, \mathcal{M}_t, f_{\theta_{t+1}}) \quad &(gatekeeper update)
$$

\]

**Central question**: Does the sequence
...
### 2. Theorem SE-1: Convergence of SCX Self-Evolution (Formal
Statement)<!-- label: theorem-se-1-convergence-of-scx-self-evolution-formal-statement -->

**Theorem SE-1 (Convergence of SCX Self-Evolution).** Let
...
\theta_{t+1} &= \theta_t - \alpha_t \nabla_\theta \ell(f_{\theta_t}(x_t), y_t), \quad (x_t, y_t) \sim Unif(\mathcal{M}_t) 

S_{t+1} &= \Pi_{[0,1]} \left[ S_t + \beta_t \cdot \bigl( SCXUpdate(S_t, \mathcal{M}_{t+1}, f_{\theta_{t+1}}) - S_t \bigr) \right]
$$
...
> **Reference-set evaluation (C10).** The gatekeeper update
> \(SCXUpdate(S_t, \mathcal{M}_{t+1}, f_{\theta_{t+1}})\) is
...
**(C1) Finite structure space.** The input space \(\mathcal{X}\) is
finite or has finite covering dimension: \(\dim(\mathcal{X}) < \infty\).

**\textgreater{} FIXED (2026-06-28, DEFECT-15):** The original
``finite \(\mathcal{X}\)'' assumption is violated in practice
(continuous \(\mathbb{R}^{d_\phi}\) feature spaces). This section
replaces it with a **covering number argument** that achieves the
...
**(C2) Lipschitz student.** The NEP student \(f_\theta\) is
...
**(C3) Lipschitz gatekeeper.** The SCX update function is Lipschitz
continuous with constant \(L_S\):

\[
\|SCXUpdate(S_1, \mathcal{M}, f) - SCXUpdate(S_2, \mathcal{M}, f)\|_\infty \leq L_S \|S_1 - S_2\|_\infty
\]

**(C4) Learning rates.** The student learning rate \(\alpha_t\)
...
> **FIXED (2026-06-28, B8):** The original C4 listed
...
> as an **explicit separate sub-condition (C4b)** --- it does NOT
> follow from \(\beta_t = o(\alpha_t)\) (C6) combined with
> \(\sum \alpha_t^2 < \infty\). See the counterexample in the C6
> discussion below.
> 
> **Resolution of the tension:** The gatekeeper exploration
...
**(C5) Conditional i.i.d. sampling.** Conditioned on \(S_t\), the
...
**(C6) Two-timescale separation (corrected --- replaces original
...
\[
\boxed{\;\beta_t = o(\alpha_t)\;}, \qquad i.e., \quad \lim_{t\to\infty} \frac{\beta_t}{\alpha_t} = 0.
\]

**Rationale (DEFECT-14 fix).** The original condition C6
...
**Jointly satisfiable scheduling scheme (corrected B8).** The
...
\[\alpha_t = t^{-a}, \quad \beta_t = t^{-b}, \quad with \quad \frac{1}{2} < a < 1 < b.\]
...
**Consequence of violation.** Without two-timescale separation
...
**FIXED (2026-06-28, B8): \(\sum \beta_t < \infty\) is a separate
...
**mathematically false**, as shown by the following counterexample:
...
controls the **squares** of \(\alpha_t\), not \(\alpha_t\) itself.
...
**explicit separate requirement** in C4 (below), independent of the
...
**(C7) Gatekeeper bounded gradient.** The SCX update increment is
bounded:

\[
\|SCXUpdate(S, \mathcal{M}, f) - S\|_\infty \leq B_S < \infty
\]

**(C8) Annealing acceptance threshold (sufficient, not
...
**(C9) Random exploration fraction (sufficient, not necessary).** A
fixed fraction \(\varepsilon_{explore} > 0\) of accepted samples
are chosen uniformly at random regardless of \(S_t\) score, ensuring all
regions remain represented in \(\mathcal{M}_t\).

> **B7 FIX --- \(\varepsilon > 0\) lower bound for importance
...
> \(\varepsilon_{explore} = 0\) (no exploration), the gatekeeper
...
> 
> **Resolution:** Condition C9 requires
> \(\varepsilon_{explore} > 0\) **strictly**. With
> \(\varepsilon_{explore} > 0\), every \(x\) with \(P_0(x) > 0\)
> has \(P_{S_t}(x) \geq \varepsilon_{explore} \cdot P_0(x) > 0\)
> (the uniform exploration component ensures non-zero probability).
> Consequently:
> \[\|w\|_\infty = \sup_{x} \frac{P_0(x)}{P_{S_t}(x)} \leq \frac{1}{\varepsilon_{explore}} = W < \infty.\]
> The bound \(W\) is finite and explicit:
> \(W = 1/\varepsilon_{explore}\). As
> \(\varepsilon_{explore} \to 0\), \(W \to \infty\), and the
> variance of the importance-weighted gradient estimator diverges ---
> confirming that **\(\varepsilon_{explore} > 0\) is a
> necessary condition** for the Lyapunov descent claim in Theorem 12.5 to
> be non-vacuous.

> **Note on C8-C9**: These are sufficient conditions to mitigate the
...
**(C10) Reference-set replay (REQUIRED for Lyapunov descent).** A
...
\[\Psi(S_t, \theta_t) = \frac{1}{|\mathcal{M}_0|} \sum_{x \in \mathcal{M}_0} (S_t(x, y(x)) - \hat{C}(x))^2 + \frac{|\mathcal{V}_0|} \sum_{(x,y) \in \mathcal{V}_0} \ell(f_{\theta_t}(x), y).\]

> **Critical dependency**: Condition C10 is the bridge between Lemma
> SE-1.1 and Theorems 12.2/12.5. Theorem 12.2 proves that Lyapunov descent
> is **formally impossible** without reference-set replay (the
> gatekeeper can trivially reduce loss by accepting only easy samples).
> Theorem 12.5 proves that **with** reference-set replay and
> importance sampling, Lyapunov descent is achievable. Lemma SE-1.1 is
> therefore **conditional on C10** --- its ``CONDITIONAL'' status in
> the table below assumes C10 holds. **Without replay, Lemma SE-1.1
...
**FIXED (2026-06-28): Covering Number Argument.** We replace the
...
**Practical interpretation.** This covering-number construction
...
**any** feature space embedded in \(\mathbb{R}^{d_\phi}\) ---
...
**Remaining gap.** The infinite-space limit \(\varepsilon \to 0\)
...
#### 3.2 Conditions C2-C3: Lipschitz
...
##### 3.2.1 Lemma SE-1.0: Indicator Discontinuity Is Measure-Zero
(B9
Fix)<!-- label: lemma-se-1.0-indicator-discontinuity-is-measure-zero-b9-fix -->

**Lemma SE-1.0 (Lipschitz-Almost-Everywhere Consensus).** Under
...
**Why this resolves the B9 gap.** The ARXIV verification identified
...
*Proof of Lemma SE-1.0.* For each expert \(m\), the student loss
...
**Implications for the proof chain:** - The Lyapunov descent (Lemma
SE-1.1) uses \(\mathbb{E}[\cdot]\), which is unchanged by null-set
modifications. - The SCX update
\(SCXUpdate(S_t, \mathcal{M}_t, f_{\theta_t})\) inherits
...
**the indicator discontinuity does NOT break the proof chain**. The
gap identified in the ARXIV verification is closed by this lemma.

**Proposition SE-1.0 (Lipschitz Composition).** Under C2 and C3,
...
#### 3.3 Conditions C4-C6: Learning Rates and
...
#### 3.4 Condition C7: Bounded Gatekeeper
...
### 4. Conclusion of Theorem
SE-1<!-- label: conclusion-of-theorem-se-1 -->

Under conditions (C1)-(C7), the sequence \((S_t, \theta_t)\) converges
to a **fixed point** \((S^*, \theta^*)\) almost surely, where:

**(a) Self-consistent gatekeeper.** \(S^*\) is self-consistent in
the Bayesian sense:

\[
S^*(x,y) = P(clean \mid x, y, \mathcal{M}_\infty), \quad \forall (x,y) \in \mathcal{X} \times \mathcal{Y}
\]
...
\[
S^* = SCXUpdate(S^*, \mathcal{M}_\infty, f_{\theta^*})
\]

**(b) Optimal NEP student (local).** \(\theta^*\) is a local
minimum of the expected NEP loss under the limiting data distribution
induced by \(S^*\):

\[
\theta^* = \arg\min_{\theta \in \Theta} \mathbb{E}_{(x,y) \sim P_{S^*}}[\ell(f_\theta(x), y)] \quad (local)
\]

where \(P_{S^*}(x,y) \propto S^*(x,y) \cdot P_0(x,y)\) is the
acceptance-biased distribution at the fixed point.

**(c) Self-consistency equation.** The fixed point satisfies the
coupled system:
...
**(d) Lyapunov convergence.** The total loss function
...
### 5. Lemma SE-1.1: Lyapunov Function
...
\[
V_t = V(S_t, \theta_t) = \mathbb{E}_{(x,y) \sim P_{S_t}}[\ell(f_{\theta_t}(x), y)] + \lambda \cdot \|S_t - S^*_{prior}\|^2_{\mathcal{L}^2}
\]

where \(\lambda > 0\) regularizes the gatekeeper toward a prior
\(S^*_{prior}\) (e.g., the initial SCX score), and \(P_S\) is the
acceptance-biased distribution under gatekeeper \(S\).

Under conditions (C2)-(C7) and **C10** (reference-set replay ---
...
\[
\eta_t = \alpha_t \cdot \|\nabla_ L_t(\theta_t)\|^2 + \beta_t \cdot \|\Delta S_t\|^2_{\mathcal{L}^2} + o(\alpha_t + \beta_t)
\]

with
\(\Delta S_t = SCXUpdate(S_t, \mathcal{M}_t, f_{\theta_t}) - S_t\).
...
**Corollary SE-1.1 (Convergence of \(V_t\)).** The Lyapunov
...
\[
\sum_{t=1}^\infty \bigl( \alpha_t \|\nabla_\theta L_t(\theta_t)\|^2 + \beta_t \|\Delta S_t\|^2_{\mathcal{L}^2} \bigr) < \infty \quad a.s.
\]

#### 5.1 Selection Bias Cycle Analysis (DEFECT-13
Fix)<!-- label: selection-bias-cycle-analysis-defect-13-fix -->

A critical concern for the Lyapunov descent proof is the
**selection bias cycle** inherent in the self-evolution loop:

\[Gatekeeper  S_t \xrightarrow{selects} Memory  \mathcal{M}_{t+1} \xrightarrow{trains} NEP  f_{\theta_{t+1}} \xrightarrow{feedback} Gatekeeper  S_{t+1}\]

**The problem.** At each iteration, the gatekeeper \(S_t\) selects
...
**Formalization.** Let \(P_0\) be the original data distribution
...
measures performance on the **acceptance-biased** distribution, not
on the original distribution \(P_0\). A decrease in \(V_t\) does
**not** guarantee improvement on \(P_0\) --- it may simply reflect
the gatekeeper becoming better at selecting easy samples.

**Quantifying the distribution shift.** The total variation between
...
**Degradation under bias accumulation.** If the selection bias is
not controlled, the effective information available to the system
degrades. Let
\(\mathcal{R}_{excluded}(t) = \{x : S_t(x,y) < \gamma_t\}\) be
the set of excluded samples. The student's generalization error on the
full distribution \(P_0\) satisfies:

\[\mathbb{E}_{P_0}[\ell(f_{\theta_t}(x), y)] = \mathbb{E}_{P_{S_t}}[\ell(f_{\theta_t}(x), y)] \cdot \mathbb{P}(S_t \geq \gamma_t) + \mathbb{E}_{\mathcal{R}_{excluded}}[\ell(f_{\theta_t}(x), y)] \cdot \mathbb{P}(S_t < \gamma_t).\]

The second term is unobserved if no exploration is performed, and can
grow without detection.

**Status.** The selection bias cycle is the primary obstacle to
proving Lyapunov descent. The mitigation strategies (C8, C9) are
**conjectured** to be sufficient, but a rigorous proof that they
break the self-reinforcing cycle remains open. Without these
mitigations, the Lyapunov decrease on \(P_{S_t}\) is **not** a
...
### 6. Lemma SE-1.2: Finite Cover of Memory Bank
Types<!-- label: lemma-se-1.2-finite-cover-of-memory-bank-types -->

**Lemma SE-1.2 (Finite Memory Bank Configurations).** Under
condition C1 (finite \(\mathcal{X}\)) and the acceptance criterion
\(S_t(x,y) \geq \gamma\):

**(a)** The number of possible memory bank configurations is
finite.

**(b)** The memory bank \(\mathcal{M}_t\) evolves monotonically
(\(\mathcal{M}_t \subseteq \mathcal{M}_{t+1}\)) and stabilizes after
finitely many additions:

\[
\mathcal{M}_t = \mathcal{M}_{t+1} = ... = \mathcal{M}_\infty \quad for all sufficiently large  t
\]

**(c)** Consequently, a.s. there exists a finite time \(T\) such
...
**Corollary SE-1.2 (Infinite \(\mathcal{X}\) Extension).** If
...
\[
\sup_{x,y} |S_t(x,y) - S_{t'}(x,y)| < \varepsilon \quad for all  t, t' \geq T_\varepsilon
\]
...
### 7. Lemma SE-1.3: Parameter Displacement
Vanishes<!-- label: lemma-se-1.3-parameter-displacement-vanishes -->

**Lemma SE-1.3 (Vanishing Parameter Displacement).** Under
conditions (C2)-(C7):

\[
\|\theta_{t+1} - \theta_t\| \xrightarrow{a.s.} 0 \quad and \quad \|S_{t+1} - S_t\|_\infty \xrightarrow{a.s.} 0
\]
...
**Corollary SE-1.3 (Joint Convergence).** The sequence
\((S_t, \theta_t)\) is Cauchy in the product space
\(\mathcal{L}^\infty(\mathcal{X} \times \mathcal{Y}) \times \mathbb{R}^{d_\theta}\)
with probability 1:

\[
\lim_{t \to \infty} \sup_{t' \geq t} \left( \|S_{t'} - S_t\|_\infty + \|\theta_{t'} - \theta_t\| \right) = 0 \quad a.s.
\]
...
### 8. Lemma SE-1.4: Limit Points Are Fixed
Points<!-- label: lemma-se-1.4-limit-points-are-fixed-points -->

**Lemma SE-1.4 (Fixed Point Characterization).** Let
...
\[
S_\infty = SCXUpdate(S_\infty, \mathcal{M}_\infty, f_{\theta_\infty})
\]
...
**(c) Cross-consistency.** The pair \((S_\infty, \theta_\infty)\)
...
**Step 2: Memory bank stabilization (Lemma SE-1.2).** Because
...
**Step 3: Displacement vanishes (Lemma SE-1.3).** From the
...
**Step 4: Convergence of parameter sequence.** The Lyapunov descent
...
**Step 5: Limit is a fixed point (Lemma SE-1.4).** Having
...
**Step 6: Structure of the fixed point.** The fixed point satisfies
the self-consistency conditions stated in the theorem:

- 
- 

**Remark on the proof.** Step 2 relies crucially on the finite
...
#### Regime 1: Classical Convergence (Conditions C1-C7 fully
...
**Formal guarantee.** Under conditions (C1)-(C7), Theorem SE-1
provides:
...
#### Regime 2: Limit Cycle (C4/C6 partially
...
**Proposition SE-1.1 (Limit Cycle Conditions).** If
...
\[
S_{t+T} = S_t, \quad \theta_{t+T} = \theta_t, \quad \forall t  sufficiently large
\]

*Proof sketch.* The system becomes a deterministic discrete-time
...
**Conjectured extension.** For infinite \(\mathcal{X}\), limit
...
#### Regime 3: Perpetual Discovery (C1 violated in data
...
**Proposition SE-1.2 (Perpetual Discovery Bound).** Under perpetual
...
**Conjectured bound.** Under perpetual discovery, the expected
student loss satisfies:

\[
\liminf_{t \to \infty} \mathbb{E}[\ell(f_{\theta_t}(x), y)] \geq \inf_ \mathbb{E}_{P_\infty}[\ell(f_\theta(x), y)] + \tau \cdot \limsup TV(P_t, P_\infty)
\]
...
#### Regime 4: Divergent Collapse (S\_t becomes
...
**Proposition SE-1.3 (Divergent Collapse Condition).** Divergent
...
**Prevention mechanisms.** The following mechanisms prevent
divergent collapse:
...
**Empirical observation.** Divergent collapse is rarely observed in
...
\begin{longtable}[]{@{}
  >{\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.0941}}
  >{\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1765}}
  >{\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1412}}
  >{\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1294}}
  >{\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.3059}}
  >{\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1529}}@{}}
\toprule\noalign{}
\begin{minipage}[b]
Regime
...
**Phase diagram (conjectured).** The behavior is governed by two
key parameters:
...
\[
Regime = \begin{cases}
1  (Classical) & if  \rho_{explore} = 0  and  r \to 0 

2  (Limit cycle) & if  \rho_{explore} = 0  and  r  is bounded away from  0 

3  (Discovery) & if  \rho_{explore} > 0 

4  (Collapse) & if  S_t  threshold never met
\end{cases}
\]

This phase diagram is **conjectured** --- a rigorous analysis of
...
### 12. Connection to Theorem 1 (Noise
Detection)<!-- label: connection-to-theorem-1-noise-detection -->

**Proposition SE-1.4 (Noise Detection During Self-Evolution).**
...
\[
F1_t \geq 1 - \frac{1} \sum_{s \in \mathcal{S}} \rho_s \cdot \exp\!\bigl(-2M\Delta_s(t)^2\bigr)
\]
...
*Proof.* Theorem 1's assumptions (A1)-(A6) are about the
**data generation process** and **expert training**, not about
...
to the **original** noise detection task, which is invariant.

**Corollary SE-1.5 (Monotonic Noise Detection Improvement).** If
...
\[
\Delta_s(t) \geq \Delta_s(0) \quad for sufficiently large  t
\]

This is **not guaranteed** --- if the gatekeeper biases the sample
distribution away from certain states, the state estimates may degrade.
In Regime 4 (collapse), \(\Delta_s(t) \to 0\).

**Implication.** The noise detection guarantee provides a
**lower bound** on performance that holds regardless of
...
**Remark on Correct Global F1 Aggregation (Lemma F Fix).**
Proposition SE-1.4 uses Theorem 1's bound, which correctly operates on
the **global confusion matrix**:

\[TP_{global} = \eta N \sum_s \rho_s \cdot TPR_s,\quad
FP_{global} = (1-\eta)N \sum_s \rho_s \cdot FPR_s,\quad
FN_{global} = \eta N \sum_s \rho_s \cdot FNR_s.\]

The global F1 score is
\(F1_{global} = 2 \cdot TP_{global} / (2 \cdot TP_{global} + FP_{global} + FN_{global})\).
The Hoeffding bound applies directly to \(FPR_{global}\) and
\(FNR_{global}\) because these error rates are **linear** in
per-state quantities
(\(FPR_{global} = \sum_s \rho_s \cdot FPR_s\),
\(FNR_{global} = \sum_s \rho_s \cdot FNR_s\)). The F1 algebraic
inequality (Lemma B) then yields the stated bound without requiring
additivity of F1 itself.

> **Corrected from original Lemma F (DEFECT-01/02).** The original
> claim \(F1_{global} = \sum_s \rho_s \cdot F1_s\) is
> **mathematically false**: F1 is a harmonic mean, not a linear
> expectation. The correct relationship expands \(1 - F1_{global}\)
> as
> \(\frac{1}{2}FNR_{global} + \frac{1-\eta}{2\eta}FPR_{global} + o(e^{-M\kappa_})\),
> where the linearity of FNR and FPR allows state-wise summation. States
> with \(\kappa_s > \kappa_\) contribute
> \(o(e^{-M(\kappa_s-\kappa_)})\) and are asymptotically negligible.

**Remark on Bahadur-Rao Lattice Correction (DEFECT-06).** For
...
corrected minimax constant \(C_^{(corr)}\) differs from the
original \(C_\) by a multiplicative factor
\(\mathcal{L}(\lambda_0^*, \lambda_1^*)\) of approximately
\(1.05\)--\(1.12\) for typical parameter ranges. **The minimax
...
constant matching \(C_{SCX}/C_^{(corr)} = 1 + o(1)\)
is preserved. Only the **claimed numerical value** of the constants
changes.

**Corrected \(C_\) formula:**
\[C_^{(corr)} = \frac{2} \left(\frac{1-\eta}\right)^{s} \frac{1/\lambda_0^* + 1/|\lambda_1^*|}{\sqrt{\theta^*(1-\theta^*)}} \times \mathcal{L}(\lambda_0^*, \lambda_1^*),\]
...
### 13. Connection to Theorem 3
(Unidentifiability)<!-- label: connection-to-theorem-3-unidentifiability -->

**Proposition SE-1.5 (Corrected --- Non-Resolution of
...
\(\mathcal{P}_{noise}\) and \(\mathcal{P}_{hard}\) with
...
The self-evolution loop **does not resolve this
...
**Why the original \(1/\sqrt{N_t}\) rate was incorrect.** The
original Proposition SE-1.5 confused two distinct problems: -
**Testing two simple hypotheses with TV \textgreater{} 0**: Error
decays as \(O(1/\sqrt{N})\) (standard Chernoff-Stein regime). -
**Testing two hypotheses with TV = 0** (Theorem 3's construction):
Error is **constant** with \(N\) --- the two hypotheses produce
...
\(TV(\mathcal{P}_{noise}, \mathcal{P}_{hard}) = 0\), the
error is \(\eta\rho/2\) for all \(N\), including \(N \to \infty\). The
claimed \(1/\sqrt{N_t}\) decay does not apply.

**What self-evolution CAN improve.** However, the self-evolution
loop can **improve state estimation** (via Theorem 5's cluster
consistency) and **reduce expert error rates** (via better feature
representations from the NEP student). These improvements indirectly
tighten the **bounding constants** in Theorem 1's F1 guarantee.
They are **orthogonal** to the unidentifiability in Theorem 3: they
improve performance on the *identifiable* portion of the data but
cannot resolve the *unidentifiable* portion.

**Corollary SE-1.6 (Corrected).** The asymptotic bound
\(\lim_{t\to\infty} Error_{minimax}(t) = 0\) claimed in
the original version does **not** hold. The correct statement is:

\[\liminf_{t\to\infty} Error_{minimax}(t) \geq \frac{\eta\rho}{2} > 0,\]

i.e., the irreducible error from unidentifiability persists even with
infinite self-evolution data.

*Proof.* Theorem 3's construction has
\(TV(\mathcal{P}_{noise}, \mathcal{P}_{hard}) = 0\),
implying
\(TV(\mathcal{P}_{noise}^n, \mathcal{P}_{hard}^n) = 0\)
for all \(n\) by the i.i.d. product property. The lower bound
\(\eta\rho/2\) is derived from a minimax argument over the two worlds
and does **not** depend on \(n\). The self-evolution loop cannot
...
\bottomrule\noalign{}
\endlastfoot
Theorem SE-1: Convergence to fixed point & **Proven under C1-C7** &
Relies on finite structure space (C1) 

Theorem SE-1: Self-consistency equation & **Proven** & Follows from
fixed point condition 

Lemma SE-1.1: Lyapunov non-increase & **CONDITIONAL** (requires Thm
...
Lemma SE-1.2: Finite memory bank types & **Proven** & Finite
\(\mathcal{X}\) guarantee 

Lemma SE-1.3: Parameter displacement vanishes & **Proven** &
Annealing condition 

Lemma SE-1.4: Limit points are fixed points & **Proven** & From
Lemmas SE-1.1 through SE-1.3 

Regime 1: Classical convergence & **Proven** & Theorem SE-1 

Regime 2: Limit cycle & **Proven for finite \(\mathcal{X}\),
constant step sizes** & Periodic orbits in finite-state DDS 

Regime 3: Perpetual discovery & **Conjectured** & Infinite
\(\mathcal{X}\) extension, rate bound is heuristic 

Regime 4: Divergent collapse & **Proven conditionally** & If
\(S_t < \gamma\) for all \((x,y)\), the dynamics are determined 

Proposition SE-1.1: Limit cycle conditions & **Proven** & Finite
state space + constant step sizes 

Proposition SE-1.2: Perpetual discovery bound & **Conjectured** &
The irreducibility bound lacks rigorous proof 

Proposition SE-1.3: Collapse condition & **Proven** & Direct
consequence of update rules 

Phase diagram & **Conjectured** & Phase transitions are not
rigorously characterized 

Proposition SE-1.4: Noise detection during evolution & **Proven** &
Theorem 1 applies invariantly 

Proposition SE-1.5: Resolution of unidentifiability & **RETRACTED**
...
**Conjectured** & Requires uniform continuity and compactness 
...
*End of 06\_fixed\_point\_convergence.md -- Central theorem of SCX
...
\bottomrule\noalign{}
\endlastfoot
2026-06-28 & DEFECT-01/02 & **Fixed Lemma F**: Added correct global
F1 aggregation derivation (Section 12 remark). The original claim
\(F1_{global} = \sum_s \rho_s \cdot F1_s\) was false due to F1's
nonlinearity. Corrected: FNR and FPR are linear in per-state quantities;
the F1 expansion absorbs the nonlinearity into
\(o(e^{-M\kappa_})\). Theorem 1's existing bound is already
correct. & FATAL 

2026-06-28 & DEFECT-03/04 & **Fixed Lyapunov function**: Updated
...
2026-06-28 & DEFECT-05 & **Retracted SE-1.5**: Replaced the false
...
2026-06-28 & DEFECT-06 & **Added Bahadur-Rao lattice correction**:
Bernoulli lattice correction \((1-e^{-\lambda^*})^{-1}\) replaces
\(1/\lambda^*\) in all asymptotic formulas. \(C_\) changes by
\(\sim 5\)--\(12\%\); minimax optimality preserved (both bounds get same
correction). & MAJOR 

2026-06-28 & DEFECT-13 & **Added selection bias cycle analysis**
...
2026-06-28 & DEFECT-14 & **Fixed exploration-stabilization
...
2026-06-28 & B1 & **Lemma SE-1.1 status changed to CONDITIONAL**:
...
2026-06-28 & B2 & **Added C10 to condition list**: Reference-set
...
2026-06-28 & B2 (SE-1 body) & **Added reference-set note to SE-1
