...
\textbf{文档状态}: 完整分析\\
\textbf{依赖}: Theorem 1--4 (theory/theorems/), SE-1, SE-2
(theory/self\_evolution/)\\
\textbf{方法论}: 严格形式推导 + 诚实暴击（不委婉）
...
设有 \(M\) 个独立专家模型 \(\{E_1, \ldots, E_M\}\)。对于状态原子 \(s_i\)
及其标签 \(y_i\)，定义：
...
其中 \(p_{s_i}\)
是专家''不同意''标签的概率。对于干净样本，\(p_{s_i} = p_{\text{clean}} < 0.5\)；对于噪声样本，\(p_{s_i} = p_{\text{noisy}} > 0.5\)。
...
其中： -
\(\rho_s = |\{i: \mathcal{C}(x_i) = s\}| / N_{\text{total}}\)：状态原子
\(s\) 的占比 -
\(\Delta_s = |p_{\text{clean},s} - p_{\text{noisy},s}|\)：状态 \(s\)
的检测边际 - \(M\)：独立专家数量 - \(\eta > 0\)：归一化因子

\textbf{证明技术}：Chernoff bound + Hoeffding 不等式。对每个状态
...
\textbf{证明技术}：Fano 不等式。定义
\(\delta = \min_{f} \mathbb{E}[\mathbf{1}[f(X) \neq Y]] - \text{Bayes error}\)，即最优可行分类器与贝叶斯最优之间的最小差距。Fano
给出条件熵与错误率的关系：
...
\textbf{证明技术}：Chernoff-Stein 引理 + Bahadur-Rao 展开。
...
\textbf{定义 1（Physical Positional Encoding）}：PPE 是一个映射
\[\text{PE}: \mathcal{P} \to \mathbb{R}^{d_{\text{pe}}}\]

编码后的状态表示： \[\boxed{h_i = \phi(s_i) + \text{PE}(p_i)}\]

其中 \(\phi: \mathbb{R}^{d_s} \to \mathbb{R}^{d}\)
是特征映射，且我们要求 \(d_{\text{pe}} = d\)。
...
\textbf{关键性质}：与 NLP
的位置编码不同，物理位置具有\textbf{多维几何结构}。一个良好的 PPE
应尊重物理对称性：
...
\textbf{核心问题}：PPE 如何改变检测边际 \(\Delta_s\)？

\textbf{命题 2.1（PPE 下的修改后检测边际）}： 在 PPE
...
\(Y\) 的条件互信息，而 \(I(Y; \text{PE}(P) | X)\)
是编码后保留的信息。\(\varepsilon_{\text{PE}}\)
量化了\textbf{位置信息在编码过程中的丢失}。

\textbf{定理 2.2（不完美 PPE 下的失效上界）}：
...
\textbf{紧致性分析}： - 当
\(\varepsilon_{\text{PE}} = 0\)（完美编码）：上界恢复为原始 Theorem 2 -
当 \(\varepsilon_{\text{PE}} > 0\)：上界以
\(\sqrt{\delta + O(\varepsilon_{\text{PE}})}\) 的速率放松 - 当
\(\varepsilon_{\text{PE}} \to I(Y; P | X)\)（完全失败的编码，丢失了所有位置信息）：上界回到完全未使用位置信息的情况

\textbf{推论 2.1（位置信息的价值）}：如果物理位置 \(P\)
本身不携带任何关于标签 \(Y\) 的信息（即 \(I(Y; P | X) = 0\)），则
\(\varepsilon_{\text{PE}} = 0\)（因为没有信息可丢失），Theorem 2
的上界\textbf{完全不变}。PPE 在这种情况下是\textbf{纯粹的参数浪费}。
...
是一个\textbf{先验固定}的函数（不依赖于训练数据或标签），则两个世界中的增强特征分布依然相同：
\[P_{W_A}(X, \text{PE}(P), Y) = P_{W_B}(X, \text{PE}(P), Y)\]

因此 Theorem 3 的不可区分性\textbf{保持不变}。PPE
没有帮助也没有损害最坏情况的可识别性。

\emph{证明}：物理位置 \(P\) 由物理结构决定，独立于标签生成过程。因此
\(P_{W_A}(P|X) = P_{W_B}(P|X)\)。因为 PE
是固定的确定性函数，\(P_{W_A}(\text{PE}(P)|X) = P_{W_B}(\text{PE}(P)|X)\)。观测分布完全相同。\(\square\)

\textbf{命题 2.4（学习型 PPE 破坏 Theorem 3）}：如果 PE 的参数
\(\theta_{\text{PE}}\) 是从数据中学习的（包括标签
\(Y\)），则在两个世界中学习到不同的编码：
\[\text{PE}_{W_A} \neq \text{PE}_{W_B}\]
...
PPE 能破坏定理 3
的前提条件，意味着\textbf{位置信息提供了区分噪声和困难样本的能力}。这不是框架的缺陷，而是框架的进步。但这确实意味着
...
\emph{证明}：由 KL 散度的链式法则： \begin{align*}
...
PPE
\textbf{从不降低}最优误差指数。如果位置信息与标签相关，\(\Delta D_{\text{KL}}^{\text{PE}} > 0\)，误差指数\textbf{严格更大}（检测更准确）。如果位置信息无关，\(\Delta D_{\text{KL}}^{\text{PE}} = 0\)，不变。
...
\textbf{物理维度约束（结构化多头）}：为了实现真正的专业化，每个头的
...
\textbf{核心问题}：\(K\) 个头是否等价于 \(K\) 个独立专家？

\textbf{答案：否。} 这是一个关键的区别。

\textbf{命题 3.1（头不是独立专家）}：Multi-Head Spring
的头输出在给定状态原子集合时是\textbf{条件依赖}的：
\[\text{Cov}(\text{head}_k(s_i), \text{head}_l(s_i) \mid \mathcal{S}) \neq 0 \quad \text{（一般情况）}\]

\emph{证明}：所有头共享相同的输入状态原子 \(\mathcal{S}\) 和相同的
...
\textbf{但这不是一个严格的界}。严格的 Chernoff bound
需要处理依赖随机变量。

\textbf{定理 3.1（Multi-Head Spring 下的严格 F1 下界）}：
...
\textbf{诚实暴击}：严格处理头之间依赖结构的 Chernoff bound
极其复杂。实际上，如果不对头之间的依赖做额外假设（如 \(\beta\)-混合或
Dobrushin 条件），Theorem 1 形式的独立指数界\textbf{不适用于} Multi-Head
...
\textbf{命题 3.2（Multi-Head Spring 下的收敛条件）}：

\begin{enumerate}
\def\labelenumi{(\arabic{enumi})}
\item
  \textbf{步长条件（\(\alpha_t\) 的渐近行为）不变}：条件
...
对于 Multi-Head
Spring，\textbf{鞅性质在联合参数空间上保持}（因为它是贝叶斯更新的直接后果），但：
...
\item
  ...
  这意味着收敛的集中度降低。Azuma-Hoeffding 给出的置信区间宽度为
  \(O(\sqrt{K \cdot t})\) 而非 \(O(\sqrt{t})\)。
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{4. 组合分析}\label{ux7ec4ux5408ux5206ux6790}
...
\textbf{次加性条件}（组合收益 \textless{}
单独收益之和）：当两者之一或交叉项为负。

\textbf{实际判断}：Multi-Head Spring 可以利用 PPE
提供的空间局部性来实现更有意义的注意力模式（例如，头 \(k\) 关注物理位置
\(p_i\) 邻域内的状态原子）。这种协同大概率是\textbf{正向}的，即
\(\delta_s^{\text{cross}} > 0\)。但如果没有明确的物理约束，多头可能学到虚假的跨维相关性，此时
...
条件，而该条件可能不满足 & \textbf{高} \\
\textbf{SE-1 + MH} & Robbins-Monro → 全局最优（凸性下） &
高度非凸损失景观（\(K!\) 对称驻点）→ 只能保证驻点，不能保证全局最优 &
\textbf{高} \\
\textbf{Theorem 2 + MH} & 紧致 Fano 上界 &
过参数化时界仍然成立但变得\textbf{过松}（vacuous），实际性能可能远差于界
& \textbf{中} \\
\textbf{Theorem 3 + 学习型 PPE} & 最坏情况不可区分 & 学习型 PE
使得两个世界可区分 → \textbf{定理 3 被破坏}（但这是好事） &
\textbf{中（实际上有益）} \\
\textbf{SE-2 + MH} & 联合鞅性质保持 & 边际鞅性质可能失效；鞅差方差
\(O(K)\) 增大，置信区间变宽 & \textbf{中} \\
\textbf{组合 Theorem 1} & 独立 Chernoff bound & 有效 \(M_{\text{eff}}\)
是启发式的，非严格。交叉项 \(\delta_s^{\text{cross}}\) 难以从理论上界定
& \textbf{高} \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{\(I(Y; P \mid X) = 0\)（位置不携带标签信息）} & PPE
是纯噪声。Theorem 2 的界不变。Theorem 1 的 \(\Delta_s\)
不变。参数全部浪费 \\
\textbf{\(\rho_{\text{eff}} = 0\)（所有头坍塌为单一模式）} & Multi-Head
等价于单头 + \(K\) 倍冗余参数。Theorem 1 无增益。SE-1 收敛到相同点 \\
\textbf{固定、信息论可逆的 PE} & 仅是变量替换。KL 散度不变 → Theorem 4
不变。无实质贡献 \\
\textbf{\(K > K_{\text{crit}}\) 且无正则化} & 过度参数化 → 泛化误差主导
→ 任何定理的界都变为空集（vacuous） \\
\textbf{物理维度不相关（例如头关注物理维度 D，但 D 与任务无关）} &
...
\textbf{Theorem 3 被破坏的条件（5 个条件，按严重程度排序）}：

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{学习型 PPE（最严重）}：§2.5 已分析。训练数据中的标签噪声导致
  \(\text{PE}_{W_A} \neq \text{PE}_{W_B}\)。结果：两个世界可区分，Theorem
  3 的不可区分性不成立。

  \emph{评价}：\textbf{好事}。Theorem 3
  是最坏情况不可能性结果。如果位置信息能打破这个不可能性，说明 PPE
  有用。
\item
  \textbf{Multi-Head Spring 中的维度泄漏}：若某个头被设计为关注物理维度
  A，但实际上通过 softmax 的全局注意力也能访问维度 B
  的信息，则头的''专业化''是虚假的。这可能导致该头学到标签相关的虚假相关性，改变
  \(P_{\text{clean}}\) 和 \(P_{\text{noisy}}\) 的估计。
\item
  \textbf{\(K > N\)（头数超过状态原子数）}：多头的注意力模式无法被数据唯一确定。存在无限多个参数配置产生相同的
  \(\text{Spring}_{\text{MH}}(s_i)\)。Theorem 3
  的构造依赖于数据分布的唯一性------当参数化是欠定的，两个世界的分布可能有细微差异。
\item
  \textbf{PPE 维度与物理维度对齐}：如果 \(\text{PE}(p_i)\) 的维度与
  Multi-Head Spring
  中某头的物理维度\textbf{正交}（即编码碰巧擦除了该头需要的信息），则该头的注意力模式会产生系统性偏差。这引入了一个
  Theorem 3 构造中未考虑的自由度。
\item
  \textbf{\(\eta(t)\) 衰减过快 + Multi-Head}：如果探索权重 \(\eta(t)\)
  衰减过快，Multi-Head Spring 可能过早地将某些模式''冻结''在次优状态。在
  Theorem 3
  的双世界构造中，一个世界可能学到更好的模式（因为标签更干净），而另一个世界学到差的模式------使两个世界可区分。
\end{enumerate}

\subsubsection{5.4
最诚实的总结}\label{ux6700ux8bdaux5b9eux7684ux603bux7ed3}

\textbf{PPE 的底线}： - \textbf{对 Theorem 1}：如果物理位置与任务相关 →
有正向贡献（\(\delta_s^{\text{PE}} > 0\)）。如果无关 →
没有影响。Chernoff 结构保持。 - \textbf{对 Theorem 2}：不完美的编码以
\(\sqrt{\delta + O(\varepsilon_{\text{PE}})}\)
扩大上界。完美编码不影响上界。 - \textbf{对 Theorem
3}：固定编码保持不可区分性。学习型编码\textbf{破坏}Theorem
3------但这是\textbf{进展}而非退步。 - \textbf{对 Theorem
4}：不减少误差指数。有用时增加指数。

\textbf{Multi-Head Spring 的底线}： - \textbf{对 Theorem
1}：头\textbf{不是}独立专家。严格界比 naive \(M \times K\)
松得多。头相关性的完整处理极其困难。 - \textbf{对
SE-1}：收敛到驻点（不保证全局最优）。有效步长上限以 \(O(1/\sqrt{K})\)
缩小。收敛更慢。 -
\textbf{过参数化}：对于典型的状态空间大小，\(K_{\text{crit}}\) 可能只有
1-3 个头。超过此数的头会产生泛化惩罚。

\textbf{组合的最诚实评价}： - PPE
是\textbf{更有价值的}组件------它在理论上更清晰，影响更可预测，且风险更低
- Multi-Head Spring 的\textbf{理论风险更高}------它破坏了 Theorem 1
最优雅的部分（i.i.d. 专家的 Chernoff bound） -
两者的\textbf{交叉项在理论上是正的}（可能产生超加性收益），但\textbf{在实际中可能为负}（额外的复杂性引入新的失败模式）
...
\subsection{6. 开放问题}\label{ux5f00ux653eux95eeux9898}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{严格的多头 Chernoff 界}：能否在头的依赖结构上建立
  \(\beta\)-mixing 条件，使得 Theorem 1 的指数界可以严格扩展到多头情况？
\item
  \textbf{PPE 的最优频率}：在正弦编码中，频率参数
  \(\omega_j = 10000^{-2j/d}\)
  是为自然语言设计的。物理位置的最优频率谱是什么？能否从物理对称性推导？
\item
  \textbf{头数 \(K\) 的自适应选择}：能否从数据中学习最优的 \(K\)（类似于
  Bayesian non-parametric 中的中餐馆过程），在过参数化之前停止？
\item
  \textbf{Theorem 3
  的修改}：如何在定理陈述中加入''前提条件：不使用数据依赖的位置编码''来保持其数学完整性？
\end{enumerate}
...
\item
  ...
  Inequalities}. Oxford. （依赖变量的集中不等式）
\item
