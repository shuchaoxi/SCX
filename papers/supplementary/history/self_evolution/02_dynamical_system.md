...
> **Purpose**: Formalize the SCX self-evolution loop as a discrete
> dynamical system, study fixed points, Lyapunov stability, attractor
> structure, and phase portraits. **Prerequisites**: Document 01
...
This coupling is **feed-forward**: \(S_t\) influences \(M_{t+1}\)
and \(\theta_{t+1}\), which in turn influence \(S_{t+1}\). There is no
instantaneous feedback loop.

**Proposition 3 (Causal Structure).** The system's information flow
...
*Proof.* By inspection of the update definitions: \(\Phi_M\)
...
**Proposition 4 (Completeness).** If \(\mathcal{D}\) has full
...
*Proof sketch.* \(L^1(\mathcal{D})\) is complete. The finite set
...
**Proposition 5 (Compactness of \(\mathcal{F}\) under
...
*Practical implication.* In implementation, neural network
...
**Definition 17 (Orbit).** The forward orbit of an initial state
\(z_0 \in \mathcal{Z}\) under \(\Phi\) is the sequence:

\[\mathcal{O}^+(z_0) = \{ z_0, \Phi(z_0), \Phi^{(2)}(z_0), \Phi^{(3)}(z_0), ... \}\]

The backward orbit (if \(\Phi\) is invertible) is defined analogously
but is not guaranteed.

#### 3.2 Types of Orbits<!-- label: types-of-orbits -->
...
**Conjecture 2 (Orbit Classification for SCX).** Under Assumptions
B1-B3, all orbits of the SCX self-evolution system are convergent. No
periodic or chaotic orbits exist.

*Status: **Conjecture**. The monotonicity of \(M_t\) (B1)
...
#### 3.3 Properties of Orbits<!-- label: properties-of-orbits -->

**Proposition 6 (Boundedness of Orbits).** Under Assumption B2
(bounded memory growth), any orbit \(\mathcal{O}^+(z_0)\) is bounded in
\(\mathcal{Z}\).

*Proof.* \(S_t\) is bounded in \([0,1]\) by construction,
...
**Definition 19 (\(\omega\)-Limit Set).** For a trajectory
\(z_t = \Phi^{(t)}(z_0)\), the \(\omega\)-limit set is:

\[\omega(z_0) = \bigl\{ z \in \mathcal{Z} : \exists \{t_k\}_{k=1}^\infty  with  t_k \to \infty  and  \lim_{k\to\infty} d_{\mathcal{Z}}(z_{t_k}, z) = 0 \bigr\}\]

The \(\alpha\)-limit set (for backward orbits) is defined analogously.

**Definition 20 (Attractor).** A set \(A \subseteq \mathcal{Z}\) is
...
**Conjecture 3 (Unique Attractor).** Under Assumptions B1-B3 and
...
### 5. Fixed Points<!-- label: fixed-points -->

#### 5.1 Definition<!-- label: definition-2 -->

**Definition 21 (Fixed Point).** A state
...
**Definition 22 (Stability of Fixed Points).** A fixed point
\(z^*\) is: - **Lyapunov stable** if \(\forall \varepsilon > 0\),
\(\exists \delta > 0\) such that \(d_{\mathcal{Z}}(z_0, z^*) < \delta\)
implies \(d_{\mathcal{Z}}(z_t, z^*) < \varepsilon\) for all
\(t \geq 0\). - **Asymptotically stable** if it is Lyapunov stable
and \(\exists \delta > 0\) such that
\(d_{\mathcal{Z}}(z_0, z^*) < \delta\) implies
\(\lim_{t\to\infty} d_{\mathcal{Z}}(z_t, z^*) = 0\). - **Unstable**
if it is not Lyapunov stable.

#### 5.2 Fixed Point Equations<!-- label: fixed-point-equations -->

**Fixed point equation for \(S^*\):**

\[S^*(x, y) = Update_{gate}\bigl(S^*, M^*, f_{\theta^*}\bigr)(x, y)\]

For a gradient-based update
\(Update_{gate}(S, M, f_\theta) = S - \alpha \nabla_S \hat{L}_{gate}(S; M)\),
this implies:

\[\nabla_S \hat{L}_{gate}(S^*; M^*) = 0\]
...
\[M^* = M^* \cup \{(x, y, S^*(x,y), S^*(x,y)) : (x,y) \in q, \; S^*(x,y) > \delta_{store}\}\]

This implies:

\[\{ (x,y) \in supp(\mathcal{D}) : S^*(x,y) > \delta_{store} \} \subseteq M^*\]
...
\[\theta^* = \arg\min_{\theta \in \Theta} \frac{1}{|M^*|} \sum_{(x_i, y_i, \cdot, \cdot) \in M^*} \ell_{nep}(f_\theta(x_i), y_i)\]

i.e., \(\theta^*\) minimizes the NEP loss on \(M^*\).

#### 5.3 Trivial Fixed Points<!-- label: trivial-fixed-points -->

**Proposition 8 (Trivial Fixed Points).** The following are fixed
points of \(\Phi\):
...
*Proof sketch.* Each satisfies the fixed point equations by
...
**Theorem 4 (Existence of Fixed Points).** Suppose: 1. \(\Phi\) is
...
*Proof.* Apply the Brouwer fixed-point theorem (for compact convex
...
**Remark.** Condition 1 (continuity of \(\Phi\)) is non-trivial:
...
\(L_{nep}(\theta; M)\) is strictly convex in \(\theta\) for any
\(M\). 3. The memory bank growth saturates:
\(\lim_{t\to\infty} \mathbb{P}(S_t(x,y) > \delta_{store}) = 0\)
or \(1\) for each \((x,y)\).

Then there exists a unique fixed point \(z^* = (S^*, M^*, \theta^*)\).

*Proof sketch.* Convexity ensures unique minimizers for \(S^*\) and
...
*Status: **Rigorous** under the stated convexity assumptions.
...
\[\mathbb{E}_{(x,y) \sim \mathcal{D}}\bigl[ \ell_{nep}(f_{\theta^*}(x), y) \mid S^*(x,y) > \delta_{store} \bigr] \leq \mathbb{E}_{(x,y) \sim \mathcal{D}}\bigl[ \ell_{nep}(f_{\theta^*}(x), y) \mid S^*(x,y) \leq \delta_{store} \bigr]\]

i.e., the NEP performs no worse on samples the gatekeeper admits than on
samples it rejects.

*Proof.* At a fixed point, the gatekeeper's storage policy and the
...
minimizes \(L_{gate}\) on \(M^*\), and \(M^*\) contains only
...
> \(V(z_t) = \mathbb{E}[L_{gate}] + \lambda \cdot \mathbb{E}[L_{nep}]\)
> evaluated on the acceptance-biased distribution. This was found to be
> **underspecified** (DEFECT-03) and to contain a **selection
...
> explicit candidate on a **fixed reference set** is adopted as a
...
**fixed reference set** (e.g., initial samples before
self-evolution begins, or an external validation set). \(M_0\) does
**not** grow with \(t\). - \(V_0 \subseteq M_0\) is the subset of
...
> **Notation note (B3 fix):** The Lyapunov function is denoted
> \(\Psi\) (Psi), NOT \(\Phi\) (Phi). The symbol \(\Phi\) is reserved for
> the **Update Operator** \(\Phi: \mathcal{Z} \to \mathcal{Z}\)
...
distribution \(P_{S_t}\). This created a **selection bias
...
**Gatekeeper term (\(\Psi_{gate}\)).** Measures the squared
...
consensus:
\[\Psi_{gate}(S_t) = \frac{1}{|M_0|} \sum_{x \in M_0} (S_t(x, y(x)) - \hat{C}(x))^2.\]

**Student term (\(\Psi_{student}\)).** Measures the NEP
...
\[\Psi_{student}(\theta_t) = \frac{1}{|V_0|} \sum_{(x,y) \in V_0} \ell(f_{\theta_t}(x), y).\]

#### 7.3 Properties<!-- label: properties -->

**Proposition 10 (Properties of \(\Psi\)).** 1.
**Non-negativity**: \(\Psi(S, \theta) \geq 0\) for all
\((S, \theta)\), with equality iff \(S_t(x, y(x)) = \hat{C}(x)\) for all
\(x \in M_0\) and \(f_{\theta_t}(x) = y\) for all \((x,y) \in V_0\). 2.
**Boundedness**: Under Assumption A3 (bounded loss
\(\ell \leq B\)), \(\Psi(S, \theta) \leq 1 + \lambda B < \infty\) since
\(S_t \in [0,1]\) and \(\hat{C} \in [0,1]\). 3. **Continuity**:
\(\Psi\) is continuous in \((S, \theta)\) under the product metric
\(d_{\mathcal{Z}}\) if \(\ell\) is continuous and \(M_0\) is finite.

*Proof.* (1) Sums of squares and non-negative losses. (2)
...
**Theorem 6 (Lyapunov Descent --- CONDITIONAL, CONJECTURED).**
...
\[\Psi(S_{t+1}, \theta_{t+1}) - \Psi(S_t, \theta_t) = \underbrace{[\Psi_{gate}(S_{t+1}) - \Psi_{gate}(S_t)]}_{(A)} + \lambda \underbrace{[\Psi_{student}(\theta_{t+1}) - \Psi_{student}(\theta_t)]}_{(B)}.\]

**Term (A) --- Gatekeeper update on reference set.** The gatekeeper
update \(S_{t+1} = \Pi_{[0,1]}[S_t + \beta_t(SCXUpdate - S_t)]\)
moves \(S_t\) toward \(\hat{C}\). However, this movement is **not
guaranteed** to reduce \(\Psi_{gate}\) on \(M_0\) because
SCXUpdate uses data from \(M_{t+1}\) (acceptance-biased), not from
\(M_0\). **Gap: The alignment between SCXUpdate's direction on
\(M_{t+1}\) and the gradient of \(\Psi_{gate}\) on \(M_0\) is not
...
**Overall assessment.** The descent inequality
\(\mathbb{E}[\Psi_{t+1} \mid \mathcal{F}_t] \leq \Psi_t - \eta_t\) has
**not been rigorously proven**. The decomposition above identifies
...
\[\sum_{t=1}^\infty \bigl( \alpha_t \|\nabla_\theta L_t(\theta_t)\|^2 + \beta_t \|\Delta S_t\|^2 \bigr) < \infty \quad a.s.,\]

and any limit point \((S_\infty, \theta_\infty)\) satisfies the
fixed-point equations. The convergence is in the **Cesàro mean**
sense rather than monotonic: the Lyapunov function may increase at
individual steps but its long-run average decreases.

> **Honest status (2026-06-28).** This is labeled a
> **Conjecture** because the Lyapunov descent property (Definition
...
### 8. Monotonicity Analysis<!-- label: monotonicity-analysis -->

#### 8.1 Expected Monotonicity<!-- label: expected-monotonicity -->

**Corollary 1 (Expected Monotonicity).** Under the conditions of
...
*Proof.* The gradient descent update ensures descent in expectation
...
\(\nabla_S \hat{L}_{gate}(S_t; M_{t+1}) = 0\). 2. \(\theta_t\) is
already optimal given \(M_{t+1}\):
\(\nabla_\theta \hat{L}_{nep}(f_{\theta_t}; M_{t+1}) = 0\).
...
**Proposition 12 (Memory Monotonicity).** The sequence
\(\{N_t\}_{t=0}^\infty\) of memory bank sizes is non-decreasing:

\[N_0 \leq N_1 \leq N_2 \leq ...\]

and converges to \(N_\infty \leq \infty\).

*Proof.* \(M_t \subseteq M_{t+1}\) (Assumption B1) directly implies
...
**Regime I: Initialization** (\(t = 0\)). - \(S_0\) derived from
...
**Regime II: Rapid Improvement** (\(0 < t < T_1\)). - The
...
**Regime III: Diminishing Returns** (\(T_1 \leq t < T_2\)). - The
...
**Regime IV: Saturation** (\(t \geq T_2\)). - \(z_t\) approaches a
fixed point \(z^*\) - \(V(z_t)\) plateaus - \(N_t\) saturates (or grows
only from genuinely novel samples) - The system has converged

#### 9.2 Parameter Dependence<!-- label: parameter-dependence -->

The qualitative behavior depends on several key parameters:

\begin{longtable}[]{@{}
  >{\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.3667}}
  >{\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.6333}}@{}}
\toprule\noalign{}
\begin{minipage}[b]
Parameter
...
\(\delta_{store}\) & Higher threshold: slower memory growth,
higher precision 
...
#### 9.3 Phase Diagram<!-- label: phase-diagram -->

We can partition the parameter space into regimes:

**Region A (Convergent)** \(\subset \mathcal{Z}\): Initial states
...
**Region B (Limit Cycle)** \(\subset \mathcal{Z}\): Initial states
leading to periodic orbits. **Conjectured empty** for SCX
(Conjecture 2).

**Region C (Divergent)** \(\subset \mathcal{Z}\): Initial states
from which the system diverges (e.g., \(N_t \to \infty\) without
convergence of \(S_t\)). This can occur if \(\delta_{store}\) is
too low, causing the system to accumulate noisy samples indefinitely.

**Proposition 14 (Divergence Condition).** If
\(\delta_{store} < \eta\) (the storage threshold is below the
...
*Proof sketch.* If \(\delta_{store} < \eta\), then even a
...
\[V(z_0) \leq 1 - F1_0 \leq \frac{1} \sum_{s \in \mathcal{S}} \rho_s \cdot \exp\bigl(-2M\Delta_s^2\bigr)\]

This bounds the initial Lyapunov value \(V(z_0)\) in terms of the noise
detection F1 score.

**Dynamical implication**: The system starts with
...
*Status: **Conjecture** --- the exact constant
...
#### 10.4 Integration<!-- label: integration -->
...
\bottomrule\noalign{}
\endlastfoot
Proposition 3 (causal structure) & **Proven** & By inspection of
update definitions 

Proposition 4 (completeness) & **Proven** & Standard topology
results 

Proposition 5 (compactness) & **Proven** & Arzela-Ascoli under
Lipschitz bound 

Proposition 6 (bounded orbits) & **Proven** & From B1-B3 

Proposition 7 (fixed points as attractors) & **Proven** & By
Lyapunov's theorem 

Proposition 8 (trivial fixed points) & **Proven** & By
verification 

Theorem 4 (general existence) & **Proven** & Brouwer/Schauder fixed
point theorem 

Theorem 5 (unique existence, convex) & **Proven** & Contraction
mapping / convex analysis 

Proposition 9 (necessary condition) & **Proven** & By
contradiction 

Proposition 10 (Lyapunov properties) & **Proven** & Non-negativity,
boundedness, continuity of the explicit \(\Psi\) are verified for finite
\(M_0\) 

Theorem 6 (Lyapunov decrease) & **CONJECTURED (was ``Proven'')** &
...
Corollary 1 (expected monotonicity) & **CONJECTURED** (depends on
Theorem 6) & Conditional on the descent property being proven 

Proposition 11 (strict decrease) & **CONJECTURED** (depends on
...
*End of Document 02: SCX Self-Evolution as a Discrete Dynamical
System*

*Next: Document 03 analyzes the online learning regret of the
...
\bottomrule\noalign{}
\endlastfoot
2026-06-28 & DEFECT-03 & **Explicit Lyapunov function defined.**
Replaced the vague composite loss
\(V(z_t) = \mathbb{E}[L_{gate}] + \lambda \mathbb{E}[L_{nep}]\)
with a concrete candidate
\(\Psi(S_t, \theta_t) = \frac{1}{|M_0|}\sum_{x \in M_0}(S_t - \hat{C})^2 + \frac{|V_0|}\sum_{V_0}\ell(f_{\theta_t}, y)\)
evaluated on a fixed reference set \(M_0\). The function is
non-negative, bounded, and well-defined for finite \(M_0\). & FATAL 

2026-06-28 & DEFECT-04 & **Honest CONJECTURE status for descent
property.** The Lyapunov descent proof (Theorem 6) is acknowledged as
**not proven**. Two critical gaps identified: (i) alignment of
...
2026-06-28 & DEFECT-03/13 & **Selection bias confound
...
2026-06-28 & DEFECT-13/14 & **Cross-reference to selection bias
