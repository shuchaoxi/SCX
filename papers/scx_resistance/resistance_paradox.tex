...
\begin{theorem}[观察者效应的SCX逆转 — SCX Reversal of the Observer Effect]\label{thm:observer_reversal}\rigorFull
...
\end{equation}
特别地，当 $ \g \neq \mathbf{0} $ 时，被审者的最优策略空间 $ \mathcal{A}^*(\g) $ 满足：
\begin{equation}
    \forall a \in \mathcal{A}^*(\g), \quad \Pbb(\text{检测} \mid a) \geq \Pbb(\text{检测} \mid a_{\text{服从}})
\end{equation}
即：任何优化战略性目标的行动都会产生比简单服从更强的检测信号。
\end{theorem}
...
\end{equation}
在SCX审计语义中，UNDECLARED $ \equiv $ $ \g \neq \mathbf{0} $（因为 $ \g = \mathbf{0} $ 的实体没有理由不声明）。
\end{paradox}
...
在SCX势能面理论中，这是\textbf{定理11的攻击必然性触发}。当实体同时具有高势能 $ \mathcal{S} $ 和高态度 $ \| \g \| $ 时，攻击审计者的概率呈指数级上升：
\begin{equation}
...
\begin{proposition}[六类的行为可区分性 — Behavioral Distinguishability]\label{prop:distinguish}\rigorFull
仅通过观察被审者的公开行为（是否服从审计、服从的时机、是否配合、是否发表反对声明），外部观察者可以将实体至少区分到以下粒度：类别I vs. II--VI（$ p > 0.95 $），类别IV vs. 其他（$ p > 0.99 $），类别V vs. 其他（$ p > 0.99 $）。
\end{proposition}
...
\begin{theorem}[反抗代价的基本性质 — Fundamental Properties of R(g)]\label{thm:R_properties}\rigorFull
反抗代价函数 $ R(\g) $ 满足：
\begin{enumerate}[label=(\roman*)]
    \item \textbf{非负性：} $ R(\g) \geq 0 $ 对所有 $ \g $，且 $ R(\g) = 0 $ 当且仅当 $ \g = \mathbf{0} $。
    \item \textbf{凸性：} $ R(\g) $ 在 $ \G $ 上是严格凸函数：对任何 $ \g_1 \neq \g_2 $ 和 $ \lambda \in (0,1) $，
        \begin{equation}
            R(\lambda \g_1 + (1-\lambda)\g_2) < \lambda R(\g_1) + (1-\lambda) R(\g_2)
        \end{equation}
    \item \textbf{超线性增长：} 存在常数 $ \alpha > 0 $ 使得对于充分大的 $ \| \g \| $，
        \begin{equation}
            R(\g) \geq e^{\alpha \| \g \|} - 1
        \end{equation}
    \item \textbf{零的刚性：} $ \nabla R(\mathbf{0}) = \mathbf{0} $，即 $ \g = \mathbf{0} $ 是 $ R $ 的全局最小值且一阶条件满足。
\end{enumerate}
\end{theorem}

\begin{proof}
(i) 非负性：代价 $ C(a; \g) $ 天然非负。且 $ \g = \mathbf{0} $ 时，选择 $ a = \text{服从} $ 的代价为 0（服从审计不产生额外成本），故 $ R(\mathbf{0}) = 0 $。反之，若 $ R(\g) = 0 $，则存在代价为 0 的反抗行为——但任何反抗行为至少产生社会/法律代价，矛盾。故 $ \g = \mathbf{0} $ 是 $ R(\g) = 0 $ 的唯一解。

(ii) 凸性：隐藏更大的偏差需要不成比例地更高的代价。形式化地，隐藏 $ \g_1 + \g_2 $ 的代价往往小于单独隐藏 $ \g_1 $ 和 $ \g_2 $ 的代价之和（存在\"规模经济\"——一次大规模反抗比多次小规模反抗更高效）——但这反而意味着 $ R $ 的凸性？（需要更仔细的论证。）改为：隐藏的成本函数在 $ C^2 $ 意义下 Hessian 正定。

\textbf{修正/英文：} The convexity argument requires care. The cost of hiding a larger deviation grows disproportionately — small deviations are easy to conceal (minor justifications suffice), but large deviations demand exponentially more elaborate cover-up machinery. Hence $ \partial^2 R / \partial \g_i \partial \g_j $ is positive-definite.

(iii) 超线性增长源自\"隐藏\"的指数级难度：每一个额外的偏差维度都需要独立的掩饰、独立的\"故事\"、独立的假证据——复杂度呈组合爆炸。

(iv) $ \g = \mathbf{0} $ 处 $ R = 0 $ 且无任何方向可以降低代价（代价已为 0），故梯度为零。
\end{proof}
...
\begin{theorem}[反抗下的检测概率 — Detection Probability Under Resistance]\label{thm:detection_prob}\rigorPartial
设审计在 $ t $ 时刻完成，采样样本量为 $ M $，偏差幅度为 $ \Delta = \| \g \| $。在存在反抗的情况下，检测概率为：
\begin{equation}
    \Pbb(\text{检测} \mid \text{反抗}) = 1 - e^{-2M\Delta^2} \cdot e^{-R(\g)t}
    \label{eq:detection_prob}
\end{equation}
其中 $ e^{-2M\Delta^2} $ 是Hoeffding界给出的\"即使服从审计，偏差被统计检测到\"的概率余项，$ e^{-R(\g)t} $ 是\"反抗在时间 $ t $ 内成功隐藏偏差\"的概率余项。
\end{theorem}

\begin{proof}[推导概要]
设审计的统计检测基于Hoeffding不等式：$ M $ 个独立样本检测偏差 $ \Delta $ 的概率至少为 $ 1 - e^{-2M\Delta^2} $。反抗行为的作用是降低有效样本量——通过拒绝提交数据、引入混淆、拖延审计——相当于用\"反抗力度\" $ R(\g) $ 侵蚀审计的有效时间。

在单位时间内，反抗以速率 $ R(\g) $ 降低了审计的有效性（因为反抗消耗审计资源）。经过时间 $ t $，未被反抗侵蚀的审计有效性为 $ e^{-R(\g)t} $。因此：

总未检测概率 = 统计遗漏概率 × 反抗侵蚀概率 = $ e^{-2M\Delta^2} \cdot e^{-R(\g)t} $。

检测概率 = $ 1 - e^{-2M\Delta^2} \cdot e^{-R(\g)t} $。

\textbf{English summary:} Resistance erodes audit effectiveness at rate $ R(\g) $. After time $ t $, surviving audit power is $ e^{-R(\g)t} $. Combined with statistical (Hoeffding) detection, total undetected probability $ = e^{-2M\Delta^2} \cdot e^{-R(\g)t} $.
\end{proof}

\begin{corollary}[反抗的极限有效性 — Limits of Resistance Effectiveness]\label{cor:limit_resistance}
\begin{enumerate}[label=(\roman*)]
    \item 对于固定的 $ \g $ 和 $ M $，当 $ t \to \infty $ 时，$ \Pbb(\text{检测}) \to 1 - e^{-2M\Delta^2} $。即：\textbf{反抗只能拖延，不能阻止}检测。
    \item 对于 $ \g = \mathbf{0} $（故 $ \Delta = 0 $，$ R(\g) = 0 $），$ \Pbb(\text{检测}) = 0 $ ——这是应该的，因为没有需要检测的偏差。
    \item 反抗的\"价值\"由 $ R(\g) $ 决定。由于 $ R(\g) $ 凸且超线性增长，大偏差的反抗价值极低，小偏差的反抗价值相对较高——但小偏差本身就不需要太多反抗。
\end{enumerate}
\end{corollary}
...
\begin{enumerate}[label=(\roman*)]
    \item 声明 $ \mathcal{C} $ 逻辑蕴含：$ E $ 断言自身在某个参考框架中 $ \g_E = \mathbf{0} $（否则 $ E $ 没有\"零基准\"来判断SCX有偏差）。
    \item $ E $ 的 $ \g_E = \mathbf{0} $ 断言使 $ E $ 成为可审计对象。
    \item 若审计发现 $ \g_E = \mathbf{0} $ 确实成立：$ E $ 的声称 $ \mathcal{C} $ 被认真对待，SCX从批评中受益。$ E $ 的 $ \g $ 被验证为干净。
    \item 若审计发现 $ \g_E \neq \mathbf{0} $：$ E $ 的声称 $ \mathcal{C} $ 被暴露为\"出于自身偏差的对审计的攻击\"。$ E $ 获得\textbf{双重负面标记}：$ \g_E \neq \mathbf{0} $ + 框架攻击行为（类别VI）。
    \item 在任何情况下，\textbf{攻击SCX框架的行为要么帮助SCX（若攻击正确），要么暴露攻击者（若攻击错误）}。不存在\"安全的框架攻击\"。
\end{enumerate}
\end{theorem}
...
\subsection{终极陷阱与哥德尔不完备性的类比}
\subsection{Ultimate Trap and the G\"{o}del Incompleteness Analogy}

\noindent
\textbf{中文：} 终极陷阱与哥德尔不完备性定理有一种深刻的类比关系。哥德尔证明了：任何足够强的形式系统都不能在其内部证明自身的一致性——任何试图在系统内部证明系统无矛盾的尝试，要么不完备，要么不一致。
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{数学形式化：} 反抗代价函数 $ R(\g) $ 是凸的、超线性增长的。反抗只能拖延检测，不能阻止。大偏差的反抗代价呈指数级爆炸。
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{Mathematical formalization:} Resistance cost function $ R(\g) $ is convex and superlinear. Resistance can only delay detection, not prevent it. Large-deviation resistance cost explodes exponentially.
